...
static xmlCharEncError asciiToAscii(
	void* vctxt, unsigned char* out, int* outlen, const unsigned char* in, int* inlen, int flush
);
static xmlCharEncError UTF8ToUTF8(
	void* vctxt, unsigned char* out, int* outlen, const unsigned char* inb, int* inlenb, int flush
);
static xmlCharEncError latin1ToUTF8(
	void* vctxt, unsigned char* out, int* outlen, const unsigned char* in, int* inlen, int flush
);
static xmlCharEncError UTF16LEToUTF8(
	void* vctxt, unsigned char* out, int* outlen, const unsigned char* inb, int* inlenb, int flush
);
static xmlCharEncError UTF16BEToUTF8(
	void* vctxt, unsigned char* out, int* outlen, const unsigned char* inb, int* inlenb, int flush
);

#ifdef LIBXML_OUTPUT_ENABLED

static xmlCharEncError UTF8ToLatin1(
	void* vctxt, unsigned char* outb, int* outlen, const unsigned char* in, int* inlen, int flush
);
static xmlCharEncError UTF8ToUTF16(
	void* vctxt, unsigned char* outb, int* outlen, const unsigned char* in, int* inlen, int flush
);
static xmlCharEncError UTF8ToUTF16LE(
	void* vctxt, unsigned char* outb, int* outlen, const unsigned char* in, int* inlen, int flush
);
static xmlCharEncError UTF8ToUTF16BE(
	void* vctxt, unsigned char* outb, int* outlen, const unsigned char* in, int* inlen, int flush
);
...
static xmlCharEncError UTF8ToHtmlWrapper(
	void* vctxt, unsigned char* out, int* outlen, const unsigned char* in, int* inlen, int flush
);
#else
...
static xmlCharEncError EightBitToUtf8(
	void* vctxt, unsigned char* out, int* outlen, const unsigned char* in, int* inlen, int flush
);
static xmlCharEncError Utf8ToEightBit(
	void* vctxt, unsigned char* out, int* outlen, const unsigned char* in, int* inlen, int flush
);
...
#define MAKE_8BIT_HANDLER(name, table) \
	...
	#define MAKE_ISO_HANDLER(name, n)                                     \
		{(char*)name, {NULL}, {NULL}, NULL, NULL, NULL, XML_HANDLER_STATIC}
...
xmlCharEncIconv(const char* name, xmlCharEncFlags flags, xmlCharEncodingHandler** out);
#endif

#ifdef LIBXML_ICU_ENABLED
static xmlParserErrors
xmlCharEncUconv(const char* name, xmlCharEncFlags flags, xmlCharEncodingHandler** out);
#endif
...
xmlCharEncoding xmlDetectCharEncoding(const unsigned char* in, int len) {
	...
	if (len >= 4) {
		...
		if ((in[0] == 0x3C) && (in[1] == 0x00) && (in[2] == 0x00) && (in[3] == 0x00)) {
		 ...
		 * UTF-16BE encodings.
		 */
		if ((in[0] == 0x3C) && (in[1] == 0x00) && (in[2] == 0x3F) && (in[3] == 0x00)) {
			return (XML_CHAR_ENCODING_UTF16LE);
		}
		if ((in[0] == 0x00) && (in[1] == 0x3C) && (in[2] == 0x00) && (in[3] == 0x3F)) {
...
int xmlAddEncodingAlias(const char* name, const char* alias) {
	...
	if (xmlCharEncodingAliasesNb >= xmlCharEncodingAliasesMax) {
		...
		newSize = xmlGrowCapacity(xmlCharEncodingAliasesMax, sizeof(tmp[0]), 20, XML_MAX_ITEMS);
		if (newSize < 0) {
			return (-1);
		}
...
int xmlDelEncodingAlias(const char* alias) {
	...
	for (i = 0; i < xmlCharEncodingAliasesNb; i++) {
		if (!strcmp(xmlCharEncodingAliases[i].alias, alias)) {
			...
			xmlCharEncodingAliasesNb--;
			memmove(
				&xmlCharEncodingAliases[i],
				&xmlCharEncodingAliases[i + 1],
...
static int xmlCompareEncTableEntries(const void* vkey, const void* ventry) {
	const char*             key   = vkey;
...
static xmlCharEncoding xmlParseCharEncodingInternal(const char* name) {
	...
	entry = bsearch(
		...
		sizeof(xmlEncTable[0]),
		xmlCompareEncTableEntries
	);
	if (entry != NULL) {
		return (entry->enc);
	}
 ...
 */
xmlCharEncodingHandler* xmlNewCharEncodingHandler(
	const char* name, xmlCharEncodingInputFunc input, xmlCharEncodingOutputFunc output
) {
...
void xmlRegisterCharEncodingHandler(xmlCharEncodingHandler* handler) {
	...
	if (globalHandlers == NULL) {
		globalHandlers = xmlMalloc(MAX_ENCODING_HANDLERS * sizeof(globalHandlers[0]));
		if (globalHandlers == NULL) {
			goto free_handler;
		}
	}
 ...
 */
static xmlParserErrors xmlFindExtraHandler(
	const char*              norig,
	const char*              name,
	...
	if (globalHandlers != NULL) {
		...
		for (i = 0; i < nbCharEncodingHandler; i++) {
			...
			if (!xmlStrcasecmp((const xmlChar*)name, (const xmlChar*)h->name)) {
...
xmlParserErrors xmlLookupCharEncodingHandler(xmlCharEncoding enc, xmlCharEncodingHandler** out) {
	...
	*out = NULL;
	...
	if ((enc == XML_CHAR_ENCODING_UTF8) || (enc == XML_CHAR_ENCODING_NONE)) {
		return (XML_ERR_OK);
	}
...
#endif
		return (xmlFindExtraHandler(handler->name, handler->name, flags, NULL, NULL, out));
	}
 ...
 */
xmlParserErrors
xmlOpenCharEncodingHandler(const char* name, int output, xmlCharEncodingHandler** out) {
...
xmlCharEncodingHandler* xmlFindCharEncodingHandler(const char* name) {
	...
	if ((xmlStrcasecmp(BAD_CAST name, BAD_CAST "UTF-8") == 0)
		...
		return ((xmlCharEncodingHandlerPtr)&defaultHandlers[XML_CHAR_ENCODING_UTF8]);
	}
 ...
 */
static xmlCharEncError xmlIconvConvert(
	void*                vctxt,
	unsigned char*       out,
	int*                 outlen,
...
static void xmlIconvFree(void* vctxt) {
	xmlIconvCtxt* ctxt = vctxt;
	...
	#if defined(FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION) && defined(__GLIBC__)
		#include <libxml/parserInternals.h>

static int xmlEncodingMatch(const char* name1, const char* name2) {
	/*
...
xmlCharEncIconv(const char* name, xmlCharEncFlags flags, xmlCharEncodingHandler** out) {
	 ...
	 */
	if ((xmlEncodingMatch(name, "TSCII")) || (xmlEncodingMatch(name, "BIG5-HKSCS"))) {
		ret = XML_ERR_UNSUPPORTED_ENCODING;
		goto error;
	}
	#endif
	...
	return (
		xmlCharEncNewCustomHandler(name, inFunc, outFunc, xmlIconvFree, inputCtxt, outputCtxt, out)
	);
 ...
 * @returns an xmlCharEncError code.
 */
static xmlCharEncError xmlUconvConvert(
	void* vctxt, unsigned char* out, int* outlen, const unsigned char* in, int* inlen, int flush
) {
...
static xmlParserErrors openIcuConverter(const char* name, int isInput, xmlUconvCtxt** out) {
	UErrorCode    status;
	xmlUconvCtxt* conv;
	...
	*out = NULL;
	...
	status = U_ZERO_ERROR;
	if (isInput) {
		ucnv_setToUCallBack(conv->uconv, UCNV_TO_U_CALLBACK_STOP, NULL, NULL, NULL, &status);
	} else {
		ucnv_setFromUCallBack(conv->uconv, UCNV_FROM_U_CALLBACK_STOP, NULL, NULL, NULL, &status);
	}
	if (U_FAILURE(status)) {
		goto error;
	}
...
static void closeIcuConverter(xmlUconvCtxt* conv) {
	if (conv == NULL) {
		return;
	}
	ucnv_close(conv->uconv);
...
static void xmlUconvFree(void* vctxt) {
...
xmlCharEncUconv(const char* name, xmlCharEncFlags flags, xmlCharEncodingHandler** out) {
	...
	return (xmlCharEncNewCustomHandler(name, inFunc, outFunc, xmlUconvFree, ucv_in, ucv_out, out));
 ...
 */
static xmlParserErrors xmlEncConvertError(xmlCharEncError code) {
	xmlParserErrors ret;
 ...
 * @returns an xmlCharEncError code.
 */
xmlCharEncError xmlEncInputChunk(
	xmlCharEncodingHandler* handler,
	unsigned char*          out,
	int*                    outlen,
	const unsigned char*    in,
 ...
 * @returns an xmlCharEncError code.
 */
static xmlCharEncError xmlEncOutputChunk(
	xmlCharEncodingHandler* handler,
	unsigned char*          out,
	int*                    outlen,
	const unsigned char*    in,
...
int xmlCharEncFirstLine(
	xmlCharEncodingHandler* handler, struct _xmlBuffer* out, struct _xmlBuffer* in
) {
 ...
 * @returns an xmlCharEncError code.
 */
xmlCharEncError xmlCharEncInput(xmlParserInputBuffer* input, size_t* sizeOut, int flush) {
	xmlBufPtr       out, in;
	...
	*sizeOut = 0;
	...
	while (1) {
		...
		ret = xmlEncInputChunk(
			input->encoder, xmlBufEnd(out), &c_out, dataIn, &c_in, flush && completeIn
		);
 ...
 */
int
xmlCharEncInFunc(xmlCharEncodingHandler* handler, struct _xmlBuffer* out, struct _xmlBuffer* in) {
	int ret;
	int written;
	int toconv;
	...
	ret = xmlEncInputChunk(
		handler, &out->content[out->use], &written, in->content, &toconv, /* flush */ 0
	);
 ...
 */
static int xmlSerializeDecCharRef(char* buf, int val) {
	char* out = buf;
	int   len, i;
 ...
 * @returns the number of bytes written or an xmlCharEncError code.
 */
int xmlCharEncOutput(xmlOutputBuffer* output, int init) {
	int       ret;
	size_t    written;
	int       writtentot = 0;
	size_t    toconv;
	...
	if ((output == NULL) || (output->encoder == NULL) || (output->buffer == NULL)
	    || (output->conv == NULL)) {
		return (XML_ENC_ERR_INTERNAL);
	}
	out = output->conv;
...
retry:
	...
	if (init) {
		...
		xmlEncOutputChunk(output->encoder, xmlBufEnd(out), &c_out, NULL, &c_in);
		xmlBufAddLen(out, c_out);
		return (c_out);
	}
	...
	c_in  = toconv;
	c_out = written;
	ret   = xmlEncOutputChunk(output->encoder, xmlBufEnd(out), &c_out, xmlBufContent(in), &c_in);
	...
	if (ret == XML_ENC_ERR_INPUT) {
		...
		c_in  = charrefLen;
		ret   = xmlEncOutputChunk(output->encoder, xmlBufEnd(out), &c_out, charref, &c_in);
...
error:
	if (((writtentot <= 0) && (ret != 0)) || (ret == XML_ENC_ERR_MEMORY)) {
		if (output->error == 0) {
 ...
 */
int
xmlCharEncOutFunc(xmlCharEncodingHandler* handler, struct _xmlBuffer* out, struct _xmlBuffer* in) {
	int ret;
	int written;
	int writtentot = 0;
	int toconv;
...
retry:
	...
	if (in == NULL) {
		toconv = 0;
		/* TODO: Check return value. */
		xmlEncOutputChunk(handler, &out->content[out->use], &written, NULL, &toconv);
		out->use               += written;
		out->content[out->use]  = 0;
		return (0);
	}
	...
	ret = xmlEncOutputChunk(handler, &out->content[out->use], &written, in->content, &toconv);
	xmlBufferShrink(in, toconv);
	out->use               += written;
	writtentot             += written;
	out->content[out->use]  = 0;
	...
	if (ret == XML_ENC_ERR_INPUT) {
		...
		written = out->size - out->use - 1;
		toconv  = charrefLen;
		ret     = xmlEncOutputChunk(handler, &out->content[out->use], &written, charref, &toconv);
...
static xmlCharEncError EightBitToUtf8(
	...
	const unsigned short* unicodetable = vctxt;
	const unsigned char*  instart      = in;
	const unsigned char*  inend;
	unsigned char*        outstart = out;
	unsigned char*        outend;
