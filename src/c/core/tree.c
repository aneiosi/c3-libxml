...
static xmlNodePtr xmlNewEntityRef(xmlDocPtr doc, xmlChar* name);

static xmlNsPtr   xmlNewReconciledNs(xmlNodePtr tree, xmlNsPtr ns);

static xmlAttrPtr xmlGetPropNodeInternal(
	const xmlNode* node, const xmlChar* name, const xmlChar* nsName, int useDTD
);

static xmlChar* xmlGetPropNodeValueInternal(const xmlAttr* prop);

static void     xmlBufGetChildContent(xmlBufPtr buf, const xmlNode* tree);

static void     xmlUnlinkNodeInternal(xmlNodePtr cur);
...
#define IS_STR_XML(str)                                                                     \
	((str != NULL) && (str[0] == 'x') && (str[1] == 'm') && (str[2] == 'l') && (str[3] == 0))
...
xmlNs* xmlNewNs(xmlNode* node, const xmlChar* href, const xmlChar* prefix) {
	...
	if (node != NULL) {
		if (node->nsDef == NULL) {
			...
			if ((xmlStrEqual(prev->prefix, cur->prefix)) && (prev->href != NULL)) {
				goto error;
			}
 ...
 */
xmlDtd*
xmlNewDtd(xmlDoc* doc, const xmlChar* name, const xmlChar* publicId, const xmlChar* systemId) {
	xmlDtdPtr cur;
 ...
 */
xmlDtd* xmlCreateIntSubset(
	xmlDoc* doc, const xmlChar* name, const xmlChar* publicId, const xmlChar* systemId
) {
	xmlDtdPtr cur;
...
void xmlFreeDtd(xmlDtd* cur) {
	...
	if (cur->children != NULL) {
		...
		while (c != NULL) {
			next = c->next;
			if ((c->type != XML_ELEMENT_DECL) && (c->type != XML_ATTRIBUTE_DECL)
 ...
 */
int xmlNodeParseAttValue(
	const xmlDoc* doc, xmlAttr* attr, const xmlChar* value, size_t len, xmlNode** listPtr
) {
		...
		if (cur[0] == '&') {
			...
			if ((remaining > 2) && (cur[1] == '#') && (cur[2] == 'x')) {
				...
				if (cur != q) {
					...
					if ((ent != NULL) && (ent->etype == XML_INTERNAL_PREDEFINED_ENTITY)) {
					...
					} else if (ent == NULL || (ent->flags & XML_ENT_EXPANDING) == 0) {
						/*
						 * Flush buffer so far
						 */
						...
						if ((ent != NULL) && ((ent->flags & XML_ENT_PARSED) == 0) && (ent->content != NULL)) {
							int res;

							ent->flags |= XML_ENT_EXPANDING;
							res         = xmlNodeParseAttValue(doc, (xmlAttr*)ent, ent->content, SIZE_MAX, NULL);
							ent->flags &= ~XML_ENT_EXPANDING;
							if (res < 0) {
								goto out;
							}
...
xmlChar* xmlNodeListGetStringInternal(const xmlNode* node, int escape, int flags) {
	...
	if ((escape == 0) && ((node->type == XML_TEXT_NODE) || (node->type == XML_CDATA_SECTION_NODE))
	    && (node->next == NULL)) {
 ...
 */
xmlChar*
xmlNodeListGetRawString(const xmlDoc* doc ATTRIBUTE_UNUSED, const xmlNode* list, int inLine) {
	int escape = 0;
	int flags  = 0;
...
static xmlAttrPtr xmlNewPropInternal(
	xmlNodePtr node, xmlNsPtr ns, const xmlChar* name, const xmlChar* value, int eatname
) {
	xmlAttrPtr cur;
	xmlDocPtr  doc = NULL;
	...
	if ((node != NULL) && (node->type != XML_ELEMENT_NODE)) {
		if ((eatname == 1)
		    && ((node->doc == NULL) || (node->doc->dict == NULL) || (!(xmlDictOwns(node->doc->dict, name))))) {
...
static xmlNodePtr
xmlNewElem(xmlDocPtr doc, xmlNsPtr ns, const xmlChar* name, const xmlChar* content) {
	xmlNodePtr cur;
	...
	if (content != NULL) {
		 ...
		 * but the API can't be changed.
		 */
		if (xmlNodeParseAttValue(doc, (xmlAttr*)cur, content, SIZE_MAX, NULL) < 0) {
...
xmlNode* xmlNewDocNodeEatName(xmlDoc* doc, xmlNs* ns, xmlChar* name, const xmlChar* content) {
	...
	if (cur == NULL) {
		...
		if ((doc == NULL) || (doc->dict == NULL) || (!xmlDictOwns(doc->dict, name))) {
			xmlFree(name);
		}
		return (NULL);
	}
...
static void xmlRemoveEntity(xmlEntityPtr ent) {
...
static int xmlNodeSetDoc(xmlNodePtr node, xmlDocPtr doc) {
	xmlDocPtr  oldDoc;
...
int xmlSetTreeDoc(xmlNode* tree, xmlDoc* doc) {
	...
	if ((tree->children != NULL) && (tree->type != XML_ENTITY_REF_NODE)) {
...
static void xmlTextSetContent(xmlNodePtr text, xmlChar* content) {
	if ((text->content != NULL) && (text->content != (xmlChar*)&text->properties)) {
		xmlDocPtr doc = text->doc;

		if ((doc == NULL) || (doc->dict == NULL) || (!xmlDictOwns(doc->dict, text->content))) {
...
static int xmlTextAddContent(xmlNodePtr text, const xmlChar* content, int len) {
	xmlChar* merged;
...
static xmlNodePtr
xmlInsertProp(xmlDocPtr doc, xmlNodePtr cur, xmlNodePtr parent, xmlNodePtr prev, xmlNodePtr next) {
	xmlAttrPtr attr;
	...
	attr = xmlGetPropNodeInternal(parent, cur->name, cur->ns ? cur->ns->href : NULL, 0);
...
static xmlNodePtr xmlInsertNode(
	xmlDocPtr doc, xmlNodePtr cur, xmlNodePtr parent, xmlNodePtr prev, xmlNodePtr next, int coalesce
) {
	...
	if ((coalesce) && (cur->type == XML_TEXT_NODE)) {
		if ((prev != NULL) && (prev->type == XML_TEXT_NODE) && (prev->name == cur->name)) {
		...
		if ((next != NULL) && (next->type == XML_TEXT_NODE) && (next->name == cur->name)) {
			if (cur->content != NULL) {
				xmlChar* merged;
...
xmlNode* xmlAddNextSibling(xmlNode* prev, xmlNode* cur) {
	if ((prev == NULL) || (prev->type == XML_NAMESPACE_DECL) || (cur == NULL)
	    || (cur->type == XML_NAMESPACE_DECL) || (cur == prev)) {
		return (NULL);
	}
...
xmlNode* xmlAddPrevSibling(xmlNode* next, xmlNode* cur) {
	if ((next == NULL) || (next->type == XML_NAMESPACE_DECL) || (cur == NULL)
 ...
 * if arguments are invalid or a memory allocation failed.
 */
xmlNode* xmlAddSibling(xmlNode* node, xmlNode* cur) {
	if ((node == NULL) || (node->type == XML_NAMESPACE_DECL) || (cur == NULL)
	    || (cur->type == XML_NAMESPACE_DECL) || (cur == node)) {
		return (NULL);
	}
...
xmlNode* xmlAddChildList(xmlNode* parent, xmlNode* cur) {
	...
	if (parent->children == NULL) {
		 ...
		 */
		if ((cur->type == XML_TEXT_NODE) && (prev->type == XML_TEXT_NODE)
		    && (cur->name == prev->name)) {
			xmlNodePtr next;
...
xmlNode* xmlAddChild(xmlNode* parent, xmlNode* cur) {
	...
	if ((parent == NULL) || (parent->type == XML_NAMESPACE_DECL) || (cur == NULL)
	    || (cur->type == XML_NAMESPACE_DECL) || (parent == cur)) {
		return (NULL);
	}
...
void xmlFreeNodeList(xmlNode* cur) {
	...
	while (1) {
		while ((cur->children != NULL) && (cur->type != XML_DOCUMENT_NODE)
		       && (cur->type != XML_HTML_DOCUMENT_NODE) && (cur->type != XML_DTD_NODE)
		       && (cur->type != XML_ENTITY_REF_NODE)) {
			cur    = cur->children;
			depth += 1;
		}
...
void xmlFreeNode(xmlNode* cur) {
	...
	} else if ((cur->content != NULL) && (cur->type != XML_ENTITY_REF_NODE)
	           && (cur->content != (xmlChar*)&(cur->properties))) {
		DICT_FREE(cur->content)
	}
...
static xmlAttrPtr xmlCopyPropInternal(xmlDocPtr doc, xmlNodePtr target, xmlAttrPtr cur) {
	 ...
	 * Try to handle IDs
	 */
	if ((target != NULL) && (cur != NULL) && (target->doc != NULL) && (cur->doc != NULL)
	    && (cur->parent != NULL) && (cur->children != NULL)) {
...
xmlNode* xmlStaticCopyNode(xmlNode* node, xmlDoc* doc, xmlNode* parent, int extended) {
	...
	if ((node->type != XML_ELEMENT_NODE) && (node->content != NULL)
	    && (node->type != XML_ENTITY_REF_NODE) && (node->type != XML_XINCLUDE_END)
	...
	if (node->type == XML_ENTITY_REF_NODE) {
		...
		while (cur != NULL) {
			...
			if ((cur->type != XML_ENTITY_REF_NODE) && (cur->children != NULL)) {
				cur    = cur->children;
				insert = copy;
				continue;
			}
 ...
 */
xmlNode* xmlStaticCopyNodeList(xmlNode* node, xmlDoc* doc, xmlNode* parent) {
	xmlNodePtr ret          = NULL;
	xmlNodePtr p            = NULL, q;
		...
		if (cur->type == XML_ENTITY_DECL) {
			...
			switch (tmp->etype) {
				...
				case XML_EXTERNAL_PARAMETER_ENTITY:
					q = (xmlNodePtr)xmlGetParameterEntityFromDtd(ret, tmp->name);
					break;
			...
			q                 = (xmlNodePtr)xmlGetDtdQElementDesc(ret, tmp->name, tmp->prefix);
		} else if (cur->type == XML_ATTRIBUTE_DECL) {
			xmlAttributePtr tmp = (xmlAttributePtr)cur;
			q                   = (xmlNodePtr)xmlGetDtdQAttrDesc(ret, tmp->elem, tmp->name, tmp->prefix);
...
static long xmlGetLineNoInternal(const xmlNode* node, int depth) {
	...
	if ((node->type == XML_ELEMENT_NODE) || (node->type == XML_TEXT_NODE)
		...
		result = xmlGetLineNoInternal(node->prev, depth + 1);
	} else if ((node->parent != NULL) && (node->parent->type == XML_ELEMENT_NODE)) {
...
xmlChar* xmlNodeGetLang(const xmlNode* cur) {
	...
	while (cur != NULL) {
		res = xmlNodeGetAttrValue(cur, BAD_CAST "lang", XML_XML_NAMESPACE, &lang);
		if (res < 0) {
			return (NULL);
		}
		if (lang != NULL) {
...
int xmlNodeGetSpacePreserve(const xmlNode* cur) {
	...
	while (cur != NULL) {
		res = xmlNodeGetAttrValue(cur, BAD_CAST "space", XML_XML_NAMESPACE, &space);
		if (res < 0) {
			return (-1);
		}
		if (space != NULL) {
...
void xmlNodeSetName(xmlNode* cur, const xmlChar* name) {
	...
	oldName   = cur->name;
	cur->name = copy;
	if ((oldName != NULL) && ((dict == NULL) || (!xmlDictOwns(dict, oldName)))) {
...
int xmlNodeGetBaseSafe(const xmlDoc* doc, const xmlNode* cur, xmlChar** baseOut) {
	...
	*baseOut = NULL;
	...
	while (cur != NULL) {
		...
		if (cur->type == XML_ELEMENT_NODE) {
			if (xmlNodeGetAttrValue(cur, BAD_CAST "base", XML_XML_NAMESPACE, &base) < 0) {
				xmlFree(ret);
				return (-1);
			}
			if (base != NULL) {
				...
				if ((!xmlStrncmp(ret, BAD_CAST "http://", 7)) || (!xmlStrncmp(ret, BAD_CAST "ftp://", 6))
...
static void xmlBufGetEntityRefContent(xmlBufPtr buf, const xmlNode* ref) {
	xmlEntityPtr ent;
...
static int xmlNodeSetContentInternal(xmlNodePtr cur, const xmlChar* content, int len) {
	if (cur == NULL) {
		return (1);
	}
	switch (cur->type) {
		...
		case XML_ATTRIBUTE_NODE: {
			 ...
			 * but the API can't be changed.
			 */
			if (xmlNodeParseAttValue(cur->doc, (xmlAttr*)cur, content, maxSize, NULL) < 0) {
				return (-1);
			}
			break;
		}
...
xmlNode* xmlTextMerge(xmlNode* first, xmlNode* second) {
	...
	if ((first->type != XML_TEXT_NODE) || (second->type != XML_TEXT_NODE) || (first == second)
 ...
 * memory allocation failed.
 */
int xmlGetNsListSafe(const xmlDoc* doc ATTRIBUTE_UNUSED, const xmlNode* node, xmlNs*** out) {
	xmlNsPtr  cur;
			...
			while (cur != NULL) {
				...
				if (i >= nbns) {
					if (nbns >= maxns) {
						...
						newSize = xmlGrowCapacity(maxns, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
						if (newSize < 0) {
							xmlFree(namespaces);
							return (-1);
						}
...
#endif
						tmp = xmlRealloc(namespaces, (newSize + 1) * sizeof(tmp[0]));
						if (tmp == NULL) {
							xmlFree(namespaces);
							return (-1);
						}
...
static xmlNsPtr xmlNewXmlNs(void) {
	xmlNsPtr ns;
 ...
 * other errors.
 */
int xmlSearchNsSafe(xmlNode* node, const xmlChar* prefix, xmlNs** out) {
	xmlNsPtr   cur;
	xmlDocPtr  doc;
	...
	*out = NULL;
	...
	while ((node != NULL) && (node->type == XML_ELEMENT_NODE)) {
		cur = node->nsDef;
		while (cur != NULL) {
			if ((xmlStrEqual(cur->prefix, prefix)) && (cur->href != NULL)) {
				*out = cur;
				return (0);
			}
			cur = cur->next;
		...
		if (orig != node) {
			cur = node->ns;
			if ((cur != NULL) && (xmlStrEqual(cur->prefix, prefix)) && (cur->href != NULL)) {
				*out = cur;
				return (0);
			}
		}
 ...
 */
xmlNs* xmlSearchNs(xmlDoc* doc ATTRIBUTE_UNUSED, xmlNode* node, const xmlChar* nameSpace) {
	xmlNsPtr cur;
 ...
 */
static int xmlNsInScope(
	xmlDocPtr doc ATTRIBUTE_UNUSED, xmlNodePtr node, xmlNodePtr ancestor, const xmlChar* prefix
) {
	xmlNsPtr tst;
	...
	while ((node != NULL) && (node != ancestor)) {
		...
		if (node->type == XML_ELEMENT_NODE) {
			tst = node->nsDef;
			while (tst != NULL) {
				if ((tst->prefix == NULL) && (prefix == NULL)) {
					return (0);
				}
				if ((tst->prefix != NULL) && (prefix != NULL) && (xmlStrEqual(tst->prefix, prefix))) {
					return (0);
				}
				tst = tst->next;
			}
 ...
 * other errors.
 */
int xmlSearchNsByHrefSafe(xmlNode* node, const xmlChar* href, xmlNs** out) {
	xmlNsPtr   cur;
	xmlDocPtr  doc;
	xmlNodePtr orig = node;
	xmlNodePtr parent;
 ...
 * namespace is queried.
 */
xmlNs* xmlSearchNsByHref(xmlDoc* doc ATTRIBUTE_UNUSED, xmlNode* node, const xmlChar* href) {
	xmlNsPtr cur;
...
static int xmlGrowNsCache(xmlNsCache** cache, int* capacity) {
	xmlNsCache* tmp;
	int         newSize;

	newSize = xmlGrowCapacity(*capacity, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
	if (newSize < 0) {
		return (-1);
	}
		...
		if (node->ns != NULL) {
			...
			if (i == nbCache) {
				...
				if (n == NULL) {
					 ...
					 */
					if ((sizeCache <= nbCache) && (xmlGrowNsCache(&cache, &sizeCache) < 0)) {
						ret = -1;
					} else {
...
static xmlAttrPtr xmlGetPropNodeInternal(
	...
	if (node->properties != NULL) {
		...
		if (nsName == NULL) {
			...
			do {
				if ((prop->ns != NULL) && xmlStrEqual(prop->name, name)
				    && ((prop->ns->href == nsName) || xmlStrEqual(prop->ns->href, nsName))) {
					return (prop);
				}
				prop = prop->next;
			} while (prop != NULL);
	...
	if ((node->doc != NULL) && (node->doc->intSubset != NULL)) {
		...
		if (nsName == NULL) {
			 ...
			 */
			attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, elemQName, name, NULL);
			if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
				attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, elemQName, name, NULL);
			}
		} else if (xmlStrEqual(nsName, XML_XML_NAMESPACE)) {
			/*
			 * The XML namespace must be bound to prefix 'xml'.
			 */
			attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, elemQName, name, BAD_CAST "xml");
			if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
				attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, elemQName, name, BAD_CAST "xml");
			}
		} else {
			xmlNsPtr *nsList, *cur;
			...
			while (*cur != NULL) {
				if (xmlStrEqual((*cur)->href, nsName)) {
					attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, elemQName, name, (*cur)->prefix);
					if (attrDecl) {
						break;
					}
					if (doc->extSubset != NULL) {
						attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, elemQName, name, (*cur)->prefix);
						if (attrDecl) {
							break;
						}
					}
 ...
 * memory allocation failed.
 */
int xmlNodeGetAttrValue(
	const xmlNode* node, const xmlChar* name, const xmlChar* nsUri, xmlChar** out
) {
	xmlAttrPtr prop;
...
xmlAttr* xmlSetNsProp(xmlNode* node, xmlNs* ns, const xmlChar* name, const xmlChar* value) {
	...
	if (prop != NULL) {
		...
		if ((prop->atype == XML_ATTRIBUTE_ID) && (xmlAddIDSafe(prop, value) < 0)) {
			return (NULL);
		}
		return (prop);
	}
 ...
 */
static xmlNsMapItemPtr xmlDOMWrapNsMapAddItem(
	xmlNsMapPtr* nsmap, int position, xmlNsPtr oldNs, xmlNsPtr newNs, int depth
) {
	xmlNsMapItemPtr ret;
	xmlNsMapPtr     map;
 ...
 *          or internal error.
 */
static xmlNsPtr xmlDOMWrapStoreNs(xmlDocPtr doc, const xmlChar* nsName, const xmlChar* prefix) {
	xmlNsPtr ns;
	...
	if (ns->next != NULL) {
		...
		ns = ns->next;
		while (ns != NULL) {
			if (((ns->prefix == prefix) || xmlStrEqual(ns->prefix, prefix))
...
static xmlNsPtr xmlTreeNSListLookupByPrefix(xmlNsPtr nsList, const xmlChar* prefix) {
	...
	}
	{
		...
		ns = nsList;
		do {
			if ((prefix == ns->prefix) || xmlStrEqual(prefix, ns->prefix)) {
				return (ns);
			}
			ns = ns->next;
		} while (ns != NULL);
 ...
 */
static int xmlDOMWrapNSNormGatherInScopeNs(xmlNsMapPtr* map, xmlNodePtr node) {
	xmlNodePtr      cur;
	xmlNsPtr        ns;
	xmlNsMapItemPtr mi;
	int             shadowed;
 ...
 */
static int xmlDOMWrapNSNormAddNsMapItem2(
...
int xmlDOMWrapRemoveNode(
	xmlDOMWrapCtxt* ctxt, xmlDoc* doc, xmlNode* node, int options ATTRIBUTE_UNUSED
) {
	xmlNsPtr* list     = NULL;
			...
			case XML_ATTRIBUTE_NODE:
				if (node->ns != NULL) {
					...
					if (ctxt != NULL) {
						 ...
						 * Add to doc's oldNs.
						 */
						ns = xmlDOMWrapStoreNs(doc, node->ns->href, node->ns->prefix);
						if (ns == NULL) {
							ret = -1;
						}
					}
...
next_sibling:
		...
		if (node->next != NULL) {
			...
			node     = node->parent;
			if ((type == XML_ATTRIBUTE_NODE) && (node != NULL) && (node->children != NULL)) {
				node = node->children;
			} else {
				goto next_sibling;
			}
 ...
 *          and internal errors.
 */
static int xmlSearchNsByNamespaceStrict(
	xmlDocPtr doc, xmlNodePtr node, const xmlChar* nsName, xmlNsPtr* retNs, int prefixed
) {
		...
		if (cur->type == XML_ELEMENT_NODE) {
			if (cur->nsDef != NULL) {
				for (ns = cur->nsDef; ns != NULL; ns = ns->next) {
					...
					if (prev != NULL) {
						...
						do {
							if ((prevns->prefix == ns->prefix)
							    || ((prevns->prefix != NULL) && (ns->prefix != NULL) && xmlStrEqual(prevns->prefix, ns->prefix))) {
								/*
								 * Shadowed.
								 */
								break;
					 ...
					 * Ns-name comparison.
					 */
					if ((nsName == ns->href) || xmlStrEqual(nsName, ns->href)) {
						/*
 ...
 */
static int
xmlSearchNsByPrefixStrict(xmlDocPtr doc, xmlNodePtr node, const xmlChar* prefix, xmlNsPtr* retNs) {
	xmlNodePtr cur;
	xmlNsPtr   ns;
 ...
 *          and internal errors.
 */
static xmlNsPtr xmlDOMWrapNSNormDeclareNsForced(
	xmlDocPtr doc, xmlNodePtr elem, const xmlChar* nsName, const xmlChar* prefix, int checkShadow
) {
	xmlNsPtr       ret;
	char           buf[50];
	const xmlChar* pref;
	...
	while (1) {
		 ...
		 */
		if ((elem->nsDef != NULL) && (xmlTreeNSListLookupByPrefix(elem->nsDef, pref) != NULL)) {
			goto ns_next_prefix;
		}
		if (checkShadow && elem->parent && ((xmlNodePtr)elem->parent->doc != elem->parent)) {
			/*
 ...
 */
static int xmlDOMWrapNSNormAcquireNormalizedNs(
	xmlDocPtr    doc,
	xmlNodePtr   elem,
	xmlNsPtr     ns,
	xmlNsPtr*    retNs,
	...
	if ((doc == NULL) || (ns == NULL) || (retNs == NULL) || (nsMap == NULL)) {
		return (-1);
	}
	...
	*retNs = NULL;
	...
	if ((XML_NSMAP_NOTEMPTY(*nsMap)) && (!(ancestorsOnly && (elem == NULL)))) {
		...
		XML_NSMAP_FOREACH(*nsMap, mi) {
			if ((mi->depth >= XML_TREE_NSMAP_PARENT) &&
			    ...
			    ((mi->newNs->href != NULL) && (mi->newNs->href[0] != 0)) &&
			    /* Ensure a prefix if wanted. */
			    ((!prefixed) || (mi->newNs->prefix != NULL)) &&
			    /* Equal ns name */
			    ((mi->newNs->href == ns->href) || xmlStrEqual(mi->newNs->href, ns->href))) {
				/* Set the mapping. */
				mi->oldNs = ns;
				*retNs    = mi->newNs;
				return (0);
	...
	if (elem == NULL) {
		...
		*retNs = tmpns;
		...
		tmpns = xmlDOMWrapNSNormDeclareNsForced(doc, elem, ns->href, ns->prefix, 0);
		if (tmpns == NULL) {
			return (-1);
		}
		...
		if (*nsMap != NULL) {
			 ...
			 */
			XML_NSMAP_FOREACH(*nsMap, mi) {
				if ((mi->depth < depth) && (mi->shadowDepth == -1)
				    && ((ns->prefix == mi->newNs->prefix) || xmlStrEqual(ns->prefix, mi->newNs->prefix))) {
					/*
					 * Shadows.
					 */
...
typedef enum { XML_DOM_RECONNS_REMOVEREDUND = 1 << 0 } xmlDOMReconcileNSOptions;
 ...
 */
int
xmlDOMWrapReconcileNamespaces(xmlDOMWrapCtxt* ctxt ATTRIBUTE_UNUSED, xmlNode* elem, int options) {
		...
		switch (cur->type) {
			case XML_ELEMENT_NODE:
				...
				if (cur->nsDef != NULL) {
					...
					while (ns != NULL) {
						if (!parnsdone) {
							if ((elem->parent) && ((xmlNodePtr)elem->parent->doc != elem->parent)) {
								/*
						...
						if (optRemoveRedundantNS && XML_NSMAP_NOTEMPTY(nsMap)) {
							XML_NSMAP_FOREACH(nsMap, mi) {
								if ((mi->depth >= XML_TREE_NSMAP_PARENT) && (mi->shadowDepth == -1)
								    && ((ns->prefix == mi->newNs->prefix) || xmlStrEqual(ns->prefix, mi->newNs->prefix))
								    && ((ns->href == mi->newNs->href) || xmlStrEqual(ns->href, mi->newNs->href))) {
									/*
									 * A redundant ns-decl was found.
									 * Add it to the list of redundant ns-decls.
									 */
									if (xmlDOMWrapNSNormAddNsMapItem2(
												&listRedund, &sizeRedund, &nbRedund, ns, mi->newNs
											)
									    == -1) {
										ret = -1;
									} else {
...
next_ns_decl:
			...
			case XML_ATTRIBUTE_NODE:
				...
				if (xmlDOMWrapNSNormAcquireNormalizedNs(
							...
							depth,
							ancestorsOnly,
							(cur->type == XML_ATTRIBUTE_NODE) ? 1 : 0
						)
				    == -1) {
					ret = -1;
				}
...
next_sibling:
		...
		if (cur->type == XML_ELEMENT_NODE) {
			if (XML_NSMAP_NOTEMPTY(nsMap)) {
				 ...
				 * Pop mappings.
				 */
				while ((nsMap->last != NULL) && (nsMap->last->depth >= depth)) {
 ...
 * @returns 0 if succeeded, -1 otherwise and on API/internal errors.
 */
static int xmlDOMWrapAdoptBranch(
	 ...
	 */
	if ((destParent == NULL) || (ctxt && ctxt->getNsForNodeFunc)) {
		parnsdone = 1;
	} else {
		parnsdone = 0;
	}
...
leave_node:
		...
		if (cur->next != NULL) {
			cur = cur->next;
		} else if ((cur->type == XML_ATTRIBUTE_NODE) && (cur->parent->children != NULL)) {
...
end_ns_reference:
		 ...
		 */
		if ((clone->type == XML_ATTRIBUTE_NODE) && (clone->parent != NULL)) {
			int res;
 ...
 * @returns 0 if succeeded, -1 otherwise and on API/internal errors.
 */
static int xmlDOMWrapAdoptAttr(
	xmlDOMWrapCtxtPtr   ctxt,
	xmlDocPtr sourceDoc ATTRIBUTE_UNUSED,
	xmlAttrPtr          attr,
	xmlDocPtr           destDoc,
		...
		if (IS_STR_XML(attr->ns->prefix)) {
			...
			if (ns == NULL) {
				ns = xmlDOMWrapNSNormDeclareNsForced(
					destDoc, destParent, attr->ns->href, attr->ns->prefix, 1
				);
			}
		}
		if (ns == NULL) {
...
int xmlDOMWrapAdoptNode(
	...
	if ((node == NULL) || (node->type == XML_NAMESPACE_DECL) || (destDoc == NULL)
	    || ((destParent != NULL) && (destParent->doc != destDoc))) {
		return (-1);
	}
	/*
...
int xmlIsXHTML(const xmlChar* systemID, const xmlChar* publicID) {
	...
	if (publicID != NULL) {
		if (xmlStrEqual(publicID, XHTML_STRICT_PUBLIC_ID)) {
			return (1);
		}
		if (xmlStrEqual(publicID, XHTML_FRAME_PUBLIC_ID)) {
			return (1);
		}
		if (xmlStrEqual(publicID, XHTML_TRANS_PUBLIC_ID)) {
			return (1);
		}
	}
	if (systemID != NULL) {
		if (xmlStrEqual(systemID, XHTML_STRICT_SYSTEM_ID)) {
			return (1);
		}
		if (xmlStrEqual(systemID, XHTML_FRAME_SYSTEM_ID)) {
			return (1);
		}
		if (xmlStrEqual(systemID, XHTML_TRANS_SYSTEM_ID)) {
			return (1);
		}
	}
	return (0);
