...
static int LIBXML_ATTR_FORMAT(14, 15) xmlSetError(
	xmlError*     err,
	void*         ctxt,
	xmlNodePtr    node,
	int           domain,
	...
	va_start(ap, fmt);
	res = xmlVSetError(
		err, ctxt, node, domain, code, level, file, line, str1, str2, str3, int1, col, fmt, ap
	);
	va_end(ap);
...
static int xmlVUpdateError(
	...
	if (node != NULL) {
		...
		for (i = 0; i < 10; i++) {
			if ((node->type == XML_ELEMENT_NODE) || (node->parent == NULL)) {
				break;
			}
			node = node->parent;
		}
...
static void xmlParserPrintFileContextInternal(
...
void xmlFormatError(const xmlError* err, xmlGenericErrorFunc channel, void* data) {
	...
	if ((domain == XML_FROM_PARSER) || (domain == XML_FROM_HTML) || (domain == XML_FROM_DTD)
	    || (domain == XML_FROM_NAMESPACE) || (domain == XML_FROM_IO) || (domain == XML_FROM_VALID)) {
		ctxt = err->ctxt;
	}

	if ((node != NULL) && (node->type == XML_ELEMENT_NODE) && (domain != XML_FROM_SCHEMASV)) {
		name = node->name;
	}
	...
	if ((ctxt != NULL) && (ctxt->input != NULL)) {
		...
		if (file != NULL) {
			channel(data, "%s:%d: ", file, line);
		           ...
		           && ((domain == XML_FROM_PARSER) || (domain == XML_FROM_SCHEMASV) || (domain == XML_FROM_SCHEMASP) || (domain == XML_FROM_DTD) || (domain == XML_FROM_RELAXNGP) || (domain == XML_FROM_RELAXNGV))) {
	...
	if (ctxt != NULL) {
		if ((input != NULL) && ((input->buf == NULL) || (input->buf->encoder == NULL))
		    && (code == XML_ERR_INVALID_ENCODING) && (input->cur < input->end)) {
			int i;
	...
	if ((domain == XML_FROM_XPATH) && (err->str1 != NULL) && (err->int1 < 100)
...
int xmlVRaiseError(
	...
	if (xmlVUpdateError(
				to, ctxt, node, domain, code, level, file, line, str1, str2, str3, int1, col, msg, ap
			)) {
		return (-1);
	}
	...
	if (schannel != NULL) {
		...
		if ((channel == xmlGenericErrorDefaultFunc) || (channel == xmlParserError)
		    || (channel == xmlParserWarning) || (channel == xmlParserValidityError)
...
static void xmlVFormatLegacyError(void* ctx, const char* level, const char* fmt, va_list ap) {
 ...
 * @param ...  arguments to format
 */
void xmlParserError(void* ctx, const char* msg ATTRIBUTE_UNUSED, ...) {
	va_list ap;
 ...
 * @param ...  arguments to format
 */
void xmlParserWarning(void* ctx, const char* msg ATTRIBUTE_UNUSED, ...) {
	va_list ap;
 ...
 * @param ...  arguments to format
 */
void xmlParserValidityError(void* ctx, const char* msg ATTRIBUTE_UNUSED, ...) {
	va_list ap;
 ...
 * @param ...  arguments to format
 */
void xmlParserValidityWarning(void* ctx, const char* msg ATTRIBUTE_UNUSED, ...) {
	va_list ap;
 ...
 */
const char* xmlErrString(xmlParserErrors code) {
	const char* errmsg;
	...
	switch (code) {
		...
		case XML_IO_ECHILD:
			...
			break;
		case XML_IO_EDEADLK:
			errmsg = "Resource deadlock avoided";
			break;
		case XML_IO_EDOM:
			errmsg = "Domain error";
			break;
		...
		case XML_IO_EMSGSIZE:
			errmsg = "Inappropriate message buffer length";
			break;
		case XML_IO_ENAMETOOLONG:
			errmsg = "Filename too long";
			break;
		case XML_IO_ENFILE:
			errmsg = "Too many open files in system";
			break;
		case XML_IO_ENODEV:
		...
		case XML_IO_ENOENT:
			...
			break;
		case XML_IO_ENOEXEC:
			errmsg = "Exec format error";
			break;
		case XML_IO_ENOLCK:
			errmsg = "No locks available";
			break;
		case XML_IO_ENOMEM:
		...
		case XML_IO_ENOTSUP:
			...
			break;
		case XML_IO_ENOTTY:
			errmsg = "Inappropriate I/O control operation";
			break;
		case XML_IO_ENXIO:
		...
		case XML_IO_ETIMEDOUT:
			...
			break;
		case XML_IO_EXDEV:
			errmsg = "Improper link";
			break;
		case XML_IO_NETWORK_ATTEMPT:
			errmsg = "Attempt to load network entity";
			break;
		case XML_IO_ENCODER:
 ...
 * @param ap  arguments
 */
void xmlVPrintErrorMessage(const char* fmt, va_list ap) {
 ...
 * @param ...  arguments
 */
void xmlPrintErrorMessage(const char* fmt, ...) {
	va_list ap;
