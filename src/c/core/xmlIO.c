...
static int xmlIODefaultMatch(const char* filename);
 ...
 */
static int xmlSerializeHexCharRef(char* buf, int val) {
	char* out   = buf;
	int   shift = 0, bits;
 ...
 */
xmlChar* xmlEscapeText(const xmlChar* string, int flags) {
	const xmlChar*     cur;
	xmlChar*           buffer;
	xmlChar*           out;
	...
	while (*cur != 0) {
		...
		if (offset >= 0) {
			if (c == 0) {
				...
				replSize = xmlEscapeContent[offset], repl = &xmlEscapeContent[offset + 1];
				cur += 1;
			}
		} else {
			int val = 0, len = 4;
		...
		if (totalSize > size - used) {
			...
			if ((size > (SIZE_MAX - 1) / 2) || (totalSize > (SIZE_MAX - 1) / 2 - size)) {
				xmlFree(buffer);
				return (NULL);
			}
...
#ifdef LIBXML_OUTPUT_ENABLED
void
xmlSerializeText(xmlOutputBuffer* buf, const xmlChar* string, size_t maxSize, unsigned flags) {
	const xmlChar*     cur;
	const signed char* tab;
	...
	while (1) {
		...
		if (offset >= 0) {
			...
			xmlOutputBufferWrite(buf, xmlEscapeContent[offset], &xmlEscapeContent[offset + 1]);
			cur += 1;
		} else {
			char tempBuf[12];
			int  tempSize;
...
#ifdef EMSGSIZE
		case EMSGSIZE:
			code = XML_IO_EMSGSIZE;
			break;
#endif
#ifdef ENAMETOOLONG
		case ENAMETOOLONG:
			code = XML_IO_ENAMETOOLONG;
			break;
#endif
#ifdef ENFILE
		case ENFILE:
...
#ifdef ECONNREFUSED
		case ECONNREFUSED:
			code = XML_IO_ECONNREFUSED;
			break;
#endif
#ifdef ENETUNREACH
		case ENETUNREACH:
			code = XML_IO_ENETUNREACH;
			break;
#endif
#ifdef EADDRINUSE
		case EADDRINUSE:
			code = XML_IO_EADDRINUSE;
			break;
#endif
#ifdef EALREADY
		case EALREADY:
...
static int xmlConvertUriToPath(const char* uri, char** out) {
	const char* escaped;
	char*       unescaped;
 ...
 * @returns an xmlParserErrors code
 */
static xmlParserErrors xmlFdOpen(const char* filename, int write, int* out) {
	char*           fromUri = NULL;
	int             flags;
	int             fd;
	xmlParserErrors ret;
...
static int xmlFdFree(void* context) {
	xmlFree(context);
	return (XML_ERR_OK);
}
 ...
 * @returns an xmlParserErrors code
 */
static xmlParserErrors xmlFileOpenSafe(const char* filename, int write, void** out) {
	char*           fromUri = NULL;
	FILE*           fd;
 ...
 * @param context  the I/O context
 */
static int xmlFileFlush(void* context) {
	FILE* file = context;
 ...
 * @returns the number of bytes read.
 */
static int xmlGzfileRead(void* context, char* buffer, int len) {
	int ret;
 ...
 * @param context  the I/O context
 */
static int xmlGzfileClose(void* context) {
...
static int xmlIODefaultMatch(const char* filename ATTRIBUTE_UNUSED) {
	return (1);
}
...
static xmlFileOffset xmlSeek(int fd, xmlFileOffset offset, int whence) {
	#ifdef _WIN32
 ...
 * @returns an xmlParserErrors code.
 */
xmlParserErrors xmlInputFromFd(xmlParserInputBuffer* buf, int fd, xmlParserInputFlags flags) {
	xmlFdIOCtxt* fdctxt;
	int          copy;
	...
	if (flags & XML_INPUT_UNZIP) {
		...
		if (gzStream == NULL) {
			...
			if ((compressed) ||
			    ...
			    (pos < 0) || (xmlSeek(fd, pos, SEEK_SET) < 0)) {
				/*
...
int xmlOutputBufferClose(xmlOutputBuffer* out) {
	...
	if (out->closecallback != NULL) {
		...
		if ((code != XML_ERR_OK) && (!xmlIsCatastrophicError(XML_ERR_FATAL, out->error))) {
			if (code < 0) {
 ...
 * @returns an xmlParserErrors code.
 */
xmlParserErrors xmlParserInputBufferCreateUrl(
	const char* URI, xmlCharEncoding enc, xmlParserInputFlags flags, xmlParserInputBuffer** out
) {
	...
	*out = NULL;
	...
	for (i = xmlInputCallbackNr - 1; i >= 0; i--) {
		...
		} else if ((cb->matchcallback != NULL) && (cb->matchcallback(URI) != 0)) {
...
xmlOutputBuffer* xmlOutputBufferCreateBuffer(xmlBuffer* buffer, xmlCharEncodingHandler* encoder) {
	...
	ret = xmlOutputBufferCreateIO(xmlBufferWrite, NULL, (void*)buffer, encoder);
...
static int xmlMemClose(void* vctxt) {
	xmlMemIOCtxt* ctxt = vctxt;
 ...
 */
xmlParserInputBuffer* xmlNewInputBufferMemory(
	const void* mem, size_t size, xmlParserInputFlags flags, xmlCharEncoding enc
) {
	xmlParserInputBufferPtr ret;

	if ((flags & XML_INPUT_BUF_STATIC) && ((flags & XML_INPUT_BUF_ZERO_TERMINATED) == 0)) {
		xmlMemIOCtxt* ctxt;
 ...
 * @returns the new input buffer or NULL.
 */
xmlParserInputBuffer* xmlNewInputBufferString(const char* str, xmlParserInputFlags flags) {
	xmlParserInputBufferPtr ret;
 ...
 * @returns the new parser input or NULL
 */
xmlParserInputBuffer* xmlParserInputBufferCreateIO(
 ...
 * @returns the new parser output or NULL
 */
xmlOutputBuffer* xmlOutputBufferCreateIO(
...
int xmlParserInputBufferPush(xmlParserInputBuffer* in, int len, const char* buf) {
	...
	if (in->encoder != NULL) {
		 ...
		 */
		nbchars = SIZE_MAX;
		if (xmlCharEncInput(in, &nbchars, /* flush */ 0) != XML_ENC_ERR_SUCCESS) {
			return (-1);
		}
		if (nbchars > INT_MAX) {
			nbchars = INT_MAX;
...
static int endOfInput(
	void* context ATTRIBUTE_UNUSED, char* buffer ATTRIBUTE_UNUSED, int len ATTRIBUTE_UNUSED
) {
	return (0);
}
...
int xmlParserInputBufferGrow(xmlParserInputBuffer* in, int len) {
	...
	if (in->encoder != NULL) {
		...
		if (xmlCharEncInput(in, &sizeOut, /* flush */ 0) != XML_ENC_ERR_SUCCESS) {
			return (-1);
		}
		res = sizeOut;
	}
 ...
 *         in case of error.
 */
int xmlOutputBufferWrite(xmlOutputBuffer* out, int len, const char* data) {
	xmlBufPtr buf     = NULL;
	size_t    written = 0;
	int       ret;
	...
	if ((buf != NULL) && (out->writecallback)) {
		...
		while (1) {
			...
			ret = out->writecallback(out->context, (const char*)xmlBufContent(buf), nbchars);
			if (ret < 0) {
...
int xmlOutputBufferWriteEscape(
	xmlOutputBuffer* out, const xmlChar* str, xmlCharEncodingOutputFunc escaping
) {
	int    ret;
	int    written = 0;
	size_t len;
...
int xmlOutputBufferFlush(xmlOutputBuffer* out) {
	...
	if ((out->conv != NULL) && (out->encoder != NULL) && (out->writecallback != NULL)) {
		ret = out->writecallback(
			out->context, (const char*)xmlBufContent(out->conv), xmlBufUse(out->conv)
		);
		if (ret >= 0) {
		...
		ret = out->writecallback(
			out->context, (const char*)xmlBufContent(out->buffer), xmlBufUse(out->buffer)
		);
		if (ret >= 0) {
 ...
 * @returns 0, 1, or 2.
 */
int xmlNoNetExists(const char* filename) {
	char* fromUri;
	int   ret;
