...
void xmlFreeEntity(xmlEntity* entity) {
	...
	if ((entity->children) && (entity == (xmlEntityPtr)entity->children->parent)) {
		xmlFreeNodeList(entity->children);
	}
	if ((entity->name != NULL) && ((dict == NULL) || (!xmlDictOwns(dict, entity->name)))) {
 ...
 */
static xmlEntityPtr xmlCreateEntity(
	xmlDocPtr      doc,
	const xmlChar* name,
	int            type,
	...
	switch (type) {
		...
		case XML_EXTERNAL_GENERAL_UNPARSED_ENTITY:
			...
			if (predef != NULL) {
				...
				if ((type == XML_INTERNAL_GENERAL_ENTITY) && (content != NULL)) {
 ...
 * @param name  its name
 */
static void xmlFreeEntityWrapper(void* entity, const xmlChar* name ATTRIBUTE_UNUSED) {
	if (entity != NULL) {
 ...
 */
static void* xmlCopyEntity(void* payload, const xmlChar* name ATTRIBUTE_UNUSED) {
 ...
 * @param name  unused
 */
static void xmlDumpEntityDeclScan(void* ent, void* save, const xmlChar* name ATTRIBUTE_UNUSED) {
