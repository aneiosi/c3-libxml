...
static unsigned xmlHashValue(
	unsigned seed, const xmlChar* key, const xmlChar* key2, const xmlChar* key3, size_t* lengths
) {
	unsigned h1, h2;
	size_t   i;
...
ATTRIBUTE_NO_SANITIZE_INTEGER
static unsigned xmlHashQNameValue(
	unsigned       seed,
...
static int xmlFastStrEqual(const xmlChar* s1, const xmlChar* s2) {
	if (s1 == NULL) {
		...
		return ((s2 != NULL) && (strcmp((const char*)s1, (const char*)s2) == 0));
	}
}
 ...
 */
ATTRIBUTE_NO_SANITIZE_INTEGER
static xmlHashEntry* xmlHashFindEntry(
	const xmlHashTable* hash,
	const xmlChar*      key,
	const xmlChar*      key2,
	const xmlChar*      key3,
	unsigned            hashValue,
	...
	if (entry->hashValue != 0) {
		...
		do {
			if (entry->hashValue == hashValue) {
				if (hash->dict) {
					if ((entry->key == key) && (entry->key2 == key2) && (entry->key3 == key3)) {
						found = 1;
						break;
					}
				}
				if ((strcmp((const char*)entry->key, (const char*)key) == 0)
				    && (xmlFastStrEqual(entry->key2, key2)) && (xmlFastStrEqual(entry->key3, key3))) {
					found = 1;
					break;
				}
			}
 ...
 */
ATTRIBUTE_NO_SANITIZE_INTEGER
static int xmlHashUpdateInternal(
	xmlHashTable*      hash,
	const xmlChar*     key,
	const xmlChar*     key2,
	const xmlChar*     key3,
	...
	if (entry->hashValue != 0) {
		...
		if (cur < entry) {
			 ...
			 */
			memmove(&hash->table[1], hash->table, (char*)cur - (char*)hash->table);
			cur            = end - 1;
			hash->table[0] = *cur;
		}
 ...
 */
int xmlHashAdd(xmlHashTable* hash, const xmlChar* key, void* payload) {
 ...
 * @returns 1 on success, 0 if an entry exists and -1 in case of error.
 */
int xmlHashAdd2(xmlHashTable* hash, const xmlChar* key, const xmlChar* key2, void* payload) {
 ...
 */
int xmlHashAdd3(
	xmlHashTable* hash, const xmlChar* key, const xmlChar* key2, const xmlChar* key3, void* payload
) {
 ...
 */
int xmlHashAddEntry(xmlHashTable* hash, const xmlChar* key, void* payload) {
 ...
 * @returns 0 on success and -1 in case of error.
 */
int xmlHashAddEntry2(xmlHashTable* hash, const xmlChar* key, const xmlChar* key2, void* payload) {
...
int xmlHashUpdateEntry(
	xmlHashTable* hash, const xmlChar* key, void* payload, xmlHashDeallocator dealloc
) {
	int res = xmlHashUpdateInternal(hash, key, NULL, NULL, payload, dealloc, 1);
...
int xmlHashUpdateEntry2(
	...
	int res = xmlHashUpdateInternal(hash, key, key2, NULL, payload, dealloc, 1);
...
int xmlHashUpdateEntry3(
	...
	int res = xmlHashUpdateInternal(hash, key, key2, key3, payload, dealloc, 1);
 ...
 */
void* xmlHashLookup(xmlHashTable* hash, const xmlChar* key) {
 ...
 * @returns a pointer to the payload or NULL if no entry was found.
 */
void* xmlHashLookup2(xmlHashTable* hash, const xmlChar* key, const xmlChar* key2) {
 ...
 */
void*
xmlHashLookup3(xmlHashTable* hash, const xmlChar* key, const xmlChar* key2, const xmlChar* key3) {
	const xmlHashEntry* entry;
	unsigned            hashValue;
	int                 found;
...
void* xmlHashQLookup3(
	...
	hashValue = xmlHashQNameValue(hash->randomSeed, prefix, name, prefix2, name2, prefix3, name3);
	mask      = hash->size - 1;
	pos       = hashValue & mask;
	entry     = &hash->table[pos];
	...
	if (entry->hashValue != 0) {
		...
		do {
			if ((hashValue == entry->hashValue) && (xmlStrQEqual(prefix, name, entry->key))
	...
	for (i = 0; i < hash->size; i++) {
		if ((entry->hashValue != 0) && (entry->payload != NULL)) {
			...
			do {
				...
				(entry->hashValue != 0) && (entry->payload != NULL)
				&& ((entry->key != old.key) || (entry->key2 != old.key2) || (entry->key3 != old.key3))
			);
		}
		if (++entry >= end) {
			entry = hash->table;
...
void xmlHashScanFull3(
	...
	for (i = 0; i < hash->size; i++) {
		if ((entry->hashValue != 0) && (entry->payload != NULL)) {
			 ...
			 */
			do {
				if (((key != NULL) && (strcmp((const char*)key, (const char*)entry->key) != 0))
 ...
 */
xmlHashTable*
xmlHashCopySafe(xmlHashTable* hash, xmlHashCopier copyFunc, xmlHashDeallocator deallocFunc) {
	const xmlHashEntry *entry, *end;
	xmlHashTablePtr     ret;
	...
	for (entry = hash->table; entry < end; entry++) {
		if (entry->hashValue != 0) {
			...
			if (xmlHashAdd3(ret, entry->key, entry->key2, entry->key3, copy) <= 0) {
				if (deallocFunc != NULL) {
 ...
 */
int xmlHashRemoveEntry(xmlHashTable* hash, const xmlChar* key, xmlHashDeallocator dealloc) {
...
int xmlHashRemoveEntry2(
	xmlHashTable* hash, const xmlChar* key, const xmlChar* key2, xmlHashDeallocator dealloc
) {
...
int xmlHashRemoveEntry3(
	...
	while (1) {
		...
		if ((next->hashValue == 0) || (((next->hashValue - nextpos) & mask) == 0)) {
			break;
		}
