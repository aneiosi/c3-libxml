 ...
 * @param str1  extra data
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlErrId(
	xmlParserCtxtPtr ctxt, xmlParserErrors error, const char* msg, const xmlChar* str1
) {
	xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_ERROR, str1, NULL, NULL, 0, msg, str1);
}
 ...
 * @param str1  an error string
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlWarnMsg(
	xmlParserCtxtPtr ctxt, xmlParserErrors error, const char* msg, const xmlChar* str1
) {
	xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_WARNING, str1, NULL, NULL, 0, msg, str1);
}
 ...
 * @param str2  an error string
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlNsWarnMsg(
	xmlParserCtxtPtr ctxt,
	xmlParserErrors  error,
	const char*      msg,
	const xmlChar*   str1,
	...
	xmlCtxtErr(
		ctxt, NULL, XML_FROM_NAMESPACE, error, XML_ERR_WARNING, str1, str2, NULL, 0, msg, str1, str2
	);
}
...
void xmlSAX2InternalSubset(
	void* ctx, const xmlChar* name, const xmlChar* publicId, const xmlChar* systemId
) {
	xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr)ctx;
	xmlDtdPtr        dtd;
	if (ctx == NULL) {
	...
	ctxt->myDoc->intSubset = xmlCreateIntSubset(ctxt->myDoc, name, publicId, systemId);
...
void xmlSAX2ExternalSubset(
	...
	if ((systemId != NULL) && ((ctxt->options & XML_PARSE_NO_XXE) == 0)
	    && (((ctxt->validate) || (ctxt->loadsubset & ~XML_SKIP_IDS)) && (ctxt->wellFormed && ctxt->myDoc))) {
		/*
		...
		if ((ctxt->sax != NULL) && (ctxt->sax->resolveEntity != NULL)) {
			input = ctxt->sax->resolveEntity(ctxt->userData, publicId, systemId);
		}
		if (input == NULL) {
			return;
		}
...
xmlParserInput* xmlSAX2ResolveEntity(void* ctx, const xmlChar* publicId, const xmlChar* systemId) {
	...
	if (systemId != NULL) {
		...
		if ((xmlStrlen(systemId) > XML_MAX_URI_LENGTH) || (xmlStrlen(base) > XML_MAX_URI_LENGTH)) {
		...
		if (URI == NULL) {
			if (res < 0) {
				...
				xmlWarnMsg(ctxt, XML_ERR_INVALID_URI, "Can't resolve URI: %s\n", systemId);
			}
			return (NULL);
		}
	...
	ret = xmlLoadResource(ctxt, (const char*)URI, (const char*)publicId, XML_RESOURCE_DTD);
...
void xmlSAX2EntityDecl(
	...
	res       = xmlAddEntity(ctxt->myDoc, extSubset, name, type, publicId, systemId, content, &ent);
	switch (res) {
		case XML_ERR_OK:
			break;
			...
			if (ctxt->pedantic) {
				if (extSubset) {
					xmlWarnMsg(
						ctxt,
						res,
						"Entity(%s) already defined in the"
						" external subset\n",
						name
					);
				} else {
...
void xmlSAX2ElementDecl(void* ctx, const xmlChar* name, int type, xmlElementContent* content) {
	...
	if (ctxt->inSubset == 1) {
		elem = xmlAddElementDecl(
			&ctxt->vctxt, ctxt->myDoc->intSubset, name, (xmlElementTypeVal)type, content
		);
	} else if (ctxt->inSubset == 2) {
		elem = xmlAddElementDecl(
			&ctxt->vctxt, ctxt->myDoc->extSubset, name, (xmlElementTypeVal)type, content
		);
	} else {
		xmlFatalErrMsg(
			ctxt,
...
void xmlSAX2NotationDecl(
	...
	if ((ctxt->validate) && (ctxt->wellFormed) && (ctxt->myDoc->intSubset != NULL)) {
...
void xmlSAX2UnparsedEntityDecl(
	...
	xmlSAX2EntityDecl(
		ctx, name, XML_EXTERNAL_GENERAL_UNPARSED_ENTITY, publicId, systemId, (xmlChar*)notationName
	);
}
...
void xmlSAX2StartDocument(void* ctx) {
	...
	if ((ctxt->myDoc != NULL) && (ctxt->myDoc->URL == NULL) && (ctxt->input != NULL)
...
#ifdef LIBXML_HTML_ENABLED
	if (ctxt->html) {
		if (((ctxt->options & HTML_PARSE_NODEFDTD) == 0) && (doc->intSubset == NULL)) {
			doc->intSubset = xmlCreateIntSubset(
				doc,
				BAD_CAST "html",
...
static void xmlSAX2AppendChild(xmlParserCtxtPtr ctxt, xmlNodePtr node) {
	xmlNodePtr parent;
	xmlNodePtr last;
	...
	if ((node->type != XML_TEXT_NODE) && (ctxt->input != NULL)) {
 ...
 * @param str2  an error string
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlNsErrMsg(
	xmlParserCtxtPtr ctxt,
	xmlParserErrors  error,
	const char*      msg,
	const xmlChar*   str1,
	...
	xmlCtxtErr(
		ctxt, NULL, XML_FROM_NAMESPACE, error, XML_ERR_ERROR, str1, str2, NULL, 0, msg, str1, str2
	);
}
 ...
 */
static void xmlSAX1Attribute(
	xmlParserCtxtPtr ctxt, const xmlChar* fullname, const xmlChar* value, const xmlChar* prefix
) {
	xmlAttrPtr     ret;
	const xmlChar* name;
	xmlChar*       ns;
	 ...
	 */
	if ((ns == NULL) && (name[0] == 'x') && (name[1] == 'm') && (name[2] == 'l') && (name[3] == 'n')
	    && (name[4] == 's') && (name[5] == 0)) {
		xmlNsPtr nsret;
		xmlChar* val;
		...
		if (val[0] != 0) {
			...
			if (uri == NULL) {
				...
				if (uri->scheme == NULL) {
					xmlNsWarnMsg(
						ctxt, XML_WAR_NS_URI_RELATIVE, "xmlns:%s: URI %s is not absolute\n", name, value
					);
				}
				xmlFreeURI(uri);
			}
		 ...
		 */
		else if (ctxt->validate && ctxt->wellFormed && ctxt->myDoc && ctxt->myDoc->intSubset) {
			ctxt->valid
				&= xmlValidateOneNamespace(&ctxt->vctxt, ctxt->myDoc, ctxt->node, prefix, nsret, val);
		}
	#endif /* LIBXML_VALID_ENABLED */
		if (val != value) {
			xmlFree(val);
	...
	if ((ns != NULL) && (ns[0] == 'x') && (ns[1] == 'm') && (ns[2] == 'l') && (ns[3] == 'n')
	    && (ns[4] == 's') && (ns[5] == 0)) {
		xmlNsPtr nsret;
		xmlChar* val;
		...
		if (val[0] == 0) {
			xmlNsErrMsg(ctxt, XML_NS_ERR_EMPTY, "Empty namespace name for prefix %s\n", name, NULL);
		}
		 ...
		 */
			...
			ctxt->valid
				&= xmlValidateOneNamespace(&ctxt->vctxt, ctxt->myDoc, ctxt->node, prefix, nsret, value);
		}
	#endif /* LIBXML_VALID_ENABLED */
		if (val != value) {
			xmlFree(val);
			...
			while (prop != NULL) {
				if (prop->ns != NULL) {
					if ((xmlStrEqual(name, prop->name))
					    && ((namespace == prop->ns) || (xmlStrEqual(namespace->href, prop->ns->href)))) {
						xmlCtxtErr(
							ctxt,
							NULL,
							XML_FROM_PARSER,
		...
		if (!ctxt->replaceEntities) {
			...
			if (val == NULL) {
				...
				nvalnorm = xmlValidCtxtNormalizeAttributeValue(
					&ctxt->vctxt, ctxt->myDoc, ctxt->node, fullname, val
				);
				if (nvalnorm != NULL) {
					xmlFree(val);
					val = nvalnorm;
	...
	#endif /* LIBXML_VALID_ENABLED */
		if (((ctxt->loadsubset & XML_SKIP_IDS) == 0) && (ctxt->input->entity == NULL) &&
		    /* Don't create IDs containing entity references */
		    (ret->children != NULL) && (ret->children->type == XML_TEXT_NODE)
			...
			if (xmlStrEqual(fullname, BAD_CAST "xml:id")) {
				...
				if (xmlValidateNCName(content, 1) != 0) {
					xmlErrId(
						ctxt, XML_DTD_XMLID_VALUE, "xml:id : attribute value %s is not an NCName\n", content
					);
				}
 ...
 * Deprecated SAX1 interface.
 */
static void xmlCheckDefaultedAttributes(
	xmlParserCtxtPtr ctxt, const xmlChar* name, const xmlChar* prefix, const xmlChar** atts
) {
	...
	if (elemDecl != NULL) {
		...
		while (attr != NULL) {
			...
			if (attr->defaultValue != NULL) {
				 ...
				 */
				if (((attr->prefix != NULL) && (xmlStrEqual(attr->prefix, BAD_CAST "xmlns")))
				    || ((attr->prefix == NULL) && (xmlStrEqual(attr->name, BAD_CAST "xmlns")))
				    || (ctxt->loadsubset & XML_COMPLETE_ATTRS)) {
					xmlAttributePtr tst;

					tst = xmlGetDtdQAttrDesc(ctxt->myDoc->intSubset, attr->elem, attr->name, attr->prefix);
					if ((tst == attr) || (tst == NULL)) {
						...
						if (att == NULL) {
							xmlSAX1Attribute(ctxt, fulln, attr->defaultValue, prefix);
						}
 ...
 */
static void xmlSAX1StartElement(void* ctx, const xmlChar* fullname, const xmlChar** atts) {
	xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr)ctx;
	xmlNodePtr       ret;
	xmlNodePtr       parent;
	xmlNsPtr         ns;
	...
	if ((ctx == NULL) || (fullname == NULL) || (ctxt->myDoc == NULL)) {
		return;
	}
	...
	if (ctxt->validate && (ctxt->myDoc->extSubset == NULL) &&
	 ...
	 ((ctxt->myDoc->intSubset->notations == NULL) &&
	  ...
	  (ctxt->myDoc->intSubset->entities == NULL)))) {
		xmlErrValid(ctxt, XML_ERR_NO_DTD, "Validation failed: no DTD found !", NULL, NULL);
		ctxt->validate = 0;
	}
	#endif
	 ...
	 * namespaces
	 */
	if ((ctxt->myDoc->intSubset != NULL) || (ctxt->myDoc->extSubset != NULL)) {
	...
	if (atts != NULL) {
		...
		while ((att != NULL) && (value != NULL)) {
			if ((att[0] == 'x') && (att[1] == 'm') && (att[2] == 'l') && (att[3] == 'n')
			    && (att[4] == 's')) {
	...
	if ((prefix != NULL) && (ns == NULL)) {
		xmlNsWarnMsg(
			ctxt, XML_NS_ERR_UNDEFINED_NAMESPACE, "Namespace prefix %s is not defined\n", prefix, NULL
		);
	 ...
	 */
	if ((ns != NULL) && (ns->href != NULL) && ((ns->href[0] != 0) || (ns->prefix != NULL))) {
		xmlSetNs(ret, ns);
	}
	 ...
	 */
	if ((ctxt->validate) && ((ctxt->vctxt.flags & XML_VCTXT_DTD_VALIDATED) == 0)) {
		int chk;
...
#ifdef LIBXML_HTML_ENABLED
static void
xmlSAX2HtmlAttribute(xmlParserCtxtPtr ctxt, const xmlChar* fullname, const xmlChar* value) {
	xmlAttrPtr ret;
	xmlChar*   nval = NULL;
 ...
 */
static void
xmlSAX2StartHtmlElement(xmlParserCtxtPtr ctxt, const xmlChar* fullname, const xmlChar** atts) {
	xmlNodePtr     ret;
	xmlNodePtr     parent;
	const xmlChar* att;
	const xmlChar* value;
...
void xmlSAX2EndElement(void* ctx, const xmlChar* name ATTRIBUTE_UNUSED) {
	...
	if (!ctxt->html && ctxt->validate && ctxt->wellFormed && ctxt->myDoc && ctxt->myDoc->intSubset) {
		ctxt->valid &= xmlValidateOneElement(&ctxt->vctxt, ctxt->myDoc, ctxt->node);
	}
#endif /* LIBXML_VALID_ENABLED */
 ...
 */
static xmlNodePtr
xmlSAX2TextNode(xmlParserCtxtPtr ctxt, xmlDocPtr doc, const xmlChar* str, int len) {
	xmlNodePtr     ret;
 ...
 */
static xmlChar*
xmlSAX2DecodeAttrEntities(xmlParserCtxtPtr ctxt, const xmlChar* str, const xmlChar* end) {
	const xmlChar* in;
 ...
 */
static xmlAttrPtr xmlSAX2AttributeNs(
	xmlParserCtxtPtr ctxt,
	...
	if ((ctxt->replaceEntities == 0) && (!ctxt->html)) {
		...
		if (*valueend != 0) {
			...
			if (xmlNodeParseAttValue(ret->doc, ret, value, valueend - value, NULL) < 0) {
				xmlSAX2ErrMemory(ctxt);
			}
		}
	} else if (value != NULL) {
...
#ifdef LIBXML_VALID_ENABLED
	if ((!ctxt->html) && ctxt->validate && ctxt->wellFormed && ctxt->myDoc
	    && ctxt->myDoc->intSubset) {
		/*
		 * If we don't substitute entities, the validation should be
		 * done on a value with replaced entities anyway.
		...
		if (!ctxt->replaceEntities) {
			...
			if (dup == NULL) {
				if (*valueend == 0) {
					ctxt->valid
						&= xmlValidateOneAttribute(&ctxt->vctxt, ctxt->myDoc, ctxt->node, ret, value);
				} else {
					/*
				...
				if (ctxt->attsSpecial != NULL) {
					...
					if (fullname == NULL) {
						...
						ctxt->vctxt.valid = 1;
						nvalnorm          = xmlValidCtxtNormalizeAttributeValue(
              &ctxt->vctxt, ctxt->myDoc, ctxt->node, fullname, dup
            );
		...
		if (((ctxt->loadsubset & XML_SKIP_IDS) == 0) && (ctxt->input->entity == NULL) &&
			 ...
			 * validation level. Otherwise we have to do specific handling here.
			 */
			if ((prefix == ctxt->str_xml) && (localname[0] == 'i') && (localname[1] == 'd')
			    && (localname[2] == 0)) {
				/*
				 * Add the xml:id value
				 *
...
void xmlSAX2StartElementNs(
	...
	if (ctxt->validate &&
	   ...
	   ((ctxt->myDoc->intSubset->notations == NULL) &&
	    ...
	    (ctxt->myDoc->intSubset->attributes == NULL) &&
	    (ctxt->myDoc->intSubset->entities == NULL)))))) {
		xmlErrValid(ctxt, XML_DTD_NO_DTD, "Validation failed: no DTD found !", NULL, NULL);
		ctxt->validate = 0;
	}
#endif /* LIBXML_VALID_ENABLED */
	...
	if (ctxt->freeElems != NULL) {
		...
		if (ctxt->dictNames) {
			ret = xmlNewDocNodeEatName(ctxt->myDoc, NULL, (xmlChar*)localname, NULL);
		} else if (lname == NULL) {
	...
	if ((URI != NULL) && (ret->ns == NULL)) {
		...
		if (ret->ns == NULL) {
			...
			if (prefix != NULL) {
				xmlNsWarnMsg(
					ctxt, XML_NS_ERR_UNDEFINED_NAMESPACE, "Namespace prefix %s was not found\n", prefix, NULL
				);
			} else {
				xmlNsWarnMsg(
					ctxt,
		...
		for (j = 0, i = 0; i < nb_attributes; i++, j += 5) {
			...
			if ((attributes[j + 1] != NULL) && (attributes[j + 2] == NULL)) {
				if (ctxt->dictNames) {
					...
					attr = xmlSAX2AttributeNs(ctxt, fullname, NULL, attributes[j + 3], attributes[j + 4]);
					goto have_attr;
				} else {
					...
					attr = xmlSAX2AttributeNs(ctxt, lname, NULL, attributes[j + 3], attributes[j + 4]);
					xmlFree(lname);
					goto have_attr;
				}
			}
			attr = xmlSAX2AttributeNs(
				ctxt, attributes[j], attributes[j + 1], attributes[j + 3], attributes[j + 4]
			);
have_attr:
			if (attr == NULL) {
				continue;
...
static void xmlSAX2Text(xmlParserCtxtPtr ctxt, const xmlChar* ch, int len, xmlElementType type) {
	 ...
	 */
	if ((lastChild == NULL) || (lastChild->type != type)
		...
		int maxSize = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH : XML_MAX_TEXT_LENGTH;
		...
		if ((content == NULL) || (capacity <= 0)) {
			xmlFatalErr(ctxt, XML_ERR_INTERNAL_ERROR, "xmlSAX2Text: no content");
			return;
		}
		...
		if (newSize >= capacity) {
			...
			if ((content == (xmlChar*)&lastChild->properties)
			    || ((ctxt->nodemem == oldSize + 1) && (xmlDictOwns(ctxt->dict, content)))) {
				xmlChar* newContent;
	...
	if ((lastChild != NULL) && (type == XML_TEXT_NODE) && (ctxt->input != NULL)) {
		if ((unsigned)ctxt->input->line < (unsigned)USHRT_MAX) {
...
void xmlSAX2IgnorableWhitespace(
	void* ctx ATTRIBUTE_UNUSED, const xmlChar* ch ATTRIBUTE_UNUSED, int len ATTRIBUTE_UNUSED
) {}
