 ...
 */
   ...
   */
	#if defined(XML_THREAD_LOCAL) && !defined(__APPLE__) && !defined(USE_WAIT_DTOR)
		#define USE_TLS
	#endif
...
const xmlSAXLocator xmlDefaultSAXLocator
	= {xmlSAX2GetPublicId, xmlSAX2GetSystemId, xmlSAX2GetLineNumber, xmlSAX2GetColumnNumber};
...
static void xmlInitGlobalState(xmlGlobalStatePtr gs);
...
static void WINAPI xmlGlobalStateDtor(void* ctxt, unsigned char timedOut ATTRIBUTE_UNUSED) {
...
static int xmlRegisterGlobalStateDtor(xmlGlobalState* gs) {
	...
	if (RegisterWaitForSingleObject(
				&waitHandle, threadHandle, xmlGlobalStateDtor, gs, INFINITE, WT_EXECUTEONLYONCE
			)
	    == 0) {
 ...
 */
static xmlGlobalStatePtr xmlNewGlobalState(int allowFailure) {
	xmlGlobalState* gs;
...
static xmlGlobalStatePtr xmlGetThreadLocalStorage(int allowFailure) {
	xmlGlobalState* gs;
...
static xmlGlobalStatePtr xmlGetThreadLocalStorage(int allowFailure ATTRIBUTE_UNUSED) {
	return (&globalState);
}
...
xmlMallocFunc* __xmlMallocAtomic(void) {
	return (&xmlGetThreadLocalStorage(0)->mallocAtomic);
}

xmlReallocFunc* __xmlRealloc(void) {
...
xmlStrdupFunc* __xmlMemStrdup(void) {
 ...
 */
unsigned* xmlGetLocalRngState(void) {
 ...
 */
xmlError* xmlGetLastErrorInternal(void) {
	return (&xmlGetThreadLocalStorage(0)->lastError);
}
...
int xmlDllMain(
	ATTRIBUTE_UNUSED void* hinstDLL, unsigned long fdwReason, ATTRIBUTE_UNUSED void* lpvReserved
)
	#else
...
XMLPUBFUN BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved);

BOOL WINAPI
DllMain(ATTRIBUTE_UNUSED HINSTANCE hinstDLL, DWORD fdwReason, ATTRIBUTE_UNUSED LPVOID lpvReserved)
	#endif
{
