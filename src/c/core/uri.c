...
#define ISA_STRICTLY_UNRESERVED(p)                                                           \
	((ISA_ALPHA(p)) || (ISA_DIGIT(p)) || ((*(p) == '-')) || ((*(p) == '.')) || ((*(p) == '_')) \
	 || ((*(p) == '~')))
...
#define ISA_PCHAR(u, p)                                                              \
	(ISA_UNRESERVED(u, p) || ISA_PCT_ENCODED(p) || ISA_SUB_DELIM(p) || ((*(p) == ':')) \
	 || ((*(p) == '@')))
...
static int xmlIsUnreserved(xmlURIPtr uri, const char* cur) {
	if (uri == NULL) {
		return (0);
	}
...
static int xmlParse3986Scheme(xmlURIPtr uri, const char** str) {
	...
	while (ISA_ALPHA(cur) || ISA_DIGIT(cur) || (*cur == '+') || (*cur == '-') || (*cur == '.')) {
		cur++;
	}
	if (uri != NULL) {
		if (uri->scheme != NULL) {
...
static int xmlParse3986Fragment(xmlURIPtr uri, const char** str) {
	...
	while ((ISA_PCHAR(uri, cur)) || (*cur == '/') || (*cur == '?') || (*cur == '[')
	       || (*cur == ']')) {
		NEXT(cur);
	}
	if (uri != NULL) {
...
static int xmlParse3986Userinfo(xmlURIPtr uri, const char** str) {
	...
	cur = *str;
	while (ISA_UNRESERVED(uri, cur) || ISA_PCT_ENCODED(cur) || ISA_SUB_DELIM(cur) || (*cur == ':')) {
		NEXT(cur);
	}
	if (*cur == '@') {
		if (uri != NULL) {
...
not_ipv4:
	 ...
	 */
	while (ISA_UNRESERVED(uri, cur) || ISA_PCT_ENCODED(cur) || ISA_SUB_DELIM(cur)) {
		NEXT(cur);
	}
found:
	if (uri != NULL) {
 ...
 * @returns 0 or the error code
 */
static int xmlParse3986Segment(xmlURIPtr uri, const char** str, char forbid, int empty) {
	const char* cur;
 ...
 */
int xmlParseURISafe(const char* str, xmlURI** uriOut) {
	xmlURIPtr uri;
	int       ret;
	...
	if (uri->opaque != NULL) {
		...
		if (uri->path != NULL) {
			...
			if ((uri->scheme != NULL) && (p[0] == '/')
			    ...
			    && (p[2] == ':') && (xmlStrEqual(BAD_CAST uri->scheme, BAD_CAST "file"))) {
				if (len + 3 >= max) {
					temp = xmlSaveUriRealloc(ret, &max);
					if (temp == NULL) {
						goto mem_error;
 ...
 */
static int xmlNormalizePath(char* path, int isFile) {
	char *cur, *out;
	int   numSeg = 0;
	...
	while (*cur != 0) {
		...
		if (*cur == '.') {
			if (cur[1] == 0) {
				...
				cur += 2;
				continue;
			} else if ((cur[1] == '.') && ((cur[2] == 0) || xmlIsPathSeparator(cur[2], isFile))) {
				if (numSeg > 0) {
					/* Handle ".." by removing last segment */
					do {
						out--;
					} while ((out > path) && !xmlIsPathSeparator(out[-1], isFile));
					numSeg--;
...
static int xmlIsAbsolutePath(const xmlChar* path) {
	int c = path[0];
 ...
 */
static int xmlResolvePath(const xmlChar* escRef, const xmlChar* base, xmlChar** out) {
 ...
 * code if URI or base are invalid.
 */
int xmlBuildURISafe(const xmlChar* URI, const xmlChar* base, xmlChar** valPtr) {
	xmlChar*  val = NULL;
	...
	*valPtr = NULL;
	...
	if ((ref->scheme == NULL) && (ref->path == NULL)
	    && ((ref->authority == NULL) && (ref->server == NULL) && (ref->port == PORT_EMPTY))) {
		if (bas->scheme != NULL) {
	 ...
	 */
	if ((ref->authority != NULL) || (ref->server != NULL) || (ref->port != PORT_EMPTY)) {
...
static int xmlParseUriOrPath(const char* str, xmlURIPtr* out, int* drive) {
	xmlURIPtr uri;
	char*     buf = NULL;
	int       ret;
		...
		if (xmlIsAbsolutePath(BAD_CAST buf)) {
			...
			if (server != NULL) {
				...
				if (end == NULL) {
					...
					uri->server = (char*)xmlStrndup(BAD_CAST server, end - server);
					path        = end;
				}
				if (uri->server == NULL) {
					ret = -1;
 ...
 * code if URI or base are invalid.
 */
int xmlBuildRelativeURISafe(const xmlChar* URI, const xmlChar* base, xmlChar** valPtr) {
	xmlChar*       val = NULL;
	int            ret = 0;
	int            ix;
	int            nbslash = 0;
	...
	*valPtr = NULL;
	...
	if ((xmlStrcmp((xmlChar*)bas->scheme, (xmlChar*)ref->scheme))
	    || (xmlStrcmp((xmlChar*)bas->server, (xmlChar*)ref->server)) || (bas->port != ref->port)
	    || (baseDrive != refDrive)) {
		goto done;
	}
