...
static xmlEntityPtr xmlLookupGeneralEntity(xmlParserCtxtPtr ctxt, const xmlChar* name, int inAttr);

static const xmlChar* xmlParseEntityRefInternal(xmlParserCtxtPtr ctxt);
...
static void              xmlCtxtParseEntity(xmlParserCtxtPtr ctxt, xmlEntityPtr ent);

static int               xmlLoadEntityContent(xmlParserCtxtPtr ctxt, xmlEntityPtr entity);

static void              xmlParsePERefInternal(xmlParserCtxt* ctxt, int markupDecl);
...
static void              xmlErrMemory(xmlParserCtxtPtr ctxt) {
 ...
 */
static void
xmlErrAttributeDup(xmlParserCtxtPtr ctxt, const xmlChar* prefix, const xmlChar* localname) {
	if (prefix == NULL) {
		xmlCtxtErr(
			ctxt,
			NULL,
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlFatalErrMsg(
	xmlParserCtxtPtr ctxt, xmlParserErrors error, const char* msg
) {
	xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, NULL, NULL, NULL, 0, "%s", msg);
}
 ...
 */
void LIBXML_ATTR_FORMAT(3, 0) xmlWarningMsg(
	...
	xmlCtxtErr(
		ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_WARNING, str1, str2, NULL, 0, msg, str1, str2
	);
}
 ...
 * @param str2  extra data
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlValidityError(
	xmlParserCtxtPtr ctxt,
	xmlParserErrors  error,
	const char*      msg,
	const xmlChar*   str1,
	...
	xmlCtxtErr(ctxt, NULL, XML_FROM_DTD, error, XML_ERR_ERROR, str1, str2, NULL, 0, msg, str1, str2);
}
#endif
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlFatalErrMsgInt(
	xmlParserCtxtPtr ctxt, xmlParserErrors error, const char* msg, int val
) {
	xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, NULL, NULL, NULL, val, msg, val);
}
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlFatalErrMsgStrIntStr(
	xmlParserCtxtPtr ctxt,
	xmlParserErrors  error,
	const char*      msg,
	const xmlChar*   str1,
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlFatalErrMsgStr(
	xmlParserCtxtPtr ctxt, xmlParserErrors error, const char* msg, const xmlChar* val
) {
	xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_FATAL, val, NULL, NULL, 0, msg, val);
}
 ...
 * @param val  a string value
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlErrMsgStr(
	xmlParserCtxtPtr ctxt, xmlParserErrors error, const char* msg, const xmlChar* val
) {
	xmlCtxtErr(ctxt, NULL, XML_FROM_PARSER, error, XML_ERR_ERROR, val, NULL, NULL, 0, msg, val);
}
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlNsErr(
	xmlParserCtxtPtr ctxt,
	xmlParserErrors  error,
	const char*      msg,
	const xmlChar*   info1,
 ...
 * @param info3  extra information string
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlNsWarn(
	xmlParserCtxtPtr ctxt,
	xmlParserErrors  error,
	const char*      msg,
	const xmlChar*   info1,
...
static int xmlParserEntityCheck(xmlParserCtxtPtr ctxt, unsigned long extra) {
	...
	if ((*expandedSize > XML_PARSER_ALLOWED_EXPANSION)
	    && ((*expandedSize >= ULONG_MAX) || (*expandedSize / ctxt->maxAmpl > consumed))) {
		xmlFatalErrMsg(
			ctxt,
...
static void xmlSBufInit(xmlSBuf* buf, unsigned max) {
	buf->mem  = NULL;
	buf->size = 0;
	buf->cap  = 0;
	buf->max  = max;
...
static int xmlSBufGrow(xmlSBuf* buf, unsigned len) {
	xmlChar* mem;
	unsigned cap;
...
static void xmlSBufAddString(xmlSBuf* buf, const xmlChar* str, unsigned len) {
...
static void xmlSBufAddCString(xmlSBuf* buf, const char* str, unsigned len) {
...
static void xmlSBufReportError(xmlSBuf* buf, xmlParserCtxtPtr ctxt, const char* errMsg) {
...
static xmlChar*
xmlSBufFinish(xmlSBuf* buf, int* sizeOut, xmlParserCtxtPtr ctxt, const char* errMsg) {
	if (buf->mem == NULL) {
		buf->mem = xmlMalloc(1);
		if (buf->mem == NULL) {
			buf->code = XML_ERR_NO_MEMORY;
...
static void xmlSBufCleanup(xmlSBuf* buf, xmlParserCtxtPtr ctxt, const char* errMsg) {
	if (buf->code != XML_ERR_OK) {
		xmlSBufReportError(buf, ctxt, errMsg);
	}
...
static int xmlUTF8MultibyteLen(xmlParserCtxtPtr ctxt, const xmlChar* str, const char* errMsg) {
	int c  = str[0];
	int c1 = str[1];
 ...
 * @param ctxt  an XML parser context
 */
static void xmlCtxtInitializeLate(xmlParserCtxtPtr ctxt) {
	...
	xmlDictSetLimit(ctxt->dict, (ctxt->options & XML_PARSE_HUGE) ? 0 : XML_MAX_DICTIONARY_LIMIT);
 ...
 */
static void xmlAddDefAttrs(
	xmlParserCtxtPtr ctxt, const xmlChar* fullname, const xmlChar* fullattr, const xmlChar* value
) {
	...
	if ((defaults == NULL) || (defaults->nbAttrs >= defaults->maxAttrs)) {
		xmlDefAttrsPtr temp;
		int            newSize;
		...
		if (defaults == NULL) {
			...
			if ((defaults->maxAttrs >= XML_MAX_ATTRS)
			    || ((size_t)defaults->maxAttrs > SIZE_MAX / 2 / sizeof(temp[0]) - sizeof(*defaults))) {
				goto mem_error;
			}
		...
		temp = xmlRealloc(defaults, sizeof(*defaults) + newSize * sizeof(xmlDefAttr));
		if (temp == NULL) {
			goto mem_error;
		}
		if (defaults == NULL) {
		...
		defaults       = temp;
		if (xmlHashUpdateEntry2(ctxt->attsDefault, name.name, prefix.name, defaults, NULL) < 0) {
			xmlFree(defaults);
			goto mem_error;
		}
	}
 ...
 */
static void xmlAddSpecialAttr(
	xmlParserCtxtPtr ctxt, const xmlChar* fullname, const xmlChar* fullattr, int type
) {
	if (ctxt->attsSpecial == NULL) {
	...
	if (xmlHashAdd2(ctxt->attsSpecial, fullname, fullattr, XML_INT_TO_PTR(type)) < 0) {
		goto mem_error;
	}
	return;
 ...
 */
static void xmlCleanSpecialAttrCallback(
	void*                 payload,
	void*                 data,
...
int xmlCheckLanguageID(const xmlChar* lang) {
	...
	if (((cur[0] == 'i') && (cur[1] == '-')) || ((cur[0] == 'I') && (cur[1] == '-'))
		 ...
		 */
		cur += 2;
		while (((cur[0] >= 'A') && (cur[0] <= 'Z')) || ((cur[0] >= 'a') && (cur[0] <= 'z'))) {
			cur++;
		}
		return (cur[0] == 0);
	}
	nxt = cur;
	while (((nxt[0] >= 'A') && (nxt[0] <= 'Z')) || ((nxt[0] >= 'a') && (nxt[0] <= 'z'))) {
		nxt++;
	}
	if (nxt - cur >= 4) {
		/*
...
static xmlChar*  xmlParseStringEntityRef(xmlParserCtxtPtr ctxt, const xmlChar** str);
 ...
 * found.
 */
static int xmlParserNsLookup(
	xmlParserCtxtPtr ctxt, const xmlHashedString* prefix, xmlParserNsBucket** bucketPtr
) {
 ...
 */
void* xmlParserNsLookupSax(xmlParserCtxt* ctxt, const xmlChar* prefix) {
 ...
 * @returns the data on success, NULL if no namespace was found.
 */
int xmlParserNsUpdateSax(xmlParserCtxt* ctxt, const xmlChar* prefix, void* saxData) {
	xmlHashedString hprefix;
	int             nsIndex;
...
static int xmlParserNsGrow(xmlParserCtxtPtr ctxt) {
	...
	newSize = xmlGrowCapacity(ctxt->nsMax, sizeof(table[0]) + sizeof(extra[0]), 16, XML_MAX_ITEMS);
	if (newSize < 0) {
		goto error;
	}
 ...
 */
static int xmlParserNsPush(
	xmlParserCtxtPtr       ctxt,
	const xmlHashedString* prefix,
	const xmlHashedString* uri,
	void*                  saxData,
	int                    defAttr
) {
	...
	if ((prefix == NULL) || (prefix->name == NULL)) {
		...
		if (oldIndex != INT_MAX) {
			...
			if ((ctxt->options & XML_PARSE_NSCLEAN) && (uri->name == ctxt->nsTab[oldIndex * 2 + 1])) {
				return (0);
			}
		}
	...
	if (oldIndex != INT_MAX) {
		...
		if ((ctxt->options & XML_PARSE_NSCLEAN) && (uri->name == ctxt->nsTab[bucket->index * 2 + 1])) {
			return (0);
		}
...
static int xmlCtxtGrowAttrs(xmlParserCtxtPtr ctxt) {
	...
	newSize = xmlGrowCapacity(
		ctxt->maxatts / 5, sizeof(atts[0]) * 5 + sizeof(attallocs[0]), 10, XML_MAX_ATTRS
	);
	if (newSize < 0) {
	...
	attallocs  = xmlRealloc(ctxt->attallocs, newSize * sizeof(attallocs[0]));
	if (attallocs == NULL) {
		goto mem_error;
	}
 ...
 */
int xmlCtxtPushInput(xmlParserCtxt* ctxt, xmlParserInput* value) {
	char* directory = NULL;
	int   maxDepth;
	...
	if (ctxt->inputNr >= ctxt->inputMax) {
		...
		newSize = xmlGrowCapacity(ctxt->inputMax, sizeof(tmp[0]), 5, maxDepth);
		if (newSize < 0) {
...
int nodePush(xmlParserCtxt* ctxt, xmlNode* value) {
	...
	if (ctxt->nodeNr >= ctxt->nodeMax) {
		...
		newSize = xmlGrowCapacity(ctxt->nodeMax, sizeof(tmp[0]), 10, maxDepth);
		if (newSize < 0) {
			xmlFatalErrMsgInt(
				ctxt,
				XML_ERR_RESOURCE_LIMIT,
...
static int nameNsPush(
	...
	if (ctxt->nameNr >= ctxt->nameMax) {
		...
		newSize = xmlGrowCapacity(ctxt->nameMax, sizeof(tmp[0]) + sizeof(tmp2[0]), 10, XML_MAX_ITEMS);
		if (newSize < 0) {
			goto mem_error;
		}
 ...
 * @returns the name just removed
 */
static const xmlChar* namePop(xmlParserCtxtPtr ctxt) {
	const xmlChar* ret;
...
static int spacePush(xmlParserCtxtPtr ctxt, int val) {
	if (ctxt->spaceNr >= ctxt->spaceMax) {
		...
		newSize = xmlGrowCapacity(ctxt->spaceMax, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlErrMemory(ctxt);
			return (-1);
		}
...
#define CMP4(s, c1, c2, c3, c4)                                                                 \
	(((unsigned char*)s)[0] == c1 && ((unsigned char*)s)[1] == c2 && ((unsigned char*)s)[2] == c3 \
...
#define CMP9(s, c1, c2, c3, c4, c5, c6, c7, c8, c9)                         \
	(CMP8(s, c1, c2, c3, c4, c5, c6, c7, c8) && ((unsigned char*)s)[8] == c9)
#define CMP10(s, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10)                        \
	(CMP9(s, c1, c2, c3, c4, c5, c6, c7, c8, c9) && ((unsigned char*)s)[9] == c10)
...
#define GROW                                                                              \
	if ((!PARSER_PROGRESSIVE(ctxt)) && (ctxt->input->end - ctxt->input->cur < INPUT_CHUNK)) \
		xmlParserGrow(ctxt);
...
#define COPY_BUF(b, i, v)               \
	if (v < 0x80)                         \
		...
		i += xmlCopyCharMultiByte(&b[i], v)

static int xmlCurrentCharRecover(xmlParserCtxtPtr ctxt, int* len) {
...
static void xmlPopPE(xmlParserCtxtPtr ctxt) {
	unsigned long consumed;
	xmlEntityPtr  ent;
	...
	if ((ent->flags & XML_ENT_CHECKED) == 0) {
		...
		xmlSaturatedAddSizeT(&consumed, ctxt->input->end - ctxt->input->base);
...
static int xmlSkipBlankCharsPE(xmlParserCtxtPtr ctxt) {
	...
	while (PARSER_STOPPED(ctxt) == 0) {
		if (IS_BLANK_CH(CUR)) { /* CHECKED tstblanks.xml */
			...
			if ((expandParam == 0) || (IS_BLANK_CH(NXT(1))) || (NXT(1) == 0)) {
				break;
			}
...
int xmlParseCharRef(xmlParserCtxt* ctxt) {
	...
	if (val >= 0x110000) {
		...
		xmlFatalErrMsgInt(
			ctxt, XML_ERR_INVALID_CHAR, "xmlParseCharRef: invalid xmlChar value %d\n", val
		);
	}
	return (val);
}
...
static int xmlParseStringCharRef(xmlParserCtxtPtr ctxt, const xmlChar** str) {
	...
	*str = ptr;
	...
	if (val >= 0x110000) {
		xmlFatalErrMsgInt(
			ctxt, XML_ERR_INVALID_CHAR, "xmlParseStringCharRef: character reference out of bounds\n", val
		);
	} else if (IS_CHAR(val)) {
		return (val);
	} else {
		xmlFatalErrMsgInt(
			ctxt, XML_ERR_INVALID_CHAR, "xmlParseStringCharRef: invalid xmlChar value %d\n", val
		);
	}
	return (0);
}
...
xmlChar* xmlStringLenDecodeEntities(
	...
	if ((str[len] != 0) || (end != 0) || (end2 != 0) || (end3 != 0)) {
		return (NULL);
	}
...
static int areBlanks(xmlParserCtxtPtr ctxt, const xmlChar* str, int len, int blank_chars) {
	...
	if (ctxt->myDoc != NULL) {
		...
		if (doc->intSubset != NULL) {
			elemDecl = xmlHashLookup2(doc->intSubset->elements, ctxt->node->name, prefix);
		}
		if ((elemDecl == NULL) && (doc->extSubset != NULL)) {
			elemDecl = xmlHashLookup2(doc->extSubset->elements, ctxt->node->name, prefix);
		}
		if (elemDecl != NULL) {
	...
	if ((ctxt->node->children == NULL) && (RAW == '<') && (NXT(1) == '/')) {
		return (0);
	}
	...
	if (lastChild == NULL) {
		...
		return (0);
	} else if ((ctxt->node->children != NULL) && (xmlNodeIsText(ctxt->node->children))) {
		return (0);
	}
	return (1);
}
...
xmlChar* xmlSplitQName(xmlParserCtxt* ctxt, const xmlChar* name, xmlChar** prefixOut) {
	xmlChar*       ret;
	const xmlChar* localname;
...
static int xmlIsNameCharNew(int c) {
	...
	if ((c != ' ') && (c != '>') && (c != '/') &&                               /* accelerators */
	    (((c >= 'a') && (c <= 'z')) || ((c >= 'A') && (c <= 'Z')) || ((c >= '0') && (c <= '9'))
	     ...
	     ((c >= 0xC0) && (c <= 0xD6)) || ((c >= 0xD8) && (c <= 0xF6))
	     || ((c >= 0xF8) && (c <= 0x2FF)) || ((c >= 0x300) && (c <= 0x36F)) ||  /* !start */
	     ((c >= 0x370) && (c <= 0x37D)) || ((c >= 0x37F) && (c <= 0x1FFF))
	     || ((c >= 0x200C) && (c <= 0x200D)) || ((c >= 0x203F) && (c <= 0x2040)) || /* !start */
	     ((c >= 0x2070) && (c <= 0x218F)) || ((c >= 0x2C00) && (c <= 0x2FEF))
	     || ((c >= 0x3001) && (c <= 0xD7FF)) || ((c >= 0xF900) && (c <= 0xFDCF))
	     || ((c >= 0xFDF0) && (c <= 0xFFFD)) || ((c >= 0x10000) && (c <= 0xEFFFF)))) {
		return (1);
...
const xmlChar* xmlParseName(xmlParserCtxt* ctxt) {
	...
	const xmlChar* ret;
	size_t         count = 0;
	size_t maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_TEXT_LENGTH : XML_MAX_NAME_LENGTH;
...
static const xmlChar* xmlParseNameAndCompare(xmlParserCtxtPtr ctxt, const xmlChar* other) {
 ...
 */
static void xmlExpandPEsInEntityValue(
	xmlParserCtxtPtr ctxt, xmlSBuf* buf, const xmlChar* str, int length, int depth
) {
	...
	while ((str < end) && (!PARSER_STOPPED(ctxt))) {
		...
		if (c >= 0x80) {
			l = xmlUTF8MultibyteLen(ctxt, str, "invalid character in entity value\n");
			if (l == 0) {
				if (chunk < str) {
			...
			if (ent->content == NULL) {
				...
				if (((ctxt->options & XML_PARSE_NO_XXE) == 0)
				    && ((ctxt->replaceEntities) || (ctxt->validate))) {
...
xmlChar* xmlParseEntityValue(xmlParserCtxt* ctxt, xmlChar** orig) {
	unsigned maxLength
		= (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH : XML_MAX_TEXT_LENGTH;
	xmlSBuf        buf;
	const xmlChar* start;
	int            quote, length;
 ...
 * @param depth  nesting depth
 */
static void xmlCheckEntityInAttValue(xmlParserCtxtPtr ctxt, xmlEntityPtr pent, int depth) {
	int            maxDepth = (ctxt->options & XML_PARSE_HUGE) ? 40 : 20;
	const xmlChar* str;
 ...
 */
static int xmlExpandEntityInAttValue(
	xmlParserCtxtPtr ctxt,
	xmlSBuf*         buf,
	const xmlChar*   str,
	xmlEntityPtr     pent,
		...
		if (c != '&') {
			...
			if ((ent != NULL) && (ent->etype == XML_INTERNAL_PREDEFINED_ENTITY)) {
				...
				*inSpace = 0;
				...
				normChange |= xmlExpandEntityInAttValue(
					ctxt, buf, ent->content, ent, normalize, inSpace, depth, check
				);
				if (pent != NULL) {
					pent->flags &= ~XML_ENT_EXPANDING;
				}
 ...
 */
xmlChar* xmlExpandEntitiesInAttValue(xmlParserCtxt* ctxt, const xmlChar* str, int normalize) {
	unsigned maxLength
		= (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH : XML_MAX_TEXT_LENGTH;
	xmlSBuf buf;
	int     inSpace = 1;
	...
	xmlExpandEntityInAttValue(
		ctxt, &buf, str, NULL, normalize, &inSpace, ctxt->inputNr, /* check */ 0
	);
 ...
 */
static xmlChar* xmlParseAttValueInternal(
	xmlParserCtxtPtr ctxt, int* attlen, int* outFlags, int special, int isNamespace
) {
	unsigned maxLength
		= (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH : XML_MAX_TEXT_LENGTH;
	xmlSBuf  buf;
	...
	while (1) {
		...
		if (c >= 0x80) {
			l = xmlUTF8MultibyteLen(ctxt, CUR_PTR, "invalid character in attribute value\n");
			if (l == 0) {
				if (chunkSize > 0) {
...
xmlChar* xmlParsePubidLiteral(xmlParserCtxt* ctxt) {
	...
	cur = CUR;
	while ((IS_PUBIDCHAR_CH(cur)) && (cur != stop) && (PARSER_STOPPED(ctxt) == 0)) { /* checked */
		if (len + 1 >= size) {
			xmlChar* tmp;
			int      newSize;
...
static const unsigned char test_char_data[256]
	= {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00,
     ...
     0x00, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x00, 0x27, /* & */
     0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36,
     0x37, 0x38, 0x39, 0x3A, 0x3B, 0x00, 0x3D, 0x3E, 0x3F, /* < */
     0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E,
     0x4F, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x5B, 0x5C, 0x00,
     0x5E, 0x5F, /* ] */
     ...
     0x7E, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /* non-ascii */
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static void xmlCharacters(xmlParserCtxtPtr ctxt, const xmlChar* buf, int size, int isBlank) {
	int checkBlanks;
	...
	checkBlanks = (!ctxt->keepBlanks) || (ctxt->sax->ignorableWhitespace != ctxt->sax->characters);
	 ...
	 */
	if ((checkBlanks) && (areBlanks(ctxt, buf, size, isBlank))) {
...
static void xmlParseCharDataComplex(xmlParserCtxtPtr ctxt, int partial) {
	...
	if (ctxt->input->cur < ctxt->input->end) {
		if ((cur == 0) && (CUR != 0)) {
			if (partial == 0) {
				xmlFatalErrMsgInt(
					ctxt, XML_ERR_INVALID_CHAR, "Incomplete UTF-8 sequence starting with %02X\n", CUR
				);
				NEXTL(1);
			}
		} else if ((cur != '<') && (cur != '&') && (cur != ']')) {
 ...
 * @param cdata  unused
 */
void xmlParseCharData(xmlParserCtxt* ctxt, ATTRIBUTE_UNUSED int cdata) {
	...
	if (CMP6(CUR_PTR, 'S', 'Y', 'S', 'T', 'E', 'M')) {
		...
		*publicId = xmlParsePubidLiteral(ctxt);
		...
		if (strict) {
			...
			if (SKIP_BLANKS == 0) {
				xmlFatalErrMsg(
					ctxt, XML_ERR_SPACE_REQUIRED, "Space required after the Public Identifier\n"
				);
			}
		} else {
			/*
...
static void xmlParseCommentComplex(xmlParserCtxtPtr ctxt, xmlChar* buf, size_t len, size_t size) {
	...
	int r, rl;
	int cur, l;
	int maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH : XML_MAX_TEXT_LENGTH;
	...
	while (IS_CHAR(cur) && /* checked */
		...
		if (len + 5 >= size) {
			...
			newSize = xmlGrowCapacity(size, 1, 1, maxLength);
			if (newSize < 0) {
				xmlFatalErrMsgStr(ctxt, XML_ERR_COMMENT_NOT_FINISHED, "Comment too big found", NULL);
				xmlFree(buf);
				return;
			}
			tmp = xmlRealloc(buf, newSize);
	...
	if (cur == 0) {
		xmlFatalErrMsgStr(
			ctxt, XML_ERR_COMMENT_NOT_FINISHED, "Comment not terminated \n<!--%.50s\n", buf
		);
	} else if (!IS_CHAR(cur)) {
		xmlFatalErrMsgInt(
			ctxt, XML_ERR_INVALID_CHAR, "xmlParseComment: invalid xmlChar value %d\n", cur
...
void xmlParseComment(xmlParserCtxt* ctxt) {
	...
	size_t   len     = 0;
	size_t maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH : XML_MAX_TEXT_LENGTH;
	const xmlChar* in;
	size_t         nbchar = 0;
	int            ccol;
...
get_more:
		ccol = ctxt->input->col;
		while (((*in > '-') && (*in <= 0x7F)) || ((*in >= 0x20) && (*in < '-')) || (*in == 0x09)) {
			in++;
			ccol++;
		}
		ctxt->input->col = ccol;
		...
		if (*in == '-') {
			if (in[1] == '-') {
				...
				if (buf != NULL) {
					...
					xmlFatalErrMsgStr(
						ctxt, XML_ERR_HYPHEN_IN_COMMENT, "Double hyphen within comment\n", NULL
					);
				}
				in++;
				ctxt->input->col++;
...
const xmlChar* xmlParsePITarget(xmlParserCtxt* ctxt) {
	...
	if ((name != NULL) && ((name[0] == 'x') || (name[0] == 'X'))
		...
		xmlWarningMsg(
			ctxt, XML_ERR_RESERVED_XML_NAME, "xmlParsePITarget: invalid name prefix 'xml'\n", NULL, NULL
		);
	}
	if ((name != NULL) && (xmlStrchr(name, ':') != NULL)) {
		xmlNsErr(
			ctxt, XML_NS_ERR_COLON, "colons are forbidden from PI names '%s'\n", name, NULL, NULL
		);
	}
	return (name);
}
...
static void xmlParseCatalogPI(xmlParserCtxtPtr ctxt, const xmlChar* catalog) {
	xmlChar*       URL = NULL;
		...
		if (target != NULL) {
			if ((RAW == '?') && (NXT(1) == '>')) {
				...
				if ((ctxt->sax) && (!ctxt->disableSAX) && (ctxt->sax->processingInstruction != NULL)) {
					ctxt->sax->processingInstruction(ctxt->userData, target, NULL);
				}
				return;
			}
			buf = xmlMalloc(size);
...
#ifdef LIBXML_CATALOG_ENABLED
				if ((ctxt->inSubset == 0) && (xmlStrEqual(target, XML_CATALOG_PI))) {
					xmlCatalogAllow allow = xmlCatalogGetDefaults();

					if ((ctxt->options & XML_PARSE_CATALOG_PI)
					    && ((allow == XML_CATA_ALLOW_DOCUMENT) || (allow == XML_CATA_ALLOW_ALL))) {
...
void xmlParseNotationDecl(xmlParserCtxt* ctxt) {
	...
	if (CMP8(CUR_PTR, 'N', 'O', 'T', 'A', 'T', 'I', 'O', 'N')) {
		...
		if (xmlStrchr(name, ':') != NULL) {
			xmlNsErr(
				ctxt, XML_NS_ERR_COLON, "colons are forbidden from notation names '%s'\n", name, NULL, NULL
			);
		}
		if (SKIP_BLANKS_PE == 0) {
...
#endif
			NEXT;
			if ((ctxt->sax != NULL) && (!ctxt->disableSAX) && (ctxt->sax->notationDecl != NULL)) {
...
void xmlParseEntityDecl(xmlParserCtxt* ctxt) {
	...
	if (CMP6(CUR_PTR, 'E', 'N', 'T', 'I', 'T', 'Y')) {
		...
		if (xmlStrchr(name, ':') != NULL) {
			xmlNsErr(
				ctxt, XML_NS_ERR_COLON, "colons are forbidden from entities names '%s'\n", name, NULL, NULL
			);
		}
		if (SKIP_BLANKS_PE == 0) {
		...
		if (isParameter) {
			if ((RAW == '"') || (RAW == '\'')) {
				...
				if (value) {
					if ((ctxt->sax != NULL) && (!ctxt->disableSAX) && (ctxt->sax->entityDecl != NULL)) {
						ctxt->sax->entityDecl(
							ctxt->userData, name, XML_INTERNAL_PARAMETER_ENTITY, NULL, NULL, value
						);
					}
				}
			} else {
				...
				if (URI) {
					if (xmlStrchr(URI, '#')) {
						...
						if ((ctxt->sax != NULL) && (!ctxt->disableSAX) && (ctxt->sax->entityDecl != NULL)) {
							ctxt->sax->entityDecl(
								ctxt->userData, name, XML_EXTERNAL_PARAMETER_ENTITY, literal, URI, NULL
							);
						}
					}
				}
			...
			if ((RAW == '"') || (RAW == '\'')) {
				...
				if ((ctxt->sax != NULL) && (!ctxt->disableSAX) && (ctxt->sax->entityDecl != NULL)) {
					ctxt->sax->entityDecl(
						ctxt->userData, name, XML_INTERNAL_GENERAL_ENTITY, NULL, NULL, value
					);
				}
				/*
				 * For expat compatibility in SAX mode.
				 */
				if ((ctxt->myDoc == NULL) || (xmlStrEqual(ctxt->myDoc->version, SAX_COMPAT_MODE))) {
					if (ctxt->myDoc == NULL) {
				...
				if (CMP5(CUR_PTR, 'N', 'D', 'A', 'T', 'A')) {
					...
					if ((ctxt->sax != NULL) && (!ctxt->disableSAX) && (ctxt->sax->entityDecl != NULL)) {
						ctxt->sax->entityDecl(
							ctxt->userData, name, XML_EXTERNAL_GENERAL_PARSED_ENTITY, literal, URI, NULL
						);
					}
					/*
					 * For expat compatibility in SAX mode.
					...
					if ((ctxt->replaceEntities != 0)
					    && ((ctxt->myDoc == NULL) || (xmlStrEqual(ctxt->myDoc->version, SAX_COMPAT_MODE)))) {
						if (ctxt->myDoc == NULL) {
							ctxt->myDoc = xmlNewDoc(SAX_COMPAT_MODE);
							if (ctxt->myDoc == NULL) {
								xmlErrMemory(ctxt);
						...
						xmlSAX2EntityDecl(ctxt, name, XML_EXTERNAL_GENERAL_PARSED_ENTITY, literal, URI, NULL);
					}
				}
			}
		}
...
int xmlParseDefaultDecl(xmlParserCtxt* ctxt, xmlChar** value) {
	...
	*value = NULL;
	...
	if (ret == NULL) {
		xmlFatalErrMsg(
			ctxt, (xmlParserErrors)ctxt->errNo, "Attribute default value declaration error\n"
		);
	} else {
		*value = ret;
	}
	...
	if (CMP7(CUR_PTR, 'A', 'T', 'T', 'L', 'I', 'S', 'T')) {
		...
		while ((RAW != '>') && (PARSER_STOPPED(ctxt) == 0)) {
			...
			if (RAW != '>') {
				if (SKIP_BLANKS_PE == 0) {
					xmlFatalErrMsg(
						ctxt, XML_ERR_SPACE_REQUIRED, "Space required after the attribute default value\n"
					);
					if (defaultValue != NULL) {
						xmlFree(defaultValue);
					}
			...
			if ((ctxt->sax != NULL) && (!ctxt->disableSAX) && (ctxt->sax->attributeDecl != NULL)) {
				ctxt->sax->attributeDecl(
					ctxt->userData, elemName, attrName, type, def, defaultValue, tree
				);
			} else if (tree != NULL) {
				xmlFreeEnumeration(tree);
			}

			if ((ctxt->sax2) && (defaultValue != NULL) && (def != XML_ATTRIBUTE_IMPLIED)
 ...
 */
static void xmlSkipBlankCharsPEBalanced(xmlParserCtxt* ctxt, int openInputNr) {
	SKIP_BLANKS;
	GROW;
 ...
 */
xmlElementContent* xmlParseElementMixedContentDecl(xmlParserCtxt* ctxt, int openInputNr) {
	...
	if (CMP7(CUR_PTR, '#', 'P', 'C', 'D', 'A', 'T', 'A')) {
		...
		while ((RAW == '|') && (PARSER_STOPPED(ctxt) == 0)) {
			...
			if (elem == NULL) {
				xmlFatalErrMsg(
					ctxt, XML_ERR_NAME_REQUIRED, "xmlParseElementMixedContentDecl : Name expected\n"
				);
...
xmlParseElementChildrenContentDeclPriv(xmlParserCtxtPtr ctxt, int openInputNr, int depth) {
	int                  maxDepth = (ctxt->options & XML_PARSE_HUGE) ? 2048 : 256;
	...
	if (RAW == '(') {
		...
		NEXT;
		cur = ret = xmlParseElementChildrenContentDeclPriv(ctxt, newInputNr, depth + 1);
		if (cur == NULL) {
			return (NULL);
		}
	} else {
	...
	while (!PARSER_STOPPED(ctxt)) {
		...
		if (RAW == '(') {
			...
			NEXT;
			last = xmlParseElementChildrenContentDeclPriv(ctxt, newInputNr, depth + 1);
			if (last == NULL) {
				if (ret != NULL) {
					xmlFreeDocElementContent(ctxt->myDoc, ret);
				}
	...
	if (RAW == '?') {
		if (ret != NULL) {
			if ((ret->ocur == XML_ELEMENT_CONTENT_PLUS) || (ret->ocur == XML_ELEMENT_CONTENT_MULT)) {
				ret->ocur = XML_ELEMENT_CONTENT_MULT;
			} else {
		...
		if (ret != NULL) {
			...
			while ((cur != NULL) && (cur->type == XML_ELEMENT_CONTENT_OR)) {
				if ((cur->c1 != NULL)
				    && ((cur->c1->ocur == XML_ELEMENT_CONTENT_OPT) || (cur->c1->ocur == XML_ELEMENT_CONTENT_MULT))) {
					cur->c1->ocur = XML_ELEMENT_CONTENT_ONCE;
				}
				if ((cur->c2 != NULL)
				    && ((cur->c2->ocur == XML_ELEMENT_CONTENT_OPT) || (cur->c2->ocur == XML_ELEMENT_CONTENT_MULT))) {
...
int
xmlParseElementContentDecl(xmlParserCtxt* ctxt, const xmlChar* name, xmlElementContent** result) {
...
int xmlParseElementDecl(xmlParserCtxt* ctxt) {
	...
	if (CMP7(CUR_PTR, 'E', 'L', 'E', 'M', 'E', 'N', 'T')) {
		...
		if (RAW != '>') {
			...
			NEXT;
			if ((ctxt->sax != NULL) && (!ctxt->disableSAX) && (ctxt->sax->elementDecl != NULL)) {
				if (content != NULL) {
					content->parent = NULL;
				}
...
static void xmlParseConditionalSections(xmlParserCtxtPtr ctxt) {
	...
	while (!PARSER_STOPPED(ctxt)) {
		...
		} else if ((depth > 0) && (RAW == ']') && (NXT(1) == ']') && (NXT(2) == '>')) {
			if (isFreshPE) {
				xmlFatalErrMsg(
					ctxt,
		...
		if (NXT(1) == '!') {
			switch (NXT(2)) {
				...
				default:
					xmlFatalErr(
						ctxt,
						ctxt->inSubset == 2 ? XML_ERR_EXT_SUBSET_NOT_FINISHED
 ...
 */
void
xmlParseExternalSubset(xmlParserCtxt* ctxt, const xmlChar* publicId, const xmlChar* systemId) {
	int oldInputNr;
...
void xmlParseReference(xmlParserCtxt* ctxt) {
	...
	if (ent == NULL) {
		 ...
		 */
		if ((ctxt->replaceEntities == 0) && (ctxt->sax != NULL) && (ctxt->disableSAX == 0)
	...
	if ((ent->name == NULL) || (ent->etype == XML_INTERNAL_PREDEFINED_ENTITY)) {
		val = ent->content;
		if (val == NULL) {
			return;
		}
	...
	if ((ent->etype == XML_INTERNAL_GENERAL_ENTITY)
	    || (((ctxt->options & XML_PARSE_NO_XXE) == 0) && ((ctxt->replaceEntities) || (ctxt->validate)))) {
	...
	if (ctxt->replaceEntities == 0) {
		...
		if ((cur->type == XML_TEXT_NODE) || (cur->type == XML_CDATA_SECTION_NODE)) {
			...
			if ((cur->type == XML_TEXT_NODE) || (ctxt->options & XML_PARSE_NOCDATA)) {
				if (ctxt->sax->characters != NULL) {
		...
		while (cur != NULL) {
			 ...
			 */
			if ((cur->next == NULL)
			    && ((cur->type == XML_TEXT_NODE) || (cur->type == XML_CDATA_SECTION_NODE))) {
				int len = xmlStrlen(cur->content);
...
static void xmlHandleUndeclaredEntity(xmlParserCtxtPtr ctxt, const xmlChar* name) {
	/*
	 * [ WFC: Entity Declared ]
	...
	if ((ctxt->standalone == 1) || ((ctxt->hasExternalSubset == 0) && (ctxt->hasPErefs == 0))) {
		 ...
		 */
		xmlValidityError(ctxt, XML_ERR_UNDECLARED_ENTITY, "Entity '%s' not defined\n", name, NULL);
#endif
	} else if ((ctxt->loadsubset & ~XML_SKIP_IDS)
	           || ((ctxt->replaceEntities) && ((ctxt->options & XML_PARSE_NO_XXE) == 0))) {
		/*
		...
		xmlWarningMsg(ctxt, XML_WAR_UNDECLARED_ENTITY, "Entity '%s' not defined\n", name, NULL);
	}
...
xmlLookupGeneralEntity(xmlParserCtxtPtr ctxt, const xmlChar* name, int inAttr) {
	 ...
	 */
		...
		xmlFatalErrMsgStr(
			ctxt, XML_ERR_UNPARSED_ENTITY, "Entity reference to unparsed entity %s\n", name
		);
		ent = NULL;
	}
	 ...
	 */
		...
		if (inAttr) {
			xmlFatalErrMsgStr(
				ctxt, XML_ERR_ENTITY_IS_EXTERNAL, "Attribute references external entity '%s'\n", name
			);
			ent = NULL;
		}
	}
...
static void xmlParsePERefInternal(xmlParserCtxt* ctxt, int markupDecl) {
	...
	if (entity == NULL) {
		...
		if ((entity->etype != XML_INTERNAL_PARAMETER_ENTITY)
			...
			xmlWarningMsg(
				ctxt, XML_WAR_UNDECLARED_ENTITY, "Internal: %%%s; is not a parameter entity\n", name, NULL
			);
		} else {
...
static int xmlLoadEntityContent(xmlParserCtxtPtr ctxt, xmlEntityPtr entity) {
	...
	if ((ctxt == NULL) || (entity == NULL)
	    || ((entity->etype != XML_EXTERNAL_PARAMETER_ENTITY) && (entity->etype != XML_EXTERNAL_GENERAL_PARSED_ENTITY))
	    || (entity->content != NULL)) {
	...
	input = xmlLoadResource(ctxt, (char*)entity->URI, (char*)entity->ExternalID, rtype);
	if (input == NULL) {
		return (-1);
	}
	...
	if ((CMP5(CUR_PTR, '<', '?', 'x', 'm', 'l')) && (IS_BLANK_CH(NXT(5)))) {
		...
		if ((xmlStrEqual(ctxt->version, BAD_CAST "1.0"))
			...
			xmlFatalErrMsg(
				ctxt, XML_ERR_VERSION_MISMATCH, "Version mismatch between document and entity\n"
			);
		}
	}
	...
	for (i = 0; i < length;) {
		...
		if ((c < 0) || (!IS_CHAR(c))) {
			xmlFatalErrMsgInt(
				ctxt, XML_ERR_INVALID_CHAR, "xmlLoadEntityContent: invalid char value %d\n", content[i]
			);
			goto error;
		}
		i += clen;
...
static xmlEntityPtr xmlParseStringPEReference(xmlParserCtxtPtr ctxt, const xmlChar** str) {
	...
	if (entity == NULL) {
		...
		if ((entity->etype != XML_INTERNAL_PARAMETER_ENTITY)
		    && (entity->etype != XML_EXTERNAL_PARAMETER_ENTITY)) {
			xmlWarningMsg(
				ctxt, XML_WAR_UNDECLARED_ENTITY, "%%%s; is not a parameter entity\n", name, NULL
			);
		}
	}
...
void xmlParseDocTypeDecl(xmlParserCtxt* ctxt) {
	...
	if (SKIP_BLANKS == 0) {
		xmlFatalErrMsg(ctxt, XML_ERR_SPACE_REQUIRED, "Space required after 'DOCTYPE'\n");
	}
	...
	if (RAW == '[') {
		...
		while (1) {
			if (PARSER_STOPPED(ctxt)) {
				...
				SKIP_BLANKS;
				break;
			} else if ((PARSER_EXTERNAL(ctxt)) && (RAW == '<') && (NXT(1) == '!') && (NXT(2) == '[')) {
				/*
...
const xmlChar* xmlParseStartTag(xmlParserCtxt* ctxt) {
	...
	while (((RAW != '>') && ((RAW != '/') || (NXT(1) != '>')) && (IS_BYTE_CHAR(RAW)))
		...
		if (attvalue != NULL) {
			...
			if (nbatts + 4 > maxatts) {
				...
				newSize = xmlGrowCapacity(maxatts, sizeof(n[0]) * 2, 11, XML_MAX_ATTRS);
				if (newSize < 0) {
					xmlErrMemory(ctxt);
					goto failed;
				}
...
failed:
	 ...
	 */
	if ((ctxt->sax != NULL) && (ctxt->sax->startElement != NULL) && (!ctxt->disableSAX)) {
		if (nbatts > 0) {
...
static void xmlParseEndTag1(xmlParserCtxtPtr ctxt, int line) {
	const xmlChar* name;
	 ...
	 * SAX: End of Tag
	 */
	if ((ctxt->sax != NULL) && (ctxt->sax->endElement != NULL) && (!ctxt->disableSAX)) {
...
static xmlHashedString xmlParseQNameHashed(xmlParserCtxtPtr ctxt, xmlHashedString* prefix) {
	...
	if ((l.name == NULL) || (CUR == ':')) {
		...
		l = xmlDictLookupHashed(ctxt->dict, BASE_PTR + start, CUR_PTR - (BASE_PTR + start));
		if (l.name == NULL) {
			xmlErrMemory(ctxt);
			return (l);
		}
...
xmlParseQNameAndCompare(xmlParserCtxtPtr ctxt, const xmlChar* name, const xmlChar* prefix) {
	const xmlChar* cmp;
	const xmlChar* in;
	const xmlChar* ret;
...
static xmlHashedString xmlParseAttribute2(
	xmlParserCtxtPtr ctxt,
	const xmlChar*   pref,
	const xmlChar*   elem,
	xmlHashedString* hprefix,
	...
	*value = NULL;
	 ...
	 */
	if (ctxt->attsSpecial != NULL) {
		special = XML_PTR_TO_INT(xmlHashQLookup2(ctxt->attsSpecial, pref, elem, prefix, name));
	}
	...
	SKIP_BLANKS;
	flags       = 0;
	isNamespace = (((prefix == NULL) && (name == ctxt->str_xmlns)) || (prefix == ctxt->str_xmlns));
	val         = xmlParseAttValueInternal(ctxt, len, &flags, special, isNamespace);
	if (val == NULL) {
		goto error;
	}
	...
	if (prefix == ctxt->str_xml) {
		...
		if ((ctxt->pedantic) && (xmlStrEqual(name, BAD_CAST "lang"))) {
			...
			if (!xmlCheckLanguageID(internal_val)) {
				xmlWarningMsg(
					ctxt, XML_WAR_LANG_VALUE, "Malformed value for xml:lang : %s\n", internal_val, NULL
				);
			}
		}
...
static int xmlAttrHashInsertQName(
	xmlParserCtxtPtr ctxt,
	unsigned         size,
	const xmlChar*   name,
	const xmlChar*   prefix,
...
static const xmlChar*
xmlParseStartTag2(xmlParserCtxtPtr ctxt, const xmlChar** pref, const xmlChar** URI, int* nbNsPtr) {
	...
	while (((RAW != '>') && ((RAW != '/') || (NXT(1) != '>')) && (IS_BYTE_CHAR(RAW)))
		...
		hattname = xmlParseAttribute2(ctxt, prefix, localname, &haprefix, &attvalue, &len, &alloc);
		...
		if ((attname == ctxt->str_xmlns) && (aprefix == NULL)) {
			...
			if (*uri != 0) {
				...
				if (parsedUri == NULL) {
					...
					if (parsedUri->scheme == NULL) {
						xmlNsWarn(
							ctxt, XML_WAR_NS_URI_RELATIVE, "xmlns: URI %s is not absolute\n", uri, NULL, NULL
						);
					}
				...
				if ((len == 29) && (xmlStrEqual(uri, BAD_CAST "http://www.w3.org/2000/xmlns/"))) {
					xmlNsErr(
						ctxt,
						XML_NS_ERR_XML_NAMESPACE,
			...
			if ((uri == NULL) || (uri[0] == 0)) {
				...
				if (parsedUri == NULL) {
					xmlNsErr(
						ctxt, XML_WAR_NS_URI, "xmlns:%s: '%s' is not a valid URI\n", attname, uri, NULL
					);
				} else {
			...
			ctxt->attallocs[nratts++] = (hattname.hashValue & 0x7FFFFFFF) | ((unsigned)alloc << 31);
			atts[nbatts++]            = attname;
			atts[nbatts++]            = aprefix;
...
next_attr:
		...
		if (defaults != NULL) {
			for (i = 0; i < defaults->nbAttrs; i++) {
				...
				if ((attname == ctxt->str_xmlns) && (aprefix == NULL)) {
					...
					if (xmlParserNsPush(ctxt, &attr->name, &attr->value, NULL, 1) > 0) {
						nbNs++;
					}
				} else {
		...
		if (defaults != NULL) {
			for (i = 0; i < defaults->nbAttrs; i++) {
				...
				if (maxAtts > 1) {
					...
					res       = xmlAttrHashInsert(ctxt, attrHashSize, attname, nsuri, hashValue, nbatts);
					if (res < 0) {
						continue;
					}
					if (res < INT_MAX) {
				 ...
				 */
				if ((ctxt->validate) && (ctxt->standalone == 1) && (attr->external != 0)) {
					xmlValidityError(
						ctxt,
	...
	if ((numDupErr == 0) && (numNsErr > 1)) {
		...
		for (i = 0, j = 0; j < nratts; i += 5, j++) {
			...
			res             = xmlAttrHashInsertQName(ctxt, attrHashSize, attname, aprefix, hashValue, i);
			if (res < INT_MAX) {
				xmlErrAttributeDup(ctxt, aprefix, attname);
			}
		}
	...
	*URI  = uri;
	...
	if ((ctxt->sax != NULL) && (ctxt->sax->startElementNs != NULL) && (!ctxt->disableSAX)) {
		if (nbNs > 0) {
			...
			ctxt->sax->startElementNs(
				ctxt->userData, localname, prefix, uri, 0, NULL, nbatts / 5, nbdef, atts
			);
		}
	}
...
static void xmlParseEndTag2(xmlParserCtxtPtr ctxt, const xmlStartTag* tag) {
	 ...
	 * SAX: End of Tag
	 */
	if ((ctxt->sax != NULL) && (ctxt->sax->endElementNs != NULL) && (!ctxt->disableSAX)) {
...
void xmlParseCDSect(xmlParserCtxt* ctxt) {
	...
	if (cur != '>') {
		xmlFatalErrMsgStr(
			ctxt, XML_ERR_CDATA_NOT_FINISHED, "CData section not finished\n%.50s\n", buf
		);
		goto out;
	}
	NEXTL(l);
	 ...
	 */
	if ((ctxt->sax != NULL) && (!ctxt->disableSAX)) {
		if ((ctxt->sax->cdataBlock != NULL) && ((ctxt->options & XML_PARSE_NOCDATA) == 0)) {
...
static void xmlParseContentInternal(xmlParserCtxtPtr ctxt) {
	...
	if ((ctxt->nameNr > oldNameNr) && (ctxt->input->cur >= ctxt->input->end) && (ctxt->wellFormed)) {
...
static int xmlParseElementStart(xmlParserCtxtPtr ctxt) {
	 ...
	 * type of the root element.
	 */
	if (ctxt->validate && ctxt->wellFormed && ctxt->myDoc && ctxt->node
...
static void xmlFinishDocument(xmlParserCtxtPtr ctxt) {
	xmlDocPtr doc;
	 ...
	 */
	doc = ctxt->myDoc;
	if ((doc != NULL) && (xmlStrEqual(doc->version, SAX_COMPAT_MODE))) {
		xmlFreeDoc(doc);
		ctxt->myDoc = NULL;
	}
}
...
int xmlParseDocument(xmlParserCtxt* ctxt) {
	...
	if ((ctxt->sax) && (ctxt->sax->setDocumentLocator)) {
		ctxt->sax->setDocumentLocator(ctxt->userData, (xmlSAXLocator*)&xmlDefaultSAXLocator);
	}
	...
	if ((ctxt->myDoc != NULL) && (ctxt->input != NULL) && (ctxt->input->buf != NULL)
	...
	if (CMP9(CUR_PTR, '<', '!', 'D', 'O', 'C', 'T', 'Y', 'P', 'E')) {
		 ...
		 */
		ctxt->inSubset = 2;
		if ((ctxt->sax != NULL) && (ctxt->sax->externalSubset != NULL) && (!ctxt->disableSAX)) {
 ...
 * @param c  character
 */
static int xmlParseLookupChar(xmlParserCtxtPtr ctxt, int c) {
	const xmlChar* cur;
 ...
 */
static const xmlChar*
xmlParseLookupString(xmlParserCtxtPtr ctxt, size_t startDelta, const char* str, size_t strLen) {
 ...
 * @param ctxt  an XML parser context
 */
static int xmlParseLookupCharData(xmlParserCtxtPtr ctxt) {
 ...
 * @param ctxt  an XML parser context
 */
static int xmlParseLookupGt(xmlParserCtxtPtr ctxt) {
	const xmlChar* cur;
	const xmlChar* end   = ctxt->input->end;
		...
		switch (ctxt->instate) {
			...
			case XML_PARSER_XML_DECL:
				...
				if ((cur == '<') && (next == '?')) {
					...
					if ((ctxt->input->cur[2] == 'x') && (ctxt->input->cur[3] == 'm')
					    && (ctxt->input->cur[4] == 'l') && (IS_BLANK_CH(ctxt->input->cur[5]))) {
						ret += 5;
						xmlParseXMLDecl(ctxt);
					} else {
						ctxt->version = xmlCharStrdup(XML_DEFAULT_VERSION);
			...
			case XML_PARSER_START_TAG: {
				...
				if ((RAW == '/') && (NXT(1) == '>')) {
					...
					xmlFatalErrMsgStr(
						ctxt, XML_ERR_GT_REQUIRED, "Couldn't find end of Start Tag %s\n", name
					);
					nodePop(ctxt);
					spacePop(ctxt);
					if (nbNs > 0) {
				...
				if (cur == '<') {
					...
					if (next == '/') {
						...
						if (next == '-') {
							...
							if (ctxt->input->cur[3] == '-') {
								if ((!terminate) && (!xmlParseLookupString(ctxt, 4, "-->", 3))) {
									goto done;
								}
								xmlParseComment(ctxt);
								ctxt->instate = XML_PARSER_CONTENT;
							...
							if ((ctxt->input->cur[2] == '[') && (ctxt->input->cur[3] == 'C')
							    && (ctxt->input->cur[4] == 'D') && (ctxt->input->cur[5] == 'A')
							    && (ctxt->input->cur[6] == 'T') && (ctxt->input->cur[7] == 'A')
							    && (ctxt->input->cur[8] == '[')) {
								if ((!terminate) && (!xmlParseLookupString(ctxt, 9, "]]>", 3))) {
									goto done;
								}
				...
				if (ctxt->input->cur[0] == '<') {
					...
					if (next == '?') {
						...
						if (ctxt->input->cur[2] == '-') {
							...
							if ((ctxt->input->cur[2] == 'D') && (ctxt->input->cur[3] == 'O')
			...
			case XML_PARSER_DTD: {
				...
				xmlParseInternalSubset(ctxt);
				ctxt->inSubset = 2;
				if ((ctxt->sax != NULL) && (!ctxt->disableSAX) && (ctxt->sax->externalSubset != NULL)) {
					ctxt->sax->externalSubset(
						ctxt->userData, ctxt->intSubName, ctxt->extSubSystem, ctxt->extSubURI
					);
				}
...
int xmlParseChunk(xmlParserCtxt* ctxt, const char* chunk, int size, int terminate) {
	...
	if ((size > 0) && (chunk != NULL) && (!terminate) && (chunk[size - 1] == '\r')) {
		end_in_lf = 1;
		size--;
	}
	...
	maxLength = (ctxt->options & XML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH : XML_MAX_LOOKUP_LIMIT;
...
xmlParserCtxt* xmlCreatePushParserCtxt(
	xmlSAXHandler* sax, void* user_data, const char* chunk, int size, const char* filename
) {
	xmlParserCtxtPtr  ctxt;
	xmlParserInputPtr input;
 ...
 */
xmlParserCtxt* xmlCreateIOParserCtxt(
	xmlSAXHandler*        sax,
	void*                 user_data,
	...
	input    = xmlCtxtNewInputFromIO(ctxt, NULL, ioread, ioclose, ioctx, encoding, 0);
	if (input == NULL) {
		xmlFreeParserCtxt(ctxt);
		return (NULL);
	}
 ...
 */
xmlDtd* xmlCtxtParseDtd(
	xmlParserCtxt* ctxt, xmlParserInput* input, const xmlChar* publicId, const xmlChar* systemId
) {
	xmlDtdPtr ret = NULL;
	...
	ctxt->myDoc->properties = XML_DOC_INTERNAL;
	ctxt->myDoc->extSubset  = xmlNewDtd(ctxt->myDoc, BAD_CAST "none", publicId, systemId);
...
xmlDtd* xmlSAXParseDTD(xmlSAXHandler* sax, const xmlChar* publicId, const xmlChar* systemId) {
	...
	if ((ctxt->sax != NULL) && (ctxt->sax->resolveEntity != NULL)) {
		input = ctxt->sax->resolveEntity(ctxt->userData, publicId, systemIdCanonic);
	}
	if (input == NULL) {
		xmlFreeParserCtxt(ctxt);
...
static xmlNodePtr xmlCtxtParseContentInternal(
	...
	if ((ctxt->wellFormed) || ((ctxt->recovery) && (!xmlCtxtIsCatastrophicError(ctxt)))) {
		if (root != NULL) {
			xmlNodePtr cur;
...
int xmlParseCtxtExternalEntity(
	xmlParserCtxt* ctxt, const xmlChar* URL, const xmlChar* ID, xmlNode** listOut
) {
	xmlParserInputPtr input;
	xmlNodePtr        list;
...
int xmlParseBalancedChunkMemory(
	xmlDoc* doc, xmlSAXHandler* sax, void* user_data, int depth, const xmlChar* string, xmlNode** lst
) {
 ...
 */
xmlNode*
xmlCtxtParseContent(xmlParserCtxt* ctxt, xmlParserInput* input, xmlNode* node, int hasTextDecl) {
	xmlDocPtr  doc;
...
xmlParserErrors xmlParseInNodeContext(
	xmlNode* node, const char* data, int datalen, int options, xmlNode** listOut
) {
	xmlParserCtxtPtr  ctxt;
	xmlParserInputPtr input;
	xmlDocPtr         doc;
	...
	*listOut = NULL;
	...
	input = xmlCtxtNewInputFromMemory(
		ctxt, NULL, data, datalen, (const char*)doc->encoding, XML_INPUT_BUF_STATIC
	);
	if (input == NULL) {
		xmlFreeParserCtxt(ctxt);
		return (XML_ERR_NO_MEMORY);
 ...
 */
xmlParserCtxt*
xmlCreateEntityParserCtxt(const xmlChar* URL, const xmlChar* ID, const xmlChar* base) {
	xmlParserCtxtPtr  ctxt;
	xmlParserInputPtr input;
	xmlChar*          uri = NULL;
	...
	input = xmlLoadResource(ctxt, (char*)URL, (char*)ID, XML_RESOURCE_UNKNOWN);
	if (input == NULL) {
		goto error;
	}
...
void xmlSetupParserForBuffer(xmlParserCtxt* ctxt, const xmlChar* buffer, const char* filename) {
	...
	input = xmlCtxtNewInputFromString(ctxt, filename, (const char*)buffer, NULL, 0);
	if (input == NULL) {
		return;
	}
	if (xmlCtxtPushInput(ctxt, input) < 0) {
...
xmlDoc* xmlSAXParseMemoryWithData(
	...
	input = xmlCtxtNewInputFromMemory(ctxt, NULL, buffer, size, NULL, XML_INPUT_BUF_STATIC);
...
int xmlCtxtResetPush(
	xmlParserCtxt* ctxt, const char* chunk, int size, const char* filename, const char* encoding
) {
	xmlParserInputPtr input;
...
static int xmlCtxtSetOptionsInternal(xmlParserCtxtPtr ctxt, int options, int keepMask) {
	int allMask;
...
xmlDoc* xmlReadDoc(const xmlChar* cur, const char* URL, const char* encoding, int options) {
	...
	input = xmlCtxtNewInputFromString(ctxt, URL, (const char*)cur, encoding, XML_INPUT_BUF_STATIC);
...
xmlDoc* xmlReadFile(const char* filename, const char* encoding, int options) {
	 ...
	 */
	if ((filename != NULL) && (filename[0] == '-') && (filename[1] == 0)) {
		input = xmlCtxtNewInputFromFd(ctxt, filename, STDIN_FILENO, encoding, 0);
	} else {
...
xmlReadMemory(const char* buffer, int size, const char* url, const char* encoding, int options) {
	...
	input = xmlCtxtNewInputFromMemory(ctxt, url, buffer, size, encoding, XML_INPUT_BUF_STATIC);
 ...
 * @returns the resulting document tree
 */
xmlDoc* xmlCtxtReadDoc(
	xmlParserCtxt* ctxt, const xmlChar* str, const char* URL, const char* encoding, int options
) {
	xmlParserInputPtr input;
	...
	input = xmlCtxtNewInputFromString(ctxt, URL, (const char*)str, encoding, XML_INPUT_BUF_STATIC);
	if (input == NULL) {
		return (NULL);
	}
 ...
 */
xmlDoc*
xmlCtxtReadFile(xmlParserCtxt* ctxt, const char* filename, const char* encoding, int options) {
	xmlParserInputPtr input;
 ...
 */
xmlDoc*
xmlCtxtReadFd(xmlParserCtxt* ctxt, int fd, const char* URL, const char* encoding, int options) {
	xmlParserInputPtr input;
