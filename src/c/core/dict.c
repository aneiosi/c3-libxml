 ...
 * @returns the pointer of the local string, or NULL in case of error.
 */
static const xmlChar* xmlDictAddQString(
	xmlDictPtr     dict,
...
static unsigned xmlDictHashName(unsigned seed, const xmlChar* data, size_t maxLen, size_t* plen) {
	unsigned h1, h2;
	size_t   i;
...
ATTRIBUTE_NO_SANITIZE_INTEGER
static unsigned xmlDictHashQName(
	unsigned seed, const xmlChar* prefix, const xmlChar* name, size_t* pplen, size_t* plen
) {
	unsigned h1, h2;
	size_t   i;
 ...
 */
ATTRIBUTE_NO_SANITIZE_INTEGER
unsigned xmlDictCombineHash(unsigned v1, unsigned v2) {
	/*
 ...
 */
ATTRIBUTE_NO_SANITIZE_INTEGER
static xmlDictEntry* xmlDictFindEntry(
	const xmlDict* dict,
	const xmlChar* prefix,
	const xmlChar* name,
	int            len,
		...
		do {
			if (entry->hashValue == hashValue) {
				if (prefix == NULL) {
					 ...
					 */
					if ((strncmp((const char*)entry->name, (const char*)name, len) == 0)
					    && (entry->name[len] == 0)) {
						found = 1;
						break;
					}
 ...
 */
ATTRIBUTE_NO_SANITIZE_INTEGER
static const xmlDictEntry* xmlDictLookupInternal(
	xmlDict* dict, const xmlChar* prefix, const xmlChar* name, int maybeLen, int update
) {
	...
	if ((dict->subdict != NULL) && (dict->subdict->size > 0)) {
		...
		if (prefix == NULL) {
			subHashValue = xmlDictHashName(dict->subdict->seed, name, len, &len);
		} else {
			subHashValue = xmlDictHashQName(dict->subdict->seed, prefix, name, &plen, &len);
		}
	...
	if (entry->hashValue != 0) {
		...
		if (cur < entry) {
			 ...
			 */
			memmove(&dict->table[1], dict->table, (char*)cur - (char*)dict->table);
			cur            = end - 1;
			dict->table[0] = *cur;
		}
 ...
 */
xmlHashedString xmlDictLookupHashed(xmlDict* dict, const xmlChar* name, int len) {
	const xmlDictEntry* entry;
	xmlHashedString     ret;
...
void xmlInitRandom(void) {
...

	{
		...
		status = BCryptGenRandom(
			NULL, (unsigned char*)globalRngState, sizeof(globalRngState), BCRYPT_USE_SYSTEM_PREFERRED_RNG
		);
		 ...
		 */
		globalRngState[0]
			= (unsigned)time(NULL) ^ HASH_ROL((unsigned)((size_t)&xmlInitRandom & 0xFFFFFFFF), 8);
...
ATTRIBUTE_NO_SANITIZE_INTEGER
static unsigned xoroshiro64ss(unsigned* s) {
	unsigned s0      = s[0];
	unsigned s1      = s[1];
