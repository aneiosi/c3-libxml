...
static int
xmlValidateAttributeValueInternal(xmlDocPtr doc, xmlAttributeType type, const xmlChar* value);
#endif
/************************************************************************
 *									*
...
static void xmlVErrMemory(xmlValidCtxtPtr ctxt) {
	if (ctxt != NULL) {
		if (ctxt->flags & XML_VCTXT_USE_PCTXT) {
			...
			xmlRaiseMemoryError(NULL, ctxt->error, ctxt->userData, XML_FROM_VALID, NULL);
		}
	} else {
...
static int xmlDoErrValid(
	...
	va_start(ap, msg);
	if (pctxt != NULL) {
		xmlCtxtVErr(pctxt, node, XML_FROM_VALID, code, level, str1, str2, str3, int1, msg, ap);
 ...
 */
static void LIBXML_ATTR_FORMAT(4, 0) xmlErrValidNode(
	xmlValidCtxtPtr ctxt,
	xmlNodePtr      node,
	...
	xmlDoErrValid(ctxt, node, error, XML_ERR_ERROR, str1, str2, str3, 0, msg, str1, str2, str3);
}
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlErrValid(
	xmlValidCtxtPtr ctxt, xmlParserErrors error, const char* msg, const char* extra
) {
	xmlDoErrValid(
		ctxt, NULL, error, XML_ERR_ERROR, (const xmlChar*)extra, NULL, NULL, 0, msg, extra
	);
}
 ...
 * @param str3  extra information
 */
static void LIBXML_ATTR_FORMAT(4, 0) xmlErrValidNodeNr(
	xmlValidCtxtPtr ctxt,
	xmlNodePtr      node,
	xmlParserErrors error,
	const char*     msg,
	...
	xmlDoErrValid(ctxt, node, error, XML_ERR_ERROR, str1, str3, NULL, int2, msg, str1, int2, str3);
}
 ...
 * @returns 0 on success, -1 if a memory allocation failed
 */
static int LIBXML_ATTR_FORMAT(4, 0) xmlErrValidWarning(
	xmlValidCtxtPtr ctxt,
	xmlNodePtr      node,
	xmlParserErrors error,
	const char*     msg,
	...
	return xmlDoErrValid(
		ctxt, node, error, XML_ERR_WARNING, str1, str2, str3, 0, msg, str1, str2, str3
	);
}
...
static int vstateVPush(xmlValidCtxtPtr ctxt, xmlElementPtr elemDecl, xmlNodePtr node) {
	if (ctxt->vstateNr >= ctxt->vstateMax) {
		xmlValidState* tmp;
		int            newSize;

		newSize = xmlGrowCapacity(ctxt->vstateMax, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlVErrMemory(ctxt);
			return (-1);
		}
	...
	if ((elemDecl != NULL) && (elemDecl->etype == XML_ELEMENT_TYPE_ELEMENT)) {
		...
		if (elemDecl->contModel != NULL) {
			ctxt->vstateTab[ctxt->vstateNr].exec = xmlRegNewExecCtxt(elemDecl->contModel, NULL, NULL);
...
static int vstateVPush(
	...
	if (ctxt->vstateNr >= ctxt->vstateMax) {
		...
		newSize = xmlGrowCapacity(ctxt->vstateMax, sizeof(tmp[0]), 8, MAX_RECURSE);
		if (newSize < 0) {
			xmlVErrMemory(ctxt);
			return (-1);
		}
	 ...
	 */
	if ((i >= 0) && (ctxt->vstateTab[i].cont == cont) && (ctxt->vstateTab[i].node == node)
	    && (ctxt->vstateTab[i].depth == depth) && (ctxt->vstateTab[i].occurs == occurs)
...
static int nodeVPush(xmlValidCtxtPtr ctxt, xmlNodePtr value) {
	if (ctxt->nodeNr >= ctxt->nodeMax) {
		xmlNodePtr* tmp;
		int         newSize;

		newSize = xmlGrowCapacity(ctxt->nodeMax, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlVErrMemory(ctxt);
			return (-1);
		}
...
static xmlNodePtr nodeVPop(xmlValidCtxtPtr ctxt) {
	xmlNodePtr ret;
...
/* TODO: use hash table for accesses to elem and attribute definitions */
	...
	#define CHECK_DTD                                                \
		if (doc == NULL)                                               \
			return (0);                                                  \
		else if ((doc->intSubset == NULL) && (doc->extSubset == NULL)) \
		return (0)
 ...
 */
static int xmlValidBuildAContentModel(
	xmlElementContentPtr content, xmlValidCtxtPtr ctxt, const xmlChar* name
) {
	if (content == NULL) {
		xmlErrValidNode(
			ctxt,
	...
	switch (content->type) {
		...
		case XML_ELEMENT_CONTENT_ELEMENT: {
			...
			switch (content->ocur) {
				case XML_ELEMENT_CONTENT_ONCE:
					ctxt->state = xmlAutomataNewTransition(ctxt->am, ctxt->state, NULL, fullname, NULL);
					break;
				...
				case XML_ELEMENT_CONTENT_PLUS:
					ctxt->state = xmlAutomataNewTransition(ctxt->am, ctxt->state, NULL, fullname, NULL);
					xmlAutomataNewTransition(ctxt->am, ctxt->state, ctxt->state, fullname, NULL);
					break;
		...
		case XML_ELEMENT_CONTENT_OR: {
			...
			ocur = content->ocur;
			if ((ocur == XML_ELEMENT_CONTENT_PLUS) || (ocur == XML_ELEMENT_CONTENT_MULT)) {
				ctxt->state = xmlAutomataNewEpsilon(ctxt->am, ctxt->state, NULL);
			}
			oldstate = ctxt->state;
		...
		default:
			xmlErrValid(
				ctxt, XML_ERR_INTERNAL_ERROR, "ContentModel broken for element %s\n", (const char*)name
			);
			return (0);
	}
	return (1);
 ...
 */
xmlElementContent*
xmlNewDocElementContent(xmlDoc* doc, const xmlChar* name, xmlElementContentType type) {
...
xmlElement* xmlAddElementDecl(
	...
	if ((dtd->doc != NULL) && (dtd->doc->intSubset != NULL)) {
		...
		if ((ret != NULL) && (ret->etype == XML_ELEMENT_TYPE_UNDEFINED)) {
			...
			ret->attributes = NULL;
			xmlHashRemoveEntry2(dtd->doc->intSubset->elements, localName, prefix, NULL);
			xmlFreeElement(ret);
		}
	}
 ...
 */
static void* xmlCopyElement(void* payload, const xmlChar* name ATTRIBUTE_UNUSED) {
 ...
 * @param name  unused
 */
static void xmlDumpElementDeclScan(void* elem, void* save, const xmlChar* name ATTRIBUTE_UNUSED) {
 ...
 */
xmlAttribute* xmlAddAttributeDecl(
	xmlValidCtxt*       ctxt,
	xmlDtd*             dtd,
	const xmlChar*      elem,
	const xmlChar*      name,
...
#ifdef LIBXML_VALID_ENABLED
	if ((ctxt != NULL) && (ctxt->flags & XML_VCTXT_VALIDATE) && (defaultValue != NULL)
	 ...
	 */
	if ((dtd->doc != NULL) && (dtd->doc->extSubset == dtd) && (dtd->doc->intSubset != NULL)
	...
	if ((xmlStrEqual(ret->name, BAD_CAST "xmlns"))
	    || (ret->prefix != NULL && (xmlStrEqual(ret->prefix, BAD_CAST "xmlns")))) {
		...
		while (
			(tmp != NULL)
			&& ((xmlStrEqual(tmp->name, BAD_CAST "xmlns")) || (ret->prefix != NULL && (xmlStrEqual(ret->prefix, BAD_CAST "xmlns"))))
		) {
			if (tmp->nexth == NULL) {
				break;
			}
 ...
 */
static void* xmlCopyAttribute(void* payload, const xmlChar* name ATTRIBUTE_UNUSED) {
 ...
 */
static void
xmlDumpAttributeDeclScan(void* attr, void* save, const xmlChar* name ATTRIBUTE_UNUSED) {
 ...
 */
xmlNotation* xmlAddNotationDecl(
	xmlValidCtxt*  ctxt,
	xmlDtd*        dtd,
	const xmlChar* name,
 ...
 */
static void* xmlCopyNotation(void* payload, const xmlChar* name ATTRIBUTE_UNUSED) {
 ...
 * allocation fails.
 */
static int xmlAddIDInternal(xmlAttrPtr attr, const xmlChar* value, xmlIDPtr* idPtr) {
	xmlDocPtr     doc;
	xmlIDPtr      id;
	xmlIDTablePtr table;
	int           ret;
...
xmlID* xmlAddID(xmlValidCtxt* ctxt, xmlDoc* doc, const xmlChar* value, xmlAttr* attr) {
	...
	if (res < 0) {
		...
		if (ctxt != NULL) {
			 ...
			 */
			xmlErrValidNode(
				ctxt, attr->parent, XML_DTD_ID_REDEFINED, "ID %s already defined\n", value, NULL, NULL
			);
		}
	}
...
int xmlIsID(xmlDoc* doc, xmlNode* elem, xmlAttr* attr) {
	...
	if ((doc != NULL) && (doc->type == XML_HTML_DOCUMENT_NODE)) {
		...
		if ((xmlStrEqual(BAD_CAST "name", attr->name)) && (xmlStrEqual(elem->name, BAD_CAST "a"))) {
			return (1);
		}
	} else {
		...
		if ((attr->ns != NULL) && (attr->ns->prefix != NULL) && (!strcmp((char*)attr->name, "id"))
		...
		if ((elem == NULL) || (elem->type != XML_ELEMENT_NODE) || (elem->name == NULL)) {
			return (0);
		}
		...
		if (fullelemname != NULL) {
			attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, fullelemname, attr->name, aprefix);
			if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
				attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, fullelemname, attr->name, aprefix);
			}
		}
 ...
 */
static int
xmlDummyCompare(const void* data0 ATTRIBUTE_UNUSED, const void* data1 ATTRIBUTE_UNUSED) {
	return (0);
}
...
int xmlIsRef(xmlDoc* doc, xmlNode* elem, xmlAttr* attr) {
	...
	if ((doc->intSubset == NULL) && (doc->extSubset == NULL)) {
		...
		aprefix  = (attr->ns != NULL) ? attr->ns->prefix : NULL;
		attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, elem->name, attr->name, aprefix);
		if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
			attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, elem->name, attr->name, aprefix);
		}

		if ((attrDecl != NULL)
		    && (attrDecl->atype == XML_ATTRIBUTE_IDREF || attrDecl->atype == XML_ATTRIBUTE_IDREFS)) {
			return (1);
		}
	}
	return (0);
...
xmlElement* xmlGetDtdElementDesc(xmlDtd* dtd, const xmlChar* name) {
	...
	if ((dtd == NULL) || (dtd->elements == NULL) || (name == NULL)) {
		return (NULL);
	}
...
xmlAttribute* xmlGetDtdAttrDesc(xmlDtd* dtd, const xmlChar* elem, const xmlChar* name) {
	...
	if ((dtd == NULL) || (dtd->attributes == NULL) || (elem == NULL) || (name == NULL)) {
		return (NULL);
	}
...
xmlAttribute*
xmlGetDtdQAttrDesc(xmlDtd* dtd, const xmlChar* elem, const xmlChar* name, const xmlChar* prefix) {
	xmlAttributeTablePtr table;
...
int xmlValidateNotationUse(xmlValidCtxt* ctxt, xmlDoc* doc, const xmlChar* notationName) {
	...
	if ((doc == NULL) || (doc->intSubset == NULL) || (notationName == NULL)) {
		return (-1);
	}
...
static int xmlValidateNameValueInternal(const xmlChar* value, int flags) {
	if ((value == NULL) || (value[0] == 0)) {
		return (0);
	}
...
static int xmlValidateNamesValueInternal(const xmlChar* value, int flags) {
	const xmlChar* cur;
...
static int xmlValidateNmtokenValueInternal(const xmlChar* value, int flags) {
	if ((value == NULL) || (value[0] == 0)) {
		return (0);
	}
...
static int xmlValidateNmtokensValueInternal(const xmlChar* value, int flags) {
	const xmlChar* cur;
...
int xmlValidateNotationDecl(
	xmlValidCtxt* ctxt ATTRIBUTE_UNUSED,
...
static int xmlValidateAttributeValue2(
	xmlValidCtxtPtr  ctxt,
	xmlDocPtr        doc,
	const xmlChar*   name,
	xmlAttributeType type,
...
xmlChar* xmlValidCtxtNormalizeAttributeValue(
	xmlValidCtxt* ctxt, xmlDoc* doc, xmlNode* elem, const xmlChar* name, const xmlChar* value
) {
	xmlChar*        ret;
	xmlAttributePtr attrDecl = NULL;
	const xmlChar*  localName;
	...
	if ((elem->ns != NULL) && (elem->ns->prefix != NULL)) {
		...
		if (doc->intSubset != NULL) {
			attrDecl = xmlHashLookup3(doc->intSubset->attributes, localName, prefix, elemname);
		}
		if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
			attrDecl = xmlHashLookup3(doc->extSubset->attributes, localName, prefix, elemname);
			if (attrDecl != NULL) {
				extsubset = 1;
			}
		}
	...
	if ((attrDecl == NULL) && (doc->intSubset != NULL)) {
		attrDecl = xmlHashLookup3(doc->intSubset->attributes, localName, prefix, elem->name);
	}
	if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
		attrDecl = xmlHashLookup3(doc->extSubset->attributes, localName, prefix, elem->name);
		if (attrDecl != NULL) {
			extsubset = 1;
		}
	}
...
xmlChar* xmlValidNormalizeAttributeValue(
	xmlDoc* doc, xmlNode* elem, const xmlChar* name, const xmlChar* value
) {
	xmlChar*        ret;
	xmlAttributePtr attrDecl = NULL;
...
static void
xmlValidateAttributeIdCallback(void* payload, void* data, const xmlChar* name ATTRIBUTE_UNUSED) {
	xmlAttributePtr attr  = (xmlAttributePtr)payload;
	int*            count = (int*)data;
...
int xmlValidateAttributeDecl(xmlValidCtxt* ctxt, xmlDoc* doc, xmlAttribute* attr) {
	...
	if (attr->atype == XML_ATTRIBUTE_ID) {
		...
		if (doc->intSubset != NULL) {
			elem = xmlHashLookup2(doc->intSubset->elements, elemLocalName, elemPrefix);
		}
		if (elem != NULL) {
			...
			if (doc->intSubset != NULL) {
				...
				xmlHashScan3(table, NULL, NULL, attr->elem, xmlValidateAttributeIdCallback, &nbId);
			}
		}
		if (nbId > 1) {
			xmlErrValidNodeNr(
			...
			int extId = 0;
			elem      = xmlHashLookup2(doc->extSubset->elements, elemLocalName, elemPrefix);
			if (elem != NULL) {
	...
	if ((attr->defaultValue != NULL) && (attr->tree != NULL)) {
		...
		while (tree != NULL) {
			if (xmlStrEqual(tree->name, attr->defaultValue)) {
				break;
			}
			tree = tree->next;
		}
...
int xmlValidateElementDecl(xmlValidCtxt* ctxt, xmlDoc* doc, xmlElement* elem) {
			...
			if (cur->c1->type == XML_ELEMENT_CONTENT_ELEMENT) {
				...
				while (next != NULL) {
					if (next->type == XML_ELEMENT_CONTENT_ELEMENT) {
						if ((xmlStrEqual(next->name, name)) && (xmlStrEqual(next->prefix, cur->c1->prefix))) {
	...
	if (doc->intSubset != NULL) {
		...
		if ((tst != NULL) && (tst != elem)
		    && ((tst->prefix == elem->prefix) || (xmlStrEqual(tst->prefix, elem->prefix)))
...
int xmlValidateOneAttribute(
	xmlValidCtxt* ctxt, xmlDoc* doc, xmlNode* elem, xmlAttr* attr, const xmlChar* value
) {
	xmlAttributePtr attrDecl = NULL;
	const xmlChar*  aprefix;
	int             val;
	...
	if ((elem->ns != NULL) && (elem->ns->prefix != NULL)) {
		...
		attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, fullname, attr->name, aprefix);
		if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
			attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, fullname, attr->name, aprefix);
		}
		if ((fullname != fn) && (fullname != elem->name)) {
			xmlFree(fullname);
		}
	...
	if ((attrDecl->def == XML_ATTRIBUTE_FIXED) && (!xmlStrEqual(attrDecl->defaultValue, value))) {
		xmlErrValidNode(
			ctxt,
			elem,
			XML_DTD_ATTRIBUTE_VALUE,
...
int xmlValidateOneNamespace(
	...
	if (prefix != NULL) {
		...
		if (ns->prefix != NULL) {
			...
			attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, fullname, BAD_CAST "xmlns", NULL);
			if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
				attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, fullname, BAD_CAST "xmlns", NULL);
			}
		}
		if ((fullname != fn) && (fullname != elem->name)) {
			xmlFree(fullname);
	...
	if (attrDecl == NULL) {
		if (ns->prefix != NULL) {
			...
			attrDecl = xmlGetDtdQAttrDesc(doc->intSubset, elem->name, BAD_CAST "xmlns", NULL);
			if ((attrDecl == NULL) && (doc->extSubset != NULL)) {
				attrDecl = xmlGetDtdQAttrDesc(doc->extSubset, elem->name, BAD_CAST "xmlns", NULL);
			}
		}
	}
...
static xmlNodePtr xmlValidateSkipIgnorable(xmlNodePtr child) {
	while (child != NULL) {
		switch (child->type) {
...
static int xmlValidateElementType(xmlValidCtxtPtr ctxt) {
	...
	if ((NODE == NULL)
	    && ((CONT->ocur == XML_ELEMENT_CONTENT_MULT) || (CONT->ocur == XML_ELEMENT_CONTENT_OPT))) {
		return (1);
	}
	if (CONT == NULL) {
		return (-1);
...
cont:
	...
	switch (CONT->type) {
		case XML_ELEMENT_CONTENT_PCDATA:
			...
			if (NODE->type == XML_TEXT_NODE) {
				...
				do {
					...
					(NODE != NULL)
					&& ((NODE->type != XML_ELEMENT_NODE) && (NODE->type != XML_TEXT_NODE) && (NODE->type != XML_CDATA_SECTION_NODE))
				);
				ret = 1;
				break;
			} else {
		...
		case XML_ELEMENT_CONTENT_ELEMENT:
			...
			ret = ((NODE->type == XML_ELEMENT_NODE) && (xmlStrEqual(NODE->name, CONT->name)));
			if (ret == 1) {
		...
		case XML_ELEMENT_CONTENT_OR:
			...
			if (CONT->c1->type == XML_ELEMENT_CONTENT_ELEMENT) {
				if ((NODE == NULL) || (!xmlStrEqual(NODE->name, CONT->c1->name))) {
					DEPTH++;
					CONT = CONT->c2;
					goto cont;
				}
			 ...
			 */
			if (vstateVPush(ctxt, CONT->c2, NODE, DEPTH + 1, OCCURS, ROLLBACK_OR) < 0) {
				return (-1);
			}
			DEPTH++;
			CONT = CONT->c1;
		...
		case XML_ELEMENT_CONTENT_SEQ:
			 ...
			 */
			if ((CONT->c1->type == XML_ELEMENT_CONTENT_ELEMENT)
			    && ((CONT->c1->ocur == XML_ELEMENT_CONTENT_OPT) || (CONT->c1->ocur == XML_ELEMENT_CONTENT_MULT))) {
				if ((NODE == NULL) || (!xmlStrEqual(NODE->name, CONT->c1->name))) {
					DEPTH++;
					CONT = CONT->c2;
					goto cont;
...
analyze:
	while (CONT != NULL) {
		 ...
		 */
		RESET_OCCURRENCE;
		if ((CONT->parent == NULL) || (CONT->parent == (xmlElementContentPtr)1)) {
			break;
		}
 ...
 */
static void xmlSnprintfElements(char* buf, int size, xmlNodePtr node, int glob) {
	xmlNodePtr cur;
	int        len;
...
static int xmlValidateElementContent(
	xmlValidCtxtPtr ctxt, xmlNodePtr child, xmlElementPtr elemDecl, int warn, xmlNodePtr parent
) {
	int ret = 1;
	#ifndef LIBXML_REGEXP_ENABLED
 ...
 * @returns 1 if valid or 0 otherwise.
 */
static int xmlValidateOneCdataElement(xmlValidCtxtPtr ctxt, xmlDocPtr doc, xmlNodePtr elem) {
	int        ret = 1;
	xmlNodePtr cur, child;

	if ((ctxt == NULL) || (doc == NULL) || (elem == NULL) || (elem->type != XML_ELEMENT_NODE)) {
		return (0);
	}
 ...
 */
static int
xmlValidateCheckMixed(xmlValidCtxtPtr ctxt, xmlElementContentPtr cont, const xmlChar* qname) {
	const xmlChar* name;
	int            plen;
	...
	if (name == NULL) {
		while (cont != NULL) {
			...
			} else if ((cont->type == XML_ELEMENT_CONTENT_OR) && (cont->c1 != NULL)
			           && (cont->c1->type == XML_ELEMENT_CONTENT_ELEMENT)) {
				if ((cont->c1->prefix == NULL) && (xmlStrEqual(cont->c1->name, qname))) {
					return (1);
				}
			} else if ((cont->type != XML_ELEMENT_CONTENT_OR) || (cont->c1 == NULL)
		...
		while (cont != NULL) {
			if (cont->type == XML_ELEMENT_CONTENT_ELEMENT) {
				if ((cont->prefix != NULL) && (xmlStrncmp(cont->prefix, qname, plen) == 0)
				    && (xmlStrEqual(cont->name, name))) {
					return (1);
				}
			} else if ((cont->type == XML_ELEMENT_CONTENT_OR) && (cont->c1 != NULL)
			           && (cont->c1->type == XML_ELEMENT_CONTENT_ELEMENT)) {
				if ((cont->c1->prefix != NULL) && (xmlStrncmp(cont->c1->prefix, qname, plen) == 0)
 ...
 */
static xmlElementPtr
xmlValidGetElemDecl(xmlValidCtxtPtr ctxt, xmlDocPtr doc, xmlNodePtr elem, int* extsubset) {
	xmlElementPtr  elemDecl = NULL;
	const xmlChar* prefix   = NULL;

	if ((ctxt == NULL) || (doc == NULL) || (elem == NULL) || (elem->name == NULL)) {
		return (NULL);
	}
	if (extsubset != NULL) {
		*extsubset = 0;
	...
	if (elemDecl == NULL) {
		xmlErrValidNode(
			ctxt, elem, XML_DTD_UNKNOWN_ELEM, "No declaration for element %s\n", elem->name, NULL, NULL
		);
	}
	return (elemDecl);
}
...
int xmlValidateOneElement(xmlValidCtxt* ctxt, xmlDoc* doc, xmlNode* elem) {
	...
	switch (elem->type) {
		...
		default:
			xmlErrValidNode(
				ctxt, elem, XML_ERR_INTERNAL_ERROR, "unexpected element type\n", NULL, NULL, NULL
			);
			return (0);
	}
				...
				while (child != NULL) {
					if (child->type == XML_ELEMENT_NODE) {
						...
						if ((child->ns != NULL) && (child->ns->prefix != NULL)) {
							...
							fullname = xmlBuildQName(child->name, child->ns->prefix, fn, 50);
							if (fullname == NULL) {
								xmlVErrMemory(ctxt);
								return (0);
							}
								...
								if (cont->type == XML_ELEMENT_CONTENT_ELEMENT) {
									...
									if (xmlStrEqual(cont->c1->name, fullname)) {
										break;
								           ...
								           || (cont->c1->type != XML_ELEMENT_CONTENT_PCDATA)) {
									xmlErrValid(
										ctxt, XML_DTD_MIXED_CORRUPT, "Internal: MIXED struct corrupted\n", NULL
									);
									break;
								}
								cont = cont->c2;
							...
							if (cont->type == XML_ELEMENT_CONTENT_ELEMENT) {
								if (xmlStrEqual(cont->name, name)) {
									break;
							           ...
							           && (cont->c1->type == XML_ELEMENT_CONTENT_ELEMENT)) {
								if (xmlStrEqual(cont->c1->name, name)) {
									break;
								}
							} else if ((cont->type != XML_ELEMENT_CONTENT_OR) || (cont->c1 == NULL)
							           || (cont->c1->type != XML_ELEMENT_CONTENT_PCDATA)) {
...
child_ok:
	...
	while (attr != NULL) {
		if (attr->def == XML_ATTRIBUTE_REQUIRED) {
			...
			if ((attr->prefix == NULL) && (xmlStrEqual(attr->name, BAD_CAST "xmlns"))) {
				xmlNsPtr ns;
...
int xmlValidateElement(xmlValidCtxt* ctxt, xmlDoc* doc, xmlNode* root) {
	xmlNodePtr elem;
	xmlAttrPtr attr;
	xmlNsPtr   ns;
	xmlChar*   value;
	...
	while (1) {
		...
		if (elem->type == XML_ELEMENT_NODE) {
			...
			while (ns != NULL) {
				if (elem->ns == NULL) {
					ret &= xmlValidateOneNamespace(ctxt, doc, elem, NULL, ns, ns->href);
				} else {
					ret &= xmlValidateOneNamespace(ctxt, doc, elem, elem->ns->prefix, ns, ns->href);
				}
				ns = ns->next;
			}
 ...
 * @returns 0 to abort the walk or 1 to continue.
 */
static int xmlWalkValidateList(const void* data, void* user) {
 ...
 * @param name  name of ID we are searching for
 */
static void xmlValidateCheckRefCallback(void* payload, void* data, const xmlChar* name) {
...
static void
xmlValidateNotationCallback(void* payload, void* data, const xmlChar* name ATTRIBUTE_UNUSED) {
...
static void
xmlValidateAttributeCallback(void* payload, void* data, const xmlChar* name ATTRIBUTE_UNUSED) {
	...
	if (cur->atype == XML_ATTRIBUTE_NOTATION) {
		...
		if ((elem == NULL) && (cur->parent != NULL) && (cur->parent->type == XML_DTD_NODE)) {
			elem = xmlHashLookup2(((xmlDtdPtr)cur->parent)->elements, elemLocalName, elemPrefix);
		}
 ...
 */
static int
xmlValidateDocumentInternal(xmlParserCtxtPtr ctxt, xmlValidCtxtPtr vctxt, xmlDocPtr doc) {
	int        ret;
	xmlNodePtr root;
	...
	if ((doc->intSubset != NULL)
	    && ((doc->intSubset->SystemID != NULL) || (doc->intSubset->ExternalID != NULL))
	    && (doc->extSubset == NULL)) {
		xmlChar* sysID = NULL;
		...
		if (ctxt != NULL) {
			...
			input = xmlLoadResource(
				ctxt, (const char*)sysID, (const char*)doc->intSubset->ExternalID, XML_RESOURCE_DTD
			);
			if (input == NULL) {
				xmlFree(sysID);
				return 0;
			...
			doc->extSubset = xmlCtxtParseDtd(ctxt, input, doc->intSubset->ExternalID, sysID);
		} else {
 ...
 * @returns 1 if valid or 0 otherwise.
 */
int xmlValidateDocument(xmlValidCtxt* vctxt, xmlDoc* doc) {
...
int
xmlValidGetPotentialChildren(xmlElementContent* ctree, const xmlChar** names, int* len, int max) {
	int i;
	...
	switch (ctree->type) {
		case XML_ELEMENT_CONTENT_PCDATA:
			for (i = 0; i < *len; i++) {
				if (xmlStrEqual(BAD_CAST "#PCDATA", names[i])) {
					return (*len);
				}
			}
		...
		case XML_ELEMENT_CONTENT_ELEMENT:
			for (i = 0; i < *len; i++) {
				if (xmlStrEqual(ctree->name, names[i])) {
					return (*len);
				}
			}
 ...
 */
static void xmlNoValidityErr(void* ctx ATTRIBUTE_UNUSED, const char* msg ATTRIBUTE_UNUSED, ...) {}
	...
	for (i = 0; i < nb_elements; i++) {
		...
		if (xmlValidateOneElement(&vctxt, parent->doc, parent)) {
			...
			for (j = 0; j < nb_valid_elements; j++) {
				if (xmlStrEqual(elements[i], names[j])) {
					break;
				}
			}
