...
static const xmlChar* xmlRelaxNGNs = (const xmlChar*)"http://relaxng.org/ns/structure/1.0";
	...
	#define IS_RELAXNG(node, typ)                                               \
		((node != NULL) && (node->ns != NULL) && (node->type == XML_ELEMENT_NODE) \
 ...
 */
static void xmlRngPErrMemory(xmlRelaxNGParserCtxtPtr ctxt) {
 ...
 */
static void xmlRngVErrMemory(xmlRelaxNGValidCtxtPtr ctxt) {
	xmlStructuredErrorFunc schannel = NULL;
	xmlGenericErrorFunc    channel  = NULL;
	void*                  data     = NULL;
 ...
 */
static void LIBXML_ATTR_FORMAT(4, 0) xmlRngPErr(
 ...
 * @param str2  extra info
 */
static void LIBXML_ATTR_FORMAT(4, 0) xmlRngVErr(
 ...
 * @returns 1 if yes, 0 if no and -1 in case of error.
 */
typedef int (*xmlRelaxNGTypeCheck)(
	void* data, const xmlChar* type, const xmlChar* value, void** result, xmlNode* node
);
 ...
 * @returns 1 if yes, 0 if no and -1 in case of error.
 */
typedef int (*xmlRelaxNGFacetCheck)(
	void*          data,
	const xmlChar* type,
	const xmlChar* facet,
	const xmlChar* val,
 ...
 * @returns 1 if yes, 0 if no and -1 in case of error.
 */
typedef int  (*xmlRelaxNGTypeCompare)(
  void*          data,
  const xmlChar* type,
...
static void xmlRelaxNGFreeValidState(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGValidStatePtr state);
 ...
 */
static void xmlRelaxNGFreeDocument(xmlRelaxNGDocumentPtr docu) {
	if (docu == NULL) {
		return;
	}
 ...
 * @returns the newly allocated structure or NULL in case or error
 */
static xmlRelaxNGDefinePtr xmlRelaxNGNewDefine(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	xmlRelaxNGDefinePtr ret;

	if (ctxt->defMax == 0) {
		ctxt->defMax = 16;
		ctxt->defNr  = 0;
		ctxt->defTab = (xmlRelaxNGDefinePtr*)xmlMalloc(ctxt->defMax * sizeof(xmlRelaxNGDefinePtr));
		if (ctxt->defTab == NULL) {
			xmlRngPErrMemory(ctxt);
			return (NULL);
		}
		...
		ctxt->defMax *= 2;
		tmp
			= (xmlRelaxNGDefinePtr*)xmlRealloc(ctxt->defTab, ctxt->defMax * sizeof(xmlRelaxNGDefinePtr));
		if (tmp == NULL) {
			xmlRngPErrMemory(ctxt);
			return (NULL);
		}
...
static xmlRelaxNGStatesPtr xmlRelaxNGNewStates(xmlRelaxNGValidCtxtPtr ctxt, int size) {
	...
	if ((ctxt != NULL) && (ctxt->freeStates != NULL) && (ctxt->freeStatesNr > 0)) {
		ctxt->freeStatesNr--;
	...
	ret = (xmlRelaxNGStatesPtr)xmlMalloc(
		sizeof(xmlRelaxNGStates) + (size - 1) * sizeof(xmlRelaxNGValidStatePtr)
	);
	if (ret == NULL) {
		xmlRngVErrMemory(ctxt);
		return (NULL);
	...
	ret->nbState  = 0;
	ret->maxState = size;
	ret->tabState = (xmlRelaxNGValidStatePtr*)xmlMalloc((size) * sizeof(xmlRelaxNGValidStatePtr));
 ...
 */
static int xmlRelaxNGAddStatesUniq(
	xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGStatesPtr states, xmlRelaxNGValidStatePtr state
) {
	if (state == NULL) {
		return (-1);
	}
	if (states->nbState >= states->maxState) {
		...
		tmp  = (xmlRelaxNGValidStatePtr*)xmlRealloc(
      states->tabState, (size) * sizeof(xmlRelaxNGValidStatePtr)
    );
		if (tmp == NULL) {
			xmlRngVErrMemory(ctxt);
			return (-1);
 ...
 * @returns 1 in case of success and 0 if this is a duplicate and -1 on error
 */
static int xmlRelaxNGAddStates(
	xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGStatesPtr states, xmlRelaxNGValidStatePtr state
) {
	int i;
 ...
 */
static void xmlRelaxNGFreeStates(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGStatesPtr states) {
	if (states == NULL) {
		return;
	}
	if ((ctxt != NULL) && (ctxt->freeStates == NULL)) {
		...
		ctxt->freeStatesNr  = 0;
		ctxt->freeStates
			= (xmlRelaxNGStatesPtr*)xmlMalloc(ctxt->freeStatesMax * sizeof(xmlRelaxNGStatesPtr));
		...
		tmp = (xmlRelaxNGStatesPtr*)xmlRealloc(
			ctxt->freeStates, 2 * ctxt->freeStatesMax * sizeof(xmlRelaxNGStatesPtr)
		);
		if (tmp == NULL) {
			xmlRngVErrMemory(ctxt);
...
xmlRelaxNGNewValidState(xmlRelaxNGValidCtxtPtr ctxt, xmlNodePtr node) {
	...
	if (nbAttrs > 0) {
		if (ret->attrs == NULL) {
			...
			tmp = (xmlAttrPtr*)xmlRealloc(ret->attrs, nbAttrs * sizeof(xmlAttrPtr));
			if (tmp == NULL) {
				xmlRngVErrMemory(ctxt);
				return (ret);
			}
...
xmlRelaxNGCopyValidState(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGValidStatePtr state) {
	...
	if (state->nbAttrs > 0) {
		if (ret->attrs == NULL) {
			...
			tmp = (xmlAttrPtr*)xmlRealloc(ret->attrs, state->maxAttrs * sizeof(xmlAttrPtr));
			if (tmp == NULL) {
				xmlRngVErrMemory(ctxt);
				ret->nbAttrs = 0;
				return (ret);
...
static int xmlRelaxNGEqualValidState(
	...
	if ((state1->value != state2->value) && (!xmlStrEqual(state1->value, state2->value))) {
		return (0);
	}
 ...
 */
int xmlRelaxParserSetFlag(xmlRelaxNGParserCtxt* ctxt, int flags) {
	if (ctxt == NULL) {
		return (-1);
	}
...
static xmlDoc*   xmlRelaxReadFile(xmlRelaxNGParserCtxtPtr ctxt, const char* filename) {
  ...
  if (ctxt->resourceLoader != NULL) {
    xmlCtxtSetResourceLoader(pctxt, ctxt->resourceLoader, ctxt->resourceCtxt);
  }
...
static xmlDoc* xmlRelaxReadMemory(xmlRelaxNGParserCtxtPtr ctxt, const char* buf, int size) {
	xmlParserCtxtPtr pctxt;
	xmlDocPtr        doc;
...
static int xmlRelaxNGIncludePush(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGIncludePtr value) {
	if (ctxt->incTab == NULL) {
		ctxt->incMax = 4;
		ctxt->incNr  = 0;
		ctxt->incTab = (xmlRelaxNGIncludePtr*)xmlMalloc(ctxt->incMax * sizeof(ctxt->incTab[0]));
		if (ctxt->incTab == NULL) {
			xmlRngPErrMemory(ctxt);
			return (0);
		}
	...
	if (ctxt->incNr >= ctxt->incMax) {
		ctxt->incMax *= 2;
		ctxt->incTab
			= (xmlRelaxNGIncludePtr*)xmlRealloc(ctxt->incTab, ctxt->incMax * sizeof(ctxt->incTab[0]));
		if (ctxt->incTab == NULL) {
			xmlRngPErrMemory(ctxt);
			return (0);
		}
 ...
 */
static xmlRelaxNGIncludePtr xmlRelaxNGIncludePop(xmlRelaxNGParserCtxtPtr ctxt) {
 ...
 */
static int xmlRelaxNGRemoveRedefine(
	xmlRelaxNGParserCtxtPtr ctxt,
	...
	while (tmp != NULL) {
		...
		if ((name == NULL) && (IS_RELAXNG(tmp, "start"))) {
			...
			if ((inc != NULL) && (inc->doc != NULL) && (inc->doc->children != NULL)) {
				if (xmlStrEqual(inc->doc->children->name, BAD_CAST "grammar")) {
					if (xmlRelaxNGRemoveRedefine(ctxt, href, xmlDocGetRootElement(inc->doc)->children, name)
					    == 1) {
						found = 1;
					}
				}
 ...
 */
static xmlRelaxNGIncludePtr xmlRelaxNGLoadInclude(
	xmlRelaxNGParserCtxtPtr ctxt, const xmlChar* URL, xmlNodePtr node, const xmlChar* ns
) {
	...
	for (i = 0; i < ctxt->incNr; i++) {
		if (xmlStrEqual(ctxt->incTab[i]->href, URL)) {
			xmlRngPErr(
				ctxt, NULL, XML_RNGP_INCLUDE_RECURSE, "Detected an Include recursion for %s\n", URL, NULL
			);
			return (NULL);
		}
	}
	...
	if (doc == NULL) {
		xmlRngPErr(ctxt, node, XML_RNGP_PARSE_ERROR, "xmlRelaxNG: could not load %s\n", URL, NULL);
		return (NULL);
	}
	...
	if (root == NULL) {
		xmlRngPErr(
			ctxt, node, XML_RNGP_EMPTY, "xmlRelaxNG: included document is empty %s\n", URL, NULL
		);
		return (NULL);
	}
 ...
 * @returns 0 in case of error, the index in the stack otherwise
 */
static int xmlRelaxNGValidErrorPush(
	xmlRelaxNGValidCtxtPtr ctxt,
	xmlRelaxNGValidErr     err,
	const xmlChar*         arg1,
	const xmlChar*         arg2,
	...
	if (ctxt->errTab == NULL) {
		ctxt->errMax = 8;
		ctxt->errNr  = 0;
		ctxt->errTab = (xmlRelaxNGValidErrorPtr)xmlMalloc(ctxt->errMax * sizeof(xmlRelaxNGValidError));
		if (ctxt->errTab == NULL) {
			xmlRngVErrMemory(ctxt);
			return (0);
		}
	...
	if (ctxt->errNr >= ctxt->errMax) {
		ctxt->errMax *= 2;
		ctxt->errTab  = (xmlRelaxNGValidErrorPtr)xmlRealloc(
      ctxt->errTab, ctxt->errMax * sizeof(xmlRelaxNGValidError)
    );
		if (ctxt->errTab == NULL) {
			xmlRngVErrMemory(ctxt);
			return (0);
	...
	if ((ctxt->err != NULL) && (ctxt->state != NULL) && (ctxt->err->node == ctxt->state->node)
	    && (ctxt->err->err == err)) {
		return (ctxt->errNr);
	}
...
static int xmlRelaxNGDocumentPush(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDocumentPtr value) {
	if (ctxt->docTab == NULL) {
		ctxt->docMax = 4;
		ctxt->docNr  = 0;
		ctxt->docTab = (xmlRelaxNGDocumentPtr*)xmlMalloc(ctxt->docMax * sizeof(ctxt->docTab[0]));
		if (ctxt->docTab == NULL) {
			xmlRngPErrMemory(ctxt);
			return (0);
		}
	...
	if (ctxt->docNr >= ctxt->docMax) {
		ctxt->docMax *= 2;
		ctxt->docTab
			= (xmlRelaxNGDocumentPtr*)xmlRealloc(ctxt->docTab, ctxt->docMax * sizeof(ctxt->docTab[0]));
		if (ctxt->docTab == NULL) {
			xmlRngPErrMemory(ctxt);
			return (0);
		}
 ...
 */
static xmlRelaxNGDocumentPtr xmlRelaxNGDocumentPop(xmlRelaxNGParserCtxtPtr ctxt) {
...
xmlRelaxNGLoadExternalRef(xmlRelaxNGParserCtxtPtr ctxt, const xmlChar* URL, const xmlChar* ns) {
...
static const char* xmlRelaxNGDefName(xmlRelaxNGDefinePtr def) {
	if (def == NULL) {
		return ("none");
	}
	switch (def->type) {
 ...
 */
static xmlChar*
xmlRelaxNGGetErrorString(xmlRelaxNGValidErr err, const xmlChar* arg1, const xmlChar* arg2) {
	char     msg[1000];
	xmlChar* result;
	...
	switch (err) {
		...
		case XML_RELAXNG_ERR_TEXTWRONG:
			snprintf(msg, 1000, "Did not expect text in element %s content\n", arg1);
			break;
 ...
 */
static void xmlRelaxNGShowValidError(
	xmlRelaxNGValidCtxtPtr ctxt,
	xmlRelaxNGValidErr     err,
	xmlNodePtr             node,
	xmlNodePtr             child,
	...
	xmlRngVErr(ctxt, child == NULL ? node : child, err, (const char*)msg, arg1, arg2);
	xmlFree(msg);
}
 ...
 */
static void xmlRelaxNGPopErrors(xmlRelaxNGValidCtxtPtr ctxt, int level) {
	int                     i;
 ...
 * @param ctxt  the validation context
 */
static void xmlRelaxNGDumpValidError(xmlRelaxNGValidCtxtPtr ctxt) {
	int                     i, j, k;
		...
		if (k < MAX_ERROR) {
			for (j = 0; j < i; j++) {
				...
				if ((err->err == dup->err) && (err->node == dup->node)
				    && (xmlStrEqual(err->arg1, dup->arg1)) && (xmlStrEqual(err->arg2, dup->arg2))) {
					goto skip;
				}
			}
			xmlRelaxNGShowValidError(ctxt, err->err, err->node, err->seq, err->arg1, err->arg2);
			k++;
		}
skip:
		if (err->flags & ERROR_IS_DUP) {
 ...
 */
static void xmlRelaxNGAddValidError(
	xmlRelaxNGValidCtxtPtr ctxt,
	xmlRelaxNGValidErr     err,
	const xmlChar*         arg1,
	const xmlChar*         arg2,
 ...
 * @returns 1 if yes, 0 if no and -1 in case of error.
 */
static int xmlRelaxNGSchemaTypeCheck(
	...
	ret = xmlSchemaValPredefTypeNode(typ, value, (xmlSchemaValPtr*)result, node);
 ...
 * @returns 1 if yes, 0 if no and -1 in case of error.
 */
static int xmlRelaxNGSchemaFacetCheck(
	void* data     ATTRIBUTE_UNUSED,
	const xmlChar* type,
 ...
 */
static int xmlRelaxNGSchemaTypeCompare(
	void* data     ATTRIBUTE_UNUSED,
	const xmlChar* type,
	const xmlChar* value1,
	xmlNodePtr     ctxt1,
 ...
 * @returns 1 if yes, 0 if no and -1 in case of error.
 */
static int xmlRelaxNGDefaultTypeCheck(
	void* data           ATTRIBUTE_UNUSED,
 ...
 * @returns 1 if yes, 0 if no and -1 in case of error.
 */
static int xmlRelaxNGDefaultTypeCompare(
	void* data       ATTRIBUTE_UNUSED,
	const xmlChar*   type,
	const xmlChar*   value1,
 ...
 */
static void xmlRelaxNGFreeTypeLibrary(void* payload, const xmlChar* namespace ATTRIBUTE_UNUSED) {
 ...
 */
static int xmlRelaxNGRegisterTypeLibrary(
	...
	if ((xmlRelaxNGRegisteredTypes == NULL) || (namespace == NULL) || (check == NULL)
	    || (comp == NULL)) {
		return (-1);
	}
...
void            xmlInitRelaxNGInternal(void) {
  xmlInitMutex(&xmlRelaxNGMutex);
}

void xmlCleanupRelaxNGInternal(void) {
 ...
 * @returns 1 if yes, 0 if no and -1 in case of error
 */
static int xmlRelaxNGIsCompilable(xmlRelaxNGDefinePtr def) {
	int ret = -1;
 ...
 * @returns 0 if success and -1 in case of error
 */
static int xmlRelaxNGCompile(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr def) {
	int                 ret = 0;
	xmlRelaxNGDefinePtr list;
	...
	switch (def->type) {
		...
		case XML_RELAXNG_ELEMENT:
			if ((ctxt->am != NULL) && (def->name != NULL)) {
				ctxt->state
					= xmlAutomataNewTransition2(ctxt->am, ctxt->state, NULL, def->name, def->ns, def);
			}
		...
		case XML_RELAXNG_TEXT: {
			...
			xmlAutomataNewTransition(ctxt->am, ctxt->state, ctxt->state, BAD_CAST "#text", NULL);
			ctxt->state = xmlAutomataNewEpsilon(ctxt->am, oldstate, NULL);
			break;
		}
		case XML_RELAXNG_EMPTY:
		...
		case XML_RELAXNG_VALUE:
			xmlRngPErr(
				...
				BAD_CAST xmlRelaxNGDefName(def),
				NULL
			);
			break;
	}
...
static int xmlRelaxNGTryCompile(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr def) {
	...
	if ((def->type == XML_RELAXNG_START) || (def->type == XML_RELAXNG_ELEMENT)) {
...
static xmlRelaxNGDefinePtr xmlRelaxNGParseAttribute(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node);
static xmlRelaxNGDefinePtr xmlRelaxNGParseElement(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node);
static xmlRelaxNGDefinePtr
xmlRelaxNGParsePatterns(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr nodes, int group);
static xmlRelaxNGDefinePtr xmlRelaxNGParsePattern(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node);
static xmlRelaxNGPtr       xmlRelaxNGParseDocument(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node);
static int xmlRelaxNGParseGrammarContent(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr nodes);
static xmlRelaxNGDefinePtr
xmlRelaxNGParseNameClass(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node, xmlRelaxNGDefinePtr def);
static xmlRelaxNGGrammarPtr xmlRelaxNGParseGrammar(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr nodes);
static int
xmlRelaxNGElementMatch(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefinePtr define, xmlNodePtr elem);
 ...
 */
static xmlChar*
xmlRelaxNGGetDataTypeLibrary(xmlRelaxNGParserCtxtPtr ctxt ATTRIBUTE_UNUSED, xmlNodePtr node) {
	xmlChar *ret, *escape;
 ...
 */
static xmlRelaxNGDefinePtr xmlRelaxNGParseValue(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	...
	if (type != NULL) {
		...
		if (xmlValidateNCName(type, 0)) {
			xmlRngPErr(
				ctxt, node, XML_RNGP_TYPE_VALUE, "value type '%s' is not an NCName\n", type, NULL
			);
		}
	...
	if (node->children == NULL) {
		...
		if (def->value == NULL) {
			xmlRngPErr(
				ctxt, node, XML_RNGP_VALUE_NO_CONTENT, "Element <value> has no content\n", NULL, NULL
			);
 ...
 * @returns the definition pointer or NULL in case of error
 */
static xmlRelaxNGDefinePtr xmlRelaxNGParseData(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	...
	xmlRelaxNGNormExtSpace(type);
	if (xmlValidateNCName(type, 0)) {
		xmlRngPErr(ctxt, node, XML_RNGP_TYPE_VALUE, "data type '%s' is not an NCName\n", type, NULL);
	}
	library = xmlRelaxNGGetDataTypeLibrary(ctxt, node);
	if (library == NULL) {
		library = xmlStrdup(BAD_CAST "http://relaxng.org/ns/structure/1.0");
	...
	if (lib == NULL) {
		...
		if (lib->have == NULL) {
			...
			} else if ((xmlStrEqual(library, BAD_CAST "http://www.w3.org/2001/XMLSchema-datatypes"))
			           && ((xmlStrEqual(def->name, BAD_CAST "IDREF")) || (xmlStrEqual(def->name, BAD_CAST "IDREFS")))) {
				ctxt->idref = 1;
			}
		}
	}
	...
	while (content != NULL) {
		...
		if (xmlStrEqual(library, BAD_CAST "http://relaxng.org/ns/structure/1.0")) {
			xmlRngPErr(
				ctxt,
				node,
			...
			while ((content != NULL) && (xmlStrEqual(content->name, BAD_CAST "param"))) {
				content = content->next;
			}
		} else {
			...
			if (param != NULL) {
				...
				if (param->name == NULL) {
					xmlRngPErr(ctxt, node, XML_RNGP_PARAM_NAME_MISSING, "param has no name\n", NULL, NULL);
				}
 ...
 * @returns 1 distinct, 0 if equal
 */
static int xmlRelaxNGCompareElemDefLists(
	xmlRelaxNGParserCtxtPtr ctxt ATTRIBUTE_UNUSED,
...
static int xmlRelaxNGGenerateAttributes(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr def) {
	...
	cur    = def;
	while (cur != NULL) {
		if ((cur->type == XML_RELAXNG_ELEMENT) || (cur->type == XML_RELAXNG_TEXT)
		    || (cur->type == XML_RELAXNG_DATATYPE) || (cur->type == XML_RELAXNG_PARAM)
		    || (cur->type == XML_RELAXNG_LIST) || (cur->type == XML_RELAXNG_VALUE)
		    || (cur->type == XML_RELAXNG_EMPTY)) {
			return (0);
		}
		if ((cur->type == XML_RELAXNG_CHOICE) || (cur->type == XML_RELAXNG_INTERLEAVE)
		    || (cur->type == XML_RELAXNG_GROUP) || (cur->type == XML_RELAXNG_ONEORMORE)
		    || (cur->type == XML_RELAXNG_ZEROORMORE) || (cur->type == XML_RELAXNG_OPTIONAL)
		    || (cur->type == XML_RELAXNG_PARENTREF) || (cur->type == XML_RELAXNG_EXTERNALREF)
		    || (cur->type == XML_RELAXNG_REF) || (cur->type == XML_RELAXNG_DEF)) {
			if (cur->content != NULL) {
				parent = cur;
				cur    = cur->content;
				tmp    = cur;
 ...
 */
static xmlRelaxNGDefinePtr*
xmlRelaxNGGetElements(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr def, int eora) {
	...
	while (cur != NULL) {
		if (((eora == 0) && ((cur->type == XML_RELAXNG_ELEMENT) ||
			...
			if (ret == NULL) {
				max = 10;
				ret = (xmlRelaxNGDefinePtr*)xmlMalloc((max + 1) * sizeof(xmlRelaxNGDefinePtr));
				if (ret == NULL) {
					xmlRngPErrMemory(ctxt);
					return (NULL);
				}
				...
				max  *= 2;
				temp  = xmlRealloc(ret, (max + 1) * sizeof(xmlRelaxNGDefinePtr));
				if (temp == NULL) {
					xmlRngPErrMemory(ctxt);
					xmlFree(ret);
					return (NULL);
			...
			ret[len++] = cur;
			ret[len]   = NULL;
		} else if ((cur->type == XML_RELAXNG_CHOICE) || (cur->type == XML_RELAXNG_INTERLEAVE)
		           || (cur->type == XML_RELAXNG_GROUP) || (cur->type == XML_RELAXNG_ONEORMORE)
		           || (cur->type == XML_RELAXNG_ZEROORMORE) || (cur->type == XML_RELAXNG_OPTIONAL)
		           || (cur->type == XML_RELAXNG_PARENTREF) || (cur->type == XML_RELAXNG_REF)
		           || (cur->type == XML_RELAXNG_DEF) || (cur->type == XML_RELAXNG_EXTERNALREF)) {
			/*
 ...
 */
static void
xmlRelaxNGCheckChoiceDeterminism(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr def) {
	xmlRelaxNGDefinePtr** list;
	...
	list = (xmlRelaxNGDefinePtr**)xmlMalloc(nbchild * sizeof(xmlRelaxNGDefinePtr*));
	if (list == NULL) {
		xmlRngPErrMemory(ctxt);
		return;
	}
	...
	while (cur != NULL) {
		...
		if ((list[i] == NULL) || (list[i][0] == NULL)) {
			...
			while ((*tmp != NULL) && (is_triable == 1)) {
				...
				} else if (((*tmp)->type == XML_RELAXNG_ELEMENT) && ((*tmp)->name != NULL)) {
 ...
 */
static void
xmlRelaxNGComputeInterleaves(void* payload, void* data, const xmlChar* name ATTRIBUTE_UNUSED) {
	...
	groups
		= (xmlRelaxNGInterleaveGroupPtr*)xmlMalloc(nbchild * sizeof(xmlRelaxNGInterleaveGroupPtr));
	if (groups == NULL) {
		goto error;
	}
	cur = def->content;
	while (cur != NULL) {
		groups[nbgroups] = (xmlRelaxNGInterleaveGroupPtr)xmlMalloc(sizeof(xmlRelaxNGInterleaveGroup));
	...
	for (i = 0; i < nbgroups; i++) {
		...
		if ((tmp != NULL) && (*tmp != NULL)) {
			while (*tmp != NULL) {
				if ((*tmp)->type == XML_RELAXNG_TEXT) {
					res
						= xmlHashAddEntry2(partitions->triage, BAD_CAST "#text", NULL, XML_INT_TO_PTR(i + 1));
					if (res != 0) {
						is_determinist = -1;
					}
				} else if (((*tmp)->type == XML_RELAXNG_ELEMENT) && ((*tmp)->name != NULL)) {
					if (((*tmp)->ns == NULL) || ((*tmp)->ns[0] == 0)) {
						res = xmlHashAddEntry2(partitions->triage, (*tmp)->name, NULL, XML_INT_TO_PTR(i + 1));
					} else {
						res = xmlHashAddEntry2(
							partitions->triage, (*tmp)->name, (*tmp)->ns, XML_INT_TO_PTR(i + 1)
						);
					}
					if (res != 0) {
						is_determinist = -1;
					...
					if (((*tmp)->ns == NULL) || ((*tmp)->ns[0] == 0)) {
						res
							= xmlHashAddEntry2(partitions->triage, BAD_CAST "#any", NULL, XML_INT_TO_PTR(i + 1));
					} else {
						res = xmlHashAddEntry2(
							partitions->triage, BAD_CAST "#any", (*tmp)->ns, XML_INT_TO_PTR(i + 1)
						);
					}
...
xmlRelaxNGParseInterleave(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	...
	if (child == NULL) {
		xmlRngPErr(
			ctxt, node, XML_RNGP_INTERLEAVE_NO_CONTENT, "Element interleave is empty\n", NULL, NULL
		);
	}
	while (child != NULL) {
 ...
 */
static int xmlRelaxNGParseInclude(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	...
	incl                     = node->psvi;
	if (incl == NULL) {
		xmlRngPErr(ctxt, node, XML_RNGP_INCLUDE_EMPTY, "Include node has no data\n", NULL, NULL);
		return (-1);
	}
	...
	if (!xmlStrEqual(root->name, BAD_CAST "grammar")) {
		xmlRngPErr(
			ctxt, node, XML_RNGP_GRAMMAR_MISSING, "Include document root is not a grammar\n", NULL, NULL
		);
		return (-1);
	}
 ...
 * @returns 0 in case of success or -1 in case of error
 */
static int xmlRelaxNGParseDefine(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	xmlChar*            name;
	int                 ret = 0, tmp;
	...
	if (name == NULL) {
		...
		if (xmlValidateNCName(name, 0)) {
			xmlRngPErr(
				ctxt, node, XML_RNGP_INVALID_DEFINE_NAME, "define name '%s' is not an NCName\n", name, NULL
			);
		}
		def = xmlRelaxNGNewDefine(ctxt, node);
		if (def == NULL) {
		...
		def->name = name;
		if (node->children == NULL) {
			xmlRngPErr(ctxt, node, XML_RNGP_DEFINE_EMPTY, "define has no children\n", NULL, NULL);
		} else {
		...
		if (ctxt->grammar->defs == NULL) {
			xmlRngPErr(
				ctxt, node, XML_RNGP_DEFINE_CREATE_FAILED, "Could not create definition hash\n", NULL, NULL
			);
			ret = -1;
		} else {
 ...
 */
static void xmlRelaxNGParseImportRef(void* payload, void* data, const xmlChar* name) {
	xmlRelaxNGParserCtxtPtr ctxt = (xmlRelaxNGParserCtxtPtr)data;
	xmlRelaxNGDefinePtr     def  = (xmlRelaxNGDefinePtr)payload;
	int                     tmp;
	...
	if (tmp < 0) {
		...
		prev = (xmlRelaxNGDefinePtr)xmlHashLookup(ctxt->grammar->refs, def->name);
		if (prev == NULL) {
			if (def->name != NULL) {
				xmlRngPErr(
					ctxt, NULL, XML_RNGP_REF_CREATE_FAILED, "Error refs definitions '%s'\n", def->name, NULL
				);
			} else {
...
static int xmlRelaxNGParseImportRefs(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGGrammarPtr grammar) {
	...
	if (ctxt->grammar->refs == NULL) {
		xmlRngPErr(
			ctxt, NULL, XML_RNGP_REF_CREATE_FAILED, "Could not create references hash\n", NULL, NULL
		);
		return (-1);
	}
...
xmlRelaxNGProcessExternalRef(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	...
	if (docu != NULL) {
		...
		if (docu->content == NULL) {
			...
			if (root == NULL) {
				xmlRngPErr(
					ctxt, node, XML_RNGP_EXTERNALREF_EMTPY, "xmlRelaxNGParse: %s is empty\n", ctxt->URL, NULL
				);
				return (NULL);
			}
			/*
			...
			ctxt->flags   = oldflags;
			if ((docu->schema != NULL) && (docu->schema->topgrammar != NULL)) {
...
static xmlRelaxNGDefinePtr xmlRelaxNGParsePattern(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	...
	if (IS_RELAXNG(node, "element")) {
		...
		if (node->children != NULL) {
			xmlRngPErr(ctxt, node, XML_RNGP_EMPTY_NOT_EMPTY, "empty: had a child node\n", NULL, NULL);
		}
		...
		if (node->children != NULL) {
			xmlRngPErr(ctxt, node, XML_RNGP_TEXT_HAS_CHILD, "text: had a child node\n", NULL, NULL);
		}
		...
		if (node->children == NULL) {
			xmlRngPErr(ctxt, node, XML_RNGP_EMPTY_CONSTRUCT, "Element %s is empty\n", node->name, NULL);
		} else {
		...
		if (ctxt->grammar->refs == NULL) {
			xmlRngPErr(
				ctxt, node, XML_RNGP_REF_CREATE_FAILED, "Could not create references hash\n", NULL, NULL
			);
			def = NULL;
		} else {
			int tmp;
			...
			if (tmp < 0) {
				...
				if (prev == NULL) {
					if (def->name != NULL) {
						...
						xmlRngPErr(
							ctxt, node, XML_RNGP_REF_CREATE_FAILED, "Error refs definitions\n", NULL, NULL
						);
					}
					def = NULL;
				} else {
		...
		def->name = xmlGetProp(node, BAD_CAST "name");
		if (def->name == NULL) {
			xmlRngPErr(ctxt, node, XML_RNGP_PARENTREF_NO_NAME, "parentRef has no name\n", NULL, NULL);
		} else {
			xmlRelaxNGNormExtSpace(def->name);
			if (xmlValidateNCName(def->name, 0)) {
				xmlRngPErr(
		...
		if (node->children != NULL) {
			xmlRngPErr(ctxt, node, XML_RNGP_PARENTREF_NOT_EMPTY, "parentRef is not empty\n", NULL, NULL);
		}
		...
		if (ctxt->parentgrammar->refs == NULL) {
			...
			if (tmp < 0) {
				...
				prev = (xmlRelaxNGDefinePtr)xmlHashLookup(ctxt->parentgrammar->refs, def->name);
				if (prev == NULL) {
					xmlRngPErr(
						ctxt,
						node,
...
xmlRelaxNGParseAttribute(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	...
	if (child != NULL) {
		...
		if (cur != NULL) {
			switch (cur->type) {
				...
				case XML_RELAXNG_EXCEPT:
					xmlRngPErr(
						ctxt, node, XML_RNGP_ATTRIBUTE_CONTENT, "attribute has invalid content\n", NULL, NULL
					);
					break;
				case XML_RELAXNG_NOOP:
					xmlRngPErr(
	...
	if (child != NULL) {
		xmlRngPErr(
			ctxt, node, XML_RNGP_ATTRIBUTE_CHILDREN, "attribute has multiple children\n", NULL, NULL
		);
	}
...
xmlRelaxNGParseExceptNameClass(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node, int attr) {
	...
	if (!IS_RELAXNG(node, "except")) {
		xmlRngPErr(ctxt, node, XML_RNGP_EXCEPT_MISSING, "Expecting an except node\n", NULL, NULL);
		return (NULL);
	}
	if (node->next != NULL) {
		xmlRngPErr(
	...
	if (IS_RELAXNG(node, "name")) {
		...
		if (xmlValidateNCName(val, 0)) {
			if (node->parent != NULL) {
				...
				xmlRngPErr(ctxt, node, XML_RNGP_ELEMENT_NAME, "name '%s' is not an NCName\n", val, NULL);
			}
		}
		ret->name = val;
		val       = xmlGetProp(node, BAD_CAST "ns");
		ret->ns   = val;
		if ((ctxt->flags & XML_RELAXNG_IN_ATTRIBUTE) && (val != NULL)
		    && (xmlStrEqual(val, BAD_CAST "http://www.w3.org/2000/xmlns"))) {
			xmlRngPErr(
				ctxt, node, XML_RNGP_XML_NS, "Attribute with namespace '%s' is not allowed\n", val, NULL
			);
		}
		if ((ctxt->flags & XML_RELAXNG_IN_ATTRIBUTE) && (val != NULL) && (val[0] == 0)
		    && (xmlStrEqual(ret->name, BAD_CAST "xmlns"))) {
			xmlRngPErr(
				ctxt, node, XML_RNGP_XMLNS_NAME, "Attribute with QName 'xmlns' is not allowed\n", val, NULL
			);
		}
		...
		if (node->children != NULL) {
			ret->nameClass
				= xmlRelaxNGParseExceptNameClass(ctxt, node->children, def->type == XML_RELAXNG_ATTRIBUTE);
		}
	} else if (IS_RELAXNG(node, "nsName")) {
		ret->name = NULL;
		ret->ns   = xmlGetProp(node, BAD_CAST "ns");
		if (ret->ns == NULL) {
			xmlRngPErr(ctxt, node, XML_RNGP_NSNAME_NO_NS, "nsName has no ns attribute\n", NULL, NULL);
		}
		if ((ctxt->flags & XML_RELAXNG_IN_ATTRIBUTE) && (ret->ns != NULL)
		...
		if (node->children == NULL) {
			xmlRngPErr(ctxt, node, XML_RNGP_CHOICE_EMPTY, "Element choice is empty\n", NULL, NULL);
		} else {
			child = node->children;
			while (child != NULL) {
 ...
 */
static int xmlRelaxNGParseStart(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr nodes) {
	int                 ret = 0;
	...
	if (IS_RELAXNG(nodes, "empty")) {
		...
		if (nodes->children != NULL) {
			xmlRngPErr(
				ctxt, nodes, XML_RNGP_NOTALLOWED_NOT_EMPTY, "element notAllowed is not empty\n", NULL, NULL
			);
		}
	} else {
 ...
 */
static void xmlRelaxNGCheckReference(void* payload, void* data, const xmlChar* name) {
	...
	if (grammar->defs != NULL) {
		...
		xmlRngPErr(
			ctxt, ref->node, XML_RNGP_REF_NO_DEF, "Reference %s has no matching definition\n", name, NULL
		);
	}
}
 ...
 * @param name  the name associated to the defines
 */
static void xmlRelaxNGCheckCombine(void* payload, void* data, const xmlChar* name) {
 ...
 */
static int
xmlRelaxNGCheckCycles(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr cur, int depth) {
	int ret = 0;
 ...
 */
static xmlRelaxNGDefinePtr xmlRelaxNGTryUnlink(
	xmlRelaxNGParserCtxtPtr ctxt ATTRIBUTE_UNUSED,
	xmlRelaxNGDefinePtr          cur,
 ...
 * @param parent  parent
 */
static void xmlRelaxNGSimplify(
	xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr cur, xmlRelaxNGDefinePtr parent
) {
		...
		if ((cur->type == XML_RELAXNG_REF) || (cur->type == XML_RELAXNG_PARENTREF)) {
			...
			if ((parent != NULL) && 
				...
				if (parent->type == XML_RELAXNG_GROUP || parent->type == XML_RELAXNG_INTERLEAVE) {
					prev = xmlRelaxNGTryUnlink(ctxt, cur, parent, prev);
				} else if (ctxt->def->parent->type == XML_RELAXNG_GROUP
				           || ctxt->def->parent->type == XML_RELAXNG_INTERLEAVE) {
					prev = xmlRelaxNGTryUnlink(ctxt, ctxt->def, ctxt->def->parent, prev);
				}
			} else {
			 ...
			 */
			if ((cur->type == XML_RELAXNG_GROUP) || (cur->type == XML_RELAXNG_INTERLEAVE)) {
				if (cur->content == NULL) {
					cur->type = XML_RELAXNG_EMPTY;
			 ...
			 */
			if ((cur->type == XML_RELAXNG_EXCEPT) && (cur->content != NULL)
 ...
 */
static xmlRelaxNGContentType
xmlRelaxNGMaxContentType(xmlRelaxNGContentType ct1, xmlRelaxNGContentType ct2) {
	if ((ct1 == XML_RELAXNG_CONTENT_ERROR) || (ct2 == XML_RELAXNG_CONTENT_ERROR)) {
		return (XML_RELAXNG_CONTENT_ERROR);
	}
	if ((ct1 == XML_RELAXNG_CONTENT_SIMPLE) || (ct2 == XML_RELAXNG_CONTENT_SIMPLE)) {
 ...
 */
static xmlRelaxNGContentType xmlRelaxNGCheckRules(
	xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr cur, int flags, xmlRelaxNGType ptype
) {
	int                   nflags;
	...
	while (cur != NULL) {
		...
		if ((cur->type == XML_RELAXNG_REF) || (cur->type == XML_RELAXNG_PARENTREF)) {
			...
			if ((cur->parent != NULL) && (cur->parent->type == XML_RELAXNG_DATATYPE)) {
...
xmlRelaxNGParseGrammar(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr nodes) {
	...
	if (ctxt->grammar == NULL) {
		xmlRngPErr(
			ctxt, nodes, XML_RNGP_GRAMMAR_CONTENT, "Failed to parse <grammar> content\n", NULL, NULL
		);
	} else if (ctxt->grammar->start == NULL) {
		xmlRngPErr(
			ctxt, nodes, XML_RNGP_GRAMMAR_NO_START, "Element <grammar> has no <start>\n", NULL, NULL
		);
	}
	...
	if (schema->topgrammar->start != NULL) {
		...
		if ((ctxt->flags & XML_RELAXNG_IN_EXTERNALREF) == 0) {
			...
			xmlRelaxNGCheckRules(
				ctxt, schema->topgrammar->start, XML_RELAXNG_IN_START, XML_RELAXNG_NOOP
			);
		}
	}
 ...
 */
static void xmlRelaxNGCleanupAttributes(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node) {
	xmlAttrPtr cur, next;

	cur = node->properties;
	while (cur != NULL) {
		next = cur->next;
		if ((cur->ns == NULL) || (xmlStrEqual(cur->ns->href, xmlRelaxNGNs))) {
 ...
 */
static void xmlRelaxNGCleanupTree(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr root) {
	...
	while (cur != NULL) {
		...
		if (cur->type == XML_ELEMENT_NODE) {
			...
			if ((cur->ns == NULL) || (!xmlStrEqual(cur->ns->href, xmlRelaxNGNs))) {
				...
				if (xmlStrEqual(cur->name, BAD_CAST "externalRef")) {
					...
					if (uri == NULL) {
						xmlRngPErr(
							ctxt, cur, XML_RNGP_HREF_ERROR, "Incorrect URI for externalRef %s\n", href, NULL
						);
						if (ns != NULL) {
							xmlFree(ns);
						}
					...
					if (URL == NULL) {
						xmlRngPErr(
							ctxt, cur, XML_RNGP_HREF_ERROR, "Failed to compute URL for include %s\n", href, NULL
						);
						if (href != NULL) {
							xmlFree(href);
						}
					...
					if (incl == NULL) {
						xmlRngPErr(
							ctxt, cur, XML_RNGP_INCLUDE_FAILURE, "Failed to load include %s\n", URL, NULL
						);
						xmlFree(URL);
						delete = cur;
						goto skip_children;
				...
				} else if ((xmlStrEqual(cur->name, BAD_CAST "except")) && (cur != root)) {
					int oldflags = ctxt->flags;

					/*
					 * 4.16
					 */
					if ((cur->parent != NULL) && (xmlStrEqual(cur->parent->name, BAD_CAST "anyName"))) {
		 ...
		 */
			...
			if (IS_BLANK_NODE(cur)) {
				if ((cur->parent != NULL) && (cur->parent->type == XML_ELEMENT_NODE)) {
		 ...
		 */
		if (cur->children != NULL) {
			if ((cur->children->type != XML_ENTITY_DECL) && (cur->children->type != XML_ENTITY_REF_NODE)
...
xmlRelaxNG* xmlRelaxNGParse(xmlRelaxNGParserCtxt* ctxt) {
	...
	if (ctxt->URL != NULL) {
		...
		if (doc == NULL) {
			xmlRngPErr(
				ctxt, NULL, XML_RNGP_PARSE_ERROR, "xmlRelaxNGParse: could not load %s\n", ctxt->URL, NULL
			);
			return (NULL);
		}
		...
		if (doc == NULL) {
			xmlRngPErr(
				ctxt, NULL, XML_RNGP_PARSE_ERROR, "xmlRelaxNGParse: could not parse schemas\n", NULL, NULL
			);
			return (NULL);
		}
	...
	if (root == NULL) {
		xmlRngPErr(
			...
			ctxt->URL ? ctxt->URL : BAD_CAST "schemas",
			NULL
		);
 ...
 */
void
xmlRelaxNGSetResourceLoader(xmlRelaxNGParserCtxt* ctxt, xmlResourceLoader loader, void* vctxt) {
	if (ctxt == NULL) {
		return;
	}
 ...
 */
static void xmlRelaxNGDumpDefines(FILE* output, xmlRelaxNGDefinePtr defines) {
	while (defines != NULL) {
 ...
 */
static void xmlRelaxNGValidateCompiledCallback(
	xmlRegExecCtxtPtr exec ATTRIBUTE_UNUSED, const xmlChar* token, void* transdata, void* inputdata
) {
	...
	if (ctxt == NULL) {
		xmlRngVErr(
			ctxt, NULL, XML_ERR_INTERNAL_ERROR, "callback on %s missing context\n", token, NULL
		);
		return;
	}
	if (define == NULL) {
		...
		xmlRngVErr(ctxt, NULL, XML_ERR_INTERNAL_ERROR, "callback on %s missing define\n", token, NULL);
	...
	if (define->type != XML_RELAXNG_ELEMENT) {
		xmlRngVErr(
			ctxt, NULL, XML_ERR_INTERNAL_ERROR, "callback on %s define is not element\n", token, NULL
		);
		if (ctxt->errNo == XML_RELAXNG_OK) {
			ctxt->errNo = XML_RELAXNG_ERR_INTERNAL;
		}
 ...
 * @returns 0 in case of success, -1 in case of error.
 */
static int xmlRelaxNGValidateCompiledContent(
...
static int  xmlRelaxNGElemPush(xmlRelaxNGValidCtxtPtr ctxt, xmlRegExecCtxtPtr exec) {
  if (ctxt->elemTab == NULL) {
    ctxt->elemMax = 10;
    ctxt->elemTab = (xmlRegExecCtxtPtr*)xmlMalloc(ctxt->elemMax * sizeof(xmlRegExecCtxtPtr));
    if (ctxt->elemTab == NULL) {
      xmlRngVErrMemory(ctxt);
      return (-1);
    }
  ...
  if (ctxt->elemNr >= ctxt->elemMax) {
    ctxt->elemMax *= 2;
    ctxt->elemTab
      = (xmlRegExecCtxtPtr*)xmlRealloc(ctxt->elemTab, ctxt->elemMax * sizeof(xmlRegExecCtxtPtr));
    if (ctxt->elemTab == NULL) {
      xmlRngVErrMemory(ctxt);
      return (-1);
    }
 ...
 */
static xmlRegExecCtxtPtr xmlRelaxNGElemPop(xmlRelaxNGValidCtxtPtr ctxt) {
 ...
 * @param inputdata  callback data, the Relax NG validation context
 */
static void xmlRelaxNGValidateProgressiveCallback(
	xmlRegExecCtxtPtr exec ATTRIBUTE_UNUSED, const xmlChar* token, void* transdata, void* inputdata
) {
	xmlRelaxNGValidCtxtPtr  ctxt   = (xmlRelaxNGValidCtxtPtr)inputdata;
	xmlRelaxNGDefinePtr     define = (xmlRelaxNGDefinePtr)transdata;
...
int xmlRelaxNGValidatePushElement(
	xmlRelaxNGValidCtxt* ctxt, xmlDoc* doc ATTRIBUTE_UNUSED, xmlNode* elem
) {
	int        ret = 1;
...
int xmlRelaxNGValidatePushCData(
	xmlRelaxNGValidCtxt* ctxt, const xmlChar* data, int len ATTRIBUTE_UNUSED
) {
	int ret = 1;
 ...
 */
static xmlNodePtr
xmlRelaxNGSkipIgnored(xmlRelaxNGValidCtxtPtr ctxt ATTRIBUTE_UNUSED, xmlNodePtr node) {
	/*
 ...
 */
static int xmlRelaxNGValidateDatatype(
	xmlRelaxNGValidCtxtPtr ctxt, const xmlChar* value, xmlRelaxNGDefinePtr define, xmlNodePtr node
) {
	int                      ret, tmp;
	xmlRelaxNGTypeLibraryPtr lib;
	void*                    result = NULL;
...
static int xmlRelaxNGValidateValue(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefinePtr define) {
	...
	switch (define->type) {
		...
		case XML_RELAXNG_VALUE: {
			if (!xmlStrEqual(value, define->value)) {
				if (define->name != NULL) {
					...
					if ((nval == NULL) || (nvalue == NULL) || (!xmlStrEqual(nval, nvalue))) {
						ret = -1;
					}
					if (nval != NULL) {
						xmlFree(nval);
		...
		case XML_RELAXNG_ZEROORMORE: {
			...
			cur          = ctxt->state->value;
			temp         = NULL;
			while ((cur != NULL) && (cur != ctxt->state->endvalue) && (temp != cur)) {
				temp = cur;
				ret  = xmlRelaxNGValidateValueList(ctxt, define->content);
				if (ret != 0) {
 ...
 */
static int xmlRelaxNGAttributeMatch(
	xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefinePtr define, xmlAttrPtr prop
) {
	int ret;
	...
	if (define->ns != NULL) {
		if (define->ns[0] == 0) {
			...
			if ((prop->ns == NULL) || (!xmlStrEqual(define->ns, prop->ns->href))) {
				return (0);
			}
		}
	}
	...
	if (define->name != NULL) {
		for (i = 0; i < ctxt->state->nbAttrs; i++) {
			...
			if ((tmp != NULL) && (xmlStrEqual(define->name, tmp->name))) {
				if ((((define->ns == NULL) || (define->ns[0] == 0)) && (tmp->ns == NULL))
				    || ((tmp->ns != NULL) && (xmlStrEqual(define->ns, tmp->ns->href)))) {
					prop = tmp;
					break;
				}
			}
		...
		for (i = 0; i < ctxt->state->nbAttrs; i++) {
			tmp = ctxt->state->attrs[i];
			if ((tmp != NULL) && (xmlRelaxNGAttributeMatch(ctxt, define, tmp) == 1)) {
				prop = tmp;
				break;
			}
		}
 ...
 */
static int xmlRelaxNGNodeMatchesList(xmlNodePtr node, xmlRelaxNGDefinePtr* list) {
	xmlRelaxNGDefinePtr cur;
	int                 i = 0, tmp;
	...
	cur = list[i++];
	while (cur != NULL) {
		if ((node->type == XML_ELEMENT_NODE) && (cur->type == XML_RELAXNG_ELEMENT)) {
...
static int xmlRelaxNGValidateInterleave(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefinePtr define) {
	...
	while (cur != NULL) {
		ctxt->state->seq = cur;
		if ((partitions->triage != NULL) && (partitions->flags & IS_DETERMINIST)) {
			void* tmp = NULL;
			...
			if ((cur->type == XML_TEXT_NODE) || (cur->type == XML_CDATA_SECTION_NODE)) {
				...
				if (cur->ns != NULL) {
					...
					if (tmp == NULL) {
						tmp = xmlHashLookup2(partitions->triage, BAD_CAST "#any", cur->ns->href);
					}
				} else {
					tmp = xmlHashLookup2(partitions->triage, cur->name, NULL);
				}
				if (tmp == NULL) {
					tmp = xmlHashLookup2(partitions->triage, BAD_CAST "#any", NULL);
				}
			}
	...
	for (i = 0; i < nbgroups; i++) {
		...
		if (ctxt->state != NULL) {
			...
			if (cur != NULL
			    ...
			    && (define->parent->type != XML_RELAXNG_DEF || !xmlStrEqual(define->parent->name, (const xmlChar*)"open-name-class"))) {
...
xmlRelaxNGElementMatch(xmlRelaxNGValidCtxtPtr ctxt, xmlRelaxNGDefinePtr define, xmlNodePtr elem) {
	...
	} else if ((elem->ns != NULL) && (define->ns != NULL) && (define->name == NULL)) {
...
static int xmlRelaxNGBestState(xmlRelaxNGValidCtxtPtr ctxt) {
	...
	if ((ctxt == NULL) || (ctxt->states == NULL) || (ctxt->states->nbState <= 0)) {
		return (-1);
	}
...
static int xmlRelaxNGValidateElementEnd(xmlRelaxNGValidCtxtPtr ctxt, int dolog) {
	...
	if (state->seq != NULL) {
		...
		if (state->seq != NULL) {
			if (dolog) {
				VALID_ERR3(XML_RELAXNG_ERR_EXTRACONTENT, state->node->name, state->seq->name);
			}
			return (-1);
		}
	}
	for (i = 0; i < state->nbAttrs; i++) {
		if (state->attrs[i] != NULL) {
			if (dolog) {
				VALID_ERR3(XML_RELAXNG_ERR_INVALIDATTR, state->attrs[i]->name, state->node->name);
			}
			return (-1 - i);
		}
	}
	...
	switch (define->type) {
		...
		case XML_RELAXNG_ZEROORMORE: {
			...
			if (ctxt->state != NULL) {
				...
				for (j = 0; j < ctxt->states->nbState; j++) {
					xmlRelaxNGAddStates(
						ctxt, res, xmlRelaxNGCopyValidState(ctxt, ctxt->states->tabState[j])
					);
				}
			}
			oldflags     = ctxt->flags;
		...
		case XML_RELAXNG_CHOICE: {
			...
			errNr                      = ctxt->errNr;
			if ((define->dflags & IS_TRIABLE) && (define->data != NULL) && (node != NULL)) {
				/*
				...
				if ((node->type == XML_TEXT_NODE) || (node->type == XML_CDATA_SECTION_NODE)) {
					...
					if (node->ns != NULL) {
						...
						if (list == NULL) {
							list = xmlHashLookup2(triage, BAD_CAST "#any", node->ns->href);
						}
					} else {
		...
		case XML_RELAXNG_DATATYPE: {
			...
			while (child != NULL) {
				if (child->type == XML_ELEMENT_NODE) {
					VALID_ERR2(XML_RELAXNG_ERR_DATAELEM, node->parent->name);
					ret = -1;
					break;
		...
		case XML_RELAXNG_VALUE: {
			...
			while (child != NULL) {
				if (child->type == XML_ELEMENT_NODE) {
					VALID_ERR2(XML_RELAXNG_ERR_VALELEM, node->parent->name);
					ret = -1;
					break;
				} else if ((child->type == XML_TEXT_NODE) || (child->type == XML_CDATA_SECTION_NODE)) {
					content = xmlStrcat(content, child->content);
		...
		case XML_RELAXNG_LIST: {
			...
			while (child != NULL) {
				if (child->type == XML_ELEMENT_NODE) {
					VALID_ERR2(XML_RELAXNG_ERR_LISTELEM, node->parent->name);
					ret = -1;
					break;
				} else if ((child->type == XML_TEXT_NODE) || (child->type == XML_CDATA_SECTION_NODE)) {
					content = xmlStrcat(content, child->content);
 ...
 *    of error.
 */
static void xmlRelaxNGCleanPSVI(xmlNodePtr node) {
	xmlNodePtr cur;

	if ((node == NULL)
	    || ((node->type != XML_ELEMENT_NODE) && (node->type != XML_DOCUMENT_NODE) && (node->type != XML_HTML_DOCUMENT_NODE))) {
		return;
	}
	if (node->type == XML_ELEMENT_NODE) {
		node->psvi = NULL;
