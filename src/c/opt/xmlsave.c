 ...
 */
static void xmlSaveErrMemory(xmlOutputBufferPtr out) {
	if (out != NULL) {
 ...
 */
static void xmlSaveErr(xmlOutputBufferPtr out, int code, xmlNodePtr node, const char* extra) {
	const char* msg = NULL;
	int         res;

	/* Don't overwrite catastrophic errors */
	if ((out != NULL) && (out->error != XML_ERR_OK)
 ...
 */
static void xmlSaveCtxtInit(xmlSaveCtxtPtr ctxt, int options) {
	if (ctxt == NULL) {
		return;
	}
	...
	if (((options & XML_SAVE_EMPTY) == 0) && (xmlSaveNoEmptyTags)) {
...
static xmlSaveCtxtPtr xmlNewSaveCtxt(const char* encoding, int options) {
	...
	if (encoding != NULL) {
		...
		res = xmlOpenCharEncodingHandler(encoding, /* output */ 1, &ret->handler);
		if (res != XML_ERR_OK) {
...
static void xmlSaveWriteText(xmlSaveCtxt* ctxt, const xmlChar* text, unsigned flags) {
	if (ctxt->encoding == NULL) {
 ...
 */
static void xmlSaveWriteAttrContent(xmlSaveCtxt* ctxt, xmlAttrPtr attr) {
	xmlNodePtr         children;
 ...
 */
static void xmlBufDumpNotationDecl(xmlOutputBufferPtr buf, xmlNotationPtr nota) {
 ...
 */
static void
xmlBufDumpNotationDeclScan(void* nota, void* buf, const xmlChar* name ATTRIBUTE_UNUSED) {
 ...
 */
static void xmlBufDumpNotationTable(xmlOutputBufferPtr buf, xmlNotationTablePtr table) {
 ...
 */
static void xmlBufDumpElementOccur(xmlOutputBufferPtr buf, xmlElementContentPtr cur) {
	switch (cur->ocur) {
 ...
 */
static void xmlBufDumpElementContent(xmlOutputBufferPtr buf, xmlElementContentPtr content) {
	...
	do {
		...
		switch (cur->type) {
			...
			case XML_ELEMENT_CONTENT_OR:
				if ((cur != content) && (cur->parent != NULL)
				    && ((cur->type != cur->parent->type) || (cur->ocur != XML_ELEMENT_CONTENT_ONCE))) {
					xmlOutputBufferWrite(buf, 1, "(");
				}
				cur = cur->c1;
				continue;
		...
		while (cur != content) {
			...
			if (((cur->type == XML_ELEMENT_CONTENT_OR) || (cur->type == XML_ELEMENT_CONTENT_SEQ))
			    && ((cur->type != parent->type) || (cur->ocur != XML_ELEMENT_CONTENT_ONCE))) {
				xmlOutputBufferWrite(buf, 1, ")");
			}
 ...
 */
static void xmlBufDumpElementDecl(xmlOutputBufferPtr buf, xmlElementPtr elem) {
 ...
 */
static void xmlBufDumpEnumeration(xmlOutputBufferPtr buf, xmlEnumerationPtr cur) {
	while (cur != NULL) {
		xmlOutputBufferWriteString(buf, (const char*)cur->name);
		if (cur->next != NULL) {
			xmlOutputBufferWrite(buf, 3, " | ");
 ...
 */
static void xmlSaveWriteAttributeDecl(xmlSaveCtxtPtr ctxt, xmlAttributePtr attr) {
	xmlOutputBufferPtr buf = ctxt->buf;
 ...
 */
static void xmlBufDumpEntityContent(xmlOutputBufferPtr buf, const xmlChar* content) {
	const char *base, *cur;
 ...
 */
static void xmlBufDumpEntityDecl(xmlOutputBufferPtr buf, xmlEntityPtr ent) {
...
static int  xmlSaveDocInternal(xmlSaveCtxtPtr ctxt, xmlDocPtr cur, const xmlChar* encoding);

static void xmlSaveWriteIndent(xmlSaveCtxtPtr ctxt, int extra) {
	int level;

	if ((ctxt->options & XML_SAVE_NO_INDENT)
	    || (((ctxt->options & XML_SAVE_INDENT) == 0) && (xmlIndentTreeOutput == 0))) {
		return;
	}
...
static void xmlOutputBufferWriteWSNonSig(xmlSaveCtxtPtr ctxt, int extra) {
	...
	for (i = 0; i < (ctxt->level + extra); i += ctxt->indent_nr) {
		xmlOutputBufferWrite(
			ctxt->buf,
			ctxt->indent_size
				* ((ctxt->level + extra - i) > ctxt->indent_nr ? ctxt->indent_nr : (ctxt->level + extra - i)),
			ctxt->indent
		);
	}
}
...
static void xmlDtdDumpOutput(xmlSaveCtxtPtr ctxt, xmlDtdPtr dtd) {
	...
	if ((dtd->entities == NULL) && (dtd->elements == NULL) && (dtd->attributes == NULL)
	    && (dtd->notations == NULL) && (dtd->pentities == NULL)) {
		xmlOutputBufferWrite(buf, 1, ">");
		return;
	}
	xmlOutputBufferWrite(buf, 3, " [\n");
	 ...
	 */
	if ((dtd->notations != NULL) && ((dtd->doc == NULL) || (dtd->doc->intSubset == dtd))) {
...
static void xmlAttrDumpOutput(xmlSaveCtxtPtr ctxt, xmlAttrPtr cur) {
	...
	xmlOutputBufferWrite(buf, 2, "=\"");
	#ifdef LIBXML_HTML_ENABLED
	if ((ctxt->options & XML_SAVE_XHTML) && (cur->ns == NULL)
	    && ((cur->children == NULL) || (cur->children->content == NULL) || (cur->children->content[0] == 0))
...
static void xmlNodeDumpOutputInternal(xmlSaveCtxtPtr ctxt, xmlNodePtr cur) {
	...
	while (1) {
		...
		while (1) {
			...
			if ((ctxt->format == 1) && (cur->type != XML_XINCLUDE_START)
...
static int xmlSaveDocInternal(xmlSaveCtxtPtr ctxt, xmlDocPtr cur, const xmlChar* encoding) {
	...
	if (((cur->type == XML_HTML_DOCUMENT_NODE) && ((ctxt->options & XML_SAVE_AS_XML) == 0)
		...
		htmlNodeDumpInternal(buf, (htmlNodePtr)cur, (char*)ctxt->encoding, format);
	#else
		return (-1);
	#endif
	} else if ((cur->type == XML_DOCUMENT_NODE) || (ctxt->options & XML_SAVE_AS_XML)
		...
		if (cur->children != NULL) {
			...
			while (child != NULL) {
				...
				if ((child->type != XML_XINCLUDE_START) && (child->type != XML_XINCLUDE_END)) {
					xmlOutputBufferWrite(buf, 1, "\n");
				}
				child = child->next;
			}
	...
	while (1) {
		switch (cur->type) {
			...
			case XML_ELEMENT_NODE:
				...
				if ((parent != NULL) && (parent->parent == (xmlNodePtr)cur->doc)
					...
					while (tmp != NULL) {
						if (xmlStrEqual(tmp->name, BAD_CAST "meta")) {
							...
							res = xmlNodeGetAttrValue(tmp, BAD_CAST "http-equiv", NULL, &httpequiv);
							if (res < 0) {
								xmlSaveErrMemory(buf);
							} else if (res == 0) {
				...
				if (cur->children == NULL) {
					...
					if (ctxt->format == 1) {
						tmp = cur->children;
						while (tmp != NULL) {
							if ((tmp->type == XML_TEXT_NODE) || (tmp->type == XML_ENTITY_REF_NODE)) {
								unformattedNode = cur;
								ctxt->format    = 0;
								break;
							}
 ...
 * @returns 0 on success or -1 in case of error.
 */
long xmlSaveTree(xmlSaveCtxt* ctxt, xmlNode* cur) {
	long ret = 0;
	...
	if (((cur->type != XML_NAMESPACE_DECL) && (cur->doc != NULL)
	     && (cur->doc->type == XML_HTML_DOCUMENT_NODE) && ((ctxt->options & XML_SAVE_AS_XML) == 0))
	    || (ctxt->options & XML_SAVE_AS_HTML)) {
 ...
 */
int xmlSaveNotationDecl(xmlSaveCtxt* ctxt, xmlNotation* cur) {
	if (ctxt == NULL) {
		return (-1);
	}
 ...
 * @returns 0 on succes, -1 on error.
 */
int xmlSaveNotationTable(xmlSaveCtxt* ctxt, xmlNotationTable* cur) {
	if (ctxt == NULL) {
		return (-1);
	}
 ...
 * @returns 0 if successful or -1 in case of error.
 */
int xmlSaveSetAttrEscape(xmlSaveCtxt* ctxt, xmlCharEncodingOutputFunc escape ATTRIBUTE_UNUSED) {
	if (ctxt == NULL) {
		return (-1);
	}
	return (0);
 ...
 */
void xmlBufAttrSerializeTxtContent(xmlOutputBuffer* buf, xmlDoc* doc, const xmlChar* string) {
...
void xmlAttrSerializeTxtContent(
	xmlBuffer* buf, xmlDoc* doc, xmlAttr* attr ATTRIBUTE_UNUSED, const xmlChar* string
) {
...
static void
xmlDocDumpInternal(xmlOutputBufferPtr buf, xmlDocPtr doc, const char* encoding, int format) {
	xmlSaveCtxt ctxt;
	int         options;
