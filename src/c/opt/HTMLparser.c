...
#include "libxml.h"
	...
	#define IS_HEX_DIGIT(c)             ((IS_ASCII_DIGIT(c)) || ((((c) | 0x20) >= 'a') && (((c) | 0x20) <= 'f')))

	#define IS_UPPER(c)                 (((c) >= 'A') && ((c) <= 'Z'))

	#define IS_ALNUM(c)                 (IS_ASCII_LETTER(c) || IS_ASCII_DIGIT(c))

typedef enum { INSERT_INITIAL = 1, INSERT_IN_HEAD = 3, INSERT_IN_BODY = 10 } htmlInsertMode;
...
static int  htmlParseElementInternal(htmlParserCtxtPtr ctxt);
 ...
 */
static void htmlErrMemory(xmlParserCtxtPtr ctxt) {
 ...
 */
static void LIBXML_ATTR_FORMAT(3, 0) htmlParseErr(
...
static int htmlnamePush(htmlParserCtxtPtr ctxt, const xmlChar* value) {
	...
	if (ctxt->nameNr >= ctxt->nameMax) {
		...
		newSize = xmlGrowCapacity(ctxt->nameMax, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
		if (newSize < 0) {
			htmlErrMemory(ctxt);
			return (-1);
		}
 ...
 */
static int htmlNodeInfoPush(htmlParserCtxtPtr ctxt, htmlParserNodeInfo* value) {
	if (ctxt->nodeInfoNr >= ctxt->nodeInfoMax) {
		xmlParserNodeInfo* tmp;
		int                newSize;

		newSize = xmlGrowCapacity(ctxt->nodeInfoMax, sizeof(tmp[0]), 5, XML_MAX_ITEMS);
		if (newSize < 0) {
			htmlErrMemory(ctxt);
			return (0);
		}
 ...
 */
static htmlParserNodeInfo* htmlNodeInfoPop(htmlParserCtxtPtr ctxt) {
 ...
 */
	...
	#define SHRINK                                                                                \
		if ((!PARSER_PROGRESSIVE(ctxt)) && (ctxt->input->cur - ctxt->input->base > 2 * INPUT_CHUNK) \
...
static xmlChar* htmlFindEncoding(xmlParserCtxtPtr ctxt) {
	...
	if ((ctxt == NULL) || (ctxt->input == NULL) || (ctxt->input->flags & XML_INPUT_HAS_ENCODING)) {
		return (NULL);
	}
	...
	cur   += 8;
	start  = cur;
	while ((IS_ALNUM(*cur)) || (*cur == '-') || (*cur == '_') || (*cur == ':') || (*cur == '/')) {
		cur++;
	}
	if (cur == start) {
		return (NULL);
...
static int htmlMaskMatch(htmlAsciiMask mask, unsigned c) {
	if (c >= 64) {
		return (0);
	}
...
static int htmlValidateUtf8(xmlParserCtxtPtr ctxt, const xmlChar* str, size_t len, int partial) {
	unsigned c = str[0];
	int      size;
	...
	if (c < 0xC2) {
		...
		if (((v & 0x00C0C0) != 0x008080) || ((v & 0x0F2000) == 0x000000)
		    || ((v & 0x0F2000) == 0x0D2000)) {
			goto invalid;
		}
		...
		if (((v & 0x00C0C0C0) != 0x00808080) || (v < 0xF0900000) || (v >= 0xF4900000)) {
			goto invalid;
		}
...
invalid:
	...
	if ((ctxt->input->flags & XML_INPUT_ENCODING_ERROR) == 0) {
		htmlParseErr(
			ctxt, XML_ERR_INVALID_ENCODING, "Invalid bytes in character encoding\n", NULL, NULL
		);
...
static const htmlElemDesc html40ElementTable[] = {
	{				 "a",0, 0,0, 0,0, 0,1,																										 "anchor ",NULL, NULL,NULL, NULL,NULL,              0																																																														},
	{			"abbr", 0, 0, 0, 0, 0, 0, 1,																						"abbreviated form", NULL, NULL, NULL, NULL, NULL,              0},
	{	 "acronym", 0, 0, 0, 0, 0, 0, 1,																														"", NULL, NULL, NULL, NULL, NULL,              0},
	{	 "address", 0, 0, 0, 0, 0, 0, 0,																			"information on author ", NULL, NULL, NULL, NULL, NULL,              0},
	{		"applet", 0, 0, 0, 0, 1, 1, 2,																								"java applet ", NULL, NULL, NULL, NULL, NULL,              0},
	{			"area", 0, 2, 2, 1, 0, 0, 0,																 "client-side image map area ", NULL, NULL, NULL, NULL, NULL,              0},
	{				 "b", 0, 3, 0, 0, 0, 0, 1,																						 "bold text style", NULL, NULL, NULL, NULL, NULL,              0},
	{			"base", 0, 2, 2, 1, 0, 0, 0,																					"document base uri ", NULL, NULL, NULL, NULL, NULL,              0},
	{  "basefont", 0, 2, 2, 1, 1, 1, 1,																						 "base font size ", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "bdo", 0, 0, 0, 0, 0, 0, 1,																				"i18n bidi over-ride ", NULL, NULL, NULL, NULL, NULL,              0},
	{	 "bgsound", 0, 0, 2, 1, 0, 0, 0,																														"", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "big", 0, 3, 0, 0, 0, 0, 1,																						"large text style", NULL, NULL, NULL, NULL, NULL,              0},
	{"blockquote", 0, 0, 0, 0, 0, 0, 0,																						 "long quotation ", NULL, NULL, NULL, NULL, NULL,              0},
	{			"body", 1, 1, 0, 0, 0, 0, 0,																							"document body ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"br", 0, 2, 2, 1, 0, 0, 1,																					"forced line break ", NULL, NULL, NULL, NULL, NULL,              0},
	{		"button", 0, 0, 0, 0, 0, 0, 2,																								"push button ", NULL, NULL, NULL, NULL, NULL,              0},
	{	 "caption", 0, 0, 0, 0, 0, 0, 0,																							"table caption ", NULL, NULL, NULL, NULL, NULL,              0},
	{		"center",
   0, 3,
   0, 0,
   1, 1,
   0,                             "shorthand for div align=center ",
   NULL, NULL,
   NULL, NULL,
   NULL,              0																																																													},
	{			"cite", 0, 0, 0, 0, 0, 0, 1,																										"citation", NULL, NULL, NULL, NULL, NULL,              0},
	{			"code", 0, 0, 0, 0, 0, 0, 1,																			"computer code fragment", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "col", 0, 2, 2, 1, 0, 0, 0,																							 "table column ", NULL, NULL, NULL, NULL, NULL,              0},
	{  "colgroup", 0, 1, 0, 0, 0, 0, 0,																				 "table column group ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"dd", 0, 1, 0, 0, 0, 0, 0,																		 "definition description ", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "del", 0, 0, 0, 0, 0, 0, 2,																							 "deleted text ", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "dfn", 0, 0, 0, 0, 0, 0, 1,																				 "instance definition", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "dir", 0, 0, 0, 0, 1, 1, 0,																							"directory list", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "div",
   0, 0,
   0, 0,
   0, 0,
   0,                            "generic language/style container",
   NULL, NULL,
   NULL, NULL,
   NULL,              0																																																													},
	{				"dl", 0, 0, 0, 0, 0, 0, 0,																						"definition list ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"dt", 0, 1, 0, 0, 0, 0, 0,																						"definition term ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"em", 0, 3, 0, 0, 0, 0, 1,																										"emphasis", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "embed", 0, 1, 2, 1, 1, 1, 1,																		"generic embedded object ", NULL, NULL, NULL, NULL, NULL,              0},
	{  "fieldset", 0, 0, 0, 0, 0, 0, 0,																				 "form control group ", NULL, NULL, NULL, NULL, NULL,              0},
	{			"font", 0, 3, 0, 0, 1, 1, 1,																			 "local change to font ", NULL, NULL, NULL, NULL, NULL,              0},
	{			"form", 0, 0, 0, 0, 0, 0, 0,																					 "interactive form ", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "frame", 0, 2, 2, 1, 0, 2, 0,																									"subwindow ", NULL, NULL, NULL, NULL, NULL,              0},
	{  "frameset", 0, 0, 0, 0, 0, 2, 0,																					"window subdivision", NULL, NULL, NULL, NULL, NULL,              0},
	{				"h1", 0, 0, 0, 0, 0, 0, 0,																										"heading ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"h2", 0, 0, 0, 0, 0, 0, 0,																										"heading ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"h3", 0, 0, 0, 0, 0, 0, 0,																										"heading ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"h4", 0, 0, 0, 0, 0, 0, 0,																										"heading ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"h5", 0, 0, 0, 0, 0, 0, 0,																										"heading ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"h6", 0, 0, 0, 0, 0, 0, 0,																										"heading ", NULL, NULL, NULL, NULL, NULL,              0},
	{			"head", 1, 1, 0, 0, 0, 0, 0,																							"document head ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"hr", 0, 2, 2, 1, 0, 0, 0,																						"horizontal rule ", NULL, NULL, NULL, NULL, NULL,              0},
	{			"html", 1, 1, 0, 0, 0, 0, 0,																			"document root element ", NULL, NULL, NULL, NULL, NULL,              0},
	{				 "i", 0, 3, 0, 0, 0, 0, 1,																					 "italic text style", NULL, NULL, NULL, NULL, NULL,              0},
	{		"iframe", 0, 0, 0, 0, 0, 1, 2,																					 "inline subwindow ", NULL, NULL, NULL, NULL, NULL,   DATA_RAWTEXT},
	{			 "img", 0, 2, 2, 1, 0, 0, 1,																						 "embedded image ", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "input", 0, 2, 2, 1, 0, 0, 1,																							 "form control ", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "ins", 0, 0, 0, 0, 0, 0, 2,																							 "inserted text", NULL, NULL, NULL, NULL, NULL,              0},
	{	 "isindex", 0, 2, 2, 1, 1, 1, 0,																				 "single line prompt ", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "kbd", 0, 0, 0, 0, 0, 0, 1,                              "text to be entered by the user", NULL, NULL, NULL, NULL, NULL,              0},
	{		"keygen", 0, 0, 2, 1, 0, 0, 0,																														"", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "label", 0, 0, 0, 0, 0, 0, 1,																			"form field label text ", NULL, NULL, NULL, NULL, NULL,              0},
	{		"legend", 0, 0, 0, 0, 0, 0, 0,																						"fieldset legend ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"li", 0, 1, 1, 0, 0, 0, 0,																									"list item ", NULL, NULL, NULL, NULL, NULL,              0},
	{			"link", 0, 2, 2, 1, 0, 0, 0,																	 "a media-independent link ", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "map", 0, 0, 0, 0, 0, 0, 2,																			"client-side image map ", NULL, NULL, NULL, NULL, NULL,              0},
	{			"menu", 0, 0, 0, 0, 1, 1, 0,																									"menu list ", NULL, NULL, NULL, NULL, NULL,              0},
	{			"meta", 0, 2, 2, 1, 0, 0, 0,																		"generic metainformation ", NULL, NULL, NULL, NULL, NULL,              0},
	{	 "noembed", 0, 0, 0, 0, 0, 0, 0,																														"", NULL, NULL, NULL, NULL, NULL,   DATA_RAWTEXT},
	{  "noframes",
   0, 0,
   0, 0,
   0, 2,
   0,  "alternate content container for non frame-based rendering ",
   NULL, NULL,
   NULL, NULL,
   NULL,   DATA_RAWTEXT																																																													},
	{  "noscript",
   ...
   0, 0,
   0, 0,
   0, "alternate content container for non script-based rendering ",
   NULL, NULL,
   NULL, NULL,
   NULL,              0																																																													},
	{		"object", 0, 0, 0, 0, 0, 0, 2,																		"generic embedded object ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"ol", 0, 0, 0, 0, 0, 0, 0,																							 "ordered list ", NULL, NULL, NULL, NULL, NULL,              0},
	{  "optgroup", 0, 0, 0, 0, 0, 0, 0,																							 "option group ", NULL, NULL, NULL, NULL, NULL,              0},
	{		"option", 0, 1, 0, 0, 0, 0, 0,																					"selectable choice ", NULL, NULL, NULL, NULL, NULL,              0},
	{				 "p", 0, 1, 0, 0, 0, 0, 0,																									"paragraph ", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "param", 0, 2, 2, 1, 0, 0, 0,																			 "named property value ", NULL, NULL, NULL, NULL, NULL,              0},
	{ "plaintext", 0, 0, 0, 0, 0, 0, 0,																														"", NULL, NULL, NULL, NULL, NULL, DATA_PLAINTEXT},
	{			 "pre", 0, 0, 0, 0, 0, 0, 0,																					"preformatted text ", NULL, NULL, NULL, NULL, NULL,              0},
	{				 "q", 0, 0, 0, 0, 0, 0, 1,																		 "short inline quotation ", NULL, NULL, NULL, NULL, NULL,              0},
	{				 "s", 0, 3, 0, 0, 1, 1, 1,																	 "strike-through text style", NULL, NULL, NULL, NULL, NULL,              0},
	{			"samp",
   0, 0,
   0, 0,
   0, 0,
   1,                        "sample program output, scripts, etc.",
   NULL, NULL,
   NULL, NULL,
   NULL,              0																																																													},
	{		"script", 0, 0, 0, 0, 0, 0, 2,																					"script statements ", NULL, NULL, NULL, NULL, NULL,    DATA_SCRIPT},
	{		"select", 0, 0, 0, 0, 0, 0, 1,																						"option selector ", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "small", 0, 3, 0, 0, 0, 0, 1,																						"small text style", NULL, NULL, NULL, NULL, NULL,              0},
	{		"source", 0, 0, 2, 1, 0, 0, 0,																														"", NULL, NULL, NULL, NULL, NULL,              0},
	{			"span",
   0, 0,
   0, 0,
   0, 0,
   1,                           "generic language/style container ",
   NULL, NULL,
   NULL, NULL,
   NULL,              0																																																													},
	{		"strike", 0, 3, 0, 0, 1, 1, 1,																				 "strike-through text", NULL, NULL, NULL, NULL, NULL,              0},
	{		"strong", 0, 3, 0, 0, 0, 0, 1,																						 "strong emphasis", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "style", 0, 0, 0, 0, 0, 0, 0,																								 "style info ", NULL, NULL, NULL, NULL, NULL,   DATA_RAWTEXT},
	{			 "sub", 0, 3, 0, 0, 0, 0, 1,																									 "subscript", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "sup", 0, 3, 0, 0, 0, 0, 1,																								"superscript ", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "table", 0, 0, 0, 0, 0, 0, 0,																														"", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "tbody", 1, 0, 0, 0, 0, 0, 0,																								 "table body ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"td", 0, 0, 0, 0, 0, 0, 0,																						 "table data cell", NULL, NULL, NULL, NULL, NULL,              0},
	{  "textarea",
   0, 0,
   0, 0,
   0, 0,
   ...
   NULL, NULL,
   NULL,    DATA_RCDATA																																																													},
	{		 "tfoot", 0, 1, 0, 0, 0, 0, 0,																							 "table footer ", NULL, NULL, NULL, NULL, NULL,              0},
	{				"th", 0, 1, 0, 0, 0, 0, 0,																					 "table header cell", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "thead", 0, 1, 0, 0, 0, 0, 0,																							 "table header ", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "title", 0, 0, 0, 0, 0, 0, 0,																						 "document title ", NULL, NULL, NULL, NULL, NULL,    DATA_RCDATA},
	{				"tr", 0, 0, 0, 0, 0, 0, 0,																									"table row ", NULL, NULL, NULL, NULL, NULL,              0},
	{		 "track", 0, 0, 2, 1, 0, 0, 0,																														"", NULL, NULL, NULL, NULL, NULL,              0},
	{				"tt",
   0, 3,
   0, 0,
   0, 0,
   1,                           "teletype or monospaced text style",
   NULL, NULL,
   NULL, NULL,
   NULL,              0																																																													},
	{				 "u", 0, 3, 0, 0, 1, 1, 1,																			 "underlined text style", NULL, NULL, NULL, NULL, NULL,              0},
	{				"ul", 0, 0, 0, 0, 0, 0, 0,																						 "unordered list ", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "var",
   0, 0,
   0, 0,
   0, 0,
   1,                  "instance of a variable or program argument",
   NULL, NULL,
   NULL, NULL,
   NULL,              0																																																													},
	{			 "wbr", 0, 0, 2, 1, 0, 0, 0,																														"", NULL, NULL, NULL, NULL, NULL,              0},
	{			 "xmp", 0, 0, 0, 0, 0, 0, 1,																														"", NULL, NULL, NULL, NULL, NULL,   DATA_RAWTEXT}
};
...
static int htmlCompareTags(const void* key, const void* member) {
...
const htmlElemDesc* htmlTagLookup(const xmlChar* tag) {
	...
	return ((const htmlElemDesc*)bsearch(
		tag,
		html40ElementTable,
...
static int htmlCompareStartClose(const void* vkey, const void* member) {
 ...
 */
static int htmlCheckAutoClose(const xmlChar* newtag, const xmlChar* oldtag) {
 ...
 * @param newtag  The new tag name
 */
static void htmlAutoCloseOnClose(htmlParserCtxtPtr ctxt, const xmlChar* newtag) {
 ...
 * @param ctxt  an HTML parser context
 */
static void htmlAutoCloseOnEnd(htmlParserCtxtPtr ctxt) {
	int i;
...
static void htmlAutoClose(htmlParserCtxtPtr ctxt, const xmlChar* newtag) {
	...
	while ((ctxt->name != NULL) && (htmlCheckAutoClose(newtag, ctxt->name))) {
		htmlParserFinishElementParsing(ctxt);
		if ((ctxt->sax != NULL) && (ctxt->sax->endElement != NULL)) {
			ctxt->sax->endElement(ctxt->userData, ctxt->name);
		}
...
int htmlAutoCloseTag(xmlDoc* doc, const xmlChar* name, xmlNode* elem) {
	...
	if (htmlCheckAutoClose(elem->name, name)) {
		return (1);
	}
	child = elem->children;
	while (child != NULL) {
		if (htmlAutoCloseTag(doc, name, child)) {
			return (1);
		}
		child = child->next;
	}
...
int htmlIsAutoClosed(xmlDoc* doc, xmlNode* elem) {
	...
	child = elem->children;
	while (child != NULL) {
		if (htmlAutoCloseTag(doc, elem->name, child)) {
			return (1);
		}
		child = child->next;
	}
...
static void htmlCheckImplied(htmlParserCtxtPtr ctxt, const xmlChar* newtag) {
	...
	if ((ctxt->nameNr <= 1)
	    && ((xmlStrEqual(newtag, BAD_CAST "script")) || (xmlStrEqual(newtag, BAD_CAST "style")) || (xmlStrEqual(newtag, BAD_CAST "meta")) || (xmlStrEqual(newtag, BAD_CAST "link")) || (xmlStrEqual(newtag, BAD_CAST "title")) || (xmlStrEqual(newtag, BAD_CAST "base")))) {
...
static void htmlStartCharData(htmlParserCtxtPtr ctxt) {
	if (ctxt->options & (HTML_PARSE_NOIMPLIED | HTML_PARSE_HTML5)) {
		return;
	}
	if (!htmlOmittedDefaultValue) {
...
static int htmlCompareEntityDesc(const void* vkey, const void* vdesc) {
...
const htmlEntityDesc* htmlEntityValueLookup(unsigned int value) {
	...
	desc
		= bsearch(&value, html40EntitiesTable, nmemb, sizeof(htmlEntityDesc), htmlCompareEntityDesc);
...
int htmlEncodeEntities(
	unsigned char* out, int* outlen, const unsigned char* in, int* inlen, int quoteChar
) {
...
static const char* const allowPCData[]
	= {"a",          "abbr",  "acronym", "address", "applet",   "b",        "bdo",    "big",
     "blockquote", "body",  "button",  "caption", "center",   "cite",     "code",   "dd",
     "del",        "dfn",   "div",     "dt",      "em",       "font",     "form",   "h1",
     "h2",         "h3",    "h4",      "h5",      "h6",       "i",        "iframe", "ins",
     "kbd",        "label", "legend",  "li",      "noframes", "noscript", "object", "p",
 ...
 */
xmlDoc* htmlNewDocNoDtD(const xmlChar* URI, const xmlChar* publicId) {
	xmlDocPtr cur;
 ...
 */
xmlDoc* htmlNewDoc(const xmlChar* URI, const xmlChar* publicId) {
...
static xmlHashedString htmlParseHTMLName(htmlParserCtxtPtr ctxt, int attr) {
	xmlHashedString ret;
	...
	while (1) {
		...
		if ((nbchar != 0) && ((c == '/') || (c == '>') || (c == stop) || (IS_WS_HTML(c)))) {
			break;
		}
...
static const xmlChar* htmlCodePointToUtf8(int c, xmlChar* out, int* osize) {
	int i = 0;
	int bits, hi;

	if ((c >= 0x80) && (c < 0xA0)) {
		c = htmlC1Remap[c - 0x80];
	} else if ((c <= 0) || ((c >= 0xD800) && (c < 0xE000)) || (c > 0x10FFFF)) {
		c = 0xFFFD;
	}
...
htmlFindEntityPrefix(const xmlChar* string, size_t slen, int isAttr, int* nlen, int* rlen) {
	...
	while (left < right) {
		...
		if (cmp == 0) {
			if (slen < len) {
				cmp = strncmp((const char*)string + soff + 1, (const char*)bytes + 2, slen - 1);
				/* Prefix can never match */
				if (cmp == 0) {
					break;
				}
			} else {
				cmp = strncmp((const char*)string + soff + 1, (const char*)bytes + 2, len - 1);
			}
		}
		...
		if (cmp < 0) {
			...
			isTerm
				= ((term == ';') || ((bytes[0] & ENT_F_SEMICOLON) && ((!isAttr) || ((!isAlnum) && (term != '=')))));
...
static xmlChar*
htmlParseData(htmlParserCtxtPtr ctxt, htmlAsciiMask mask, int comment, int refs, int maxLength) {
	...
	#endif
						if (guess == NULL) {
							xmlSwitchEncoding(ctxt, XML_CHAR_ENCODING_WINDOWS_1252);
						} else {
...
next_chunk:
		...
		if (extraSize > maxLength - used) {
			htmlParseErr(ctxt, XML_ERR_RESOURCE_LIMIT, "value too long\n", NULL, NULL);
			goto error;
		}
...
htmlParseEntityRef(htmlParserCtxt* ctxt ATTRIBUTE_UNUSED, const xmlChar** str ATTRIBUTE_UNUSED) {
	return (NULL);
}
...
static xmlChar* htmlParseAttValue(htmlParserCtxtPtr ctxt) {
	xmlChar* ret  = NULL;
	int maxLength = (ctxt->options & HTML_PARSE_HUGE) ? XML_MAX_HUGE_LENGTH : XML_MAX_TEXT_LENGTH;
...
static void
htmlCharDataSAXCallback(htmlParserCtxtPtr ctxt, const xmlChar* buf, int size, int mode) {
	if ((ctxt->sax == NULL) || (ctxt->disableSAX)) {
		return;
	}

	if ((mode == 0) || (mode == DATA_RCDATA) || (ctxt->sax->cdataBlock == NULL)) {
		if ((ctxt->name == NULL) || (xmlStrEqual(ctxt->name, BAD_CAST "html"))
		...
		if ((mode == 0) && (!ctxt->keepBlanks) && (areBlanks(ctxt, buf, size) > 0)) {
			if (ctxt->sax->ignorableWhitespace != NULL) {
...
static int htmlParseCharData(htmlParserCtxtPtr ctxt, int partial) {
	xmlParserInputPtr input = ctxt->input;
	xmlChar           utf8Char[4];
	int               complete = 0;
	int               done     = 0;
					...
					if (j < avail) {
						if ((mode == DATA_SCRIPT) && (in[j] == '!')) {
							...
							if ((solidus) || (mode == DATA_SCRIPT_ESC1)) {
								while ((j < avail) && (ctxt->name[i] != 0) && (ctxt->name[i] == (in[j] | 0x20))) {
									i += 1;
									j += 1;
								}
				...
				case '&':
					...
					if ((j < avail) && (in[j] == '#')) {
						...
						if (partial) {
							...
							for (i = 1; i < avail; i++) {
								if ((i >= 32)
								    || (!IS_ASCII_LETTER(in[i]) && ((i < 2) || !IS_ASCII_DIGIT(in[i])))) {
									terminated = 1;
									break;
								}
							}
...
static void htmlParseComment(htmlParserCtxtPtr ctxt, int bogus) {
	...
	if (bogus) {
		...
		if ((!PARSER_PROGRESSIVE(ctxt)) && (ctxt->input->end - ctxt->input->cur < 2)) {
			xmlParserGrow(ctxt);
		}
 ...
 * @returns 0
 */
int htmlParseCharRef(htmlParserCtxt* ctxt ATTRIBUTE_UNUSED) {
	return (0);
}
...
static xmlChar* htmlParseDoctypeLiteral(htmlParserCtxtPtr ctxt) {
	xmlChar* ret;
	int maxLength = (ctxt->options & HTML_PARSE_HUGE) ? XML_MAX_TEXT_LENGTH : XML_MAX_NAME_LENGTH;
...
static void htmlSkipBogusDoctype(htmlParserCtxtPtr ctxt) {
	const xmlChar* in;
	size_t         avail;
	int            eof = PARSER_PROGRESSIVE(ctxt);
	int            line, col;
...
static void htmlParseDocTypeDecl(htmlParserCtxtPtr ctxt) {
	 ...
	 */
	if ((UPPER == 'P') && (UPP(1) == 'U') && (UPP(2) == 'B') && (UPP(3) == 'L') && (UPP(4) == 'I')
	    && (UPP(5) == 'C')) {
		SKIP(6);
		SKIP_BLANKS;
	...
	} else if ((UPPER == 'S') && (UPP(1) == 'Y') && (UPP(2) == 'S') && (UPP(3) == 'T')
	           && (UPP(4) == 'E') && (UPP(5) == 'M')) {
		SKIP(6);
		SKIP_BLANKS;
		URI = htmlParseDoctypeLiteral(ctxt);
...
static xmlHashedString htmlParseAttribute(htmlParserCtxtPtr ctxt, xmlChar** value) {
...
static int htmlCharEncCheckAsciiCompatible(htmlParserCtxt* ctxt, const xmlChar* encoding) {
	...
	res = xmlCreateCharEncodingHandler(
		(const char*)encoding, XML_ENC_INPUT | XML_ENC_HTML, ctxt->convImpl, ctxt->convCtxt, &handler
	);
	if (res != XML_ERR_OK) {
	...
	if ((res != XML_ENC_ERR_SUCCESS) || (inlen != 8) || (outlen != 8) || (memcmp(in, out, 8) != 0)) {
		htmlParseErr(
			ctxt, XML_ERR_UNSUPPORTED_ENCODING, "Encoding %s isn't ASCII-compatible", encoding, NULL
		);
		return (-1);
	}
 ...
 */
static void htmlCheckMeta(htmlParserCtxtPtr ctxt, const xmlChar** atts) {
	int            i;
 ...
 */
static int htmlAttrHashInsert(
	xmlParserCtxtPtr ctxt, unsigned size, const xmlChar* name, unsigned hashValue, int aindex
) {
...
static void htmlParseStartTag(htmlParserCtxtPtr ctxt) {
	const xmlChar*  name;
	...
	if ((ctxt->options & HTML_PARSE_HTML5) == 0) {
		...
		if ((ctxt->nameNr != 1) && (xmlStrEqual(name, BAD_CAST "head"))) {
			htmlParseErr(
				ctxt, XML_HTML_STRUCURE_ERROR, "htmlParseStartTag: misplaced <head> tag\n", name, NULL
			);
			discardtag = 1;
			ctxt->depth++;
		}
		if (xmlStrEqual(name, BAD_CAST "body")) {
			...
			for (indx = 0; indx < ctxt->nameNr; indx++) {
				if (xmlStrEqual(ctxt->nameTab[indx], BAD_CAST "body")) {
					htmlParseErr(
						ctxt, XML_HTML_STRUCURE_ERROR, "htmlParseStartTag: misplaced <body> tag\n", name, NULL
					);
					discardtag = 1;
					ctxt->depth++;
				}
	 ...
	 */
	SKIP_BLANKS;
	while ((ctxt->input->cur < ctxt->input->end) && (CUR != '>') && ((CUR != '/') || (NXT(1) != '>'))
		...
		if (attname != NULL) {
			...
			if (nbatts + 4 > maxatts) {
				...
				newSize
					= xmlGrowCapacity(maxatts, sizeof(tmp[0]) * 2 + sizeof(utmp[0]), 11, HTML_MAX_ATTRS);
				if (newSize < 0) {
					htmlErrMemory(ctxt);
					goto failed;
				}
...
failed:
	...
	if (nbatts > 2) {
		...
		for (i = 0, j = 0, k = 0; i < nbatts; i += 2, k++) {
			...
			res       = htmlAttrHashInsert(ctxt, attrHashSize, attname, hashValue, j);
			if (res < 0) {
				continue;
			}
...
static void htmlParseEndTag(htmlParserCtxtPtr ctxt) {
	const xmlChar* name;
	...
	if (ctxt->input->cur >= ctxt->input->end) {
		htmlStartCharData(ctxt);
		if ((ctxt->sax != NULL) && (!ctxt->disableSAX) && (ctxt->sax->characters != NULL)) {
			ctxt->sax->characters(ctxt->userData, BAD_CAST "</", 2);
		}
		return;
	}
	 ...
	 */
	if ((ctxt->depth > 0)
	    && (xmlStrEqual(name, BAD_CAST "html") || xmlStrEqual(name, BAD_CAST "body") || xmlStrEqual(name, BAD_CAST "head"))) {
		ctxt->depth--;
		return;
	}
	...
	if (i < 0) {
		htmlParseErr(ctxt, XML_ERR_TAG_NAME_MISMATCH, "Unexpected end tag : %s\n", name, NULL);
		return;
	}
...
static void htmlParseContent(htmlParserCtxtPtr ctxt) {
	...
	while ((PARSER_STOPPED(ctxt) == 0) && (ctxt->input->cur < ctxt->input->end)) {
		int mode;
		...
		if ((mode == 0) && (CUR == '<')) {
			if (NXT(1) == '/') {
				 ...
				 */
				if ((UPP(2) == 'D') && (UPP(3) == 'O') && (UPP(4) == 'C') && (UPP(5) == 'T')
 ...
 * @returns a node list.
 */
xmlNode* htmlCtxtParseContentInternal(htmlParserCtxt* ctxt, xmlParserInput* input) {
	xmlNodePtr root;
	...
	while (CUR == '<') {
		if (NXT(1) == '!') {
			if ((NXT(2) == '-') && (NXT(3) == '-')) {
				SKIP(4);
				htmlParseComment(ctxt, /* bogus */ 0);
			} else if ((UPP(2) == 'D') && (UPP(3) == 'O') && (UPP(4) == 'C') && (UPP(5) == 'T')
			           && (UPP(6) == 'Y') && (UPP(7) == 'P') && (UPP(8) == 'E')) {
				htmlParseDocTypeDecl(ctxt);
 ...
 */
static int htmlInitParserCtxt(htmlParserCtxtPtr ctxt, const htmlSAXHandler* sax, void* userData) {
	#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
	...
	ctxt->inputTab = (htmlParserInputPtr*)xmlMalloc(sizeof(htmlParserInputPtr));
...
static htmlParserCtxtPtr htmlCreateMemoryParserCtxtInternal(
	const char* url, const char* buffer, size_t size, const char* encoding
) {
	xmlParserCtxtPtr  ctxt;
...
htmlCreateDocParserCtxt(const xmlChar* str, const char* url, const char* encoding) {
	xmlParserCtxtPtr  ctxt;
	xmlParserInputPtr input;
	...
	input = xmlCtxtNewInputFromString(ctxt, url, (const char*)str, encoding, 0);
	if (input == NULL) {
		xmlFreeParserCtxt(ctxt);
		return (NULL);
	}
 ...
 * @param ctxt  an HTML parser context
 */
static int htmlParseLookupGt(xmlParserCtxtPtr ctxt) {
	const xmlChar* cur;
	...
	while (cur < end) {
		...
		if (state != LSTATE_ATTR_VALUE_SQUOTED && state != LSTATE_ATTR_VALUE_DQUOTED) {
			if (c == '/' && state != LSTATE_BEFORE_ATTR_VALUE && state != LSTATE_ATTR_VALUE_UNQUOTED) {
 ...
 */
static int htmlParseLookupString(
	xmlParserCtxtPtr ctxt, size_t startDelta, const char* str, size_t strLen, size_t extraLen
) {
	const xmlChar* end = ctxt->input->end;
	...
	if ((term != NULL) && ((size_t)(ctxt->input->end - term) >= extraLen + 1)) {
		ctxt->checkIndex = 0;
	...
	while (PARSER_STOPPED(ctxt) == 0) {
		...
		switch (ctxt->instate) {
			...
			case XML_PARSER_XML_DECL:
				...
				if ((ctxt->instate == XML_PARSER_START) || (ctxt->instate == XML_PARSER_XML_DECL)) {
					ctxt->instate = XML_PARSER_MISC;
				}
				break;

			case XML_PARSER_START_TAG:
				if ((!terminate) && (htmlParseLookupGt(ctxt) < 0)) {
					return;
				}
			...
			case XML_PARSER_CONTENT: {
				...
				if ((ctxt->instate == XML_PARSER_MISC) || (ctxt->instate == XML_PARSER_PROLOG)) {
					SKIP_BLANKS;
					avail = in->end - in->cur;
				}
				...
				if (mode != 0) {
					...
					if (next == '!') {
						...
						if ((UPP(2) == 'D') && (UPP(3) == 'O') && (UPP(4) == 'C') && (UPP(5) == 'T')
						    && (UPP(6) == 'Y') && (UPP(7) == 'P') && (UPP(8) == 'E')) {
							if ((!terminate) && (htmlParseLookupString(ctxt, 9, ">", 1, 0) < 0)) {
								return;
							}
							htmlParseDocTypeDecl(ctxt);
							...
							if ((!terminate) && (htmlParseLookupString(ctxt, 2, ">", 1, 0) < 0)) {
								return;
							}
							SKIP(2);
							htmlParseComment(ctxt, /* bogus */ 1);
					...
					if (avail < HTML_PARSER_BIG_BUFFER_SIZE) {
						if ((!terminate) && (htmlParseLookupString(ctxt, 0, "<", 1, 0) < 0)) {
							return;
						}
					}
					ctxt->checkIndex = 0;
...
int htmlParseChunk(htmlParserCtxt* ctxt, const char* chunk, int size, int terminate) {
	if ((ctxt == NULL) || (ctxt->input == NULL) || (ctxt->input->buf == NULL) || (size < 0)
	    || ((size > 0) && (chunk == NULL))) {
	...
	if (size > 0) {
		...
		if (res < 0) {
			htmlParseErr(ctxt, ctxt->input->buf->error, "xmlParserInputBufferPush failed", NULL, NULL);
			return (ctxt->errNo);
		}
	}
 ...
 * failed.
 */
htmlParserCtxt* htmlCreatePushParserCtxt(
	htmlSAXHandler* sax,
	void*           user_data,
	const char*     chunk,
	int             size,
...
xmlDoc*
htmlSAXParseDoc(const xmlChar* cur, const char* encoding, htmlSAXHandler* sax, void* userData) {
	htmlDocPtr        ret;
	htmlParserCtxtPtr ctxt;
...
xmlDoc*
htmlSAXParseFile(const char* filename, const char* encoding, htmlSAXHandler* sax, void* userData) {
	htmlDocPtr        ret;
	htmlParserCtxtPtr ctxt;
...
int htmlElementAllowedHere(
	const htmlElemDesc* parent ATTRIBUTE_UNUSED, const xmlChar* elt ATTRIBUTE_UNUSED
) {
	return (1);
}
...
htmlStatus htmlElementStatusHere(
	const htmlElemDesc* parent ATTRIBUTE_UNUSED, const htmlElemDesc* elt ATTRIBUTE_UNUSED
) {
	return (HTML_VALID);
}
 ...
 * @returns HTML_VALID
 */
htmlStatus htmlNodeStatus(xmlNode* node ATTRIBUTE_UNUSED, int legacy ATTRIBUTE_UNUSED) {
	return (HTML_VALID);
}
...
static int htmlCtxtSetOptionsInternal(xmlParserCtxtPtr ctxt, int options, int keepMask) {
	...
	allMask = HTML_PARSE_RECOVER | HTML_PARSE_HTML5 | HTML_PARSE_NODEFDTD | HTML_PARSE_NOERROR
	        | HTML_PARSE_NOWARNING | HTML_PARSE_PEDANTIC | HTML_PARSE_NOBLANKS | HTML_PARSE_NONET
	        | HTML_PARSE_NOIMPLIED | HTML_PARSE_COMPACT | HTML_PARSE_HUGE | HTML_PARSE_IGNORE_ENC
...
int htmlCtxtUseOptions(htmlParserCtxt* ctxt, int options) {
	 ...
	 */
	keepMask = HTML_PARSE_NODEFDTD | HTML_PARSE_NOERROR | HTML_PARSE_NOWARNING | HTML_PARSE_NOIMPLIED
	         | HTML_PARSE_COMPACT | HTML_PARSE_HUGE | HTML_PARSE_IGNORE_ENC | HTML_PARSE_BIG_LINES;
 ...
 * @returns the resulting document tree.
 */
xmlDoc* htmlReadDoc(const xmlChar* str, const char* url, const char* encoding, int options) {
	htmlParserCtxtPtr ctxt;
	xmlParserInputPtr input;
	htmlDocPtr        doc = NULL;
 ...
 */
xmlDoc*
htmlCtxtReadFile(xmlParserCtxt* ctxt, const char* filename, const char* encoding, int options) {
	xmlParserInputPtr input;
 ...
 */
xmlDoc*
htmlCtxtReadFd(xmlParserCtxt* ctxt, int fd, const char* URL, const char* encoding, int options) {
	xmlParserInputPtr input;
