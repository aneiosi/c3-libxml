...
static htmlNodePtr htmlFindFirstChild(htmlNodePtr parent, const char* name) {
	htmlNodePtr child;

	for (child = parent->children; child != NULL; child = child->next) {
		if ((child->type == XML_ELEMENT_NODE) && (xmlStrcasecmp(child->name, BAD_CAST name) == 0)) {
			return (child);
		}
	}
...
static htmlNodePtr htmlFindHead(htmlDocPtr doc) {
	htmlNodePtr html;
...
static xmlAttrPtr htmlFindMetaEncodingAttr(htmlNodePtr elem, int* outIsContentType) {
	...
	for (attr = elem->properties; attr != NULL; attr = attr->next) {
		...
		if ((xmlStrcasecmp(attr->name, BAD_CAST "http-equiv") == 0) && (attr->children != NULL)
		    && (attr->children->type == XML_TEXT_NODE) && (attr->children->next == NULL)
		    && (xmlStrcasecmp(attr->children->content, BAD_CAST "Content-Type") == 0)) {
			isContentType = 1;
		}
	}
...
static int htmlParseMetaEncoding(htmlNodePtr elem, htmlMetaEncoding* menc) {
	xmlAttrPtr     attr;
	const xmlChar* val = NULL;
	int            isContentType;

	if ((elem->type != XML_ELEMENT_NODE) || (xmlStrcasecmp(elem->name, BAD_CAST "meta") != 0)) {
		return (0);
	}
	...
	if ((attr->children != NULL) && (attr->children->type == XML_TEXT_NODE)
	    && (attr->children->next == NULL) && (attr->children->content != NULL)) {
...
static xmlChar* htmlUpdateMetaEncoding(htmlMetaEncoding* menc, const char* encoding) {
	xmlChar *newVal, *p;
...
static xmlParserErrors htmlFindOutputEncoder(const char* encoding, xmlCharEncodingHandler** out) {
	/*
 ...
 */
static size_t
htmlBufNodeDumpFormat(xmlBufPtr buf, xmlDocPtr doc ATTRIBUTE_UNUSED, xmlNodePtr cur, int format) {
	size_t             use;
	size_t             ret;
 ...
 */
int htmlNodeDumpFileFormat(
	FILE* out, xmlDoc* doc ATTRIBUTE_UNUSED, xmlNode* cur, const char* encoding, int format
) {
 ...
 */
static void
htmlDtdDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr doc, const char* encoding ATTRIBUTE_UNUSED) {
	...
	} else if (cur->SystemID != NULL && xmlStrcmp(cur->SystemID, BAD_CAST "about:legacy-compat")) {
...
static void htmlSerializeUri(xmlOutputBufferPtr buf, const xmlChar* content) {
 ...
 */
static void htmlAttrDumpOutput(xmlOutputBufferPtr buf, xmlAttrPtr cur) {
	xmlOutputBufferWrite(buf, 1, " ");
	...
	if ((cur->children != NULL) && (!htmlIsBooleanAttr(cur->name))) {
		 ...
		 */
		isUri
			= (cur->ns == NULL) && (cur->parent != NULL) && (cur->parent->ns == NULL)
		 && ((!xmlStrcasecmp(cur->name, BAD_CAST "href"))
		     || (!xmlStrcasecmp(cur->name, BAD_CAST "action"))
		     || (!xmlStrcasecmp(cur->name, BAD_CAST "src"))
		     || ((!xmlStrcasecmp(cur->name, BAD_CAST "name")) && (!xmlStrcasecmp(cur->parent->name, BAD_CAST "a"))));
		...
		for (child = cur->children; child != NULL; child = child->next) {
			if (child->type == XML_TEXT_NODE) {
				...
				if (isUri) {
					...
					xmlSerializeText(buf, content, SIZE_MAX, XML_ESCAPE_HTML | XML_ESCAPE_ATTR);
				}
 ...
 * @param format  should formatting newlines been added
 */
void htmlNodeDumpInternal(xmlOutputBuffer* buf, xmlNode* cur, const char* encoding, int format) {
	...
	while (1) {
		switch (cur->type) {
			...
			case XML_ELEMENT_NODE: {
				...
				if (encoding != NULL) {
					 ...
					 */
					if ((xmlStrcasecmp(BAD_CAST encoding, BAD_CAST "HTML") != 0)
					    && (xmlStrcasecmp(cur->name, BAD_CAST "head") == 0) && (parent != NULL)
					    && (xmlStrcasecmp(parent->name, BAD_CAST "html") == 0) && (parent->parent != NULL)
					    && (parent->parent->parent == NULL) && (metaHead == NULL)) {
						xmlNodePtr n;
				...
				while (attr != NULL) {
					if ((!isMeta) || (attr != menc.attr)) {
						...
						xmlOutputBufferWrite(buf, 2, "=\"");
						xmlSerializeText(
							buf, menc.attrValue, menc.off.start, XML_ESCAPE_HTML | XML_ESCAPE_ATTR
						);
						xmlSerializeText(buf, BAD_CAST encoding, SIZE_MAX, XML_ESCAPE_HTML | XML_ESCAPE_ATTR);
						xmlSerializeText(
							buf,
							menc.attrValue + menc.off.end,
				...
				if ((info != NULL) && (info->empty)) {
					...
					if (addMeta) {
						...
						xmlSerializeText(buf, BAD_CAST encoding, SIZE_MAX, XML_ESCAPE_HTML | XML_ESCAPE_ATTR);
						xmlOutputBufferWrite(buf, 2, "\">");
						if ((format) && (cur->children->type != HTML_TEXT_NODE)
						    && (cur->children->type != HTML_ENTITY_REF_NODE)) {
							xmlOutputBufferWrite(buf, 1, "\n");
						}
					}
				...
				if ((format) && (cur->next != NULL) && (info != NULL) && (!info->isinline)) {
			...
			case HTML_TEXT_NODE:
				...
				if ((cur->name == (const xmlChar*)xmlStringTextNoenc) || (isRaw)) {
		...
		while (1) {
			...
			if ((cur->type == XML_HTML_DOCUMENT_NODE) || (cur->type == XML_DOCUMENT_NODE)) {
				...
				if ((format) && (info != NULL) && (!info->isinline) && (cur->last->type != HTML_TEXT_NODE)
				    && (cur->last->type != HTML_ENTITY_REF_NODE)
				    && ((cur->children != cur->last) || (cur == metaHead)) && (cur->name != NULL)
				    && (cur->name[0] != 'p')) { /* p, pre, param */
					xmlOutputBufferWrite(buf, 1, "\n");
				}
				...
				if ((format) && (info != NULL) && (!info->isinline) && (cur->next != NULL)) {
					if ((cur->next->type != HTML_TEXT_NODE) && (cur->next->type != HTML_ENTITY_REF_NODE)
					    && (parent != NULL) && (parent->name != NULL)
					    && (parent->name[0] != 'p')) { /* p, pre, param */
						xmlOutputBufferWrite(buf, 1, "\n");
...
void htmlDocContentDumpFormatOutput(
	xmlOutputBuffer* buf, xmlDoc* cur, const char* encoding ATTRIBUTE_UNUSED, int format
) {
...
void htmlDocContentDumpOutput(
	xmlOutputBuffer* buf, xmlDoc* cur, const char* encoding ATTRIBUTE_UNUSED
) {
