...
static void xmlTextReaderErr(xmlParserErrors code, const char* msg, ...) {
	va_list ap;
	int     res;
...
static void xmlTextReaderErrMemory(xmlTextReaderPtr reader) {
	if (reader == NULL) {
		xmlRaiseMemoryError(NULL, NULL, NULL, XML_FROM_PARSER, NULL);
		return;
	}
...
static xmlChar* readerStrdup(xmlTextReaderPtr reader, const xmlChar* string) {
	xmlChar* copy;
...
static const xmlChar* constString(xmlTextReaderPtr reader, const xmlChar* string) {
...
constQString(xmlTextReaderPtr reader, const xmlChar* prefix, const xmlChar* name) {
	const xmlChar* dictString;
...
static void xmlTextReaderFreeProp(xmlTextReaderPtr reader, xmlAttrPtr cur) {
	...
	if ((reader != NULL) && (reader->ctxt != NULL) && (reader->ctxt->freeAttrsNr < MAX_FREE_NODES)) {
 ...
 */
static void xmlTextReaderFreePropList(xmlTextReaderPtr reader, xmlAttrPtr cur) {
	xmlAttrPtr next;
...
static void xmlTextReaderFreeNodeList(xmlTextReaderPtr reader, xmlNodePtr cur) {
	...
	while (1) {
		while ((cur->type != XML_DTD_NODE) && (cur->type != XML_ENTITY_REF_NODE)
		       && (cur->children != NULL) && (cur->children->parent == cur)) {
			cur    = cur->children;
			depth += 1;
		}
		...
		if (cur->type != XML_DTD_NODE) {
			...
			if ((cur->content != (xmlChar*)&(cur->properties)) && (cur->type != XML_ELEMENT_NODE)
			    && (cur->type != XML_XINCLUDE_START) && (cur->type != XML_XINCLUDE_END)
			...
			if (((cur->type == XML_ELEMENT_NODE) || (cur->type == XML_TEXT_NODE)) && (reader != NULL)
			    && (reader->ctxt != NULL) && (reader->ctxt->freeElemsNr < MAX_FREE_NODES)) {
...
static void xmlTextReaderStructuredRelay(void* userData, const xmlError* error) {
	...
	if (reader->sErrorFunc != NULL) {
		...
		if ((error->domain == XML_FROM_VALID) || (error->domain == XML_FROM_DTD)) {
		...
		reader->errorFunc(reader->errorFuncArg, error->message, severity, reader->ctxt);
	}
}
...
static int xmlTextReaderEntPush(xmlTextReaderPtr reader, xmlNodePtr value) {
	if (reader->entNr >= reader->entMax) {
		...
		newSize = xmlGrowCapacity(reader->entMax, sizeof(tmp[0]), 10, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlTextReaderErrMemory(reader);
			return (-1);
		}
...
static void xmlTextReaderStartElement(void* ctx, const xmlChar* fullname, const xmlChar** atts) {
	...
	if ((reader != NULL) && (reader->startElement != NULL)) {
		...
		if ((ctxt->node != NULL) && (ctxt->input != NULL) && (ctxt->input->cur != NULL)
 ...
 *               attribute values.
 */
static void xmlTextReaderStartElementNs(
	void*           ctx,
 ...
 * @param URI  the element namespace name if available
 */
static void xmlTextReaderEndElementNs(
 ...
 */
static int xmlTextReaderPushData(xmlTextReaderPtr reader) {
	xmlBufPtr          inbuf;
	int                val, s;
	...
	while (reader->state == XML_TEXTREADER_NONE) {
		...
		if (xmlBufUse(inbuf) >= reader->cur + CHUNK_SIZE) {
			val = xmlParseChunk(
				reader->ctxt, (const char*)xmlBufContent(inbuf) + reader->cur, CHUNK_SIZE, 0
			);
 ...
 * @param reader  the xmlTextReader used
 */
static int xmlTextReaderValidatePush(xmlTextReaderPtr reader) {
	xmlNodePtr node = reader->node;

		#ifdef LIBXML_VALID_ENABLED
	if ((reader->validate == XML_TEXTREADER_VALIDATE_DTD) && (reader->ctxt != NULL)
	    && (reader->ctxt->validate == 1)) {
		if ((node->ns == NULL) || (node->ns->prefix == NULL)) {
			reader->ctxt->valid
				&= xmlValidatePushElement(&reader->ctxt->vctxt, reader->ctxt->myDoc, node, node->name);
		} else {
			xmlChar  buf[50];
			xmlChar* qname;
			...
			reader->ctxt->valid
				&= xmlValidatePushElement(&reader->ctxt->vctxt, reader->ctxt->myDoc, node, qname);
			if (qname != buf) {
				xmlFree(qname);
			}
		}
		...
		#ifdef LIBXML_RELAXNG_ENABLED
	if ((reader->validate == XML_TEXTREADER_VALIDATE_RNG) && (reader->rngValidCtxt != NULL)) {
		int ret;
		...
		if (ret == 0) {
			...
			if (node == NULL) {
				...
				ret = xmlRelaxNGValidateFullElement(reader->rngValidCtxt, reader->ctxt->myDoc, node);
 ...
 */
static void xmlTextReaderValidateCData(xmlTextReaderPtr reader, const xmlChar* data, int len) {
		#ifdef LIBXML_VALID_ENABLED
	if ((reader->validate == XML_TEXTREADER_VALIDATE_DTD) && (reader->ctxt != NULL)
	    && (reader->ctxt->validate == 1)) {
 ...
 * @param reader  the xmlTextReader used
 */
static int xmlTextReaderValidatePop(xmlTextReaderPtr reader) {
	xmlNodePtr node = reader->node;
	...
	if ((reader->validate == XML_TEXTREADER_VALIDATE_DTD) && (reader->ctxt != NULL)
		...
		if ((node->ns == NULL) || (node->ns->prefix == NULL)) {
			reader->ctxt->valid
				&= xmlValidatePopElement(&reader->ctxt->vctxt, reader->ctxt->myDoc, node, node->name);
		} else {
			xmlChar  buf[50];
			xmlChar* qname;
			...
			reader->ctxt->valid
				&= xmlValidatePopElement(&reader->ctxt->vctxt, reader->ctxt->myDoc, node, qname);
			if (qname != buf) {
				xmlFree(qname);
			}
		}
 ...
 * @param reader  the xmlTextReader used
 */
static int xmlTextReaderValidateEntity(xmlTextReaderPtr reader) {
	xmlNodePtr oldnode = reader->node;
	xmlNodePtr node    = reader->node;

	do {
		if (node->type == XML_ENTITY_REF_NODE) {
			if ((node->children != NULL) && (node->children->type == XML_ENTITY_DECL)
...
skip_children:
		...
		do {
			...
			if ((node->type == XML_ENTITY_DECL) && (reader->ent != NULL)
 ...
 */
static int xmlTextReaderDoExpand(xmlTextReaderPtr reader) {
	int val;
...
int xmlTextReaderRead(xmlTextReader* reader) {
	...
	if (reader->mode == XML_TEXTREADER_MODE_INITIAL) {
		...
		do {
			...
			&& ((reader->mode != XML_TEXTREADER_MODE_EOF) && (reader->state != XML_TEXTREADER_DONE))
		);
...
get_next_node:
	...
	if (oldstate != XML_TEXTREADER_BACKTRACK) {
		if ((reader->node->children != NULL) && (reader->node->type != XML_ENTITY_REF_NODE)
	...
	if (reader->node->next != NULL) {
		if (
			(oldstate == XML_TEXTREADER_ELEMENT) && (reader->node->type == XML_ELEMENT_NODE)
			&& (reader->node->children == NULL) && ((reader->node->extra & NODE_IS_EMPTY) == 0)
	#ifdef LIBXML_XINCLUDE_ENABLED
	...
	#endif /* LIBXML_REGEXP_ENABLED */
		if ((reader->preserves > 0) && (reader->node->extra & NODE_IS_SPRESERVED)) {
			reader->preserves--;
		}
		...
		if ((reader->preserves == 0) &&
		    ...
		    (reader->in_xinclude == 0) &&
	#endif
		    (reader->entNr == 0) && (reader->node->prev != NULL)
	...
	if ((oldstate == XML_TEXTREADER_ELEMENT) && (reader->node->type == XML_ELEMENT_NODE)
	    && (reader->node->children == NULL) && ((reader->node->extra & NODE_IS_EMPTY) == 0)) {
		;
		reader->state = XML_TEXTREADER_END;
		goto node_found;
	...
	if ((reader->node == NULL) || (reader->node->type == XML_DOCUMENT_NODE)
	    || (reader->node->type == XML_HTML_DOCUMENT_NODE)) {
		if (reader->mode != XML_TEXTREADER_MODE_EOF) {
		...
		if ((oldnode != NULL) && (reader->preserves == 0) &&
		    ...
		    (reader->in_xinclude == 0) &&
	#endif
		    (reader->entNr == 0) && (oldnode->type != XML_DTD_NODE)
	...
	if ((reader->preserves == 0) &&
	    ...
	    (reader->in_xinclude == 0) &&
	#endif
	    (reader->entNr == 0) && (reader->node->last != NULL)
...
node_found:
	 ...
	 */
	if ((reader->node != NULL) && (reader->node->next == NULL)
	...
	if ((reader->xinclude) && (reader->in_xinclude == 0)
	    && (reader->state != XML_TEXTREADER_BACKTRACK) && (reader->node != NULL)
	    && (reader->node->type == XML_ELEMENT_NODE) && (reader->node->ns != NULL)
	    && ((xmlStrEqual(reader->node->ns->href, XINCLUDE_NS)) || (xmlStrEqual(reader->node->ns->href, XINCLUDE_OLD_NS)))) {
	 ...
	 */
	if ((reader->node != NULL) && (reader->node->type == XML_ENTITY_REF_NODE)
	    && (reader->ctxt != NULL) && (reader->ctxt->replaceEntities == 1)) {
		if ((reader->node->children != NULL) && (reader->node->children->type == XML_ENTITY_DECL)
	...
	#ifdef LIBXML_REGEXP_ENABLED
	} else if ((reader->node != NULL) && (reader->node->type == XML_ENTITY_REF_NODE)
	...
	if ((reader->node != NULL) && (reader->node->type == XML_ENTITY_DECL) && (reader->ent != NULL)
...
static void
xmlTextReaderDumpCopy(xmlTextReaderPtr reader, xmlOutputBufferPtr output, xmlNodePtr node) {
	if ((node->type == XML_DTD_NODE) || (node->type == XML_ELEMENT_DECL)
	    || (node->type == XML_ATTRIBUTE_DECL) || (node->type == XML_ENTITY_DECL)) {
		return;
	}
...
xmlChar* xmlTextReaderReadString(xmlTextReader* reader) {
	...
	switch (node->type) {
		...
		case XML_ELEMENT_NODE:
			if ((xmlTextReaderDoExpand(reader) == -1) || (node->children == NULL)) {
				return (NULL);
			}
			break;
		default:
...
next_node:
	...
	if ((reader->state != XML_TEXTREADER_BACKTRACK) && (reader->node->type != XML_DTD_NODE)
...
found_node:
	if ((reader->node->type == XML_XINCLUDE_START) || (reader->node->type == XML_XINCLUDE_END)) {
		goto next_node;
	}
...
xmlTextReader* xmlNewTextReader(xmlParserInputBuffer* input, const char* URI) {
	...
	if (xmlBufUse(ret->input->buffer) >= 4) {
		ret->ctxt = xmlCreatePushParserCtxt(
			ret->sax, NULL, (const char*)xmlBufContent(ret->input->buffer), 4, URI
		);
		ret->base = 0;
		ret->cur  = 4;
	} else {
...
xmlTextReader* xmlNewTextReaderFilename(const char* URI) {
	...
	if (xmlParserInputBufferCreateFilenameValue != NULL) {
		input = xmlParserInputBufferCreateFilenameValue(URI, XML_CHAR_ENCODING_NONE);
		if (input == NULL) {
		 ...
		 */
		code = xmlParserInputBufferCreateUrl(URI, XML_CHAR_ENCODING_NONE, XML_INPUT_UNZIP, &input);
		if (code != XML_ERR_OK) {
...
int xmlTextReaderClose(xmlTextReader* reader) {
	...
	if (reader->ctxt != NULL) {
	#ifdef LIBXML_VALID_ENABLED
		if ((reader->ctxt->vctxt.vstateTab != NULL) && (reader->ctxt->vctxt.vstateMax > 0)) {
		#ifdef LIBXML_REGEXP_ENABLED
...
xmlChar* xmlTextReaderGetAttribute(xmlTextReader* reader, const xmlChar* name) {
	...
	if (xmlStrEqual(prefix, BAD_CAST "xmlns")) {
		...
		if (ns != NULL) {
			result = xmlNodeGetAttrValue(reader->node, localname, ns->href, &ret);
			if (result < 0) {
				xmlTextReaderErrMemory(reader);
			}
		}
...
xmlChar* xmlTextReaderGetAttributeNs(
	xmlTextReader* reader, const xmlChar* localName, const xmlChar* namespaceURI
) {
	xmlChar* ret    = NULL;
	xmlChar* prefix = NULL;
	xmlNsPtr ns;
...
int xmlTextReaderMoveToAttribute(xmlTextReader* reader, const xmlChar* name) {
	...
	while (prop != NULL) {
		 ...
		 *   - and the attribute carrying that namespace
		 */
		if ((xmlStrEqual(prop->name, localname)) && (prop->ns != NULL)
...
int xmlTextReaderMoveToAttributeNs(
	...
	while (prop != NULL) {
		...
		if (xmlStrEqual(prop->name, localName)
		    && ((prop->ns != NULL) && (xmlStrEqual(prop->ns->href, namespaceURI)))) {
			reader->curnode = (xmlNodePtr)prop;
			return (1);
		}
		prop = prop->next;
...
int xmlTextReaderMoveToNextAttribute(xmlTextReader* reader) {
	...
	if (reader->curnode->type == XML_NAMESPACE_DECL) {
		...
		return (0);
	} else if ((reader->curnode->type == XML_ATTRIBUTE_NODE) && (reader->curnode->next != NULL)) {
	...
	if (reader->curnode->type == XML_ATTRIBUTE_NODE) {
		...
		if (reader->faketext == NULL) {
			...
			if ((reader->faketext->content != NULL)
			    && (reader->faketext->content != (xmlChar*)&(reader->faketext->properties))) {
...
int xmlTextReaderHasAttributes(xmlTextReader* reader) {
	...
	if ((node->type == XML_ELEMENT_NODE) && ((node->properties != NULL) || (node->nsDef != NULL))) {
		return (1);
	}
	/* TODO: handle the xmlDecl */
	return (0);
...
int xmlTextReaderPreservePattern(
	xmlTextReader* reader, const xmlChar* pattern, const xmlChar** namespaces
) {
	xmlPatternPtr comp;
	...
	if (reader->patternNr >= reader->patternMax) {
		...
		newSize = xmlGrowCapacity(reader->patternMax, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlTextReaderErrMemory(reader);
			return (-1);
		}
...
int xmlTextReaderSetSchema(xmlTextReader* reader, xmlSchema* schema) {
	...
	reader->xsdPlug
		= xmlSchemaSAXPlug(reader->xsdValidCtxt, &(reader->ctxt->sax), &(reader->ctxt->userData));
	if (reader->xsdPlug == NULL) {
		xmlSchemaFree(reader->xsdSchemas);
		reader->xsdSchemas = NULL;
		xmlSchemaFreeValidCtxt(reader->xsdValidCtxt);
 ...
 *	   -1 in case of error.
 */
static int xmlTextReaderRelaxNGValidateInternal(
	xmlTextReaderPtr       reader,
	const char*            rng,
	...
	if (((rng != NULL) || (ctxt != NULL))
	    && ((reader->mode != XML_TEXTREADER_MODE_INITIAL) || (reader->ctxt == NULL))) {
		return (-1);
	}
	...
	if (rng != NULL) {
		...
		if ((reader->errorFunc != NULL) || (reader->sErrorFunc != NULL)) {
			xmlRelaxNGSetParserStructuredErrors(pctxt, xmlTextReaderStructuredRelay, reader);
		}
 ...
 *         -1 in case of error.
 */
static int xmlTextReaderSchemaValidateInternal(
	xmlTextReaderPtr      reader,
	const char*           xsd,
	...
	if (xsd != NULL) {
		...
		if ((reader->errorFunc != NULL) || (reader->sErrorFunc != NULL)) {
			xmlSchemaSetParserStructuredErrors(pctxt, xmlTextReaderStructuredRelay, reader);
		}
 ...
 */
int
xmlTextReaderRelaxNGValidateCtxt(xmlTextReader* reader, xmlRelaxNGValidCtxt* ctxt, int options) {
...
void xmlTextReaderSetErrorHandler(xmlTextReader* reader, xmlTextReaderErrorFunc f, void* arg) {
	...
	if (f != NULL) {
		...
		if (reader->rngValidCtxt) {
			xmlRelaxNGSetValidStructuredErrors(
				reader->rngValidCtxt, xmlTextReaderStructuredRelay, reader
			);
		}
	#endif
	#ifdef LIBXML_SCHEMAS_ENABLED
	...
	#ifdef LIBXML_RELAXNG_ENABLED
		if (reader->rngValidCtxt) {
			xmlRelaxNGSetValidStructuredErrors(reader->rngValidCtxt, NULL, NULL);
		}
	#endif
	#ifdef LIBXML_SCHEMAS_ENABLED
		if (reader->xsdValidCtxt) {
			xmlSchemaSetValidStructuredErrors(reader->xsdValidCtxt, NULL, NULL);
		}
	#endif
	}
}
...
void xmlTextReaderSetStructuredErrorHandler(
	xmlTextReader* reader, xmlStructuredErrorFunc f, void* arg
) {
	if (reader == NULL) {
		return;
	}
...
int xmlTextReaderSetup(
	...
	if ((input != NULL) && (reader->input != NULL) && (reader->allocs & XML_TEXTREADER_INPUT)) {
		xmlFreeParserInputBuffer(reader->input);
		reader->input   = NULL;
		reader->allocs -= XML_TEXTREADER_INPUT;
	}
		...
		if (reader->ctxt == NULL) {
			if (xmlBufUse(reader->input->buffer) >= 4) {
				reader->ctxt = xmlCreatePushParserCtxt(
					reader->sax, NULL, (const char*)xmlBufContent(reader->input->buffer), 4, URL
				);
				reader->base = 0;
				reader->cur  = 4;
			} else {
 ...
 * @returns the new reader or NULL in case of error.
 */
xmlTextReader* xmlReaderForMemory(
 ...
 */
int xmlReaderNewDoc(
	xmlTextReader* reader, const xmlChar* cur, const char* URL, const char* encoding, int options
) {
	int len;
 ...
 */
int
xmlReaderNewFile(xmlTextReader* reader, const char* filename, const char* encoding, int options) {
	xmlParserInputBufferPtr input;
	...
	if (xmlParserInputBufferCreateFilenameValue != NULL) {
		input = xmlParserInputBufferCreateFilenameValue(filename, XML_CHAR_ENCODING_NONE);
		if (input == NULL) {
		...
		code = xmlParserInputBufferCreateUrl(filename, XML_CHAR_ENCODING_NONE, flags, &input);
		if (code != XML_ERR_OK) {
 ...
 */
int
xmlReaderNewFd(xmlTextReader* reader, int fd, const char* URL, const char* encoding, int options) {
	xmlParserInputBufferPtr input;
	xmlParserErrors         code;
