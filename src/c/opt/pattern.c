   ...
   */
	...
	#define XML_PAT_COPY_NSNAME(c, r, nsname)                              \
		if ((c)->comp->dict)                                                 \
			...
			r = xmlStrdup(BAD_CAST nsname);

	#define XML_PAT_FREE_STRING(c, r) \
		if ((c)->comp->dict == NULL)    \
			xmlFree(r);
 ...
 */
static xmlPatternPtr xmlNewPattern(void) {
	xmlPatternPtr cur;
...
static void xmlFreePatternInternal(xmlPatternPtr comp) {
	xmlStepOpPtr op;
	int          i;
...
xmlNewPatParserContext(const xmlChar* pattern, xmlDictPtr dict, const xmlChar** namespaces) {
...
static int xmlPatternGrow(xmlPatternPtr comp) {
	xmlStepOpPtr temp;
	int          newSize;

	newSize = xmlGrowCapacity(comp->maxStep, sizeof(temp[0]), 10, XML_MAX_ITEMS);
	if (newSize < 0) {
		return (-1);
	}
 ...
 */
static int xmlPatternAdd(
	xmlPatParserContextPtr ctxt, xmlPatternPtr comp, xmlPatOp op, xmlChar* value, xmlChar* value2
) {
...
static int xmlPatPushState(xmlStepStates* states, int step, xmlNodePtr node) {
	if (states->maxstates <= states->nbstates) {
		...
		newSize = xmlGrowCapacity(states->maxstates, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			return (-1);
		}
	...
	for (; i < comp->nbStep; i++) {
		...
		switch (step->op) {
			...
			case XML_OP_CHILD: {
				...
				if (step->value != NULL) {
					while (lst != NULL) {
						if ((lst->type == XML_ELEMENT_NODE) && (step->value[0] == lst->name[0])
			...
			case XML_OP_ANCESTOR:
				...
				node = node->parent;
				while (node != NULL) {
					if ((node->type == XML_ELEMENT_NODE) && (step->value[0] == node->name[0])
 ...
 ************************************************************************/
	...
	#define PUSH(op, val, val2)                                 \
		if (xmlPatternAdd(ctxt, ctxt->comp, (op), (val), (val2))) \
			goto error;
	...
	if (CUR == ':') {
		...
		if ((prefix[0] == 'x') && (prefix[1] == 'm') && (prefix[2] == 'l') && (prefix[3] == 0)) {
			...
			if (i >= ctxt->nb_namespaces) {
				ERROR5(
					NULL, NULL, NULL, "xmlCompileAttributeTest : no namespace bound to prefix %s\n", prefix
				);
				ctxt->error = 1;
				goto error;
			}
	...
	if (CUR == ':') {
		...
		if (CUR != ':') {
			...
			if ((prefix[0] == 'x') && (prefix[1] == 'm') && (prefix[2] == 'l') && (prefix[3] == 0)) {
				...
				if (i >= ctxt->nb_namespaces) {
					ERROR5(
						NULL, NULL, NULL, "xmlCompileStepPattern : no namespace bound to prefix %s\n", prefix
					);
					ctxt->error = 1;
					goto error;
				}
 ...
 */
static int xmlStreamCompAddStep(
	xmlStreamCompPtr comp, const xmlChar* name, const xmlChar* ns, int nodeType, int flags
) {
	...
	if (comp->nbStep >= comp->maxStep) {
		...
		newSize = xmlGrowCapacity(comp->maxStep, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			ERROR(NULL, NULL, NULL, "xmlNewStreamComp: growCapacity failed\n");
			return (-1);
		}
...
static int xmlStreamCompile(xmlPatternPtr comp) {
	 ...
	 * special case for .
	 */
	if ((comp->nbStep == 1) && (comp->steps[0].op == XML_OP_ELEM) && (comp->steps[0].value == NULL)
...
static int xmlStreamCtxtAddState(xmlStreamCtxtPtr comp, int idx, int level) {
	...
	if (comp->nbState >= comp->maxState) {
		...
		newSize = xmlGrowCapacity(comp->maxState, sizeof(tmp[0]) * 2, 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			ERROR(NULL, NULL, NULL, "xmlNewStreamCtxt: growCapacity failed\n");
			return (-1);
		}
 ...
 */
static int xmlStreamPushInternal(
	xmlStreamCtxtPtr stream, const xmlChar* name, const xmlChar* ns, int nodeType
) {
	...
	while (stream != NULL) {
		...
		if ((nodeType == XML_ELEMENT_NODE) && (name == NULL) && (ns == NULL)) {
			...
			if (comp->flags & XML_STREAM_FROM_ROOT) {
				if (comp->nbStep == 0) {
					...
					if ((comp->nbStep == 1) && (comp->steps[0].nodeType == XML_STREAM_ANY_NODE)
		...
		while (i < m) {
			if ((comp->flags & XML_STREAM_DESC) == 0) {
				 ...
				 */
				if (stream->states[(2 * (stream->nbState - 1)) + 1] < stream->level) {
					return (-1);
				}
				desc = 0;
				/* loop-stopper */
			...
			if (match) {
				...
				if (final) {
					ret = 1;
				} else if (xmlStreamCtxtAddState(stream, stepNr + 1, stream->level + 1) < 0) {
					return (-1);
				}
 ...
 */
int
xmlStreamPushNode(xmlStreamCtxt* stream, const xmlChar* name, const xmlChar* ns, int nodeType) {
 ...
 */
xmlPattern*
xmlPatterncompile(const xmlChar* pattern, xmlDict* dict, int flags, const xmlChar** namespaces) {
	xmlPatternPtr ret;
