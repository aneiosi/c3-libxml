...
static const xmlChar* xmlOldSchematronNs = SCT_OLD_NS;
	...
	#define IS_SCHEMATRON(node, elem)                                                                            \
		(                                                                                                          \
			(node != NULL) && (node->type == XML_ELEMENT_NODE) && (node->ns != NULL)                                 \
			&& (xmlStrEqual(node->name, (const xmlChar*)elem))                                                       \
			&& ((xmlStrEqual(node->ns->href, xmlSchematronNs)) || (xmlStrEqual(node->ns->href, xmlOldSchematronNs))) \
		)
...
typedef enum { XML_SCHEMATRON_ASSERT = 1, XML_SCHEMATRON_REPORT = 2 } xmlSchematronTestType;
 ...
 */
static void xmlSchematronPErrMemory(xmlSchematronParserCtxtPtr ctxt) {
	if (ctxt != NULL) {
		ctxt->nberrors++;
	}
 ...
 */
static void LIBXML_ATTR_FORMAT(4, 0) xmlSchematronPErr(
 ...
 */
static void xmlSchematronVErrMemory(xmlSchematronValidCtxtPtr ctxt) {
	if (ctxt != NULL) {
		ctxt->nberrors++;
 ...
 * @param str1  extra data
 */
static void LIBXML_ATTR_FORMAT(3, 0) xmlSchematronVErr(
	xmlSchematronValidCtxtPtr ctxt, int error, const char* msg, const xmlChar* str1
) {
	xmlGenericErrorFunc    channel  = NULL;
	xmlStructuredErrorFunc schannel = NULL;
	void*                  data     = NULL;
 ...
 */
static xmlSchematronTestPtr xmlSchematronAddTest(
	xmlSchematronParserCtxtPtr ctxt,
	...
	if (comp == NULL) {
		xmlSchematronPErr(
			ctxt, node, XML_SCHEMAP_NOROOT, "Failed to compile test expression %s", test, NULL
		);
		return (NULL);
	}
 ...
 */
static void xmlSchematronFreeLets(xmlSchematronLetPtr lets) {
 ...
 * @returns the new pointer or NULL in case of error
 */
static xmlSchematronRulePtr xmlSchematronAddRule(
	xmlSchematronParserCtxtPtr ctxt,
	xmlSchematronPtr           schema,
	...
	if ((ctxt == NULL) || (schema == NULL) || (node == NULL) || (context == NULL)) {
		return (NULL);
	}
	...
	if (pattern == NULL) {
		xmlSchematronPErr(
			ctxt, node, XML_SCHEMAP_NOROOT, "Failed to compile context expression %s", context, NULL
		);
	}
 ...
 * @returns the new pointer or NULL in case of error
 */
static xmlSchematronPatternPtr xmlSchematronAddPattern(
	xmlSchematronParserCtxtPtr ctxt, xmlSchematronPtr schema, xmlNodePtr node, xmlChar* name
) {
...
xmlSchematronParserCtxt* xmlSchematronNewParserCtxt(const char* URL) {
	...
	ret = (xmlSchematronParserCtxtPtr)xmlMalloc(sizeof(xmlSchematronParserCtxt));
	if (ret == NULL) {
 ...
 */
static void xmlSchematronAddNamespace(
	xmlSchematronParserCtxtPtr ctxt, const xmlChar* prefix, const xmlChar* ns
) {
	if (ctxt->namespaces == NULL) {
		ctxt->maxNamespaces = 10;
		ctxt->namespaces
			= (const xmlChar**)xmlMalloc(ctxt->maxNamespaces * 2 * sizeof(const xmlChar*));
		if (ctxt->namespaces == NULL) {
			xmlSchematronPErrMemory(NULL);
			return;
		}
		...
		tmp = (const xmlChar**)xmlRealloc(
			(xmlChar**)ctxt->namespaces, ctxt->maxNamespaces * 4 * sizeof(const xmlChar*)
		);
		if (tmp == NULL) {
			xmlSchematronPErrMemory(NULL);
			return;
 ...
 */
static void xmlSchematronParseTestReportMsg(xmlSchematronParserCtxtPtr ctxt, xmlNodePtr con) {
	xmlNodePtr          child;
	xmlXPathCompExprPtr comp;
	...
	while (child != NULL) {
		...
		/* Do Nothing */
		{
			...
			if (select == NULL) {
				xmlSchematronPErr(
					ctxt, child, XML_SCHEMAV_ATTRINVALID, "value-of has no select attribute", NULL, NULL
				);
			} else {
				/*
				 * try first to compile the test expression
 ...
 */
static void xmlSchematronParseRule(
	xmlSchematronParserCtxtPtr ctxt, xmlSchematronPatternPtr pattern, xmlNodePtr rule
) {
	xmlNodePtr           cur;
	int                  nbChecks = 0;
	xmlChar*             test;
	...
	if (context == NULL) {
		...
		xmlSchematronPErr(
			ctxt, rule, XML_SCHEMAP_NOROOT, "rule has an empty context attribute", NULL, NULL
		);
		xmlFree(context);
		return;
	} else {
	...
	while (cur != NULL) {
		if (IS_SCHEMATRON(cur, "let")) {
			...
			if (name == NULL) {
				xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT, "let has no name attribute", NULL, NULL);
				return;
			} else if (name[0] == 0) {
				xmlSchematronPErr(
					ctxt, cur, XML_SCHEMAP_NOROOT, "let has an empty name attribute", NULL, NULL
				);
				xmlFree(name);
				return;
			}
			...
			if (value == NULL) {
				xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT, "let has no value attribute", NULL, NULL);
				return;
			} else if (value[0] == 0) {
				xmlSchematronPErr(
					ctxt, cur, XML_SCHEMAP_NOROOT, "let has an empty value attribute", NULL, NULL
				);
				xmlFree(value);
				return;
			}
			...
			if (var_comp == NULL) {
				xmlSchematronPErr(
					ctxt, cur, XML_SCHEMAP_NOROOT, "Failed to compile let expression %s", value, NULL
				);
				return;
			}
			...
			if (test == NULL) {
				xmlSchematronPErr(
					ctxt, cur, XML_SCHEMAP_NOROOT, "assert has no test attribute", NULL, NULL
				);
			} else if (test[0] == 0) {
				xmlSchematronPErr(
					ctxt, cur, XML_SCHEMAP_NOROOT, "assert has an empty test attribute", NULL, NULL
				);
				xmlFree(test);
			} else {
	...
	if (nbChecks == 0) {
		xmlSchematronPErr(
			ctxt, rule, XML_SCHEMAP_NOROOT, "rule has no assert nor report element", NULL, NULL
		);
	}
}
...
static void xmlSchematronParsePattern(xmlSchematronParserCtxtPtr ctxt, xmlNodePtr pat) {
	...
	while (cur != NULL) {
		if (IS_SCHEMATRON(cur, "rule")) {
			...
			xmlSchematronPErr(
				ctxt, cur, XML_SCHEMAP_NOROOT, "Expecting a rule element instead of %s", cur->name, NULL
			);
		}
		cur = cur->next;
		NEXT_SCHEMATRON(cur);
...
xmlSchematron* xmlSchematronParse(xmlSchematronParserCtxt* ctxt) {
	...
	if (ctxt->URL != NULL) {
		...
		if (doc == NULL) {
			xmlSchematronPErr(
				ctxt, NULL, XML_SCHEMAP_FAILED_PARSE, "xmlSchematronParse: could not parse.\n", NULL, NULL
			);
			return (NULL);
		}
	...
	while (IS_SCHEMATRON(cur, "ns")) {
		...
		if ((uri == NULL) || (uri[0] == 0)) {
			xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT, "ns element has no uri", NULL, NULL);
		}
		if ((prefix == NULL) || (prefix[0] == 0)) {
			xmlSchematronPErr(ctxt, cur, XML_SCHEMAP_NOROOT, "ns element has no prefix", NULL, NULL);
		}
		if ((prefix) && (uri)) {
	...
	while (cur != NULL) {
		if (IS_SCHEMATRON(cur, "pattern")) {
			...
			xmlSchematronPErr(
				ctxt, cur, XML_SCHEMAP_NOROOT, "Expecting a pattern element instead of %s", cur->name, NULL
			);
		}
		cur = cur->next;
		NEXT_SCHEMATRON(cur);
 ...
 */
static void xmlSchematronReportOutput(
	xmlSchematronValidCtxtPtr ctxt ATTRIBUTE_UNUSED, xmlNodePtr cur ATTRIBUTE_UNUSED, const char* msg
) {
	/* TODO */
 ...
 */
static xmlChar*
xmlSchematronFormatReport(xmlSchematronValidCtxtPtr ctxt, xmlNodePtr test, xmlNodePtr cur) {
	xmlChar*            ret = NULL;
		...
		if ((child->type == XML_TEXT_NODE) || (child->type == XML_CDATA_SECTION_NODE)) {
			...
			switch (eval->type) {
				...
				case XPATH_STRING:
					...
					break;
				default:
					xmlSchematronVErr(ctxt, XML_ERR_INTERNAL_ERROR, "Unsupported XPATH Type\n", NULL);
			}
 ...
 */
static void xmlSchematronReportSuccess(
	...
	if ((ctxt->flags & XML_SCHEMATRON_OUT_QUIET) && ((ctxt->flags & XML_SCHEMATRON_OUT_XML) == 0)
 ...
 */
static void
xmlSchematronReportPattern(xmlSchematronValidCtxtPtr ctxt, xmlSchematronPatternPtr pattern) {
	if ((ctxt == NULL) || (pattern == NULL)) {
		return;
	}
	if ((ctxt->flags & XML_SCHEMATRON_OUT_QUIET)
	    || (ctxt->flags & XML_SCHEMATRON_OUT_ERROR)) { /* Error gives pattern name as part of error
		                                                  */
		return;
	}
	if (ctxt->flags & XML_SCHEMATRON_OUT_XML) {
...
void xmlSchematronSetValidStructuredErrors(
	xmlSchematronValidCtxt* ctxt, xmlStructuredErrorFunc serror, void* ctx
) {
	if (ctxt == NULL) {
		return;
	}
 ...
 */
static int xmlSchematronRunTest(
	xmlSchematronValidCtxtPtr ctxt,
	xmlSchematronTestPtr      test,
	xmlDocPtr                 instance,
	xmlNodePtr                cur,
 ...
 */
static int xmlSchematronRegisterVariables(
	...
	while (let != NULL) {
		...
		if (let_eval == NULL) {
			xmlSchematronVErr(
				vctxt, XML_ERR_INTERNAL_ERROR, "Evaluation of compiled expression failed\n", NULL
			);
			return -1;
		}
		if (xmlXPathRegisterVariableNS(ctxt, let->name, NULL, let_eval)) {
			xmlSchematronVErr(
				vctxt, XML_ERR_INTERNAL_ERROR, "Registering a let variable failed\n", NULL
			);
			return -1;
		}
		let = let->next;
 ...
 * @returns -1 in case of errors, otherwise 0
 */
static int xmlSchematronUnregisterVariables(
	xmlSchematronValidCtxtPtr vctxt, xmlXPathContextPtr ctxt, xmlSchematronLetPtr let
) {
	while (let != NULL) {
		if (xmlXPathRegisterVariableNS(ctxt, let->name, NULL, NULL)) {
			xmlSchematronVErr(
				vctxt, XML_ERR_INTERNAL_ERROR, "Unregistering a let variable failed\n", NULL
			);
			return -1;
		}
		let = let->next;
...
int xmlSchematronValidateDoc(xmlSchematronValidCtxt* ctxt, xmlDoc* instance) {
	...
	if ((ctxt == NULL) || (ctxt->schema == NULL) || (ctxt->schema->rules == NULL)
	    || (instance == NULL)) {
		return (-1);
	}
	ctxt->nberrors = 0;
		...
		while (cur != NULL) {
			...
			while (rule != NULL) {
				if (xmlPatternMatch(rule->pattern, cur) == 1) {
					...
					if (xmlSchematronRegisterVariables(ctxt, ctxt->xctxt, rule->lets, instance, cur)) {
						return -1;
					}

					while (test != NULL) {
						xmlSchematronRunTest(
							ctxt, test, instance, cur, (xmlSchematronPatternPtr)rule->pattern
						);
						test = test->next;
					}

					if (xmlSchematronUnregisterVariables(ctxt, ctxt->xctxt, rule->lets)) {
						return -1;
					}
				}
				rule = rule->next;
		...
		while (pattern != NULL) {
			...
			while (cur != NULL) {
				...
				while (rule != NULL) {
					if (xmlPatternMatch(rule->pattern, cur) == 1) {
						test = rule->tests;
						xmlSchematronRegisterVariables(ctxt, ctxt->xctxt, rule->lets, instance, cur);
						...
						xmlSchematronUnregisterVariables(ctxt, ctxt->xctxt, rule->lets);
					}
					rule = rule->patnext;
				}
