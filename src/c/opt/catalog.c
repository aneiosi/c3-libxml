 ...
 */
static void             xmlCatalogErrMemory(void) {
 ...
 */
static void LIBXML_ATTR_FORMAT(4, 0) xmlCatalogErr(
...
static void xmlCatalogPrintDebug(const char* fmt, ...) {
	va_list ap;
 ...
 */
static xmlCatalogEntryPtr xmlNewCatalogEntry(
	xmlCatalogEntryType type,
	const xmlChar*      name,
	const xmlChar*      value,
	const xmlChar*      URL,
 ...
 * @param name  unused
 */
static void xmlFreeCatalogEntry(void* payload, const xmlChar* name ATTRIBUTE_UNUSED) {
 ...
 * @param name  unused
 */
static void xmlFreeCatalogHashEntryList(void* payload, const xmlChar* name ATTRIBUTE_UNUSED) {
 ...
 * @param name  unused
 */
static void xmlCatalogDumpEntry(void* payload, void* data, const xmlChar* name ATTRIBUTE_UNUSED) {
	...
	switch (entry->type) {
		...
		case SGML_CATA_LINKTYPE:
		case SGML_CATA_NOTATION:
			fprintf(out, "%s", (const char*)entry->name);
			break;
		case SGML_CATA_PUBLIC:
		case SGML_CATA_SYSTEM:
		case SGML_CATA_SGMLDECL:
 ...
 */
static void xmlDumpXMLCatalogNode(
	xmlCatalogEntryPtr catal,
	xmlNodePtr         catalog,
	xmlDocPtr          doc,
	xmlNsPtr           ns,
 ...
 */
static void
xmlCatalogConvertEntry(void* payload, void* data, const xmlChar* name ATTRIBUTE_UNUSED) {
	xmlCatalogEntryPtr entry = (xmlCatalogEntryPtr)payload;
	xmlCatalogPtr      catal = (xmlCatalogPtr)data;
	if ((entry == NULL) || (catal == NULL) || (catal->sgml == NULL) || (catal->xml == NULL)) {
		return;
	}
	switch (entry->type) {
		case SGML_CATA_ENTITY:
...
static void               xmlParseXMLCatalogNodeList(
								xmlNodePtr cur, xmlCatalogPrefer prefer, xmlCatalogEntryPtr parent, xmlCatalogEntryPtr cgroup
							);
static xmlChar*
xmlCatalogListXMLResolve(xmlCatalogEntryPtr catal, const xmlChar* pubID, const xmlChar* sysID);
 ...
 */
static xmlCatalogEntryPtr xmlParseXMLCatalogOneNode(
	xmlNodePtr          cur,
	xmlCatalogEntryType type,
	const xmlChar*      name,
	...
	if (attrName != NULL) {
		...
		if (nameValue == NULL) {
			xmlCatalogErr(
				ret, cur, XML_CATALOG_MISSING_ATTR, "%s entry lacks '%s'\n", name, attrName, NULL
			);
			ok = 0;
		}
	}
 ...
 * @param cgroup  the group which includes this node
 */
static void xmlParseXMLCatalogNode(
	xmlNodePtr cur, xmlCatalogPrefer prefer, xmlCatalogEntryPtr parent, xmlCatalogEntryPtr cgroup
) {
	xmlChar*           base  = NULL;
	...
	if (xmlStrEqual(cur->name, BAD_CAST "group")) {
		...
		entry = xmlParseXMLCatalogOneNode(
			cur, XML_CATA_PUBLIC, BAD_CAST "public", BAD_CAST "publicId", BAD_CAST "uri", prefer, cgroup
		);
	} else if (xmlStrEqual(cur->name, BAD_CAST "system")) {
		entry = xmlParseXMLCatalogOneNode(
			cur, XML_CATA_SYSTEM, BAD_CAST "system", BAD_CAST "systemId", BAD_CAST "uri", prefer, cgroup
		);
		...
		entry = xmlParseXMLCatalogOneNode(
			cur, XML_CATA_URI, BAD_CAST "uri", BAD_CAST "name", BAD_CAST "uri", prefer, cgroup
		);
		...
		entry = xmlParseXMLCatalogOneNode(
			cur, XML_CATA_NEXT_CATALOG, BAD_CAST "nextCatalog", NULL, BAD_CAST "catalog", prefer, cgroup
		);
	}
	if (entry != NULL) {
		if (parent != NULL) {
...
xmlParseXMLCatalogFile(xmlCatalogPrefer prefer, const xmlChar* filename) {
	...
	if (xmlDebugCatalogs) {
		xmlCatalogPrintDebug("Parsing catalog %s\n", filename);
	}

	cur = xmlDocGetRootElement(doc);
	if ((cur != NULL) && (xmlStrEqual(cur->name, BAD_CAST "catalog")) && (cur->ns != NULL)
	    && (cur->ns->href != NULL) && (xmlStrEqual(cur->ns->href, XML_CATALOGS_NAMESPACE))) {
		parent
			= xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, (const xmlChar*)filename, NULL, prefer, NULL);
		if (parent == NULL) {
			xmlFreeDoc(doc);
			return (NULL);
		}
		...
		if (prop != NULL) {
			if (xmlStrEqual(prop, BAD_CAST "system")) {
				...
				xmlCatalogErr(
					NULL, cur, XML_CATALOG_PREFER_VALUE, "Invalid value for prefer: '%s'\n", prop, NULL, NULL
				);
			}
			xmlFree(prop);
		}
 ...
 */
static int xmlAddXMLCatalog(
	xmlCatalogEntryPtr catal, const xmlChar* type, const xmlChar* orig, const xmlChar* replace
) {
	xmlCatalogEntryPtr  cur;
	...
	if ((catal == NULL)
	    || ((catal->type != XML_CATA_CATALOG) && (catal->type != XML_CATA_BROKEN_CATALOG))) {
		return (-1);
	}
	if (catal->children == NULL) {
	...
	if (typ == XML_CATA_NONE) {
		if (xmlDebugCatalogs) {
			xmlCatalogPrintDebug("Failed to add unknown element %s to catalog\n", type);
		}
		return (-1);
	}
	...
	if (cur != NULL) {
		while (cur != NULL) {
			if ((orig != NULL) && (cur->type == typ) && (xmlStrEqual(orig, cur->name))) {
				if (xmlDebugCatalogs) {
	...
	if (cur == NULL) {
		catal->children = xmlNewCatalogEntry(typ, orig, replace, NULL, catal->prefer, NULL);
	} else {
		cur->next = xmlNewCatalogEntry(typ, orig, replace, NULL, catal->prefer, NULL);
	}
	if (doregister) {
 ...
 */
static xmlChar*
xmlCatalogXMLResolve(xmlCatalogEntryPtr catal, const xmlChar* pubID, const xmlChar* sysID) {
	xmlChar*           ret = NULL;
	xmlCatalogEntryPtr cur;
	int                haveDelegate = 0;
	int                haveNext     = 0;
	...
	if (pubID != NULL) {
		...
		if (haveDelegate) {
			...
			cur                   = catal;
			while (cur != NULL) {
				if ((cur->type == XML_CATA_DELEGATE_PUBLIC) && (cur->prefer == XML_CATA_PREFER_PUBLIC)
					...
					if (cur->children != NULL) {
						...
						ret = xmlCatalogListXMLResolve(cur->children, pubID, NULL);
						if (ret != NULL) {
							catal->depth--;
							return (ret);
						}
...
static xmlChar* xmlCatalogXMLResolveURI(xmlCatalogEntryPtr catal, const xmlChar* URI) {
	...
	if (haveDelegate) {
		...
		cur                   = catal;
		while (cur != NULL) {
			if (((cur->type == XML_CATA_DELEGATE_SYSTEM) || (cur->type == XML_CATA_DELEGATE_URI))
...
xmlCatalogListXMLResolve(xmlCatalogEntryPtr catal, const xmlChar* pubID, const xmlChar* sysID) {
	...
	if (!xmlStrncmp(sysID, BAD_CAST XML_URN_PUBID, sizeof(XML_URN_PUBID) - 1)) {
		...
		if (xmlDebugCatalogs) {
			if (urnID == NULL) {
				xmlCatalogPrintDebug("System URN ID %s expanded to NULL\n", sysID);
			} else {
				xmlCatalogPrintDebug("System URN ID expanded to %s\n", urnID);
			}
		}
		if (pubID == NULL) {
 ...
 */
static int
xmlParseSGMLCatalog(xmlCatalogPtr catal, const xmlChar* value, const char* file, int super) {
	...
	while ((cur != NULL) && (cur[0] != 0)) {
		...
		if ((cur[0] == '-') && (cur[1] == '-')) {
			...
			if (cur == NULL) {
			...
			} else if ((type == SGML_CATA_PUBLIC) || (type == SGML_CATA_SYSTEM)) {
				xmlChar* filename;
				...
				if (filename != NULL) {
					...
					entry = xmlNewCatalogEntry(type, name, filename, NULL, XML_CATA_PREFER_NONE, NULL);
				...
				if (super) {
					...
					entry = xmlNewCatalogEntry(type, sysid, NULL, NULL, XML_CATA_PREFER_NONE, NULL);
...
xmlCatalogSGMLResolve(xmlCatalogPtr catal, const xmlChar* pubID, const xmlChar* sysID) {
...
xmlCatalog* xmlLoadACatalog(const char* filename) {
	...
	#endif /* LIBXML_SGML_CATALOG_ENABLED */
	{
		...
		catal->xml = xmlNewCatalogEntry(
			XML_CATA_CATALOG, NULL, NULL, BAD_CAST filename, xmlCatalogDefaultPrefer, NULL
		);
	}
	xmlFree(content);
	return (catal);
 ...
 */
int xmlACatalogAdd(
	xmlCatalog* catal, const xmlChar* type, const xmlChar* orig, const xmlChar* replace
) {
	int res = -1;
	...
	if (catal->type == XML_SGML_CATALOG_TYPE) {
		...
		if (cattype != XML_CATA_NONE) {
			...
			entry = xmlNewCatalogEntry(cattype, orig, replace, NULL, XML_CATA_PREFER_NONE, NULL);
			if (catal->sgml == NULL) {
...
int xmlCatalogIsEmpty(xmlCatalog* catal) {
	...
	#endif /* LIBXML_SGML_CATALOG_ENABLED */
	{
		...
		if ((catal->xml->type != XML_CATA_CATALOG) && (catal->xml->type != XML_CATA_BROKEN_CATALOG)) {
			return (-1);
		}
		if (catal->xml->children == NULL) {
			return (1);
 ...
 */
void xmlInitCatalogInternal(void) {
				...
				if (*cur != 0) {
					...
					if (path != NULL) {
						*nextent = xmlNewCatalogEntry(
							XML_CATA_CATALOG, NULL, NULL, BAD_CAST path, xmlCatalogDefaultPrefer, NULL
						);
						if (*nextent != NULL) {
 ...
 * Free global data.
 */
void xmlCleanupCatalogInternal(void) {
...
int xmlCatalogAdd(const xmlChar* type, const xmlChar* orig, const xmlChar* replace) {
	 ...
	 */
	if ((xmlDefaultCatalog == NULL) && (xmlStrEqual(type, BAD_CAST "catalog"))) {
		xmlDefaultCatalog = xmlCreateNewCatalog(XML_XML_CATALOG_TYPE, xmlCatalogDefaultPrefer);
		if (xmlDefaultCatalog != NULL) {
			xmlDefaultCatalog->xml
				= xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, orig, NULL, xmlCatalogDefaultPrefer, NULL);
		}
		xmlRMutexUnlock(&xmlCatalogMutex);
		xmlCatalogInitialized = 1;
		return (0);
...
void* xmlCatalogAddLocal(void* catalogs, const xmlChar* URL) {
	...
	add = xmlNewCatalogEntry(XML_CATA_CATALOG, NULL, URL, NULL, xmlCatalogDefaultPrefer, NULL);
	if (add == NULL) {
		return (catalogs);
	}
