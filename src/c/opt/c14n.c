...
static int xmlC14NVisibleNsStackAdd(xmlC14NVisibleNsStackPtr cur, xmlNsPtr ns, xmlNodePtr node);
static void
...
static int xmlC14NIsNodeInNodeset(void* user_data, xmlNodePtr node, xmlNodePtr parent);
...
static xmlChar* xmlC11NNormalizeString(const xmlChar* input, xmlC14NNormalizationMode mode);
	...
	#define xmlC14NIsVisible(ctx, node, parent)                                                   \
		(((ctx)->is_visible_callback != NULL)                                                       \
		   ? (ctx)->is_visible_callback((ctx)->user_data, (xmlNodePtr)(node), (xmlNodePtr)(parent)) \
		   : 1)
 ...
 */
static void xmlC14NErrMemory(xmlC14NCtxPtr ctxt) {
	if (ctxt != NULL) {
...
static void xmlC14NErrFull(
	xmlC14NCtxPtr ctxt, xmlNodePtr node, int code, const char* str1, const char* msg, ...
) {
	va_list ap;
	int     res;
 ...
 * @param ctxt  a C14N evaluation context
 */
static void xmlC14NErrParam(xmlC14NCtxPtr ctxt) {
	xmlC14NErrFull(ctxt, NULL, XML_ERR_ARGUMENT, NULL, "Invalid argument\n", NULL);
}
 ...
 */
static void xmlC14NErrInvalidNode(xmlC14NCtxPtr ctxt, const char* node_type, const char* extra) {
	xmlC14NErrFull(
		ctxt, NULL, XML_C14N_INVALID_NODE, extra, "Node %s is invalid here : %s\n", node_type, extra
	);
}
 ...
 * @param extra  extra information
 */
static void xmlC14NErrUnknownNode(xmlC14NCtxPtr ctxt, int node_type, const char* extra) {
	xmlC14NErrFull(
		ctxt, NULL, XML_C14N_UNKNOW_NODE, extra, "Unknown node type %d found : %s\n", node_type, extra
	);
}
 ...
 */
static void xmlC14NErrRelativeNamespace(xmlC14NCtxPtr ctxt, const char* ns_uri) {
	xmlC14NErrFull(
		ctxt,
		NULL,
...
static int xmlC14NVisibleNsStackAdd(xmlC14NVisibleNsStackPtr cur, xmlNsPtr ns, xmlNodePtr node) {
	if ((cur == NULL) || ((cur->nsTab == NULL) && (cur->nodeTab != NULL))
	...
	if (cur->nsMax <= cur->nsCurEnd) {
		...
		newSize = xmlGrowCapacity(
			cur->nsMax, sizeof(tmp1[0]) + sizeof(tmp2[0]), XML_NAMESPACES_DEFAULT, XML_MAX_ITEMS
		);
...
static int xmlC14NIsXmlNs(xmlNsPtr ns) {
	return (
		(ns != NULL) && (xmlStrEqual(ns->prefix, BAD_CAST "xml"))
 ...
 */
static int xmlC14NNsCompare(const void* data1, const void* data2) {
...
static int xmlC14NPrintNamespacesWalker(const void* ns, void* ctx) {
...
static int xmlExcC14NProcessNamespacesAxis(xmlC14NCtxPtr ctx, xmlNodePtr cur, int visible) {
	...
	for (attr = cur->properties; attr != NULL; attr = attr->next) {
		...
		} else if ((attr->ns != NULL) && (xmlStrlen(attr->ns->prefix) == 0)
		           && (xmlStrlen(attr->ns->href) == 0)) {
			has_visibly_utilized_empty_ns = 1;
		}
	}
	 ...
	 * Process xmlns=""
	 */
	if (visible && has_visibly_utilized_empty_ns && !has_empty_ns
 ...
 */
static int xmlC14NAttrsCompare(const void* data1, const void* data2) {
 ...
 * @returns 1 on success or 0 on fail.
 */
static int xmlC14NPrintAttrs(const void* data, void* user) {
 ...
 */
static xmlAttrPtr xmlC14NFindHiddenParentAttr(
	xmlC14NCtxPtr ctx, xmlNodePtr cur, const xmlChar* name, const xmlChar* ns
) {
	xmlAttrPtr res;
...
static int xmlC14NProcessAttrsAxis(xmlC14NCtxPtr ctx, xmlNodePtr cur, int parent_visible) {
	...
	switch (ctx->mode) {
		case XML_C14N_1_0:
			 ...
			 * given as input and the element's parent is omitted from the node-set. The method for
			 * processing the attribute axis of an element E in the node-set is enhanced. All element
			 * nodes along E's ancestor axis are examined for nearest occurrences of attributes in the
			 * xml namespace, such as xml:lang and xml:space (whether or not they are in the node-set).
			 * From this list of attributes, remove any that are in E's attribute axis (whether or not
			 * they are in the node-set). Then, lexicographically merge this attribute list with the
			 * nodes of E's attribute axis that are in the node-set. The result of visiting the attribute
			 * axis is computed by processing the attribute nodes in this merged attribute list.
			 */
		...
		case XML_C14N_1_1:
			 ...
			 *
			 * Simple inheritable attributes are attributes that have a value that requires at most a
			 * simple redeclaration. This redeclaration is done by supplying a new value in the child
			 * axis. The redeclaration of a simple inheritable attribute A contained in one of E's
			 * ancestors is done by supplying a value to an attribute Ae inside E with the same name.
			 * Simple inheritable attributes are xml:lang and xml:space.
			 *
			 * The method for processing the attribute axis of an element E in the node-set is hence
			 * enhanced. All element nodes along E's ancestor axis are examined for the nearest
			 * occurrences of simple inheritable attributes in the xml namespace, such as xml:lang and
			 * xml:space (whether or not they are in the node-set). From this list of attributes, any
			 * simple inheritable attributes that are already in E's attribute axis (whether or not they
			 * are in the node-set) are removed. Then, lexicographically merge this attribute list with
			 * the nodes of E's attribute axis that are in the node-set. The result of visiting the
			 * attribute axis is computed by processing the attribute nodes in this merged attribute
			 * list.
			 *
			 * The xml:id attribute is not a simple inheritable attribute and no processing of these
			 * attributes is performed.
			 *
			 * The xml:base attribute is not a simple inheritable attribute and requires special
			 * processing beyond a simple redeclaration.
			 *
			 * Attributes in the XML namespace other than xml:base, xml:id, xml:lang, and xml:space MUST
			 * be processed as ordinary attributes.
			 */
			...
			while (attr != NULL) {
				...
				if ((!parent_visible) || (xmlC14NIsXmlAttr(attr) == 0)) {
					...
					if ((!matched) && (xml_space_attr == NULL)
					    && xmlStrEqual(attr->name, BAD_CAST "space")) {
						xml_space_attr = attr;
						matched        = 1;
					}
			...
			if (parent_visible) {
				...
				if (xml_lang_attr == NULL) {
					xml_lang_attr
						= xmlC14NFindHiddenParentAttr(ctx, cur->parent, BAD_CAST "lang", XML_XML_NAMESPACE);
				}
				if (xml_lang_attr != NULL) {
				...
				if (xml_space_attr == NULL) {
					xml_space_attr
						= xmlC14NFindHiddenParentAttr(ctx, cur->parent, BAD_CAST "space", XML_XML_NAMESPACE);
				}
				if (xml_space_attr != NULL) {
				...
				if (xml_base_attr == NULL) {
					...
					xml_base_attr
						= xmlC14NFindHiddenParentAttr(ctx, cur->parent, BAD_CAST "base", XML_XML_NAMESPACE);
				}
				if (xml_base_attr != NULL) {
		...
		if (xmlStrlen(ns->href) > 0) {
			...
			if (uri == NULL) {
				if (code < 0) {
					...
					xmlC14NErr(ctx, cur, XML_ERR_INVALID_URI, "parsing namespace uri");
				}
				return (-1);
			}
 ...
 */
static xmlC14NCtxPtr xmlC14NNewCtx(
	xmlDocPtr                doc,
	xmlC14NIsVisibleCallback is_visible_callback,
	void*                    user_data,
	xmlC14NMode              mode,
...
int xmlC14NExecute(
	...
	ctx = xmlC14NNewCtx(
		doc, is_visible_callback, user_data, c14n_mode, inclusive_ns_prefixes, with_comments, buf
	);
	if (ctx == NULL) {
		xmlC14NErr(
			NULL,
...
int xmlC14NDocSaveTo(
	...
	return (xmlC14NExecute(
		doc, xmlC14NIsNodeInNodeset, nodes, mode, inclusive_ns_prefixes, with_comments, buf
	));
}
...
static xmlChar* xmlC11NNormalizeString(const xmlChar* input, xmlC14NNormalizationMode mode) {
	...
	while (*cur != '\0') {
		...
		if ((*cur == '<') && ((mode == XMLC14N_NORMALIZE_ATTR) || (mode == XMLC14N_NORMALIZE_TEXT))) {
			*out++ = '&';
			*out++ = 'l';
			*out++ = 't';
			*out++ = ';';
			...
			*out++ = ';';
		           ...
		           && ((mode == XMLC14N_NORMALIZE_ATTR) || (mode == XMLC14N_NORMALIZE_TEXT) || (mode == XMLC14N_NORMALIZE_COMMENT) || (mode == XMLC14N_NORMALIZE_PI))) {
			*out++ = '&';
			*out++ = '#';
			*out++ = 'x';
			*out++ = 'D';
