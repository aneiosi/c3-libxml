...
xmlUnicodeLookup(const xmlUnicodeRange* sptr, int numentries, const char* tname) {
 ...
 */
static int xmlUCSIsBlock(int code, const char* block) {
	const xmlChRangeGroup* group;

	group = xmlUnicodeLookup(
		xmlUnicodeBlocks, sizeof(xmlUnicodeBlocks) / sizeof(xmlUnicodeBlocks[0]), block
	);
	if (group == NULL) {
		return (-1);
	}
...
static int  xmlRegCheckCharacterRange(
	 xmlRegAtomType type, int codepoint, int neg, int start, int end, const xmlChar* blockName
 );
 ...
 */
static void xmlRegexpErrMemory(xmlRegParserCtxtPtr ctxt) {
	if (ctxt != NULL) {
 ...
 */
static void xmlRegexpErrCompile(xmlRegParserCtxtPtr ctxt, const char* extra) {
 ...
 */
static void* xmlRegCalloc2(size_t dim1, size_t dim2, size_t elemSize) {
	size_t numElems, totalSize;
	void*  ret;

	/* Check for overflow */
	if ((dim2 == 0) || (elemSize == 0) || (dim1 > SIZE_MAX / dim2 / elemSize)) {
		return (NULL);
	}
	numElems = dim1 * dim2;
...
static xmlRegexpPtr xmlRegEpxFromParse(xmlRegParserCtxtPtr ctxt) {
	...
	if ((ret->determinist != 0) && (ret->nbCounters == 0) && (ctxt->negs == 0)
	    && (ret->atoms != NULL) && (ret->atoms[0] != NULL)
		...
		transitions = (int*)xmlRegCalloc2(nbstates + 1, nbatoms + 1, sizeof(int));
		if (transitions == NULL) {
			xmlFree(stateRemap);
			xmlFree(stringRemap);
			for (i = 0; i < nbatoms; i++) {
 ...
 */
static xmlRegParserCtxtPtr xmlRegNewParserCtxt(const xmlChar* string) {
...
static void xmlRegPrintAtomType(FILE* output, xmlRegAtomType type) {
	switch (type) {
		...
		case XML_REGEXP_CHARVAL:
			fprintf(output, "charval ");
			break;
		case XML_REGEXP_RANGES:
			fprintf(output, "ranges ");
			break;
		case XML_REGEXP_SUBREG:
			fprintf(output, "subexpr ");
			break;
		case XML_REGEXP_STRING:
			fprintf(output, "string ");
			break;
		case XML_REGEXP_ANYCHAR:
			fprintf(output, "anychar ");
			break;
		case XML_REGEXP_ANYSPACE:
			fprintf(output, "anyspace ");
			break;
		case XML_REGEXP_NOTSPACE:
			fprintf(output, "notspace ");
			break;
		case XML_REGEXP_INITNAME:
			fprintf(output, "initname ");
			break;
		case XML_REGEXP_NOTINITNAME:
			fprintf(output, "notinitname ");
			break;
		case XML_REGEXP_NAMECHAR:
			fprintf(output, "namechar ");
			break;
		case XML_REGEXP_NOTNAMECHAR:
			fprintf(output, "notnamechar ");
			break;
		case XML_REGEXP_DECIMAL:
			fprintf(output, "decimal ");
			break;
		case XML_REGEXP_NOTDECIMAL:
			fprintf(output, "notdecimal ");
			break;
		case XML_REGEXP_REALCHAR:
			fprintf(output, "realchar ");
			break;
		case XML_REGEXP_NOTREALCHAR:
			fprintf(output, "notrealchar ");
			break;
		case XML_REGEXP_LETTER:
			fprintf(output, "LETTER ");
			break;
		case XML_REGEXP_LETTER_UPPERCASE:
			fprintf(output, "LETTER_UPPERCASE ");
			break;
		case XML_REGEXP_LETTER_LOWERCASE:
			fprintf(output, "LETTER_LOWERCASE ");
			break;
		case XML_REGEXP_LETTER_TITLECASE:
			fprintf(output, "LETTER_TITLECASE ");
			break;
		case XML_REGEXP_LETTER_MODIFIER:
			fprintf(output, "LETTER_MODIFIER ");
			break;
		case XML_REGEXP_LETTER_OTHERS:
			fprintf(output, "LETTER_OTHERS ");
			break;
		case XML_REGEXP_MARK:
			fprintf(output, "MARK ");
			break;
		case XML_REGEXP_MARK_NONSPACING:
			fprintf(output, "MARK_NONSPACING ");
			break;
		case XML_REGEXP_MARK_SPACECOMBINING:
			fprintf(output, "MARK_SPACECOMBINING ");
			break;
		case XML_REGEXP_MARK_ENCLOSING:
			fprintf(output, "MARK_ENCLOSING ");
			break;
		case XML_REGEXP_NUMBER:
			fprintf(output, "NUMBER ");
			break;
		case XML_REGEXP_NUMBER_DECIMAL:
			fprintf(output, "NUMBER_DECIMAL ");
			break;
		case XML_REGEXP_NUMBER_LETTER:
			fprintf(output, "NUMBER_LETTER ");
			break;
		case XML_REGEXP_NUMBER_OTHERS:
			fprintf(output, "NUMBER_OTHERS ");
			break;
		case XML_REGEXP_PUNCT:
			fprintf(output, "PUNCT ");
			break;
		case XML_REGEXP_PUNCT_CONNECTOR:
			fprintf(output, "PUNCT_CONNECTOR ");
			break;
		case XML_REGEXP_PUNCT_DASH:
			fprintf(output, "PUNCT_DASH ");
			break;
		case XML_REGEXP_PUNCT_OPEN:
			fprintf(output, "PUNCT_OPEN ");
			break;
		case XML_REGEXP_PUNCT_CLOSE:
			fprintf(output, "PUNCT_CLOSE ");
			break;
		case XML_REGEXP_PUNCT_INITQUOTE:
			fprintf(output, "PUNCT_INITQUOTE ");
			break;
		case XML_REGEXP_PUNCT_FINQUOTE:
			fprintf(output, "PUNCT_FINQUOTE ");
			break;
		case XML_REGEXP_PUNCT_OTHERS:
			fprintf(output, "PUNCT_OTHERS ");
			break;
		case XML_REGEXP_SEPAR:
			fprintf(output, "SEPAR ");
			break;
		case XML_REGEXP_SEPAR_SPACE:
			fprintf(output, "SEPAR_SPACE ");
			break;
		case XML_REGEXP_SEPAR_LINE:
			fprintf(output, "SEPAR_LINE ");
			break;
		case XML_REGEXP_SEPAR_PARA:
			fprintf(output, "SEPAR_PARA ");
			break;
		case XML_REGEXP_SYMBOL:
			fprintf(output, "SYMBOL ");
			break;
		case XML_REGEXP_SYMBOL_MATH:
			fprintf(output, "SYMBOL_MATH ");
			break;
		case XML_REGEXP_SYMBOL_CURRENCY:
			fprintf(output, "SYMBOL_CURRENCY ");
			break;
		case XML_REGEXP_SYMBOL_MODIFIER:
			fprintf(output, "SYMBOL_MODIFIER ");
			break;
		case XML_REGEXP_SYMBOL_OTHERS:
			fprintf(output, "SYMBOL_OTHERS ");
			break;
		...
		case XML_REGEXP_OTHER_FORMAT:
			fprintf(output, "OTHER_FORMAT ");
			break;
		case XML_REGEXP_OTHER_PRIVATE:
			fprintf(output, "OTHER_PRIVATE ");
			break;
		case XML_REGEXP_OTHER_NA:
			fprintf(output, "OTHER_NA ");
			break;
...
static void xmlRegPrintAtom(FILE* output, xmlRegAtomPtr atom) {
	fprintf(output, " atom: ");
	if (atom == NULL) {
		fprintf(output, "NULL\n");
		return;
...
static void xmlRegPrintAtomCompact(FILE* output, xmlRegexpPtr regexp, int atom) {
	if (output == NULL || regexp == NULL || atom < 0 || atom >= regexp->nbstrings) {
		return;
	}
	fprintf(output, " atom: ");
...
static void xmlRegPrintTransCompact(FILE* output, xmlRegexpPtr regexp, int state, int atom) {
	int target;
	if (output == NULL || regexp == NULL || regexp->compact == NULL || state < 0 || atom < 0) {
		return;
	}
...
static void xmlRegPrintStateCompact(FILE* output, xmlRegexpPtr regexp, int state) {
	int             nbTrans = 0;
	int             i;
	int             target;
	xmlRegStateType stateType;

	if (output == NULL || regexp == NULL || regexp->compact == NULL || state < 0) {
		return;
	}
	...
	for (i = 0; i < regexp->nbstrings; i++) {
		...
		if (target > 0 && target <= regexp->nbstates
		    && regexp->compact[(target - 1) * (regexp->nbstrings + 1)] == XML_REGEXP_SINK_STATE) {
			nbTrans++;
		}
	}
 ...
 */
static void xmlRegPrintCompact(FILE* output, xmlRegexpPtr regexp) {
	int i;
...
static void xmlRegexpPrintInternal(FILE* output, xmlRegexpPtr regexp) {
	int i;
...
static xmlRegRangePtr xmlRegAtomAddRange(
	...
	if (atom->nbRanges >= atom->maxRanges) {
		...
		newSize = xmlGrowCapacity(atom->maxRanges, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlRegexpErrMemory(ctxt);
			return (NULL);
		}
...
static int xmlRegGetCounter(xmlRegParserCtxtPtr ctxt) {
	if (ctxt->nbCounters >= ctxt->maxCounters) {
		...
		newSize = xmlGrowCapacity(ctxt->maxCounters, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlRegexpErrMemory(ctxt);
			return (-1);
		}
...
static int xmlRegAtomPush(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom) {
	...
	if (ctxt->nbAtoms >= ctxt->maxAtoms) {
		...
		newSize = xmlGrowCapacity(ctxt->maxAtoms, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlRegexpErrMemory(ctxt);
			return (-1);
		}
...
static void xmlRegStateAddTransTo(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr target, int from) {
	if (target->nbTransTo >= target->maxTransTo) {
		...
		newSize = xmlGrowCapacity(target->maxTransTo, sizeof(tmp[0]), 8, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlRegexpErrMemory(ctxt);
			return;
		}
...
static void xmlRegStateAddTrans(
	xmlRegParserCtxtPtr ctxt,
	xmlRegStatePtr      state,
	xmlRegAtomPtr       atom,
	...
	for (nrtrans = state->nbTrans - 1; nrtrans >= 0; nrtrans--) {
		...
		if ((trans->atom == atom) && (trans->to == target->no) && (trans->counter == counter)
		    && (trans->count == count)) {
			return;
		}
	}
	...
	if (state->nbTrans >= state->maxTrans) {
		...
		newSize = xmlGrowCapacity(state->maxTrans, sizeof(tmp[0]), 8, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlRegexpErrMemory(ctxt);
			return;
		}
...
static xmlRegStatePtr xmlRegStatePush(xmlRegParserCtxtPtr ctxt) {
	xmlRegStatePtr state;
	...
	if (ctxt->nbStates >= ctxt->maxStates) {
		...
		newSize = xmlGrowCapacity(ctxt->maxStates, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlRegexpErrMemory(ctxt);
			return (NULL);
		}
 ...
 * @param lax
 */
static int xmlFAGenerateAllTransition(
 ...
 */
static int
xmlFAGenerateEpsilonTransition(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr from, xmlRegStatePtr to) {
	if (to == NULL) {
		to = xmlRegStatePush(ctxt);
		if (to == NULL) {
			return (-1);
 ...
 */
static int xmlFAGenerateCountedEpsilonTransition(
 ...
 * @param counter  the counter for that transition
 */
static int xmlFAGenerateCountedTransition(
	xmlRegParserCtxtPtr ctxt, xmlRegStatePtr from, xmlRegStatePtr to, int counter
) {
	if (to == NULL) {
		to = xmlRegStatePush(ctxt);
 ...
 */
static int xmlFAGenerateTransitions(
	xmlRegParserCtxtPtr ctxt, xmlRegStatePtr from, xmlRegStatePtr to, xmlRegAtomPtr atom
) {
	xmlRegStatePtr end;
	int            nullable = 0;
 ...
 */
static void
xmlFAReduceEpsilonTransitions(xmlRegParserCtxtPtr ctxt, int fromnr, int tonr, int counter) {
	int            transnr;
	xmlRegStatePtr from;
	xmlRegStatePtr to;
	...
	for (transnr = 0; transnr < to->nbTrans; transnr++) {
		...
		if (t1->atom == NULL) {
			...
			if (t1->to != fromnr) {
				if (t1->count >= 0) {
					xmlRegStateAddTrans(ctxt, from, NULL, ctxt->states[t1->to], -1, t1->count);
				} else {
					xmlFAReduceEpsilonTransitions(ctxt, fromnr, t1->to, tcounter);
				}
			}
		} else {
			xmlRegStateAddTrans(ctxt, from, t1->atom, ctxt->states[t1->to], tcounter, -1);
		}
	}
}
 ...
 * @param tonr  the to state
 */
static void xmlFAFinishReduceEpsilonTransitions(xmlRegParserCtxtPtr ctxt, int tonr) {
	int            transnr;
	xmlRegStatePtr to;
	...
	if ((to->mark == XML_REGEXP_MARK_START) || (to->mark == XML_REGEXP_MARK_NORMAL)) {
		return;
	}
 ...
 * @param ctxt  a regexp parser context
 */
static void xmlFAEliminateSimpleEpsilonTransitions(xmlRegParserCtxtPtr ctxt) {
	...
	for (statenr = 0; statenr < ctxt->nbStates; statenr++) {
		...
		if ((state->trans[0].atom == NULL) && (state->trans[0].to >= 0)
		    && (state->trans[0].to != statenr) && (state->trans[0].counter < 0)
 ...
 * @param ctxt  a regexp parser context
 */
static void xmlFAEliminateEpsilonTransitions(xmlRegParserCtxtPtr ctxt) {
	int            statenr, transnr;
	xmlRegStatePtr state;
	int            has_epsilon;
	...
	if (has_epsilon) {
		for (statenr = 0; statenr < ctxt->nbStates; statenr++) {
			...
			for (transnr = 0; transnr < state->nbTrans; transnr++) {
				...
				if ((trans->atom == NULL) && (trans->count < 0) && (trans->to >= 0)) {
					trans->to = -1;
				}
			}
		}
	...
	while (state != NULL) {
		...
		for (transnr = 0; transnr < state->nbTrans; transnr++) {
			if ((state->trans[transnr].to >= 0)
			    && ((state->trans[transnr].atom != NULL) || (state->trans[transnr].count >= 0))) {
				int newto = state->trans[transnr].to;
	...
	if ((range1->type == XML_REGEXP_ANYCHAR) || (range2->type == XML_REGEXP_ANYCHAR)) {
		...
		for (codepoint = range1->start; codepoint <= range1->end; codepoint++) {
			ret = xmlRegCheckCharacterRange(
				range2->type, codepoint, 0, range2->start, range2->end, range2->blockName
			);
			if (ret < 0) {
				return (-1);
			}
		...
		switch (range1->type) {
			...
			case XML_REGEXP_NUMBER:
				...
				break;
			case XML_REGEXP_PUNCT:
				if ((range2->type == XML_REGEXP_PUNCT_CONNECTOR) || (range2->type == XML_REGEXP_PUNCT_DASH)
				    || (range2->type == XML_REGEXP_PUNCT_OPEN) || (range2->type == XML_REGEXP_PUNCT_CLOSE)
				...
				break;
			case XML_REGEXP_SEPAR:
				if ((range2->type == XML_REGEXP_SEPAR_SPACE) || (range2->type == XML_REGEXP_SEPAR_LINE)
			...
			case XML_REGEXP_SYMBOL:
				...
				break;
			case XML_REGEXP_OTHER:
				if ((range2->type == XML_REGEXP_OTHER_CONTROL) || (range2->type == XML_REGEXP_OTHER_FORMAT)
...
static int xmlFACompareAtomTypes(xmlRegAtomType type1, xmlRegAtomType type2) {
	if ((type1 == XML_REGEXP_EPSILON) || (type1 == XML_REGEXP_CHARVAL)
	    || (type1 == XML_REGEXP_RANGES) || (type1 == XML_REGEXP_SUBREG)
	    || (type1 == XML_REGEXP_STRING) || (type1 == XML_REGEXP_ANYCHAR)) {
		return (1);
	}
	if ((type2 == XML_REGEXP_EPSILON) || (type2 == XML_REGEXP_CHARVAL)
	    || (type2 == XML_REGEXP_RANGES) || (type2 == XML_REGEXP_SUBREG)
	    || (type2 == XML_REGEXP_STRING) || (type2 == XML_REGEXP_ANYCHAR)) {
		return (1);
	}
	...
	switch (type1) {
		...
		case XML_REGEXP_DECIMAL:     /* \d */
			...
			if ((type2 == XML_REGEXP_NOTDECIMAL) || (type2 == XML_REGEXP_REALCHAR)
			    || ((type2 >= XML_REGEXP_LETTER) && (type2 <= XML_REGEXP_LETTER_OTHERS))
			    || ((type2 >= XML_REGEXP_MARK) && (type2 <= XML_REGEXP_MARK_ENCLOSING))
			    || ((type2 >= XML_REGEXP_PUNCT) && (type2 <= XML_REGEXP_PUNCT_OTHERS))
			    || ((type2 >= XML_REGEXP_SEPAR) && (type2 <= XML_REGEXP_SEPAR_PARA))
 ...
 */
static int xmlFAEqualAtoms(xmlRegAtomPtr atom1, xmlRegAtomPtr atom2, int deep) {
	int ret = 0;
	...
	switch (atom1->type) {
		...
		case XML_REGEXP_STRING:
			if (!deep) {
				...
				ret = xmlStrEqual((xmlChar*)atom1->valuep, (xmlChar*)atom2->valuep);
			}
			break;
		case XML_REGEXP_CHARVAL:
 ...
 */
static int xmlFACompareAtoms(xmlRegAtomPtr atom1, xmlRegAtomPtr atom2, int deep) {
	int ret = 1;
 ...
 * @param atom  the atom
 */
static int xmlFARecurseDeterminism(
	...
	for (transnr = 0; transnr < nbTrans; transnr++) {
		...
		if (t1->atom == NULL) {
			...
			res          = xmlFARecurseDeterminism(ctxt, ctxt->states[t1->to], fromnr, tonr, atom);
			if (res == 0) {
				ret = 0;
				/* t1->nd = 1; */
			}
		...
		if (xmlFACompareAtoms(t1->atom, atom, deep)) {
			...
			if ((t1->to != tonr) || (!xmlFAEqualAtoms(t1->atom, atom, deep))) {
				ret = 0;
			}
 ...
 * @param state  regexp state
 */
static void xmlFAFinishRecurseDeterminism(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state) {
	int transnr, nbTrans;
			...
			for (i = 0; i < transnr; i++) {
				...
				if (t2->atom != NULL) {
					if (t1->to == t2->to) {
						 ...
						 */
						if (xmlFAEqualAtoms(t1->atom, t2->atom, deep) && (t1->counter == t2->counter)
						    && (t1->count == t2->count)) {
							t2->to = -1; /* eliminated */
						}
					}
		...
		for (transnr = 0; transnr < state->nbTrans; transnr++) {
			...
			for (i = 0; i < transnr; i++) {
				...
				if (t2->atom != NULL) {
					 ...
					 */
					res = xmlFARecurseDeterminism(ctxt, ctxt->states[t2->to], statenr, t1->to, t1->atom);
...
static int xmlRegCheckCharacterRange(
	...
	switch (type) {
		...
		case XML_REGEXP_NAMECHAR:
			ret
				= (IS_LETTER(codepoint) || IS_DIGIT(codepoint) || (codepoint == '.') || (codepoint == '-') || (codepoint == '_') || (codepoint == ':') || IS_COMBINING(codepoint) || IS_EXTENDER(codepoint));
			break;
		case XML_REGEXP_NOTDECIMAL:
			neg = !neg;
			/* Falls through. */
	...
	switch (atom->type) {
		...
		case XML_REGEXP_RANGES: {
			...
			for (i = 0; i < atom->nbRanges; i++) {
				...
				if (range->neg == 2) {
					ret = xmlRegCheckCharacterRange(
						range->type, codepoint, 0, range->start, range->end, range->blockName
					);
					if (ret != 0) {
		...
		case XML_REGEXP_BLOCK_NAME:
			ret
				= xmlRegCheckCharacterRange(atom->type, codepoint, 0, 0, 0, (const xmlChar*)atom->valuep);
			if (atom->neg) {
				ret = !ret;
			}
			break;
...
static void xmlFARegExecSave(xmlRegExecCtxtPtr exec) {
	...
	if (exec->nbRollbacks >= exec->maxRollbacks) {
		...
		newSize = xmlGrowCapacity(exec->maxRollbacks, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
	...
	if (exec->comp->nbCounters > 0) {
		if (exec->rollbacks[exec->nbRollbacks].counts == NULL) {
			exec->rollbacks[exec->nbRollbacks].counts
				= (int*)xmlMalloc(exec->comp->nbCounters * sizeof(int));
			if (exec->rollbacks[exec->nbRollbacks].counts == NULL) {
				exec->status = XML_REGEXP_OUT_OF_MEMORY;
				return;
			}
		...
		memcpy(
			exec->rollbacks[exec->nbRollbacks].counts, exec->counts, exec->comp->nbCounters * sizeof(int)
		);
	}
	exec->nbRollbacks++;
}
...
static int xmlFARegExec(xmlRegexpPtr comp, const xmlChar* content) {
	...
	while (
		(exec->status == XML_REGEXP_OK) && (exec->state != NULL)
		&& ((exec->inputString[exec->index] != 0) || ((exec->state != NULL) && (exec->state->type != XML_REGEXP_FINAL_STATE)))
	) {
		xmlRegTransPtr trans;
		xmlRegAtomPtr  atom;
		...
		for (; exec->transno < exec->state->nbTrans; exec->transno++) {
			...
			if (trans->count >= 0) {
				...
				len       = 4;
				codepoint = xmlGetUTF8Char(&exec->inputString[exec->index], &len);
				if (codepoint < 0) {
			...
			if (ret == 1) {
				if ((trans->nd == 1)
				    || ((trans->count >= 0) && (deter == 0) && (exec->state->nbTrans > exec->transno + 1))) {
					xmlFARegExecSave(exec);
					if (exec->status != XML_REGEXP_OK) {
						goto error;
					}
...
static int xmlRegExecSetErrString(xmlRegExecCtxtPtr exec, const xmlChar* value) {
	if (exec->errString != NULL) {
		xmlFree(exec->errString);
	}
	if (value == NULL) {
...
static void xmlFARegExecSaveInputString(xmlRegExecCtxtPtr exec, const xmlChar* value, void* data) {
	if (exec->inputStackNr + 1 >= exec->inputStackMax) {
		...
		newSize = xmlGrowCapacity(exec->inputStackMax, sizeof(tmp[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			exec->status = XML_REGEXP_OUT_OF_MEMORY;
			return;
		}
 ...
 */
static int xmlRegCompactPushString(
	xmlRegExecCtxtPtr exec, xmlRegexpPtr comp, const xmlChar* value, void* data
) {
		...
		if ((target > 0) && (target <= comp->nbstates)) {
			...
			if (xmlRegStrEqualWildcard(comp->stringMap[i], value)) {
				...
				if ((exec->callback != NULL) && (comp->transdata != NULL)) {
					exec->callback(exec->data, value, comp->transdata[state * comp->nbstrings + i], data);
				}
 ...
 *     a negative value in case of error.
 */
static int xmlRegExecPushStringInternal(
	xmlRegExecCtxtPtr exec, const xmlChar* value, void* data, int compound
) {
	xmlRegTransPtr trans;
	xmlRegAtomPtr  atom;
	int            ret;
	...
	while ((exec->status == XML_REGEXP_OK)
	       && ((value != NULL) || ((final == 1) && (exec->state->type != XML_REGEXP_FINAL_STATE)))) {
		/*
		...
		for (; exec->transno < exec->state->nbTrans; exec->transno++) {
			...
			if (trans->count == REGEXP_ALL_LAX_COUNTER) {
				...
				if ((value == NULL) && (final)) {
					...
					for (i = 0; i < exec->state->nbTrans; i++) {
						...
						if ((count < counter->max) && (t->atom != NULL)
						    && (xmlStrEqual(value, t->atom->valuep))) {
							ret = 0;
							break;
						}
						if ((count >= counter->min) && (count < counter->max) && (t->atom != NULL)
						    && (xmlStrEqual(value, t->atom->valuep))) {
							ret = 1;
							break;
						}
			...
			if (ret == 1) {
				if ((exec->callback != NULL) && (atom != NULL) && (data != NULL)) {
					exec->callback(exec->data, atom->valuep, atom->data, data);
				}
				if (exec->state->nbTrans > exec->transno + 1) {
					if (exec->inputStackNr <= 0) {
						xmlFARegExecSaveInputString(exec, value, data);
				...
				if ((exec->comp->states[trans->to] != NULL)
				    && (exec->comp->states[trans->to]->type == XML_REGEXP_SINK_STATE)) {
					/*
...
rollback:
			...
			exec->determinist = 0;
			xmlFARegExecRollBack(exec);
			if ((exec->inputStack != NULL) && (exec->status == XML_REGEXP_OK)) {
				value = exec->inputStack[exec->index].value;
				data  = exec->inputStack[exec->index].data;
			}
		}
...
int xmlRegExecPushString2(
	xmlRegExecCtxt* exec, const xmlChar* value, const xmlChar* value2, void* data
) {
	xmlChar  buf[150];
	int      lenn, lenp, ret;
	xmlChar* str;
 ...
 */
static int xmlRegExecGetValues(
	xmlRegExecCtxtPtr exec, int err, int* nbval, int* nbneg, xmlChar** values, int* terminal
) {
	int maxval;
	int nb = 0;
	...
	*nbneg = 0;
	if ((exec->comp != NULL) && (exec->comp->compact != NULL)) {
		...
		for (i = 0; (i < comp->nbstrings) && (nb < maxval); i++) {
			target = comp->compact[state * (comp->nbstrings + 1) + i + 1];
			if ((target > 0) && (target <= comp->nbstates)
			    && (comp->compact[(target - 1) * (comp->nbstrings + 1)] != XML_REGEXP_SINK_STATE)) {
...
int xmlRegExecNextValues(
	xmlRegExecCtxt* exec, int* nbval, int* nbneg, xmlChar** values, int* terminal
) {
 ...
 */
void xmlRegExecClearErrors(xmlRegExecCtxt* exec) {
	exec->status     = 0;
	exec->errState   = NULL;
	exec->errStateNo = -1;
	xmlFree(exec->errString);
 ...
 * @param ctxt  a regexp parser context
 */
static int xmlFAIsChar(xmlRegParserCtxtPtr ctxt) {
	int cur;
	int len;
	...
	if ((cur == '.') || (cur == '\\') || (cur == '?') || (cur == '*') || (cur == '+') || (cur == '(')
	    || (cur == ')') || (cur == '|') || (cur == 0x5B) || (cur == 0x5D) || (cur == 0)) {
		return (-1);
	}
	return (cur);
}
 ...
 * @param ctxt  a regexp parser context
 */
static void xmlFAParseCharProp(xmlRegParserCtxtPtr ctxt) {
	int            cur;
	...
	if (ctxt->atom == NULL) {
		...
		if (xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg, type, 0, 0, blockName) == NULL) {
			xmlFree(blockName);
		}
	}
}
 ...
 * @param ctxt  a regexp parser context
 */
static void xmlFAParseCharClassEsc(xmlRegParserCtxtPtr ctxt) {
	int cur;
	...
	if (CUR == '.') {
		if (ctxt->atom == NULL) {
			...
			xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg, XML_REGEXP_ANYCHAR, 0, 0, NULL);
		}
		NEXT;
		return;
	}
	...
	if (cur == 'p') {
		...
		NEXT;
	} else if ((cur == 'n') || (cur == 'r') || (cur == 't') || (cur == '\\') || (cur == '|')
	           || (cur == '.') || (cur == '?') || (cur == '*') || (cur == '+') || (cur == '(')
	           || (cur == ')') || (cur == '{') || (cur == '}') || (cur == 0x2D) || (cur == 0x5B)
	           || (cur == 0x5D) || (cur == 0x5E) ||
		...
		if (ctxt->atom == NULL) {
			...
			xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg, XML_REGEXP_CHARVAL, cur, cur, NULL);
		}
		NEXT;
	} else if ((cur == 's') || (cur == 'S') || (cur == 'i') || (cur == 'I') || (cur == 'c')
	           || (cur == 'C') || (cur == 'd') || (cur == 'D') || (cur == 'w') || (cur == 'W')) {
		...
		if (ctxt->atom == NULL) {
			...
			xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg, type, 0, 0, NULL);
		}
	} else {
...
static void xmlFAParseCharRange(xmlRegParserCtxtPtr ctxt) {
	...
	cur = CUR;
	if ((cur != '-') || (NXT(1) == '[') || (NXT(1) == ']')) {
		xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg, XML_REGEXP_CHARVAL, start, end, NULL);
		return;
	}
	NEXT;
	cur = CUR;
 ...
 * @param ctxt  a regexp parser context
 */
static void xmlFAParsePosCharGroup(xmlRegParserCtxtPtr ctxt) {
	do {
		if (CUR == '\\') {
	...
	} while ((CUR != ']') && (CUR != '-') && (CUR != 0) && (ctxt->error == 0));
}
 ...
 * @param ctxt  a regexp parser context
 */
static void xmlFAParseCharGroup(xmlRegParserCtxtPtr ctxt) {
	int neg = ctxt->neg;
 ...
 * @param ctxt  a regexp parser context
 */
static void xmlFAParseCharClass(xmlRegParserCtxtPtr ctxt) {
	if (CUR == '[') {
		NEXT;
 ...
 */
static int xmlFAParseQuantExact(xmlRegParserCtxtPtr ctxt) {
	int ret      = 0;
	int ok       = 0;
	int overflow = 0;
 ...
 * @param ctxt  a regexp parser context
 */
static int xmlFAParseQuantifier(xmlRegParserCtxtPtr ctxt) {
	int cur;
 ...
 * @param ctxt  a regexp parser context
 */
static int xmlFAParseAtom(xmlRegParserCtxtPtr ctxt) {
	int codepoint, len;
 ...
 * @param ctxt  a regexp parser context
 */
static int xmlFAParsePiece(xmlRegParserCtxtPtr ctxt) {
	int ret;
 ...
 */
static int xmlFAParseBranch(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr to) {
	...
	if (ret == 0) {
		...
		if (xmlFAGenerateTransitions(
					ctxt, previous, (CUR == '|' || CUR == ')' || CUR == 0) ? to : NULL, ctxt->atom
				)
		    < 0) {
 ...
 */
void       xmlRegexpPrint(FILE* output ATTRIBUTE_UNUSED, xmlRegexp* regexp ATTRIBUTE_UNUSED) {}
 ...
 */
xmlAutomataState* xmlAutomataNewTransition(
	xmlAutomata* am, xmlAutomataState* from, xmlAutomataState* to, const xmlChar* token, void* data
) {
	xmlRegAtomPtr atom;
 ...
 * @returns the target state or NULL in case of error
 */
xmlAutomataState* xmlAutomataNewTransition2(
	xmlAutomata*      am,
 ...
 * @returns the target state or NULL in case of error
 */
xmlAutomataState* xmlAutomataNewNegTrans(
	xmlAutomata*      am,
	xmlAutomataState* from,
	xmlAutomataState* to,
	const xmlChar*    token,
 ...
 * @returns the target state or NULL in case of error
 */
xmlAutomataState* xmlAutomataNewCountTrans2(
	xmlAutomata*      am,
	xmlAutomataState* from,
	xmlAutomataState* to,
	const xmlChar*    token,
 ...
 * @returns the target state or NULL in case of error
 */
xmlAutomataState* xmlAutomataNewCountTrans(
	xmlAutomata*      am,
	xmlAutomataState* from,
	xmlAutomataState* to,
	const xmlChar*    token,
 ...
 * @returns the target state or NULL in case of error
 */
xmlAutomataState* xmlAutomataNewOnceTrans2(
	xmlAutomata*      am,
	xmlAutomataState* from,
	xmlAutomataState* to,
	const xmlChar*    token,
 ...
 * @returns the target state or NULL in case of error
 */
xmlAutomataState* xmlAutomataNewOnceTrans(
	xmlAutomata*      am,
	xmlAutomataState* from,
	xmlAutomataState* to,
	const xmlChar*    token,
 ...
 */
xmlAutomataState*
xmlAutomataNewEpsilon(xmlAutomata* am, xmlAutomataState* from, xmlAutomataState* to) {
 ...
 */
xmlAutomataState*
xmlAutomataNewAllTrans(xmlAutomata* am, xmlAutomataState* from, xmlAutomataState* to, int lax) {
	if ((am == NULL) || (from == NULL)) {
		return (NULL);
	}
 ...
 * @returns the target state or NULL in case of error
 */
xmlAutomataState* xmlAutomataNewCountedTrans(
	xmlAutomata* am, xmlAutomataState* from, xmlAutomataState* to, int counter
) {
 ...
 * @returns the target state or NULL in case of error
 */
xmlAutomataState* xmlAutomataNewCounterTrans(
	xmlAutomata* am, xmlAutomataState* from, xmlAutomataState* to, int counter
) {
	if ((am == NULL) || (from == NULL) || (counter < 0)) {
		return (NULL);
