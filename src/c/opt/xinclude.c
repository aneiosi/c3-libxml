...
static xmlXIncludeRefPtr xmlXIncludeExpandNode(xmlXIncludeCtxtPtr ctxt, xmlNodePtr node);

static int               xmlXIncludeLoadNode(xmlXIncludeCtxtPtr ctxt, xmlXIncludeRefPtr ref);

static int               xmlXIncludeDoProcess(xmlXIncludeCtxtPtr ctxt, xmlNodePtr tree);
 ...
 */
static void              xmlXIncludeErrMemory(xmlXIncludeCtxtPtr ctxt) {
  ctxt->errNo    = XML_ERR_NO_MEMORY;
  ctxt->fatalErr = 1;
  ctxt->nbErrors++;

  xmlRaiseMemoryError(ctxt->errorHandler, NULL, ctxt->errorCtxt, XML_FROM_XINCLUDE, NULL);
}
 ...
 */
static void LIBXML_ATTR_FORMAT(4, 0) xmlXIncludeErr(
	xmlXIncludeCtxtPtr ctxt, xmlNodePtr node, int error, const char* msg, const xmlChar* extra
) {
 ...
 */
static xmlChar* xmlXIncludeGetProp(xmlXIncludeCtxtPtr ctxt, xmlNodePtr cur, const xmlChar* name) {
	xmlChar* ret;
 ...
 */
static xmlXIncludeRefPtr xmlXIncludeAddNode(xmlXIncludeCtxtPtr ctxt, xmlNodePtr cur) {
	...
	if (href == NULL) {
		if (fragment == NULL) {
			xmlXIncludeErr(ctxt, cur, XML_XINCLUDE_NO_HREF, "href or xpointer must be present\n", parse);
			goto error;
		}
	...
	if (uri == NULL) {
		if (res < 0) {
			...
			xmlXIncludeErr(ctxt, cur, XML_XINCLUDE_HREF_URI, "invalid value href %s\n", href);
		}
		goto error;
	}
	 ...
	 * xml:base fixup
	 */
	if (((ctxt->parseFlags & XML_PARSE_NOBASEFIX) == 0) && (cur->doc != NULL)
	...
	if (ctxt->incNr >= ctxt->incMax) {
		...
		newSize = xmlGrowCapacity(ctxt->incMax, sizeof(table[0]), 4, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlXIncludeErrMemory(ctxt);
			goto error;
		}
...
static void xmlXIncludeBaseFixup(
	xmlXIncludeCtxtPtr ctxt, xmlNodePtr cur, xmlNodePtr copy, const xmlChar* targetBase
) {
	xmlChar* base    = NULL;
	...
	if ((base != NULL) && !xmlStrEqual(base, targetBase)) {
		if ((xmlStrlen(base) > XML_MAX_URI_LENGTH) || (xmlStrlen(targetBase) > XML_MAX_URI_LENGTH)) {
		...
		if (relBase == NULL) {
			xmlXIncludeErr(ctxt, cur, XML_XINCLUDE_HREF_URI, "Building relative URI failed: %s\n", base);
			goto done;
		}
 ...
 */
static xmlNodePtr xmlXIncludeCopyNode(
	xmlXIncludeCtxtPtr ctxt, xmlNodePtr elem, int copyChildren, const xmlChar* targetBase
) {
	...
	while (1) {
		...
		if ((cur->type == XML_DOCUMENT_NODE) || (cur->type == XML_DTD_NODE)) {
			;
		           ...
		           && ((xmlStrEqual(cur->ns->href, XINCLUDE_NS)) || (xmlStrEqual(cur->ns->href, XINCLUDE_OLD_NS)))) {
			...
			recurse = (cur->type != XML_ENTITY_REF_NODE) && (cur->children != NULL);
		}
 ...
 */
static xmlNodePtr xmlXIncludeCopyXPointer(
	xmlXIncludeCtxtPtr ctxt, xmlXPathObjectPtr obj, const xmlChar* targetBase
) {
			...
			for (i = 0; i < set->nodeNr; i++) {
				...
				switch (set->nodeTab[i]->type) {
					...
					case XML_HTML_DOCUMENT_NODE:
						node = xmlDocGetRootElement((xmlDocPtr)set->nodeTab[i]);
						if (node == NULL) {
							xmlXIncludeErr(
								ctxt, set->nodeTab[i], XML_ERR_INTERNAL_ERROR, "document without root\n", NULL
							);
							continue;
						}
						break;
 ...
 */
static void
xmlXIncludeMergeEntity(void* payload, void* vdata, const xmlChar* name ATTRIBUTE_UNUSED) {
	...
	if (prev == NULL) {
		ret
			= xmlAddDocEntity(doc, ent->name, ent->etype, ent->ExternalID, ent->SystemID, ent->content);
		if (ret == NULL) {
			xmlXIncludeErrMemory(ctxt);
			return;
		}
...
static int xmlXIncludeMergeEntities(xmlXIncludeCtxtPtr ctxt, xmlDocPtr doc, xmlDocPtr from) {
	...
	if ((source != NULL) && (source->entities != NULL)) {
		...
		xmlHashScan((xmlHashTablePtr)source->entities, xmlXIncludeMergeEntity, &data);
	}
 ...
 */
static int xmlXIncludeLoadDoc(xmlXIncludeCtxtPtr ctxt, xmlXIncludeRefPtr ref) {
	...
	for (i = 0; i < ctxt->urlNr; i++) {
		if (xmlStrEqual(url, ctxt->urlTab[i].url)) {
			if (ctxt->urlTab[i].expanding) {
				xmlXIncludeErr(ctxt, ref->elem, XML_XINCLUDE_RECURSION, "inclusion loop detected\n", NULL);
				goto error;
			}
	...
	if (ctxt->urlNr >= ctxt->urlMax) {
		...
		newSize = xmlGrowCapacity(ctxt->urlMax, sizeof(tmp[0]), 8, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlXIncludeErrMemory(ctxt);
			xmlFreeDoc(doc);
			goto error;
...
loaded:
	if (fragment == NULL) {
		...
		if (root == NULL) {
			xmlXIncludeErr(ctxt, ref->elem, XML_ERR_INTERNAL_ERROR, "document without root\n", NULL);
			goto error;
		}
		...
		if (ctxt->xpctxt == NULL) {
			...
			if (ctxt->errorHandler != NULL) {
				xmlXPathSetErrorHandler(ctxt->xpctxt, ctxt->errorHandler, ctxt->errorCtxt);
			}
		#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
		...
		if (ctxt->xpctxt->lastError.code != XML_ERR_OK) {
			if (ctxt->xpctxt->lastError.code == XML_ERR_NO_MEMORY) {
				...
				xmlXIncludeErr(
					ctxt, ref->elem, XML_XINCLUDE_XPTR_FAILED, "XPointer evaluation failed: #%s\n", fragment
				);
			}
			goto error;
		}
		...
		switch (xptr->type) {
			...
			case XPATH_XSLT_TREE:
				xmlXIncludeErr(
					ctxt, ref->elem, XML_XINCLUDE_XPTR_RESULT, "XPointer is not a range: #%s\n", fragment
				);
 ...
 * @returns 0 in case of success, -1 in case of failure
 */
static int xmlXIncludeLoadTxt(xmlXIncludeCtxtPtr ctxt, xmlXIncludeRefPtr ref) {
	...
	if (encoding != NULL) {
		...
		code = xmlOpenCharEncodingHandler(
			(const char*)encoding,
			/* output */ 0,
			&handler
		);
		...
		if (code != XML_ERR_OK) {
			if (code == XML_ERR_NO_MEMORY) {
				...
				xmlXIncludeErr(ctxt, ref->elem, code, "unexpected error from iconv or ICU\n", NULL);
				goto error;
			}
		}
	}
	...
	inputStream = xmlLoadResource(pctxt, (const char*)url, NULL, XML_RESOURCE_XINCLUDE_TEXT);
	if (inputStream == NULL) {
		/*
		...
		if (pctxt->errNo == XML_ERR_NO_MEMORY) {
			xmlXIncludeErrMemory(ctxt);
		} else if ((pctxt->errNo != XML_ERR_OK) && (pctxt->errNo != XML_IO_ENOENT)
	...
	for (i = 0; i < len;) {
		...
		if ((cur < 0) || (!IS_CHAR(cur))) {
			xmlXIncludeErr(
				ctxt, ref->elem, XML_XINCLUDE_INVALID_CHAR, "%s contains invalid char\n", url
			);
			goto error;
		}
	...
	if (ctxt->txtNr >= ctxt->txtMax) {
		...
		newSize = xmlGrowCapacity(ctxt->txtMax, sizeof(tmp[0]), 8, XML_MAX_ITEMS);
		if (newSize < 0) {
			xmlXIncludeErrMemory(ctxt);
			goto error;
		}
	...
	if ((node->content != NULL) && (ctxt->txtTab[ctxt->txtNr].text == NULL)) {
		xmlXIncludeErrMemory(ctxt);
		goto error;
	}
 ...
 */
static int
xmlXIncludeLoadFallback(xmlXIncludeCtxtPtr ctxt, xmlNodePtr fallback, xmlXIncludeRefPtr ref) {
	int ret = 0;
	int oldNbErrors;

	if ((fallback == NULL) || (fallback->type == XML_NAMESPACE_DECL) || (ctxt == NULL)) {
		return (-1);
	}
...
static xmlXIncludeRefPtr xmlXIncludeExpandNode(xmlXIncludeCtxtPtr ctxt, xmlNodePtr node) {
	...
	if (ctxt->depth >= XINCLUDE_MAX_DEPTH) {
		xmlXIncludeErr(ctxt, node, XML_XINCLUDE_RECURSION, "maximum recursion depth exceeded\n", NULL);
		ctxt->fatalErr = 1;
		return (NULL);
	}
	...
	for (i = 0; i < ctxt->incNr; i++) {
		if (ctxt->incTab[i]->elem == node) {
			if (ctxt->incTab[i]->expanding) {
				xmlXIncludeErr(ctxt, node, XML_XINCLUDE_RECURSION, "inclusion loop detected\n", NULL);
				return (NULL);
			}
...
static int xmlXIncludeLoadNode(xmlXIncludeCtxtPtr ctxt, xmlXIncludeRefPtr ref) {
	...
	if (ret < 0) {
		...
		while (children != NULL) {
			if ((children->type == XML_ELEMENT_NODE) && (children->ns != NULL)
			    && (xmlStrEqual(children->name, XINCLUDE_FALLBACK))
			    && ((xmlStrEqual(children->ns->href, XINCLUDE_NS)) || (xmlStrEqual(children->ns->href, XINCLUDE_OLD_NS)))) {
 ...
 * @returns 0 if substitution succeeded, -1 if some processing failed
 */
static int xmlXIncludeIncludeNode(xmlXIncludeCtxtPtr ctxt, xmlXIncludeRefPtr ref) {
...
static int xmlXIncludeTestNode(xmlXIncludeCtxtPtr ctxt, xmlNodePtr node) {
	...
	if ((xmlStrEqual(node->ns->href, XINCLUDE_NS))
		...
		if (xmlStrEqual(node->name, XINCLUDE_NODE)) {
			...
			while (child != NULL) {
				if ((child->type == XML_ELEMENT_NODE) && (child->ns != NULL)
				    && ((xmlStrEqual(child->ns->href, XINCLUDE_NS)) || (xmlStrEqual(child->ns->href, XINCLUDE_OLD_NS)))) {
		...
		if (xmlStrEqual(node->name, XINCLUDE_FALLBACK)) {
			if ((node->parent == NULL) || (node->parent->type != XML_ELEMENT_NODE)
			    || (node->parent->ns == NULL)
			    || ((!xmlStrEqual(node->parent->ns->href, XINCLUDE_NS)) && (!xmlStrEqual(node->parent->ns->href, XINCLUDE_OLD_NS)))
		...
		if (xmlXIncludeTestNode(ctxt, cur) == 1) {
			...
			if (ref != NULL) {
				ref->replace = 1;
		           ...
		           && ((cur->type == XML_DOCUMENT_NODE) || (cur->type == XML_ELEMENT_NODE))) {
			cur = cur->children;
			continue;
		}
		do {
 ...
 *    or the number of substitutions done.
 */
static int xmlXIncludeDoProcessRoot(xmlXIncludeCtxtPtr ctxt, xmlNodePtr tree) {
 ...
 */
void
xmlXIncludeSetErrorHandler(xmlXIncludeCtxt* ctxt, xmlStructuredErrorFunc handler, void* data) {
	if (ctxt == NULL) {
		return;
	}
 ...
 * @returns 0 in case of success and -1 in case of error.
 */
int xmlXIncludeSetStreamingMode(xmlXIncludeCtxt* ctxt, int mode) {
	if (ctxt == NULL) {
		return (-1);
	}
	ctxt->isStream = !!mode;
 ...
 *    or the number of substitutions done.
 */
int xmlXIncludeProcessNode(xmlXIncludeCtxt* ctxt, xmlNode* node) {
	int ret = 0;

	if ((node == NULL) || (node->type == XML_NAMESPACE_DECL) || (node->doc == NULL)
	    || (ctxt == NULL)) {
		return (-1);
	}
