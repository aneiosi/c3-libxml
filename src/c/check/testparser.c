...
static void ignoreError(void* ctxt ATTRIBUTE_UNUSED, const xmlError* error ATTRIBUTE_UNUSED) {}
#endif

static int testNewDocNode(void) {
	xmlNodePtr node;
	int        err = 0;
...
static int testStandaloneWithEncoding(void) {
	xmlDocPtr   doc;
	const char* str
...
static int testUnsupportedEncoding(void) {
	xmlDocPtr       doc;
	...
	doc = xmlReadDoc(BAD_CAST "<doc/>", NULL, "#unsupported", XML_PARSE_NOWARNING);
	if (doc == NULL) {
	...
	if (error == NULL || error->code != XML_ERR_UNSUPPORTED_ENCODING
...
static int testNodeGetContent(void) {
	xmlDocPtr doc;
	xmlChar*  content;
	int       err = 0;
 ...
 */
static int testUndeclEntInContent(void) {
	...
	res  = xmlParseInNodeContext(root, content, sizeof(content) - 1, options, &list);
	...
	res = xmlParseBalancedChunkMemory(doc, NULL, NULL, 0, BAD_CAST content, &list);
	if (res != XML_ERR_UNDECLARED_ENTITY || list != NULL) {
...
static int testInvalidCharRecovery(void) {
	const char* xml = "<doc>&#x10;</doc>";
	xmlDoc*     doc;
	int         err = 0;

	doc             = xmlReadDoc(BAD_CAST xml, NULL, NULL, XML_PARSE_RECOVER | XML_PARSE_NOERROR);
...
static void testCtxtInputGetterError(void* errCtxt, const xmlError* error) {
	int*           err  = errCtxt;
	...
	if (strcmp(filename, "test.xml") != 0 || line != 4 || col != 11 || bytePos != 62) {
	...
	if (strncmp((char*)start, "<doc>&ent;", 10) != 0 || size != 16 || offset != 10) {
	...
	if (filename != NULL || line != 1 || col != 11 || bytePos != 10) {
		fprintf(stderr, "unexpected position: %s %d %d %lu\n", filename, line, col, bytePos);
		*err = 1;
	}
	...
	if (strncmp((char*)start, "xxx &fail;", 10) != 0 || size != 14 || offset != 10) {
		fprintf(stderr, "unexpected window: %.10s %d %d\n", start, size, offset);
		*err = 1;
	}
}

static int testCtxtInputGetters(void) {
	const char* xml
		= "<!DOCTYPE doc [\n"
...
static void testSwitchDtdExtSubset(
	void*                     vctxt,
...
static int testSwitchDtd(void) {
	const char dtdContent[] = "<!ENTITY test '<elem1/><elem2/>'>\n";
	...
	input
		= xmlParserInputBufferCreateStatic(dtdContent, sizeof(dtdContent) - 1, XML_CHAR_ENCODING_NONE);
    ...
    ctxt, docContent, sizeof(docContent) - 1, NULL, NULL, XML_PARSE_NOENT | XML_PARSE_DTDLOAD
  );
	xmlFreeParserCtxt(ctxt);
...
static xmlChar* dumpNodeList(xmlNodePtr list) {
	xmlBufferPtr   buffer;
	xmlSaveCtxtPtr save;
	xmlNodePtr     cur;
	xmlChar*       ret;
...
static int testCtxtParseContent(void) {
	xmlParserCtxtPtr         ctxt;
	...
	static const char* const tests[]
		= {"<!-- c -->\xF0\x9F\x98\x84<a/><b/>end", "text<a:foo><b:foo/></a:foo>text<!-- c -->"};

	doc  = xmlReadDoc(BAD_CAST "<doc xmlns:a='a'><elem xmlns:b='b'/></doc>", NULL, NULL, 0);
	...
	for (i = 0; (size_t)i < sizeof(tests) / sizeof(tests[0]); i++) {
		...
		for (j = 0; j < 2; j++) {
			if (j == 0) {
				...
				xmlParseInNodeContext(node, content, strlen(content), 0, &list);
			}

			output = dumpNodeList(list);

			if ((j == 0 && ctxt->nsWellFormed == 0) || strcmp((char*)output, content) != 0) {
				fprintf(
					stderr,
					"%s failed test %d, got:\n%s\n",
					j == 0 ? "xmlCtxtParseContent" : "xmlParseInNodeContext",
					i,
					output
				);
				err = 1;
...
static int testSaveNullEncDoc(const char* xml, const char* expect) {
	xmlDocPtr      doc;
	xmlBufferPtr   buffer;
	xmlSaveCtxtPtr save;
...
static int testSaveNullEnc(void) {
	int err  = 0;

	err     |= testSaveNullEncDoc(
    "<?xml version=\"1.0\"?><doc>\xC3\x98</doc>", "<?xml version=\"1.0\"?>\n<doc>&#xD8;</doc>\n"
  );
	err |= testSaveNullEncDoc(
...
static int testDocDumpFormatMemoryEnc(void) {
...
#ifdef LIBXML_SAX1_ENABLED
static int testBalancedChunk(void) {
	xmlNodePtr list;
	xmlNodePtr elem;
	int        ret;
	int        err = 0;

	ret            = xmlParseBalancedChunkMemory(
    NULL, NULL, NULL, 0, BAD_CAST "start <node xml:lang='en'>abc</node> end", &list
  );

	if ((ret != XML_ERR_OK) || (list == NULL) || ((elem = list->next) == NULL)
	    || (elem->type != XML_ELEMENT_NODE) || (elem->nsDef == NULL)
...
static int testHugeEncodedChunk(void) {
	xmlBufferPtr     buf;
	xmlChar*         chunk;
	xmlParserCtxtPtr ctxt;
	int              err = 0, i;
	 ...
	 */
	buf                  = xmlBufferCreate();
	xmlBufferCat(buf, BAD_CAST "<?xml version='1.0' encoding='ISO-8859-1'?>\n");
...
static int testPushCDataEnd(void) {
	int err = 0;
	int k;
...
static int testHtmlIds(void) {
	const char* htmlContent = "<html><body><div id='myId'>Hello, World!</div></body></html>";
	htmlDocPtr  doc;
	xmlAttrPtr  node;
	...
	#ifdef LIBXML_OUTPUT_ENABLED
static int testHtmlInsertMetaEncoding(void) {
...
static int testHtmlUpdateMetaEncoding(void) {
	...
	const char* html
		= "<html>\n"
			...
			" content='text/html; charset = \"  foo  '>\n"
			"        <" MHE
			" content=\"charset ; charset = bar; baz\">\n"
			"        <" MHE
			" content=\"text/html\">\n"
			"        <" MHE
			" content=\"\">\n"
	...
	const char* expect
		= "<html>\n"
			...
			" content=\"text/html; charset = &quot;  foo  \">\n"
			"        <" MHE
			" content=\"charset ; charset = utf-8; baz\">\n"
			"        <" MHE
			" content=\"text/html\">\n"
			"        <" MHE
			" content=\"\">\n"
	...
	#ifdef LIBXML_PUSH_ENABLED
static int testHtmlPushWithEncoding(void) {
...
static int testReaderEncoding(void) {
	xmlBuffer*     buf;
	...
	#ifdef LIBXML_OUTPUT_ENABLED
static int testReaderContent(void) {
	xmlTextReader* reader;
...
static int testReaderNode(xmlTextReader* reader) {
	xmlChar* string;
	int      type;
	int      err = 0;
...
static void testStructuredReaderError(void* arg, const xmlError* error) {
	testReaderErrorCtxt* ctxt = arg;
...
static int testReaderXIncludeError(void) {
	/*
	...
	if (errorCtxt.message == NULL || strstr(errorCtxt.message, "href or xpointer") == NULL) {
	...
	reader = xmlReaderForDoc(BAD_CAST doc, NULL, NULL, XML_PARSE_XINCLUDE);
	xmlTextReaderSetStructuredErrorHandler(reader, testStructuredReaderError, &errorCtxt);
	errorCtxt.message = NULL;
	errorCtxt.code    = 0;
	while (xmlTextReaderRead(reader) > 0)
		;

	if (errorCtxt.code != XML_XINCLUDE_NO_HREF || errorCtxt.message == NULL
...
static int testWriterIOWrite(void* ctxt, const char* data, int len) {
	(void)ctxt;
	(void)data;
...
static int testWriterIOClose(void* ctxt) {
	(void)ctxt;

	return XML_IO_ENAMETOOLONG;
}

static int testWriterClose(void) {
	xmlOutputBufferPtr out;
	xmlTextWriterPtr   writer;
	int                err = 0;
	int                result;

	out    = xmlOutputBufferCreateIO(testWriterIOWrite, testWriterIOClose, NULL, NULL);
...
static int testBuildRelativeUri(void) {
	xmlChar*                        res;
	int                             err = 0;
	int                             i;

	static const xmlRelativeUriTest tests[] = {
		{										"/a/b1/c1",											"/a/b2/c2",                 "../b1/c1"},
		{										 "a/b1/c1",											 "a/b2/c2",                 "../b1/c1"},
		{"a/././b1/x/y/../z/../.././c1",               "./a/./b2/././b2",                 "../b1/c1"},
		{						 "file:///a/b1/c1",                      "/a/b2/c2",                       NULL},
		{										"/a/b1/c1",               "file:///a/b2/c2",                       NULL},
		{										 "a/b1/c1",											"/a/b2/c2",                       NULL},
		{										"/a/b1/c1",											 "a/b2/c2",                       NULL},
		{  "http://example.org/a/b1/c1",    "http://example.org/a/b2/c2",                 "../b1/c1"},
		{  "http://example.org/a/b1/c1",   "https://example.org/a/b2/c2",                       NULL},
		{  "http://example.org/a/b1/c1",      "http://localhost/a/b2/c2",                       NULL},
		{					"with space/x x/y y",              "with space/b2/c2",           "../x%20x/y%20y"},
		...
		{								 "\\a\\b1\\c1",											"/a/b2/c2",                 "../b1/c1"},
		{									 "a\\b1\\c1",											 "a/b2/c2",                 "../b1/c1"},
		{			 "file://server/a/b1/c1", "\\\\?\\UNC\\server\\a\\b2\\c2",                 "../b1/c1"},
		{			 "file://server/a/b1/c1",         "\\\\server\\a\\b2\\c2",                 "../b1/c1"},
		{					"file:///x:/a/b1/c1",                 "x:\\a\\b2\\c2",                 "../b1/c1"},
		{					"file:///x:/a/b1/c1",          "\\\\?\\x:\\a\\b2\\c2",                 "../b1/c1"},
		{					"file:///x:/a/b1/c1",            "file:///y:/a/b2/c2",                       NULL},
		{									"x:/a/b1/c1",										"y:/a/b2/c2",       "file:///x:/a/b1/c1"},
		{										"/a/b1/c1",										"y:/a/b2/c2",                       NULL},
		{			 "\\\\server\\a\\b1\\c1",                       "a/b2/c2",    "file://server/a/b1/c1"}
#endif
	};
	...
	for (i = 0; (size_t)i < sizeof(tests) / sizeof(tests[0]); i++) {
		...
		if (!xmlStrEqual(res, BAD_CAST expect)) {
			fprintf(
				stderr,
				"xmlBuildRelativeURI failed uri=%s base=%s "
				"result=%s expected=%s\n",
				test->uri,
				test->base,
				res,
...
#if defined(_WIN32) || defined(__CYGWIN__)
static int testWindowsUri(void) {
	...
	static const xmlRelativeUriTest tests[] = {
		{				"c:/a%20b/file.txt",               "base.xml",           "c:/a b/file.txt"},
		{"file:///c:/a%20b/file.txt",               "base.xml", "file:///c:/a%20b/file.txt"},
		{				"Z:/a%20b/file.txt",    "http://example.com/",           "Z:/a b/file.txt"},
		{							"a%20b/b1/c1",             "C:/a/b2/c2",         "C:/a/b2/a b/b1/c1"},
		{							"a%20b/b1/c1",            "\\a\\b2\\c2",           "/a/b2/a b/b1/c1"},
		{							"a%20b/b1/c1",       "\\\\?\\a\\b2\\c2",        "//?/a/b2/a b/b1/c1"},
		{							"a%20b/b1/c1", "\\\\\\\\server\\b2\\c2",     "//server/b2/a b/b1/c1"}
	};
	...
	for (i = 0; (size_t)i < sizeof(tests) / sizeof(tests[0]); i++) {
		...
		if (res == NULL || !xmlStrEqual(res, BAD_CAST test->result)) {
			fprintf(
				stderr,
				"xmlBuildURI failed uri=%s base=%s "
				"result=%s expected=%s\n",
				test->uri,
				test->base,
				res,
...
static int testTruncatedMultiByte(void) {
	const char xml[]
...
static xmlCharEncError rot13Convert(
	void*                vctxt,
	unsigned char*       out,
	int*                 outlen,
...
static void rot13ConvCtxtDtor(void* vctxt) {
	xmlFree(vctxt);
}

static xmlParserErrors rot13ConvImpl(
	...
	*inputCtxt = 13;
	...
	return xmlCharEncNewCustomHandler(
		name, rot13Convert, NULL, rot13ConvCtxtDtor, inputCtxt, NULL, out
	);
}

static int testCharEncConvImpl(void) {
	xmlParserCtxtPtr ctxt;
	xmlDocPtr        doc;
	xmlNodePtr       root;
	int              err = 0;
	...
	doc = xmlCtxtReadDoc(ctxt, BAD_CAST "<?kzy irefvba='1.0'?><qbp/>", NULL, "rot13", 0);
