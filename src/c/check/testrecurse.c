...
static void globfree(glob_t* pglob ATTRIBUTE_UNUSED) {}
...
static const xmlHugeDocParts hugeDocTable[] = {
   ...
   "<!ENTITY d '&e;&e;'> "
   "]> "
   "<foo>", "  <bar attr='&e; &f; &d;'>&a; &b; &c; &e; &f; &d;</bar>\n"
...
static void initSAX(xmlParserCtxtPtr ctxt) {
 ...
 */
static int recursiveDetectTest(
	const char*        filename,
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int notRecursiveDetectTest(
	const char*        filename,
	const char* result ATTRIBUTE_UNUSED,
	const char* err    ATTRIBUTE_UNUSED,
	int                options
 ...
 * @returns 0 in case of success, an error code otherwise
 */
static int notRecursiveHugeTest(
	...
	if (doc == NULL) {
		...
		e_size = f_size * 2 + xmlStrlen(BAD_CAST "&f;") * 2 + fixed_cost * 2;
		...
		d_size = e_size * 2 + xmlStrlen(BAD_CAST "&e;") * 2 + fixed_cost * 2;
		...
		total_size = (f_size + e_size + d_size + 3 * fixed_cost) * (MAX_NODES - 1) * 3;
		if (ctxt->sizeentcopy != total_size) {
			fprintf(
				stderr, "Wrong total entity size: %lu (expected %lu)\n", ctxt->sizeentcopy, total_size
			);
			res = 1;
		}
...
static int hugeDtdTest(
	...
	doc = xmlCtxtReadFile(ctxt, "test/recurse/huge_dtd.xml", NULL, parserOptions);
	if (doc == NULL) {
		...
		total_size = /* internal */
			...
			+ (a_size + e_size + f_size + fixed_cost * 3) * (MAX_NODES - 1) * 2 +
			/* external */
			(a_size + b_size + c_size + fixed_cost * 3) * (MAX_NODES - 1) * 2 +
		...
		total_size = strlen(hugeDocParts->start) + strlen(hugeDocParts->segment) * (MAX_NODES - 1)
		...
		if (ctxt->sizeentities != total_size) {
			fprintf(
				stderr, "Wrong parsed entity size: %lu (expected %lu)\n", ctxt->sizeentities, total_size
			);
			res = 1;
		}
	}
...
static testDesc testDescriptions[] = {
   ...
   recursiveDetectTest,  "./test/recurse/lol*.xml",
   NULL, NULL,
   NULL,                      0																																																												},
   ...
   recursiveDetectTest,  "./test/recurse/lol*.xml",
   NULL, NULL,
   NULL, OPT_SAX | OPT_NO_SUBST																																																												},
	{											"Parsing non-recursive test cases",
   notRecursiveDetectTest, "./test/recurse/good*.xml",
   NULL, NULL,
   NULL,                      0																																																												},
   ...
   notRecursiveHugeTest,                       NULL,
   NULL, NULL,
   NULL,           OPT_NO_SUBST																																																												},
	{								 "Parsing non-recursive huge case (SAX)", notRecursiveHugeTest,                       NULL, NULL, NULL, NULL,                OPT_SAX},
	{"Parsing non-recursive huge case (SAX, no substitution)",
   notRecursiveHugeTest,                       NULL,
   NULL, NULL,
   NULL, OPT_SAX | OPT_NO_SUBST																																																												},
	{									 "Parsing non-recursive huge DTD case",          hugeDtdTest,                       NULL, NULL, NULL, NULL,                      0},
...
static int launchTests(testDescPtr tst) {
	...
	if (tst->in != NULL) {
		...
		for (i = 0; i < globbuf.gl_pathc; i++) {
			...
			if (tst->err != NULL) {
				error = resultFilename(globbuf.gl_pathv[i], tst->out, tst->err);
				if (error == NULL) {
					fprintf(stderr, "Out of memory !\n");
					fatalError();
				}
			...
			if ((result) && (!checkTestFile(result))) {
				...
				mem = xmlMemUsed();
				res = tst->func(globbuf.gl_pathv[i], result, error, tst->options | XML_PARSE_COMPACT);
				xmlResetLastError();
				if (res != 0) {
