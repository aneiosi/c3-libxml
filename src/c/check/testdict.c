...
static void            fill_string_pool(xmlChar** strings, const char** seeds) {
  int i, j, k;
  ...
  for (j = 0, k = 0; (j < NB_STRINGS_PREFIX) && (i < NB_STRINGS_MAX); i++, j++) {
 ...
 */
static int test_subdict(xmlDictPtr parent) {
	int            i, j;
	xmlDictPtr     dict;
	int            ret = 0;
	xmlChar        prefix[40];
 ...
 */
static int test_dict(xmlDict* dict) {
	int            i, j;
	int            ret = 0;
	xmlChar        prefix[40];
	xmlChar *      cur, *pref;
...
static xmlChar* gen_random_string(xmlChar id) {
...
static StringPool* pool_new(size_t num_entries, size_t num_keys, xmlChar id) {
	StringPool* ret;
	size_t      num_strings;
...
static int pool_done(StringPool* pool) {
	return pool->index >= pool->num_strings;
}

static void pool_reset(StringPool* pool) {
	pool->index = 0;
}

static int pool_bulk_insert(StringPool* pool, xmlHashTablePtr hash, size_t num) {
	size_t i, j;
	int    ret = 0;
		...
		while (1) {
			...
			switch (pool->num_keys) {
				case 1:
					...
					if (res == 0 && xmlHashUpdateEntry(hash, str[0], str[0], NULL) != 0) {
						ret = -1;
					}
					break;
				case 2:
					res = xmlHashAddEntry2(hash, str[0], str[1], tmp_key);
					if (res == 0 && xmlHashUpdateEntry2(hash, str[0], str[1], str[0], NULL) != 0) {
						ret = -1;
					}
					break;
				case 3:
					res = xmlHashAddEntry3(hash, str[0], str[1], str[2], tmp_key);
					if (res == 0 && xmlHashUpdateEntry3(hash, str[0], str[1], str[2], str[0], NULL) != 0) {
						ret = -1;
					}
					break;
			}
...
static xmlChar* hash_qlookup(xmlHashTable* hash, xmlChar** names, size_t num_keys) {
	xmlChar*       prefix[3];
	...
	res = xmlHashQLookup3(hash, prefix[0], local[0], prefix[1], local[1], prefix[2], local[2]);
...
static int pool_bulk_lookup(StringPool* pool, xmlHashTablePtr hash, size_t num, int existing) {
	size_t i, j;
	int    ret = 0;
...
static int pool_bulk_remove(StringPool* pool, xmlHashTablePtr hash, size_t num) {
	size_t i, j;
	int    ret = 0;
...
static int test_hash(size_t num_entries, size_t num_keys, int use_dict) {
	xmlDict*      dict = NULL;
	xmlHashTable* hash;
