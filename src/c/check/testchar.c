...
static int  testDocumentRangeByte1(
	...
	for (i = 0; i <= 0xFF; i++) {
		...
		if ((i == forbid1) || (i == forbid2)) {
			if ((lastError == 0) || (res != NULL)) {
				fprintf(stderr, "Failed to detect invalid char for Byte 0x%02X: %c\n", i, i);
				return (1);
			}
		}
			...
			if ((lastError == 0) || (res != NULL)) {
				fprintf(stderr, "Failed to detect illegal char %c for Byte 0x%02X\n", i, i);
				return (1);
			}
		} else if (((i < 0x20) || (i >= 0x80)) && (i != 0x9) && (i != 0xA) && (i != 0xD)) {
			if ((lastError != XML_ERR_INVALID_CHAR) && (res != NULL)) {
				fprintf(stderr, "Failed to detect invalid char for Byte 0x%02X\n", i);
				return (1);
			}
		} else if (res == NULL) {
			fprintf(stderr, "Failed to parse valid char for Byte 0x%02X : %c\n", i, i);
			return (1);
		}
		if (res != NULL) {
			xmlFreeDoc(res);
...
static int testDocumentRangeByte2(xmlParserCtxtPtr ctxt, char* document, int len, char* data) {
	int       i, j;
	xmlDocPtr res;
	...
	for (i = 0x80; i <= 0xFF; i++) {
		for (j = 0; j <= 0xFF; j++) {
			...
			if ((i & 0x80) && ((i & 0x40) == 0)) {
				if ((lastError == 0) || (res != NULL)) {
					fprintf(stderr, "Failed to detect invalid char for Bytes 0x%02X 0x%02X\n", i, j);
					return (1);
				}
			}
			 ...
			 */
				...
				if ((lastError == 0) || (res != NULL)) {
					fprintf(stderr, "Failed to detect invalid char for Bytes 0x%02X 0x%02X 0x00\n", i, j);
					return (1);
				}
			}
			 ...
			 */
				...
				fprintf(stderr, "Failed to parse document for Bytes 0x%02X 0x%02X\n", i, j);
				return (1);
			}
			if (res != NULL) {
				xmlFreeDoc(res);
...
static int testDocumentRanges(void) {
	...
	test_ret += testDocumentRangeByte1(ctxt, &document1[0], strlen(document1), data, -1, -1);
	printf(" 2");
	fflush(stdout);
	data[0]   = ' ';
	data[1]   = ' ';
	...
	test_ret += testDocumentRangeByte1(ctxt, &document1[0], strlen(document1), data + 3, -1, -1);
	...
	data[3]   = ' ';
	/* test 1 byte injection at beginning of area */
	test_ret += testDocumentRangeByte1(ctxt, &document2[0], strlen(document2), data, '\'', -1);
	printf(" 4");
	fflush(stdout);
	data[0]   = ' ';
	data[1]   = ' ';
	...
	data[3]   = ' ';
	/* test 1 byte injection at end of area */
	test_ret += testDocumentRangeByte1(ctxt, &document2[0], strlen(document2), data + 3, '\'', -1);
	printf(" done\n");
	...
	test_ret += testDocumentRangeByte2(ctxt, &document1[0], strlen(document1), data);
	printf(" 2");
	fflush(stdout);
	data[0]   = ' ';
	data[1]   = ' ';
	...
	test_ret += testDocumentRangeByte2(ctxt, &document1[0], strlen(document1), data + 2);
	...
	data[3]   = ' ';
	/* test 2 byte injection at beginning of area */
	test_ret += testDocumentRangeByte2(ctxt, &document2[0], strlen(document2), data);
	printf(" 4");
	fflush(stdout);
	data[0]   = ' ';
	data[1]   = ' ';
	...
	data[3]   = ' ';
	/* test 2 byte injection at end of area */
	test_ret += testDocumentRangeByte2(ctxt, &document2[0], strlen(document2), data + 2);
	printf(" done\n");
...
static int testCurrentChar(xmlParserCtxtPtr ctxt, int* len) {
	...
	if ((*ctxt->input->cur != 0) && (err != lastError)) {
		fprintf(
			stderr,
			"xmlCurrentChar and xmlNextChar report different "
			"errors: %d %d\n",
			err,
			lastError
		);
	...
	for (i = 0x80; i <= 0xFF; i++) {
		for (j = 0; j <= 0xFF; j++) {
			 ...
			 */
				...
				if (lastError != XML_ERR_INVALID_ENCODING) {
					fprintf(stderr, "Failed to detect invalid char for Bytes 0x%02X 0x%02X: %d\n", i, j, c);
					return (1);
				}
			}
			 ...
			 */
				...
				fprintf(stderr, "Failed to parse char for Bytes 0x%02X 0x%02X\n", i, j);
				return (1);
			}
		...
		for (j = 0; j <= 0xFF; j++) {
			for (k = 0; k < 6; k++) {
				 ...
				 */
					...
					if (lastError != XML_ERR_INVALID_ENCODING) {
						fprintf(
							stderr, "Failed to detect invalid char for Bytes 0x%02X 0x%02X 0x%02X\n", i, j, K
						);
						return (1);
					}
				}
				 ...
				 */
					...
					fprintf(stderr, "Failed to parse char for Bytes 0x%02X 0x%02X 0x%02X\n", i, j, K);
					return (1);
				}
		...
		for (j = 0; j <= 0xFF; j++) {
			for (k = 0; k < 6; k++) {
				for (l = 0; l < 6; l++) {
					...
					L              = lows[l];
					data[3]        = (char)L;
					value          = (L & 0x3F) + ((K & 0x3F) << 6) + ((j & 0x3F) << 12) + ((i & 0x7) << 18);
					ctxt->nbErrors = 0;
					 ...
					 */
					else if (((j & 0xC0) != 0x80) || ((K & 0xC0) != 0x80) || ((L & 0xC0) != 0x80)) {
						if (lastError != XML_ERR_INVALID_ENCODING) {
							fprintf(
								stderr,
								"Failed to detect invalid char for Bytes 0x%02X 0x%02X 0x%02X 0x%02X\n",
					 ...
					 */
					else if (((value > 0xD7FF) && (value < 0xE000)) || (value > 0x10FFFF)) {
						if (lastError != XML_ERR_INVALID_ENCODING) {
							fprintf(
								stderr,
...
static int testCharRanges(void) {
	...
	buf = xmlParserInputBufferCreateStatic(data, sizeof(data), XML_CHAR_ENCODING_NONE);
	if (buf == NULL) {
...
static int testUTF8Chunks(void) {
	xmlParserCtxtPtr ctxt;
	xmlChar*         out;
	int              outSize;
	char*            buf;
	...
	if (strcmp((char*)out, "<?xml version=\"1.0\"?>\n<d>&#x1F60A;</d>\n") != 0) {
...
static int testUTF16(void) {
	...
	if (testEncHandler(handler16LE, utf8, sizeof(utf8) - 1, utf16LE, sizeof(utf16LE) - 1) != 0) {
		ret = -1;
	}
	if (testEncHandler(handler16BE, utf8, sizeof(utf8) - 1, utf16BE, sizeof(utf16BE) - 1) != 0) {
		ret = -1;
	}
