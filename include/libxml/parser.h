...
#define XML_SKIP_IDS       8
	 ...
	 */
	typedef xmlParserInput* (*resolveEntitySAXFunc)(
	...
	typedef void (*elementDeclSAXFunc)(
		void* ctx, const xmlChar* name, int type, xmlElementContent* content
	);
	/**
	...
	typedef void (*processingInstructionSAXFunc)(
		void* ctx, const xmlChar* target, const xmlChar* data
	);
	/**
	 ...
	 */
	typedef void (*warningSAXFunc)(void* ctx, const char* msg, ...) LIBXML_ATTR_FORMAT(2, 3);
	/**
	 ...
	 * @param ...  extra parameters for the message display
	 */
	typedef void (*errorSAXFunc)(void* ctx, const char* msg, ...) LIBXML_ATTR_FORMAT(2, 3);
	/**
	 ...
	 * @param ...  extra parameters for the message display
	 */
	typedef void (*fatalErrorSAXFunc)(void* ctx, const char* msg, ...) LIBXML_ATTR_FORMAT(2, 3);
	/**
...
#define XML_SAX2_MAGIC 0xDEEDBEAF
	 ...
	 */
	typedef xmlParserInput* (*xmlExternalEntityLoader)(
		const char* URL, const char* publicId, xmlParserCtxt* context
	);
 ...
 */
	...
	XML_DEPRECATED
	XMLPUBFUN xmlDtd*
	xmlSAXParseDTD(xmlSAXHandler* sax, const xmlChar* publicId, const xmlChar* systemId);
	 ...
	 */
	XMLPUBFUN xmlParserCtxt* xmlCreatePushParserCtxt(
		xmlSAXHandler* sax, void* user_data, const char* chunk, int size, const char* filename
	);
	XMLPUBFUN int xmlParseChunk(xmlParserCtxt* ctxt, const char* chunk, int size, int terminate);
	...
	XMLPUBFUN xmlParserCtxt* xmlCreateIOParserCtxt(
		xmlSAXHandler*        sax,
		void*                 user_data,
	...
	xmlNewIOInputStream(xmlParserCtxt* ctxt, xmlParserInputBuffer* input, xmlCharEncoding enc);
	...
	XMLPUBFUN xmlParserInput*
	xmlLoadExternalEntity(const char* URL, const char* ID, xmlParserCtxt* ctxt);
	...
	XMLPUBFUN int  xmlCtxtResetPush(
	...
	XMLPUBFUN int xmlCtxtGetInputWindow(
		xmlParserCtxt* ctxt, int inputIndex, const xmlChar** startOut, int* sizeInOut, int* offsetOut
	);
	XMLPUBFUN void
	xmlCtxtSetErrorHandler(xmlParserCtxt* ctxt, xmlStructuredErrorFunc handler, void* data);
	XMLPUBFUN void
	xmlCtxtSetResourceLoader(xmlParserCtxt* ctxt, xmlResourceLoader loader, void* vctxt);
	XMLPUBFUN void
	xmlCtxtSetCharEncConvImpl(xmlParserCtxt* ctxt, xmlCharEncConvImpl impl, void* vctxt);
	...
	XMLPUBFUN xmlDoc* xmlCtxtReadDoc(
		xmlParserCtxt* ctxt, const xmlChar* cur, const char* URL, const char* encoding, int options
	);
	XMLPUBFUN xmlDoc*
	xmlCtxtReadFile(xmlParserCtxt* ctxt, const char* filename, const char* encoding, int options);
	XMLPUBFUN xmlDoc* xmlCtxtReadMemory(
		xmlParserCtxt* ctxt,
		const char*    buffer,
		int            size,
		const char*    URL,
	...
	XMLPUBFUN xmlDoc* xmlCtxtReadIO(
		xmlParserCtxt*        ctxt,
		xmlInputReadCallback  ioread,
		xmlInputCloseCallback ioclose,
		void*                 ioctx,
	...
	XMLPUBFUN xmlParserInput* xmlNewInputFromIO(
		const char*           url,
		xmlInputReadCallback  ioRead,
		xmlInputCloseCallback ioClose,
		void*                 ioCtxt,
