...
extern "C" {
	...
	XMLPUBFUN void xmlSAX2EntityDecl(
		void*          ctx,
		const xmlChar* name,
		int            type,
	...
	XMLPUBFUN void xmlSAX2AttributeDecl(
		void*           ctx,
		const xmlChar*  elem,
	...
	XMLPUBFUN void
	xmlSAX2ElementDecl(void* ctx, const xmlChar* name, int type, xmlElementContent* content);
	XMLPUBFUN void xmlSAX2NotationDecl(
		void* ctx, const xmlChar* name, const xmlChar* publicId, const xmlChar* systemId
	);
	XMLPUBFUN void xmlSAX2UnparsedEntityDecl(
		void*          ctx,
		const xmlChar* name,
		const xmlChar* publicId,
		const xmlChar* systemId,
	...
	XMLPUBFUN void xmlSAX2StartElementNs(
		void*           ctx,
	...
	XMLPUBFUN void xmlSAX2EndElementNs(
	...
	XMLPUBFUN void
	xmlSAX2ProcessingInstruction(void* ctx, const xmlChar* target, const xmlChar* data);
