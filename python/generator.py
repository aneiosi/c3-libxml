...
functions = {
	"xmlRegisterXPathFunction": (
		...
		[
			["ctx", "xmlXPathContext *", "the xpathContext"],
			["name", "xmlChar *", "the function name"],
			["ns_uri", "xmlChar *", "the namespace or NULL"],
			["f", "pythonObject", "the python function"],
		],
		"python",
	...
	"xmlXPathRegisterVariable": (
		...
		[
			["ctx", "xmlXPathContext *", "the xpathContext"],
			["name", "xmlChar *", "the variable name"],
			["ns_uri", "xmlChar *", "the namespace or NULL"],
			["value", "pythonObject", "the value"],
		],
		"python",
		"defined(LIBXML_XPATH_ENABLED)",
	),
	...
	"xmlCreatePushParser": (
		...
		[
			["SAX", "pythonObject", "the SAX callback object or None"],
			["chunk", "xmlChar *", "the initial data"],
			["size", "int", "the size of the initial data"],
			["URI", "xmlChar *", "The URI used for base computations"],
		],
		"python",
		None,
	),
	...
	"xmlSAXParseFile": (
		...
		[
			["SAX", "pythonObject", "the SAX callback object or None"],
			["URI", "xmlChar *", "The URI of the resource"],
			["recover", "int", "allow recovery in case of error"],
		],
		"python",
		None,
	),
	"htmlSAXParseFile": (
		...
		[
			["SAX", "pythonObject", "the SAX callback object or None"],
			["URI", "xmlChar *", "The URI of the resource"],
			["encoding", "const char *", "encoding or None"],
		],
		"python",
		"defined(LIBXML_HTML_ENABLED)",
	),
	"xmlCreateOutputBuffer": (
		...
		[
			["file", "pythonObject", "the Python file"],
			["encoding", "xmlChar *", "an optional encoding"],
		],
		"python",
		None,
	),
	...
	"xmlParserSetValidate": (
		...
		[
			["ctxt", "xmlParserCtxt *", "the parser context"],
			["validate", "int", "1 to activate validation"],
		],
		"python_accessor",
		None,
	),
	"xmlParserSetReplaceEntities": (
		...
		[
			["ctxt", "xmlParserCtxt *", "the parser context"],
			["replaceEntities", "int", "1 to replace entities"],
		],
		"python_accessor",
		None,
	),
	"xmlParserSetPedantic": (
		...
		[
			["ctxt", "xmlParserCtxt *", "the parser context"],
			["pedantic", "int", "1 to run in pedantic mode"],
		],
		"python_accessor",
		None,
	),
	"xmlParserSetLoadSubset": (
		...
		[
			["ctxt", "xmlParserCtxt *", "the parser context"],
			["loadsubset", "int", "1 to load the DTD"],
		],
		"python_accessor",
		None,
	),
	...
	for arg in args:
		...
		if arg[1] in py_types:
			...
			if t != None:
				...
				c_convert = c_convert + "    %s = (%s) Py%s_Get(pyobj_%s);\n" % (
					arg[0],
					arg[1],
					t,
					arg[0],
			...
			if t == "File":
				c_release = c_release + "    PyFile_Release(%s);\n" % (arg[0])
		else:
			...
			if ret[0] != "void":
				if ret[0] in classes_type:
					...
					if name in functions_noexcept:
						...
						classes.write("    if ret is None:raise uriError('%s() failed')\n" % (name))
	...
	for classname in classes_list:
		if classname == "None":
			...
			if classname in classes_ancestor:
				...
				if (
					classes_ancestor[classname] == "xmlCore"
					or classes_ancestor[classname] == "xmlNode"
				):
				...
				if (
					...
					classes.write(
						'        return "%s" %% (self.name, int(pos_id (self)))\n\n' % (format)
					)
			else:
