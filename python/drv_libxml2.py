				...
				if nodeType == 1:
					if self.__ns:
						...
						while reader.MoveToNextAttribute():
							...
							attName = (_d(reader.NamespaceUri()), _d(reader.LocalName()))
							qnames[attName] = qname
							attrs[attName] = value
						reader.MoveToElement()
						self._cont_handler.startElementNS(eltName, eltQName, attributesNSImpl)
				...
				# EndElement
					...
					if self.__ns:
						self._cont_handler.endElementNS(
							(_d(reader.NamespaceUri()), _d(reader.LocalName())), _d(reader.Name())
						)
